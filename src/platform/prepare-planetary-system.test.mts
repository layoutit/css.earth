...
import type { BodyId, DwarfPlanetId } from '@cssearth/astronomy';
import type { Matrix3, Vector3 } from '../renderers/css/solar-system/types.ts';

type PlanetarySystem = Awaited<ReturnType<typeof preparePlanetarySystem>>;
type SystemBody = PlanetarySystem['bodies'][number];
type OrbitBody = SystemBody & { orbit: NonNullable<SystemBody['orbit']>; aphelionAu: number };
function present<T>(value: T | null | undefined, label: string): T {
  assert.ok(value !== null && value !== undefined, label);
  return value;
}
function requireOrbitBody(body: SystemBody | undefined, label: string): asserts body is OrbitBody {
  assert.ok(body && body.orbit && body.aphelionAu !== null, `${label}: closed orbit required`);
}
function systemOrbitBody(system: PlanetarySystem, id: BodyId): OrbitBody {
  const body = system.bodies.find(candidate => candidate.id === id);
  requireOrbitBody(body, `${id}: closed orbit`);
  return body;
}
function isOrbitBody(body: SystemBody): body is OrbitBody {
  return body.orbit !== null && body.aphelionAu !== null;
}
function isVector3(value: unknown): value is Vector3 {
  return Array.isArray(value) && value.length === 3 && value.every(item => typeof item === 'number' && Number.isFinite(item));
}
function isDwarfPlanetId(value: BodyId): value is DwarfPlanetId {
  return DWARFS.some(id => id === value);
}
...
const DWARFS: readonly DwarfPlanetId[] = ["ceres", "pluto", "haumea", "makemake", "eris"];
const ALL_OTHER_BODIES = [...OTHER_BODIES, ...DWARFS];
const planetsOf = (system: PlanetarySystem): OrbitBody[] => system.bodies.filter((body): body is OrbitBody => body.kind === "planet" && isOrbitBody(body));
const dwarfsOf = (system: PlanetarySystem): OrbitBody[] => system.bodies.filter((body): body is OrbitBody => body.kind === "dwarf-planet" && isOrbitBody(body));

// Textbook semi-major axes (au), used only as a sanity bound independent of
// the checked-in VSOP87A-derived elements.
const TEXTBOOK_SEMI_MAJOR_AXIS_AU: Partial<Record<BodyId, number>> = {
  venus: 0.723,
  earth: 1.000,
  mars: 1.524,
  jupiter: 5.203,
...
async function prepareMercurySystem(overrides: Partial<Parameters<typeof preparePlanetarySystem>[0]> = {}): Promise<PlanetarySystem> {
...
test("an interstellar observer prepares an open trajectory from its actual source geometry", async () => {
  ...
  assert.deepEqual(plan.orbit.vertices[present(plan.orbit.bodyVertexIndex, 'interstellar body vertex')], [0, 0, 0]);
...
test("unbound context bodies retain their epoch markers without closed planetary rings", async () => {
  ...
  const visitor = present(system.bodies.find(body => body.id === "oumuamua"), 'interstellar visitor');
  ...
  const point = present(present(view.system, 'system projection').bodies.find(body => body.id === "oumuamua"), 'visitor projection');
...
test("an outer observer's long orbit is enclosed together with the other planets", async () => {
  ...
  const preparedSystem = present(plan.system, 'outer observer system');
  assert.doesNotThrow(() => validatePreparedPlanetarySystem(preparedSystem, plan));
  assert.ok(plan.orbit.vertices.every(vertex => Math.hypot(...vertex) <= preparedSystem.maximumExtentUnits));
  assert.deepEqual(preparedSystem.bodies, system.bodies, 'Other body positions and geometry stay unchanged.');
});
...
test("satellite parent frames contain both close and distant moon orbits without changing physical positions", async () => {
  ...
  const satellites: readonly BodyId[] = ["moon", "io", "europa", "ganymede", "callisto", "dimorphos"];
  for (const id of satellites) {
    ...
    const parentId = present(astronomy.BODIES[id].parent, `${id}: parent`);
    const parent = present(system.bodies.find(body => body.id === parentId), `${id}: prepared parent`);
...
test("companion parent rings reproduce the retained primary state, including its velocity and orbital plane", async () => {
  ...
  for (const id of companions) {
    ...
    const sourceState = requireRecord(state);
    const positionKm = sourceState.positionKm, velocityKmPerDay = sourceState.velocityKmPerDay;
    assert.ok(isVector3(positionKm));
    assert.ok(isVector3(velocityKmPerDay));
    const frame = prepareEclipticPresentationFrame(id);
    const kilometersPerUnit = astronomy.BODIES[id].meanRadiusKm / 230;
    const system = await preparePlanetarySystem({ bodyId: id, presentationFrame: frame, kilometersPerUnit, astronomy });
    const parentId = present(astronomy.BODIES[id].parent, `${id}: parent`);
    const parent = present(system.bodies.find(body => body.id === parentId), `${id}: prepared parent`);
    requireOrbitBody(parent, `${id}: parent ring`);
    const toScene = (vector: Vector3): Vector3 => frame.toPresentation(applyMatrix(transposeMatrix(BODY_FIXED_TO_ICRF_MATRICES[id]), vector));
    const sourcePosition = toScene(positionKm);
    const sourceVelocity = toScene(scale(velocityKmPerDay, 1 / 86400));
    ...
    for (const vertex of parent.orbit.vertices) {
      ...
      const expectedRadius: number = semiLatusRectumKm / kilometersPerUnit /
...
test("all observers use the same canonical primary states and matching conics", async () => {
  ...
  const primaryIds: readonly BodyId[] = ["haumea", "patroclus", "sylvia"];
  for (const id of primaryIds) {
    const parent = present(system.bodies.find(body => body.id === id), `${id}: prepared primary`);
    const state = present(BODY_HELIOCENTRIC_STATES[id], `${id}: source state`);
    assert.equal(parent.orbitSource, "source-primary-state-vector-via-solar-geometry");
...
function applyMatrix(matrix: Matrix3, [x, y, z]: Vector3): Vector3 {
  return [
...
function directPositionUnits(id: BodyId): Vector3 {
  const heliocentricIcrfKm = (bodyId: BodyId): Vector3 => scale(
...
test("prepares the other seven planets innermost first, then the five dwarf planets, each vertex 0 at the body", async () => {
  ...
  for (const body of [...planetsOf(system), ...dwarfsOf(system)]) {
...
test("agrees with an independent frame-tree-free derivation of every body's position", async () => {
  ...
  const transposed: Matrix3 = [mercuryMatrix[0], mercuryMatrix[3], mercuryMatrix[6], mercuryMatrix[1], mercuryMatrix[4],
  ...
  for (const body of dwarfsOf(system)) {
    assert.ok(isDwarfPlanetId(body.id));
...
test("nests every orbit strictly outside the one before it, mercury included", async () => {
  ...
  assert.ok(
    BODY_ORBITS.venus.perihelionAu > present(BODY_ORBITS.mercury.aphelionAu, 'Mercury aphelion'),
...
test("keeps each body's semi-major axis within half a percent of the textbook value", async () => {
  const system = await prepareMercurySystem();
  for (const body of [...planetsOf(system), ...dwarfsOf(system)]) {
    const textbook = present(TEXTBOOK_SEMI_MAJOR_AXIS_AU[body.id], `${body.id}: textbook semi-major axis`);
...
test("rejects an unlisted body, a non-positive unit scale and a reflected presentation frame", async () => {
  await assert.rejects(
    // Deliberately crosses the typed preparation boundary to exercise its runtime validator.
    Reflect.apply(prepareMercurySystem, undefined, [{ bodyId: "unknown" }]),
...
test("lights every body from Mercury's vantage: phase from Sun-body-observer, only Venus can ever be a crescent", async () => {
  ...
  for (const body of system.bodies) {
    ...
    if (body.id !== "venus") {
      const bound = Math.asin(present(BODY_ORBITS.mercury.aphelionAu, 'Mercury aphelion') / body.perihelionAu) * 180 / Math.PI;
...
test("marker brightness falls with magnitudes below the brightest and floors, Venus opaque, Neptune on the floor", async () => {
  ...
  const brightest = present(byFlux[0], 'brightest body'), faintest = present(byFlux.at(-1), 'faintest body');
  assert.equal(brightest.id, "venus");
  // Every dwarf planet sits on the floor; Neptune is the faintest planet.
  assert.equal(faintest.kind, "dwarf-planet");
  assert.equal(present(planetsOf(system).sort((a, b) => a.illumination.flux - b.illumination.flux)[0], 'faintest planet').id, "neptune");
  assert.equal(brightest.illumination.markerOpacity, 1);
  assert.equal(faintest.illumination.markerOpacity, MARKER_BRIGHTNESS.floor);
...
test("the dwarf planets ride the same frame tree, Pluto's orbit really crosses Neptune's, and Eris sets the extent", async () => {
  const system = await prepareMercurySystem();
  const pluto = present(system.bodies.find(body => body.id === 'pluto'), 'Pluto');
  const neptune = systemOrbitBody(system, 'neptune');
  const eris = systemOrbitBody(system, 'eris');
  assert.ok(pluto.perihelionAu < neptune.aphelionAu, "Pluto's perihelion lies inside Neptune's aphelion");
  assert.ok(pluto.inclinationDegrees > 15, "Pluto's orbit is steeply inclined");
  for (const body of dwarfsOf(system)) {
  ...
  assert.ok(Math.abs(extentAu - eris.aphelionAu) < 0.6);
});
