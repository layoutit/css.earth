...
import type { PreparedPlanetarySystem } from "./heliocentric-view.mts";
export interface HeliocentricPreparationOptions {
  bodyId: string;
  presentationFrame: { model: string; basis: readonly Vector3[]; toPresentation(direction: Vector3): Vector3 };
  bodyRadiusUnits: number; bodyRadiusKilometers: number;
  sunSprite: { opaqueCoreDiameterShare: number; imagePixels: number };
  system?: PreparedPlanetarySystem & { units: { kilometersPerUnit: number } };
}
...
import { ORBIT_TRAIL_MODEL } from "./heliocentric-view.mts";
...
export function chordBehindTurns(offsets: readonly number[]) {
  if (!isArray(offsets) || offsets.length < 2 || offsets[0] !== 0 ||
