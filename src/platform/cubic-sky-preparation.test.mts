...
test("rejects malformed cubic-sky pixel buffers", () => {
  ...
  assert.throws(
    () => Reflect.apply(prepareStandardCubicSkyPixels, undefined, [new Uint8Array([0, 0, 0])]),
    /invalid/u,
  );
});

function luminance(pixels: Uint8Array, offset: number) {
  return 0.2126 * pixels[offset] + 0.7152 * pixels[offset + 1] +
    0.0722 * pixels[offset + 2];
}

function chroma(pixels: Uint8Array, offset: number) {
