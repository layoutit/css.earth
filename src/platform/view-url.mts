import type { LegacyCameraPose, PhysicalCameraPose } from "../renderers/css/navigation/types.ts";
...
function isPhysicalCamera(camera: LegacySharedCamera | PhysicalSharedCamera): camera is PhysicalSharedCamera { return camera.pose.schema === "cssearth-camera-pose@2"; }
...
function parseBinaryShared(bytes: Uint8Array): SharedView {
  ...
  const pose: LegacyCameraPose = { schema: SHARED_POSE_SCHEMA, scene: "", skybox: "", sunView: "" };
  camera.pose = pose;
  for (const [index, field] of POSE_FIELDS.entries()) {
    let matrix;
    if (flags & (1 << (5 + index))) {
    ...
    pose[field] = serializeMatrix(matrix);
  }
