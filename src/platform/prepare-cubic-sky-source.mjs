...
export async function preparePlanetCubicSky({
  ...
  if (!/^[a-z][a-z0-9-]*$/u.test(objectId) ||
      ...
      !["photographic", "lossless"].includes(imageEncoding) ||
      (cameraContract !== null && (
...
for (const density of [1, 2]) {
  ...
  for (const {
    ...
    }).webp(imageEncoding === "lossless" ? { lossless: true, effort: 6 } : { quality: 95, effort: 6, smartSubsample: true }).toFile(resolve(
      publicRoot,
