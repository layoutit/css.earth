import type { PreparedPage, PageMountOptions, PageSelection } from "../../renderers/css/paging/types.ts";
import type { ObjectRuntimeView } from "../../renderers/css/runtime/object-runtime-types.ts";
...
import { createPreparedProjectiveTextureLeaf } from "../prepared-projective-texture-leaf.mts";
import { selectCityPages } from "./city-page-selection.mts";
import { createCityIndex } from "./city-index.mts";
import { normalizeCityAssetOrigin, isPreparedCityAssetUrl, isPreparedAssetPath } from "./city-asset-url.mts";
import { createApiImageTransport } from "./api-image-transport.mts";
...
export function mountPreparedMapPages({ plan, carrier, system, scene, camera, stage, className, textureClassName, lensIds, own, onError = error => { throw error; } }: PageMountOptions) {
  ...
  let view: Pick<ObjectRuntimeView, "zoom"> | null = null;
  let activeLoads = 0;
  let requests = 0;
  let aborts = 0;
  let evictions = 0;
  ...
  return Object.freeze({
    publish(nextView: Pick<ObjectRuntimeView, "zoom">) { view = nextView; schedule(); },
