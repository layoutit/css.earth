import { isPreparedBlockReference, preparedReferenceKey } from "./prepared-block-transport.mjs";
...
const corners = value => Array.isArray(value) && [4, 8].includes(value.length) && value.every(finiteVector);

export function requireGeographicRootReference(ref, assetPath) {
  if (ref?.encoding !== "gzip" || !hash(ref.sha256) || !hash(ref.decodedSha256) ||
      ref.url !== `${assetPath}geographic-roots-${ref.sha256.slice(0, 16)}.pack` ||
      ![ref.bytes, ref.decodedBytes].every(value => Number.isSafeInteger(value) && value > 0 && value <= GEOGRAPHIC_INDEX_LIMITS.rootBytes)) {
    throw new Error("Invalid geographic root directory reference.");
  }
  return ref;
}

function address(node, plan) {
  const match = /^wmts-tile-(\d+)-(\d+)-(\d+)$/u.exec(node?.key ?? "");
  if (!match || !corners(node.corners) || !finiteVector(node.normal)) throw new Error("Invalid prepared geographic tile bounds.");
...
export function requireGeographicDirectoryReference(ref, plan) {
  if (!isPreparedBlockReference(ref, plan.assetPath) || ref.offset === undefined ||
      ...
      ref.bytes > plan.index.maximumDirectoryBytes || ref.decodedBytes > plan.index.maximumDirectoryBytes) {
    throw new Error("Invalid or stale geographic geometry directory.");
  }
  return ref;
}

function stub(node, plan) {
...
export function requireGeographicRoots(data, plan) {
...
export function requireGeographicDirectory(data, ref, plan, expectedKeys) {
