...
export interface PreparationCommand { objectName: string; projectRoot: string; script: string; argumentsList: readonly string[]; }
...
export async function runPreparationSteps({
...
}: { objectName: string; toolDirectory: string; steps: readonly (readonly string[])[]; projectRoot?: string; runCommand?: (command: PreparationCommand) => Promise<unknown> }) {
  if (typeof objectName !== "string" || objectName.length === 0 ||
      typeof toolDirectory !== "string" || typeof projectRoot !== "string" || !isArray(steps) ||
      steps.length === 0) {
  ...
  for (const step of steps) {
    if (!isArray(step) || step.length === 0 ||
        ...
        isAbsolute(step[0]) ||
        !/^[a-z0-9][a-z0-9._-]*\.(?:m?[jt]s)$/u.test((step[0].split("/").at(-1) ?? "")) ||
        step[0].split("/").slice(0, -1).some((part: string) => !/^(?:\.{1,2}|[a-z0-9][a-z0-9._-]*)$/u.test(part))) {
...
async function spawnPreparedStep({ objectName, projectRoot, script, argumentsList }: PreparationCommand) {
