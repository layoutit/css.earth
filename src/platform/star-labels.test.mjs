...
const star = (hip, name, magnitude = 1, direction = [0, 0, -1]) => ({ hip, name, magnitude, direction });
const select = (stars, overrides = {}) => selectStarLabel({ ...view, stars, ...overrides });
...
test("the one ordinary caption names the brightest proper-named star regardless of input order", () => {
  ...
  assert.equal(select([star(1, "", -4), star(2, null, -5)]), null);
  assert.equal(select([star(1, "", -4), star(2, null, -5), faint]).name, "Faint");
