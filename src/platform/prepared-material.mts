import type { PitchCalibration } from "../renderers/css/navigation/types.ts";
import type { PreparedView } from "../renderers/css/rendering/prepared-presentation.ts";
import type { PreparedResources } from "../renderers/css/rendering/prepared-residency.ts";
...
import { createPreparedPlanarRotationPublisher } from "./prepared-planar-rotation.mts";
import { createPreparedEllipsoidProjection } from "./prepared-ellipsoid-projection.mts";
...
export function preparedMaterialFrame(mapping: PreparedMaterialFrameMapping, view: Pick<PreparedView, "sunViewDirection">) {
  if (view.sunViewDirection === null) throw new TypeError("Prepared material needs a Sun direction.");
...
export function preparedMaterialState(track: PreparedMaterialTrack, selected: PreparedMaterialSelection, view: PreparedMaterialView, _camera?: PitchCalibration, _committedPlan?: PreparedMaterialDemand) {
  ...
  if (direction === null) throw new TypeError("Prepared material needs a Sun direction.");
  ...
  if (!bank) throw new TypeError("Prepared material bank is not declared.");
...
export function createPreparedMaterialPublisher(track: PreparedMaterialTrack,element: HTMLElement,camera?: PitchCalibration) {
  ...
  const write=(name: string,value: string)=>{
    const current: unknown=name.startsWith("--")?element.style.getPropertyValue(name):Reflect.get(element.style, name);
    if(current===value)return false;
    if(name.startsWith("--"))element.style.setProperty(name,value);else Reflect.set(element.style, name, value);
    return true;
  };
  return Object.freeze({
    publish(selected: PreparedMaterialSelection,view: PreparedView,resources: PreparedResources,committedPlan?: PreparedMaterialDemand){
