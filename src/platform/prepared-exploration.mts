...
export interface ExplorationImage { readonly id: string; readonly src: string; readonly width: number; readonly height: number; readonly bytes: number; readonly sha256: string; readonly kind: string; readonly sourceUrl: string; readonly credit: string; readonly subject?: ExplorationSubject; }
export function parseExplorationImage(raw: unknown): ExplorationImage {
  ...
  return Object.freeze({ id: explorationId(image.id), src, sha256, width: number(image.width), height: number(image.height), bytes: number(image.bytes), kind: explorationText(image.kind), sourceUrl: explorationUrl(image.sourceUrl), credit: explorationText(image.credit), ...(subject ? { subject } : {}) });
}
