...
function harness({ capacity = 3, concurrency = 2, reuse = true } = {}) {
  ...
  const store = createPreparedImageStore({
    ...
    createImage() {
      const image = { naturalWidth: 1, naturalHeight: 1, src: "", decode() {
        return new Promise((resolve, reject) => decodes.push({ image, url: image.src, resolve, reject }));
      } };
      images.push(image);
      return image;
    },
  });
  const load = (lease, url) => lease.load(url, { pool: "material" });
...
test("a URL shared across catalog pools cannot strand the released role's reusable slot", async () => {
  ...
  const store = createPreparedImageStore({ pools: ["row", "full-phase"].map(id => ({ id, capacity: 1, concurrency: 1, reuse: true })),
    createImage() { return { naturalWidth: 1, naturalHeight: 1, decode() { return new Promise(resolve => jobs.push(resolve)); } }; },
  });
...
test("declared native decoding hints survive pooled reuse without introducing another image owner", async () => {
  ...
  const store = createPreparedImageStore({ pools: [{ id: "material", capacity: 1, concurrency: 1, reuse: true, decoding: "sync" }],
    createImage() { const image = { naturalWidth: 1, naturalHeight: 1, decode: () => Promise.resolve(),
      removeAttribute(name) { if (name === "src") this.src = ""; } }; images.push(image); return image; } });
  ...
  assert.throws(() => createPreparedImageStore({ pools: [{ id: "bad", capacity: 1, concurrency: 1, decoding: "invented" }] }), /policy/);
});
...
test("unsettled resources cannot be handed off and cancellation still clears their native URL", async () => {
  const image = { src: "", naturalWidth: 1, naturalHeight: 1, decode: () => new Promise(() => {}) };
