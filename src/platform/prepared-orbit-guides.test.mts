...
import { preparedFixture } from "./orbit-guide-test-fixture.mts";
...
test("prepares a planet-owned orbit-guide plan and vector assets", async (t) => {
  ...
  const generated = await import(pathToFileURL(request.outputModulePath).href);
...
function fixtureRequest(directory: string) {
  return {
    planetId: "earth",
    bodies: [
      {
