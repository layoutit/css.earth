import type { PreparedSurfaceHit } from "../renderers/css/navigation/prepared-surface-hit.ts";
import type { PreparedFacingPlane } from "../renderers/css/rendering/prepared-facing.ts";
import type { PreparedDepthPartitions } from "../renderers/css/rendering/prepared-depth-partitions.ts";
import type { ObjectSelection } from "../renderers/css/runtime/object-contract.ts";
import type { PreparedMaterialTrack, PreparedMaterialSelection, PreparedMaterialDemand } from "../renderers/css/rendering/prepared-material.ts";
import type { PreparedResources, PreparedResourceDemand } from "../renderers/css/rendering/prepared-residency.ts";
import type { PreparedAnimationOptions } from "../renderers/css/rendering/prepared-playback.ts";
export type PreparedSelection = ObjectSelection;
export interface PreparedView {
  readonly projection?: import("../renderers/css/rendering/physical-projection.ts").PhysicalProjection;
...
export interface PreparedPageLayer {
  id: string; carrier: number; system: number; plan: import("../renderers/css/paging/types.ts").PreparedPagePlan;
...
import { createPreparedMaterialPublisher } from "./prepared-material.mts";
import { resolvePreparedMaterialDemand } from "./prepared-material-demand.mts";
...
export function resolvePreparedPresentation(definition: PreparedPresentationDefinition, { selection, view, previousPlan }: { selection: ObjectSelection; view: import("../renderers/css/rendering/prepared-material.ts").PreparedMaterialView | null; previousPlan?: PreparedPresentationPlan | null }): PreparedPresentationPlan {
  ...
  for (const selected of variant.materials) {
    ...
    if (!track) throw new TypeError(`Unknown prepared material track: ${selected.track}.`);
    if (!view) throw new TypeError("Prepared material requires the current view.");
...
const datasetKey = (name: string) => name.slice(5).replace(/-([a-z])/g, (_: string, letter: string) => letter.toUpperCase());
...
const styleValue = (element: HTMLElement, name: string) => name.startsWith("--") ? element.style.getPropertyValue(name) : Reflect.get(element.style, name) as string;
function writeStyle(element: HTMLElement, name: string, value: string) {
  if (name.startsWith("--")) element.style.setProperty(name, value);
  else Reflect.set(element.style, name, value);
}
...
export function mountPreparedPresentation(stage: HTMLElement, context: PreparedPresentationContext, definition: PreparedPresentationDefinition) {
  ...
  for (const record of definition.tree.nodes) {
    ...
    for (const propertyId of record.properties) {
      ...
      else Reflect.set(node.style, property.name, property.value);
    }
  ...
  for (const binding of stageBindings.values()) {
    ...
    context.own(() => {
      if (!owned()) return;
      if (binding.kind === "attribute") writeAttribute(stage, binding.name, previous as string | null);
      else if (binding.kind === "class") stage.classList.toggle(binding.name, previous as boolean);
      else writeStyle(stage, binding.name, previous as string);
    });
  }
