...
function visibleStar(...args: Parameters<typeof starPresentation>) {
  const value = starPresentation(...args); assert.ok(value, 'Expected a visible prepared star'); return value;
}
...
test("the photometric chain matches the reference at its fixed points", () => {
  ...
  const faint = visibleStar(exposure, limits.limitingMagnitude - 0.1);
...
test("the reference caps the disc, so magnitude reaches the eye as luminance and halo; colour follows temperature", () => {
  const sirius = visibleStar(exposure, -1.44);
  const polaris = visibleStar(exposure, 1.97);
  const faintest = visibleStar(exposure, 5.0);
  ...
  assert.ok(Math.abs(visibleStar(lit, 5.0).luminance - 0.107) < 0.01);
  assert.equal(visibleStar(lit, -1.44).luminance, EXPOSURE_KNOBS.intensityMax);
  assert.ok(Math.abs(exposureLimits(lit).limitingMagnitude - 6.029) < 0.01);
  assert.equal(visibleStar(createExposure({ fovDegrees: 60, intensityMax: 0.75 }), -1.44).luminance, 0.75);
  assert.equal(visibleStar(createExposure({ fovDegrees: 60, maxRadiusPx: 2 }), -1.44).radiusPx, 2);
...
test("the cube face inverse agrees with the faces' own sampling and the point transform faces the eye", () => {
  ...
  const yawMatch = /rotateY\(([-0-9.]+)deg\)/u.exec(transform); assert.ok(yawMatch);
  const yaw = Number(yawMatch[1]) * Math.PI / 180;
  const pitchMatch = /rotateX\(([-0-9.]+)deg\)/u.exec(transform); assert.ok(pitchMatch);
  const pitch = Number(pitchMatch[1]) * Math.PI / 180;
