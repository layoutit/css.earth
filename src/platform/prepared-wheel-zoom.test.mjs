...
test("a perspective dolly scales the distance by the wheel magnitude and never turns the scene", async t => {
  ...
  t.after(() => { globalThis.HTMLElement = prior; });
  const surface = new Surface();
  const camera = { state: { zoom: 1, distance: 1000 } };
  const published = [];
  const controls = createPreparedWheelZoomControls({
    inputSurface: surface, camera, minimumZoom: 0.001, maximumZoom: 4,
    dolly: { stepPerDelta: 0.006 },
    trackballMetrics: () => ({ centerX: 300, centerY: 300, opticalCenterX: 300,
      opticalCenterY: 300, radius: 120, surfaceRadius: 120, focalLength: 600 }),
  ...
  const near = (actual, expected) => Math.abs(actual - expected) < 1e-9 * expected;
  ...
  assert.throws(() => createPreparedWheelZoomControls({
    inputSurface: surface, camera, minimumZoom: 0.001, maximumZoom: 4,
