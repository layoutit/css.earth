...
function harness(controls: ObjectControls = moonControls, mutate: HarnessMutation = () => {}) {
  ...
  const motion = new Input({ name: "motion" }), planetaryLabels = new Input({ name: "planetaryLabels" }), heliosphere = new Input({ name: "heliosphere" });
  const illustrationModels = new Input({ name: "illustrationModels" });
  const minimap = new Input({ name: "minimap" });
  settingInputs.push(motion, planetaryLabels, heliosphere, illustrationModels, minimap);
  ...
  return { binding, lensInputs, settingInputs, lensRoot, settingsRoot, motion, planetaryLabels, heliosphere, illustrationModels, minimap, errors, actions, initial,
...
for (const object of SCENE_OBJECTS) test(`${object.id}: one binder consumes every actual control and owns no shell preference listener`, async () => {
  ...
  assert.ok(h.settingInputs.filter(input => !["motion", "planetaryLabels", "heliosphere", "illustrationModels", "minimap"].includes(input.name)).every(input => input.disabled));
  assert.equal(h.motion.disabled, false); assert.equal(h.planetaryLabels.disabled, false);
  assert.equal(h.illustrationModels.disabled, false);
  assert.equal(h.heliosphere.disabled, false);
  assert.equal(h.minimap.disabled, false);
  ...
  h.motion.emit("change"); h.planetaryLabels.emit("change"); h.heliosphere.emit("change"); h.illustrationModels.emit("change");
  h.minimap.emit("change"); assert.equal(h.actions.length, count);
