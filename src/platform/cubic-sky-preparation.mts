...
export interface CubicSkySunOracle { schema: string; canonicalUrl: string; asset: { width: number; height: number; url: string; sha256: string }; rayPeaks: readonly unknown[]; presentation: { sourceTreatment: string }; qualification: { redistribution: string }; }
import sharp from "sharp";

import { CUBIC_SKY_STANDARD } from "./cubic-sky-contract.mts";

export const CUBIC_SKY_SUN_ORACLE_SCHEMA =
  "cssearth-google-maps-sun-oracle@1";

export function prepareStandardCubicSkyPixels(highContrastPixels: Buffer) {
...
export function validateCubicSkySunOracle<T extends CubicSkySunOracle>(oracle: T) {
...
export function compositeSunIntoCubicSkyFace({
  ...
  faceSize,
  localDirection,
}: { faceId: string; facePixels: Buffer; sunPixels: Buffer; sunSize: number; faceSize: number; localDirection: Vector3 }) {
  const standard = CUBIC_SKY_STANDARD.sun;
...
function sampleBilinearRgba(pixels: Uint8Array, width: number, height: number, sourceX: number, sourceY: number) {
...
function cross(left: Vector3, right: Vector3) {
  return [
