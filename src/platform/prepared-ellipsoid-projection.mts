import type { Matrix4 } from "../renderers/css/solar-system/types.ts";
...
export function createPreparedEllipsoidProjection({ projection, width, height = width }: {projection:EllipsoidProjectionPlan;width:number;height?:number}) {
  ...
  for (const key of ["bodySystemMatrix", "bodyMeshMatrix", "materialSystemMatrix", "materialMeshMatrix",
    "baseProjection", "centerTranslation", "inverseCenterTranslation"] as const) {
    matrices[key] = Object.freeze([...requirePreparedMatrix4(projection[key])]);
  }
  const counterTransport: CounterTransport = Object.freeze(Object.fromEntries((["counterPrecision", "counterFractionDigits", "counterFractionScale"] as const)
    .filter(key => projection[key] !== undefined).map(key => [key, projection[key]])));
  const precision = (value: unknown): value is number => typeof value === "number" && Number.isInteger(value) && value >= 1 && value <= 16;
  if (counterTransport.counterPrecision !== undefined && !precision(counterTransport.counterPrecision) ||
      counterTransport.counterFractionDigits !== undefined && (!precision(counterTransport.counterPrecision) ||
        !precision(counterTransport.counterFractionDigits) || !((counterTransport.counterFractionScale ?? Number.NaN) > 0) || !Number.isFinite(counterTransport.counterFractionScale)) ||
  ...
  return ({ degrees, sceneMatrix, counterMatrix, preserveDefault = false }: EllipsoidView) => {
...
export function readPreparedCounterMatrix(value: Matrix4 | string | undefined, projection: CounterTransport): Matrix4 {
  ...
  return values.map((number, index) => {
    ...
    if (fastDecimal && fraction && projection.counterFractionDigits !== undefined && fraction.length >= projection.counterFractionDigits) {
...
function invertMatrix2(matrix: Matrix2): Matrix2 {
