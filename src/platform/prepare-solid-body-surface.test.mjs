...
function coordinatesAt(leaf, u, v) {
  const m = leaf.style.match(/matrix3d\(([^)]+)/)[1].split(",").map(Number);
  const x = u * Number(leaf.style.match(/atlas-width:([\d.]+)/)[1]);
  const y = v * Number(leaf.style.match(/atlas-height:([\d.]+)/)[1]);
...
const longitudeByte = longitude => (longitude * width - 0.5) / (width - 1) * 255;
const latitudeByte = latitude => ((0.5 - latitude / Math.PI) * height - 0.5) / (height - 1) * 255;
