...
import { requireGeographicLensReference, requireGeographicLensPackage } from "./geographic-lens-contract.mjs";
...
export function createGeographicLensRuntime({ pages, capacity, getEntity, selectBase, onChange = () => {}, fetcher = fetch }) {
  ...
  return Object.freeze({
    ...
    async select(id) {
      ...
      try {
        ...
        const content = requireGeographicLensPackage(await readPreparedJson(response, descriptor.package), descriptor, entity.id, capacity);
