...
export interface TemperaturePressureLayer { pressure: number; temperature: number; }
export interface PhotometricPhasePoint { phaseAngle: number; dimmingMagnitude: number; }
const FONT =
...
export function renderTemperaturePressureChart({
  ...
  temperatureMaximum,
  pressureTicks,
}: ChartIdentity & { layers: readonly TemperaturePressureLayer[]; pressureMinimum: number; pressureMaximum: number; temperatureMinimum: number; temperatureMaximum: number; pressureTicks: readonly { pressure: number; label: string }[] }) {
  validateChartIdentity({ id, title, description, metadata });
  if (!isArray(layers) || layers.length < 2 ||
      ...
      !isArray(pressureTicks) || pressureTicks.length === 0 ||
...
export function renderPhotometricPhaseChart({
  ...
  metadata,
  points,
}: ChartIdentity & { points: readonly PhotometricPhasePoint[] }) {
  validateChartIdentity({ id, title, description, metadata });
...
function validateChartIdentity({ id, title, description, metadata }: ChartIdentity) {
  if (!/^[a-z][a-z0-9-]*$/u.test(id) ||
      ...
      !metadata || typeof metadata !== "object" || isArray(metadata)) {
