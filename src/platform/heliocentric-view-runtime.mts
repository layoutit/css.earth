...
import type { LabelPolicy, LabelEntry, LabelDeclutter, LabelSlots, CatalogueStar, StarLabel } from "@cssearth/engine";
import type { Vector2, Matrix3, VisibleRect } from "../renderers/css/solar-system/types.ts";
import type { StarCaptionPolicy } from "../renderers/css/solar-system/heliocentric-captions.ts";
import type {CaptionPlan,LabelPolicyOptions} from "../renderers/css/solar-system/heliocentric-captions.ts";
import type {SpriteWithUrl,SystemMarkers,PhaseAtlas} from "../renderers/css/solar-system/heliocentric-sprites.ts";
import type {HeliocentricViewPlan,HeliocentricProjection,HeliocentricProjectionInput,TrailSpans,OrbitSegment} from "./heliocentric-view.mts";
...
import { selectStarLabel } from "./star-labels.mts";
import { createExposure } from "./star-photometry.mts";
...
export function mountRetainedHeliocentricView({
  ...
  let labelField: CaptionField | null = null;
...
function writePieces(pool: readonly HTMLElement[], segments: readonly OrbitSegment[], previousCount: number) {
...
function validPhaseAtlas(atlas: PhaseAtlas | null | undefined): atlas is PhaseAtlas {
...
function phaseFrameFor(atlas: PhaseAtlas, lightViewZ: number) {
...
function validSprite(sprite: { index: number; count: number; size: number } | null | undefined): sprite is { index: number; count: number; size: number } {
...
function applySprite(element: HTMLElement, sprite: SpriteWithUrl) {
