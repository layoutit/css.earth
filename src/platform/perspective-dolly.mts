import type { CameraPlan, PerspectiveCameraPlan, CameraUpdate, LevelOfDetailPlan, OrbitLineFade, PlanetarySystemFade, SunMarkerFade } from "../renderers/css/navigation/types.ts";
import type { BodyProjection, HeliocentricProjection } from "./heliocentric-view.mts";
import type { VisibleRect } from "../renderers/css/solar-system/types.ts";
import type { PositionM } from '@cssearth/engine';
import type { PhysicalProjection } from "../renderers/css/rendering/physical-projection.ts";
import type { CameraViewport } from "../renderers/css/navigation/camera-viewport.ts";
import type { PreparedWorldCameraFrame, WorldCameraPose, WorldCameraViewport } from "../renderers/css/navigation/world-camera.ts";
import type { mountRetainedHeliocentricView } from "./heliocentric-view-runtime.mts";
...
function assertPerspectiveCameraPlan(plan: CameraPlan): asserts plan is PerspectiveCameraPlan {
  if (plan?.projection?.model !== PERSPECTIVE_PROJECTION_MODEL ||
      ...
      !plan.orbitLineFade || !Number.isFinite(plan.orbitLineFade.visibleBelowDiscHeightShare) ||
...
export function createPerspectiveDolly({
  ...
  heliocentric: mountedHeliocentric,
  cameraElement,
  sceneElement,
  skyElement,
  stage,
  ...
  const heliocentric = mountedHeliocentric;
  if (!heliocentric) throw new TypeError("Perspective dolly requires the mounted heliocentric view.");
  ...
  const measure = () => {
    ...
    if (!view) throw new Error("Perspective camera requires an attached window.");
