...
test("orbit label priority follows Galaxio's angular, near-body and eligibility rules", () => {
  ...
  const priorityAt = (angleDegrees: number, bodyDistance = 1000) => planetOrbitLabelPriority(policy, 2 / (angleDegrees * Math.PI / 180), bodyDistance, 8);
...
test("projection carries physical discs and small camera-relative cores for every far system body", async () => {
  ...
  assert.ok(closer.system);
  const venusBody = closer.system.bodies.find(({ id }) => id === "venus");
  assert.ok(venusBody);
  const venus = venusBody.marker;
  ...
  const occluded = projectHeliocentricView(behindBody, nearOptions);
  assert.ok(occluded.system);
  const hidden = occluded.system.bodies[0].marker;
