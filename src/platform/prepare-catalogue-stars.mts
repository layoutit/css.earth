...
export interface PreparedCatalogueStar { hip: number; name: string | null; direction: readonly number[]; distanceParsecs: number; magnitude: number; band: string; presentation: string; radiusPx: number; rawRadiusPx: number; luminance: number; haloAlpha: number; color: readonly number[]; }
export interface CatalogueStarBand { id: string; faintestMagnitude: number; presentation: string; }
export interface CatalogueStarPreparationOptions { cataloguePath?: string; fovDegrees: number; bands?: readonly CatalogueStarBand[]; catalogue?: Catalog | null; }
export type PreparedCatalogueStars = Awaited<ReturnType<typeof prepareCatalogueStars>>;
...
import { SKIP_RADIUS_PX, createExposure, exposureLimits, starPresentation } from "./star-photometry.mts";
...
} from "./star-photometry.mts";

// Apparent magnitude from the catalogue's absolute magnitude and distance.
export function apparentMagnitude(absoluteMagnitude: number, distanceParsecs: number) {
...
export function teffToRgb(teffK: number) {
...
export function bvToTeff(colorIndexBv: number) {
  const bv = Math.min(4, Math.max(-0.4, colorIndexBv));
  return 4600 * (1 / (0.92 * bv + 1.7) + 1 / (0.92 * bv + 0.62));
}

export function starColor(teffK: number, colorIndexBv: number) {
...
export async function loadCatalogue(path: string) {
...
export async function prepareCatalogueStars({
  ...
  catalogue = null,
}: CatalogueStarPreparationOptions) {
  if (!(fovDegrees > 0)) {
  ...
  const stars: PreparedCatalogueStar[] = [];
...
// the cube's centre.
export function cubeFaceOf([x, y, z]: Vector3) {
...
export function retainedStarTransform([x, y, z]: Vector3) {
  const share = RETAINED_STAR_RADIUS_SHARE_OF_HALF_SIDE;
  const place = (component: number) => `calc(${Number((component * share).toFixed(6))} * var(--planet-cubic-sky-half-side))`;
