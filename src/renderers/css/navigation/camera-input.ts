...
import type { FlyToMotion } from './camera-input-options.js';
...
export function createUnboundedMatrixDragControls({
  ...
  const projectSurfaceRotation = (pointer: SphereDragInput) => activeTrackball?.surfaceSphere
    ? projectPhysicalSurfaceDrag(pointer, activeTrackball.surfaceSphere) : projectSphereDrag(pointer);
  ...
  const applyPointerSamples = (event: PointerEvent) => {
    ...
    for (const sampleEvent of sampleEvents) {
      ...
      const sampleRotation = (skyGesture ? projectSkyRotation : projectSurfaceRotation)({
        previousX,
        previousY,
        currentX: sampleEvent.clientX,
        currentY: sampleEvent.clientY,
  ...
  const endPointer = (event: PointerEvent) => {
    ...
    const throwState = wasDragging && event.type === "pointerup" && freshRelease
      ? estimateGoogleEarthDragThrow({ history, releaseTimestamp: event.timeStamp,
        ...
        projectRotation: skyGesture ? projectSkyRotation : projectSurfaceRotation }) : null;
  ...
  return Object.freeze({
    flyTo({ sample, durationMilliseconds = 4500 }: DestinationMotion): Promise<MotionCompletion> {
    ...
    stats() {
      return dragControlDiagnostics({ skyGesture, pointerActive: pointerId !== null,
        inertiaActive: inertiaFrame !== null, flyToActive: flyToFrame !== null,
        destinationActive: Boolean(flyToMotion?.sample), surfaceFlyToEnabled: surfaceFlyToState !== null,
