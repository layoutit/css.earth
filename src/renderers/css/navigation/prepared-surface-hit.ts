...
export interface PreparedSurfaceHit { readonly target: number; readonly triangles: readonly SurfaceTriangle[]; readonly frontFace?: SurfaceFrontFace; }
...
export function bindPreparedSurfaceHit(plan: PreparedSurfaceHit, target: HTMLElement, scene: HTMLElement,
  camera: HTMLElement): (clientX: number, clientY: number) => boolean {
  ...
  return (clientX, clientY) => {
    ...
    return rayHitsPreparedTriangles([eye.x, eye.y, eye.z], [ray.x, ray.y, ray.z], plan.triangles, plan.frontFace);
  };
}
