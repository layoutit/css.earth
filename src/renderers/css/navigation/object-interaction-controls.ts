...
import type { NavigationCamera, TrackballMetrics, CameraDelta, ControlsUpdate, DestinationMotion, WheelDolly } from './types.js';
export interface ObjectInteractionOptions { inputSurface: HTMLElement; runtimePolicy: RuntimePolicy; camera: NavigationCamera; trackballMetrics(): TrackballMetrics; sceneMatrix(): string; rotate(delta: CameraDelta): void; minimumZoom: number; maximumZoom: number; dolly?: WheelDolly | null; surfaceFlyToHitTest?: ((clientX: number, clientY: number) => boolean) | null; onStart(): void; onEnd(): void; onError?: ((error: unknown) => void) | null; }
...
export function createObjectInteractionControls({
  ...
  const interactionTrackballMetrics = () => {
    ...
    return metrics.surfaceSphere ? metrics : googleEarthInteractionTrackball(metrics);
  };
