...
export function savedWorldCamera(saved: SharedView, frame: PreparedWorldCameraFrame, viewport: WorldCameraViewport) {
  ...
  // Body-relative links predate the optional astronomical date tag. Resolve
  // those against the selected prepared frame; an explicit different date
  // still cannot be silently reinterpreted.
  if (view.preparedEpochJdTt != null && view.preparedEpochJdTt !== frame.epochJdTt) throw new TypeError('Saved camera has a different prepared epoch.');
