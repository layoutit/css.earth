...
export type PreparedMaterialView = Pick<PreparedView, "sunViewDirection" | "sceneMatrix" | "reference" | "levelOfDetail"> & Partial<Pick<PreparedView, "zoom">>;
...
export interface PreparedMaterialTrack {
  id: string; target: number; frame: PreparedMaterialFrameMapping; defaultFrame: number; farBank?: string; maximumZoom?: number;
...
export function preparedMaterialState(track: PreparedMaterialTrack, selected: PreparedMaterialSelection, view: PreparedMaterialView) {
  ...
  const zoomHidden = track.maximumZoom !== undefined && (view.zoom ?? 0) > track.maximumZoom;
