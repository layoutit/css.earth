...
export interface PreparedPageLayer {
  geographic?: boolean; id: string; carrier: number; system: number; plan: unknown;
  className: string; textureClassName: string; lensIds: string[];
}
export interface PreparedPresentationDefinition {
  observationSurface?: { slots: readonly PreparedTextureSlot[] };
...
export function mountPreparedPresentation(stage: HTMLElement, context: PreparedPresentationContext, definition: PreparedPresentationDefinition, preparedTree?: PreparedTreeLease, initialProjection?: import('./physical-projection.js').PhysicalProjection, progressiveActivation = false) {
  ...
  return Object.freeze({
    ...(observationSurface ? { observationSurface } : {}), cameraElement, sceneElement, activate,
    ...
    commitSelection({ selection, resources }: { selection: ObjectSelection; resources: PreparedResources; plan?: PreparedPresentationPlan; view?: PreparedView | null }) {
      ...
      const writes = variant.writes.map(binding => {
        ...
        return { kind: "texture" as const, target: binding.target, name: binding.name, value: url === null ? "none" : binding.quoted ? `url(${JSON.stringify(url)})` : `url(${url})` };
      });
      for (const binding of writes) {
        ...
        if (binding.kind === "attribute") writeAttribute(element, binding.name, binding.value);
          ...
          if (binding.kind !== "texture" || !observationSurface?.write(binding.target, binding.name, binding.value)) writeStyle(element, binding.name, binding.value);
          styleWrites++;
        }
      }
