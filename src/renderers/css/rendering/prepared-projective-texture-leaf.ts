...
export function composePreparedProjectiveTransform(frame: string | readonly number[], texture: string | readonly number[]): string {
  ...
  // Chromium's fast decimal parser truncates fractional digits. After frame
  // composition, truncating a small homogeneous coefficient can move the page
  // behind its base. Exponent notation preserves the prepared double values.
  return `matrix3d(${product.map(value => value.toExponential(16)).join(',')})`;
}
