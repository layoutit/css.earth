...
export interface PreparedOrbit {closed?:boolean;bodyVertexIndex?:number;displayExtentAu?:number;eccentricity?:number;normal:Vector3;perihelionDirection:Vector3;semiMajorAxisUnits:number;maximumExtentUnits:number;vertices:readonly Vector3[];vertexCount:number;trail:readonly number[];chordBehindTurns:readonly number[];trailSpans:TrailSpans|null;}
...
export interface PreparedSystemBody {id:string;eccentricity?:number;position:Vector3;semiMajorAxisUnits:number;radiusUnits:number;pointPresentation:PreparedPlanetPoint;orbit:(Omit<PreparedOrbit,"semiMajorAxisUnits"|"maximumExtentUnits"> & {labelPresentation:PlanetOrbitLabelPolicy})|null;illumination:PreparedIllumination;}
...
export function projectHeliocentricView(plan: HeliocentricViewPlan, {
  ...
  const segments = projectRing(plan.orbit.vertices, plan.orbit.closed === false ? plan.orbit.trail : trailWeights?.own ?? plan.orbit.trail,
    undefined, false, undefined, plan.orbit.closed !== false);
