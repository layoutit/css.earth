...
const scope = globalThis as unknown as {
  onmessage: (event: MessageEvent<{ plan: PreparedWorldContext } | { id: number; view: WorldContextView }>) => void;
  postMessage(value: unknown): void;
};
let calculate: ReturnType<typeof createWorldContextPlanner>;
scope.onmessage = ({ data }) => {
  try {
    if ('plan' in data) { calculate = createWorldContextPlanner(data.plan); scope.postMessage({ ready: true }); }
