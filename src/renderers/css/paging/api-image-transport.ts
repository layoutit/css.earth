import type { PreparedImage, PreparedPagePlan } from './types.js';
interface ImageReference { api: boolean; decodedBytes: number; bound: number; signature: string; }
interface Member { refs: number; lastUsed: number; idleUntil: number; }
interface ImageEntry extends ImageReference {
  key: string; controller: AbortController; url: string | null; bytes: number | null;
  members: Map<ImageScope, Member>; confirmed: boolean; valid: boolean; expiresAt: number;
  image: Pick<HTMLImageElement, 'src' | 'decode' | 'naturalWidth' | 'naturalHeight'> | null;
  empty: boolean; ready: Promise<string | null>;
}
interface ImageScope {
  members: Map<ImageEntry, Member>; maximumEntries: number; maximumDecodedBytes: number;
  maximumEncodedBytes: number; hits: number; misses: number; evictions: number; destroyed: boolean;
}
export interface ImageScopeOptions { maximumEntries: number; maximumDecodedBytes: number; idleMilliseconds?: number; }
interface TransportOptions {
  fetchImage?: typeof fetch; createImage?: (page: PreparedImage) => NonNullable<ImageEntry['image']>;
  wait?: typeof delay; now?: () => number; schedule?: typeof setTimeout; unschedule?: typeof clearTimeout;
}
export type ApiImageTransport = ReturnType<typeof createApiImageTransport>;
export type ApiImageScope = ReturnType<ApiImageTransport['createScope']>;
export type ApiImageLease = ReturnType<ApiImageScope['acquire']>;
import { isPreparedWmsImage, readWmsImage } from "./wms-image.js";
import { isPreparedWmtsImage } from "./wmts-image.js";
import { isPreparedCityAssetUrl } from "./city-asset-url.js";
import { readPreparedBytes } from "./prepared-json-transport.js";
...
async function untilAborted(promise: Promise<unknown>, signal: AbortSignal) {
...
function imageReference(page: PreparedImage, plan?: PreparedPagePlan): ImageReference {
  ...
  if (!api && !(local || isPreparedCityAssetUrl(plan ?? null, page.url, "page", page.sha256 ?? ""))) throw new Error("Invalid prepared imagery request.");
  const decodedBytes = page.width * page.height * 4;
  const bound = api ? decodedBytes + 65536 : page.bytes;
  if (!Number.isSafeInteger(decodedBytes) || !Number.isSafeInteger(bound) || (bound ?? 0) < 1) throw new Error("Invalid prepared image size.");
  return { api, decodedBytes, bound: bound!, signature: JSON.stringify([page.width, page.height, api ? page.provider?.emptyImage ?? null : [page.sha256, page.bytes]]) };
}

function expiresAt(headers: Headers, now: number) {
...
export function createApiImageTransport({ fetchImage = fetch, createImage = () => new Image(), wait = delay, now = Date.now,
  schedule = setTimeout, unschedule = clearTimeout }: TransportOptions = {}) {
  const entries = new Map<string, ImageEntry>(), owned = new Set<ImageEntry>(), scopes = new Set<ImageScope>();
  let destroyed = false, timer: ReturnType<typeof setTimeout> | null = null, requests = 0, retries = 0, activeRequests = 0, receivedBytes = 0, sharedAcquisitions = 0;
  function remove(entry: ImageEntry) {
  ...
  function totals(scope: ImageScope) {
  ...
  function fit(scope: ImageScope, extraEntries: number, extraDecoded: number, extraEncoded: number, keep?: ImageEntry) {
  ...
  function invalidate(entry: ImageEntry) {
    entry.valid = false; entry.controller.abort();
    if (entries.get(entry.key) === entry) entries.delete(entry.key);
    prune(); arm();
  }
  async function load(entry: ImageEntry, page: PreparedImage): Promise<string | null> {
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        try {
          ...
          const blob = entry.api ? await readWmsImage(response, page) : new Blob([await readPreparedBytes(response, { url: page.url, bytes: entry.bound, sha256: page.sha256! })], { type: "image/webp" });
  ...
  return Object.freeze({
    createScope({ maximumEntries, maximumDecodedBytes, idleMilliseconds = 120000 }: ImageScopeOptions) {
      ...
      const scope: ImageScope = { members: new Map(), maximumEntries, maximumDecodedBytes,
        maximumEncodedBytes: maximumDecodedBytes + maximumEntries * 65536, hits: 0, misses: 0, evictions: 0, destroyed: false };
      scopes.add(scope);
      return Object.freeze({
        acquire(page: PreparedImage, { plan, signal }: { plan?: PreparedPagePlan; signal?: AbortSignal } = {}) {
          ...
          if (!entry) {
            scope.misses++;
            entry = { ...ref, key, controller: new AbortController(), url: null, bytes: null, members: new Map(), confirmed: false, valid: true, expiresAt: 0, image: null, empty: false, ready: Promise.resolve(null) };
          ...
          let released = false, rejectAbort: ((reason: unknown) => void) | null = null;
          ...
          const ready = signal ? Promise.race([resource.ready, new Promise<string | null>((_resolve, reject) => { rejectAbort = reject; signal.addEventListener("abort", abort, { once: true }); })]) : resource.ready;
