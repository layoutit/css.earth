...
export function isPreparedBlockReference(ref: PreparedReference, assetPath = /^\/scenes\/[a-z][a-z0-9-]*\//u.exec(ref?.url ?? "")?.[0]) {
  return ref?.encoding === PREPARED_BLOCK_ENCODING && HASH.test(ref.sha256 ?? "") && HASH.test(ref.decodedSha256 ?? "") &&
    ...
    [ref.bytes, ref.decodedBytes].every(value => Number.isSafeInteger(value) && (value ?? 0) > 0 && (value ?? 0) <= PREPARED_BLOCK_LIMITS.bytes) &&
...
export class PreparedBlockTransferError extends Error {
  constructor(readonly expectedBytes: number, readonly receivedBytes: number) {
    super('Prepared block byte length mismatch.'); this.name = 'PreparedBlockTransferError';
  }
}

async function readBounded(stream: ReadableStream<Uint8Array> | null, expected: number, signal?: AbortSignal, transfer = false) {
...
export async function readPreparedWmtsBlock(response: Response, ref: PreparedReference, signal?: AbortSignal): Promise<PreparedDirectory> {
  ...
  const result = await decodePreparedBlockAsync(decoded, { signal, withEnvelope: true });
  if (Array.isArray(result)) throw new Error("Prepared block envelope is missing.");
  ...
  if (envelope?.schema === "cssearth-city-index@1" && Array.isArray(envelope.nodes)) {
    ...
    if(used.size!==pages.length||external.some(n=>!n.stub||!n.directory||!isPreparedBlockReference(n.directory,assetPath)))throw new Error("Incomplete prepared WMTS subtree.");
