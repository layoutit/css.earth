import type { PreparedPage, PageGroup, PublicationSlot, PublicationLimits } from './types.js';
...
export function selectPagePublication(groups: PageGroup[], slots: PublicationSlot[], limits: PublicationLimits, fallbacks: PageGroup[] = []) {
  ...
  const previous = new Map<string, PageGroup>();
  ...
  const components: Array<{groups: PageGroup[]; previous: PageGroup[]}> = groups.map(group => ({ groups: [group], previous: [] }));
  ...
  const reduction = (component: {groups: PageGroup[]; previous: PageGroup[]}) => component.previous.reduce((sum, group) => sum + group.pages.length, 0) -
    component.groups.reduce((sum, group) => sum + group.pages.length, 0);
  components.sort((a, b) => reduction(b) - reduction(a));
  const apply = (keys: string[], previous: PageGroup[]) => {
...
export function selectPageDemand(groups: PageGroup[], slots: PublicationSlot[], nodes: ReadonlyMap<string, PreparedPage>, limits: PublicationLimits, fallbacks: PageGroup[] = []) {
  const byKey=new Map(slots.filter(slot=>slot.key).map(slot=>[slot.key,slot]));
  ...
  const bytes=(key: string)=>byKey.get(key)?.decodedBytes??nodes.get(key)!.width*nodes.get(key)!.height*4;
...
const related = (a: PageGroup, b: PageGroup) => a.lineage.includes(b.key) || b.lineage.includes(a.key);
const readyGroup = (group: PageGroup, byKey: ReadonlyMap<string | null, PublicationSlot>) => !group.pending && group.pages.every(key => byKey.get(key)?.ready);

export function usefulFallbacks(groups: PageGroup[], slots: PublicationSlot[], fallbacks: PageGroup[]) {
  const byKey = new Map(slots.filter(slot => slot.key).map(slot => [slot.key, slot]));
...
export function selectPageFallbacks(groups: PageGroup[], nodes: ReadonlyMap<string, PreparedPage>, slots: PublicationSlot[], limits: PublicationLimits, visible: (page: PreparedPage) => boolean) {
  const byKey = new Map(slots.filter(slot => slot.key).map(slot => [slot.key, slot]));
  const selected = new Set(groups.map(group => group.key)), candidates = new Map<string, PageGroup>();
