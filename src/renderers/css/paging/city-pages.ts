import type { PreparedPage, PreparedPagePlan, PageMountOptions, PageSelection, PageGroup, PublicationSlot } from './types.js';
import type { ObjectRuntimeView } from '../runtime/object-runtime-types.js';
import type { ApiImageScope, ApiImageTransport, ApiImageLease } from './api-image-transport.js';
...
interface PageSlot extends PublicationSlot {
  leaf: HTMLElement; apiTexture: HTMLElement | null; apiHandle: ApiImageLease | null;
  controller: AbortController | null; generation: number; empty: boolean;
}
import { createPreparedProjectiveTextureLeaf } from "../rendering/prepared-projective-texture-leaf.js";
import { selectCityPages, projectCityPage } from "./city-page-selection.js";
import { createCityIndex } from "./city-index.js";
import { normalizeCityAssetOrigin, isPreparedAssetPath } from "./city-asset-url.js";
import { createApiImageTransport } from "./api-image-transport.js";
import { selectPagePublication, selectPageFallbacks, usefulFallbacks, selectPageDemand } from "./page-publication.js";

export function mountPreparedMapPages({ plan, carrier, system, scene, camera, stage, className, textureClassName, lensIds, own, paintLayer = 0, images = null, onStatus = () => {}, onError = error => { throw error; } }: PageMountOptions) {
  ...
  if(plan.backing && (plan.topology!=="wmts-quadtree@1" || !Array.isArray(plan.backing.roots) || plan.backing.roots.length>plan.poolSize ||
      ...
      !Number.isSafeInteger(plan.backing.rootDecodedBytes) || (plan.backing.rootDecodedBytes ?? 0)<1 || (plan.backing.rootDecodedBytes ?? Infinity)>=plan.index.maximumBytes ||
  ...
  let desired: string[] = [];
  let desiredGroups: PageGroup[] = [];
  let fallbackGroups: PageGroup[] = [];
  let pageGroups = new Map<string, PageGroup>();
  ...
  let index: ReturnType<typeof createCityIndex>, apiImages: ApiImageScope, imageTransport: ApiImageTransport | null = null, observer: ResizeObserver | null = null, shellObserver: MutationObserver | null = null;
  const schedule = () => {
  ...
  function refresh() {
    ...
    const physical = view.projection && requirePhysicalProjection(view.projection);
    const matrix = (physical ? new DOMMatrix(Array.from(physical.eyeFromScene)) : new DOMMatrix(getComputedStyle(scene).transform))
      .multiply(new DOMMatrix(getComputedStyle(system).transform))
      .multiply(new DOMMatrix(getComputedStyle(carrier).transform));
    const scale = physical ? 1 : Number.parseFloat(getComputedStyle(camera).scale);
  ...
  function publishReadyView() {
    if(plan.topology === "wmts-quadtree@1"){
      ...
      for(const slot of slots)if(slot.key && publication.release.includes(slot.key))release(slot);
      for(const slot of slots)if(slot.key && publication.publish.includes(slot.key))publishSlot(slot);
  ...
  function publishSlot(slot: PageSlot) {
  ...
  async function load(slot: PageSlot, page: PreparedPage) {
    ...
    try {
      ...
      if (!blobUrl) throw new Error('Prepared image did not return a decoded resource.');
      publishPreparedPageTexture(slot.leaf, slot.apiTexture, page, blobUrl, loadPlan.rasterScale, paintLayer);
      slot.leaf.dataset.cityPage = page.key;
      slot.ready = true;
      ...
      if (!controller.signal.aborted && !destroyed && generation === slot.generation) {
        ...
        errors.set(page.key, error instanceof Error ? error.message : String(error));
  ...
  return Object.freeze({
    ...
    whenIdle: () => activeLoads === 0 || destroyed ? Promise.resolve() : new Promise<void>(resolve => idleWaiters.add(resolve)),
    replacePlan(next: PreparedPagePlan | null) {
      if (destroyed) return;
