import type { WmtsRasterSource, PreparedPage, PreparedImage } from './types.js';
...
export function requireWmtsRasterSource(value: unknown): WmtsRasterSource {
  const source = value as WmtsRasterSource | undefined;
  let url;
...
export function preparedWmtsRasterUrl(source: WmtsRasterSource, page: PreparedImage) {
  if (typeof page.level !== "number" || !Number.isInteger(page.level) || page.level < 0 || page.level >= source.levels.length ||
      ![page.x, page.y].every(value => typeof value === "number" && Number.isInteger(value) && value >= 0 && value < 2 ** page.level!)) {
...
export function bindPreparedWmtsRaster(page: PreparedPage, source: WmtsRasterSource) {
...
export function isPreparedProviderWmtsImage(page: PreparedImage) {
  if (page?.rasterSource !== "prepared-wmts-raster@1" || page.width !== 256 || page.height !== 256) return false;
  try { const source = requireWmtsRasterSource(page.provider); return page.url === preparedWmtsRasterUrl(source, page); }
  catch { return false; }
}
