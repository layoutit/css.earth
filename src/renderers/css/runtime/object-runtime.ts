...
import { createApiImageTransport } from '../paging/api-image-transport.js';
import { GEOGRAPHIC_OVERVIEW_LIMITS } from '../paging/geographic-lens-contract.js';
import { createGeographicLensRuntime } from '../paging/geographic-lens-runtime.js';
...
import { initialObjectSelection, objectLensAvailable, requireObjectRuntimeDefinition } from "./object-contract.js";
...
export function createObjectRuntime(definition: ObjectRuntimeDefinition, services: Partial<ObjectRuntimeServices> = {}) {
  ...
  return function mountObject(stage: HTMLElement, { onError, onMotionRequest = () => {}, inputSurface, runtimePolicy, mobilePreviewElement = null, diagnostics = false, capabilities = {}, worldFrame, worldContext, externalWorldContext = false, preparedResources, preparedTree, initialWorldCamera, initialProjection }: ObjectMountOptions) {
    ...
    let geographic: ReturnType<typeof createGeographicLensRuntime> | null = null;
    let geographicImages: ReturnType<typeof createApiImageTransport> | null = null;
    let lastGeographicStatus: string | null = null;
    ...
    function geographicStatusKey() {
      const state = geographic?.state();
      return state ? `${state.id}:${state.status}:${state.resolution}` : 'idle';
    }
    function publishControls(state?: Readonly<ObjectSelectionState>) {
      lastGeographicStatus = geographicStatusKey(); controls?.publish(state);
    }
    function publishGeographicStatus() {
      if (!lifetime.disposed && lastGeographicStatus !== geographicStatusKey()) publishControls();
    }
    ...
    const lensState = () => {
      ...
      // Preserve the user's selection in navigation while the previous image
      // remains displayed during acquisition. Reload must keep that intent.
      return { id: overlay?.id ?? state?.desired.lensId ?? initialSelection.lensId,
        ready: Boolean(state?.ready && !state.pending && !state.error &&
          (!overlay?.id || ['ready', 'no-coverage'].includes(overlay.status))) };
    };
    function dispatchAction(action: import('./object-contract.js').ObjectAction): Promise<boolean> {
      if (action.kind === 'lens') {
    ...
    const controller = Object.freeze({ ready, sharedView, ...(destinations ? { destinations: Object.freeze({ ...destinations, ready, lens: lensState,
      selectLens: (id: string) => dispatchAction({ kind: 'lens', id }),
    ...
    async function start() {
      ...
      for (const [paintLayer, layer] of (mounted.pageLayers ?? []).entries()) {
        if (!capabilities.mountPages) throw new TypeError('Prepared pages require an injected runtime capability.');
        const plan = parsePreparedPagePlan(layer.plan, {lensIds: layer.lensIds, geographic: layer.geographic});
        const images = geographicImages!.createScope({
          maximumEntries: plan.poolSize + (layer.geographic ? GEOGRAPHIC_OVERVIEW_LIMITS.images : 0),
          maximumDecodedBytes: plan.maximumDecodedBytes,
        });
        context.own(() => images.destroy());
        const pages = capabilities.mountPages({ ...layer, paintLayer, plan, stage, scene: mounted.sceneElement, camera: mounted.cameraElement,
        ...
        if (layer.geographic) {
          geographic = createGeographicLensRuntime({ pages, images, capacity: plan, surface: mounted.observationSurface,
            objectId: definition.id, getEntity: selectedEntity,
            selectBase: id => id === definition.destinations?.defaultLens ? getSelection().dispatch({kind: 'lens', id}) : Promise.resolve(false),
      ...
      if (diagnostics) publishObjectDiagnostics({ stage, definition, mounted, orbit, cubicSky, heliocentric, selection, controls, resources, playback, lifetime, context, initialSelection, startupDecodedAssets, pageLayers, getCurrentView: () => currentView,
        exploration: { lens: lensState, selectLens: id => dispatchAction({kind: 'lens', id}),
          destinationStats: () => destinations?.stats() ?? null, destination: () => destinations?.state() ?? null,
          destinationCatalog: () => definition.destinations?.catalog ?? null,
          geographicLens: () => geographic?.state() ?? null, geographicSurface: () => geographic?.stats() ?? null,
          geographicImages: () => geographicImages?.stats() ?? null } });
      settled = true;
      resolveReady();
    }
  };
