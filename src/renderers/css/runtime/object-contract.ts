...
export function requireObjectControls(content: ObjectControls, objectId = 'unknown'): ObjectControls {
  ...
  if (names.some(name => !nonempty(name) || ['motion', 'skyContrast', 'heliosphere', 'asteroidBodies', 'asteroidOrbits', 'asteroidLabels'].includes(name)) || new Set(names).size !== names.length ||
