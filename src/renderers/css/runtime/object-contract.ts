import type { GeographicEntity } from '../paging/geographic-types.js';
...
export interface ObjectControls {
  readonly lenses: { readonly geographicCapacity?: number; readonly defaultLens: string; readonly controls: readonly LensControl[]; readonly [key: string]: unknown } | null;
...
export function requireObjectControls(content: ObjectControls, objectId = 'unknown'): ObjectControls {
  ...
  if (names.some(name => !nonempty(name) || ['motion', 'skyContrast', 'heliosphere', 'asteroidOrbits', 'asteroidLabels'].includes(name)) || new Set(names).size !== names.length ||
...
export function objectLensAvailable(controls: ObjectControls, entity: GeographicEntity | null, id: string): boolean {
  ...
  return Boolean(entity.lensIds?.includes(id) && (!lens.entityIds || lens.entityIds.includes(entity.id)));
}
