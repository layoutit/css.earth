import type { PolyTextureLeafGeometry } from '@layoutit/polycss';
type Band = { y: number; height: number };
type LayoutInput = { width: number; height: number; bandCount: number; bands?: readonly Band[]; gutter: number };
type Rect = { x: number; y: number; width: number; height: number };
type PresentationInput = { sourceWidth: number; sourceHeight: number; sourceRect: Rect;
  addressSourceWidth?: number; addressSourceHeight?: number; addressSourceRect?: Rect;
  backgroundPosition: readonly number[]; backgroundSize: readonly number[]; leafWidth: number; leafHeight: number;
  bandCount: number; bands?: readonly Band[]; gutter: number; overscan?: number };
...
export function prepareProjectiveTextureLayer(matrixValue: string, rasterScale = 1) {
...
export function fitProjectiveTextureGeometryToStableLayout(
  ...
  return Object.freeze({
    ...
    backgroundPosition: geometry.backgroundPosition.map(value => value * layoutScale) as [number, number],
    backgroundSize: geometry.backgroundSize.map(value => value * layoutScale) as [number, number],
  });
}
