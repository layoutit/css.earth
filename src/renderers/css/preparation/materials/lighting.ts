...
// One bank of row shards and one billboard atlas, at two texels per presentation pixel.
const density = 2;
export async function prepareLighting(config: RasterRecipe, recipe: LightingRecipe, publicDirectory: string) {
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
        ...
        const file = outputName(recipe.rowOutput).replace('{row}', String(rowIndex).padStart(2, '0'));
    ...
    const bbFile = outputName(recipe.billboardOutput), bbPath = resolve(publicDirectory, bbFile), bbUrl = config.publicBase + bbFile;
    ...
    const bank = { schema: recipe.bankSchema, frameSize, presentationFrameSize: recipe.presentationSize,
    ...
    return { ...recipe.metadata, frameCount: recipe.frameCount, presentationFrameSize: recipe.presentationSize, defaultFrame: recipe.defaultFrame, bank };
}
