import { requireRootEntity, requireObservationSurface } from './geographic.js';
...
export function requireOptionalPresentation(plan: Record<string, unknown>, tree: PreparedTree, controls: ObjectControls): void {
  ...
  if (plan.pageLayers !== undefined) {
    ...
    if (layers.filter(input => record(input, 'page layer').geographic === true).length > 1) fail('only one geographic pool may mount');
    for (const input of layers) {
      const layer = record(input, 'page layer', ['id', 'plan', 'carrier', 'system', 'className', 'textureClassName', 'lensIds', 'geographic']);
      ...
      if (layer.geographic !== undefined && (layer.geographic !== true || !plan.destinations || !controls.lenses?.geographicCapacity)) fail('geographic pool requires destinations and retained controls');
      parsePreparedPagePlan(layer.plan, { lensIds: lenses, geographic: layer.geographic === true });
    }
  }
  if (plan.observationSurface !== undefined) requireObservationSurface(plan.observationSurface, plan, tree);
  if (plan.destinations !== undefined) {
    const destinations = record(plan.destinations, 'destinations', ['catalog', 'defaultLens', 'statuses', 'rootEntity']), catalog = record(destinations.catalog, 'destination catalog');
    ...
    if (destinations.rootEntity !== undefined) requireRootEntity(destinations.rootEntity, lensIds, /^\/scenes\/[a-z][a-z0-9-]*\//u.exec(String(catalog.url))?.[0] ?? 'INVALID');
