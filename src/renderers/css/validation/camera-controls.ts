import { GEOGRAPHIC_LENS_CAPACITY } from '../paging/geographic-types.js';
...
export function requireControls(value: unknown): asserts value is ObjectControls {
  ...
  if (controls.lenses !== null) {
    ...
    if (lenses.geographicCapacity !== undefined && integer(lenses.geographicCapacity, 'geographic capacity') > GEOGRAPHIC_LENS_CAPACITY) fail('geographic controls exceed capacity');
  ...
  if (controls.settings !== null) {
    ...
    for (const input of array(settings.controls, 'setting controls')) {
      ...
      if (['motion', 'skyContrast', 'heliosphere'].includes(name)) fail('setting belongs to shared shell');
...
export function requireCamera(value: unknown): asserts value is CameraPlan {
  ...
  if (camera.drag !== undefined) choice(record(camera.drag, 'drag').model, ['screen-axis-tumble', 'surface-grab'], 'drag model');
