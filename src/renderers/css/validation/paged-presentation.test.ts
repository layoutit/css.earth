...
const original: unknown = JSON.parse(await readFile(new URL('../../../planets/earth/prepared/runtime.json', import.meta.url), 'utf8'));
...
test('the external runtime boundary preserves both actual Earth page plans and metadata stubs', () => {
  ...
  for (const page of parsed.pageLayers ?? []) {
    assert.equal(parsePreparedPagePlan(page.plan, { lensIds: page.lensIds, geographic: page.geographic }), page.plan);
  }
...
test('internal page lens overrides cannot silently disagree with the validated binding', () => {
  for (const lensIds of [[], ['not-a-lens'], ['normal', 'normal'], ['normal', 'buenos-aires-noise', 'elevation']]) {
