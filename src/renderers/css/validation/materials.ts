...
export function requireMaterials(value: unknown, tree: PreparedTree, resources: ReadonlySet<string>): asserts value is readonly PreparedMaterialTrack[] {
  ...
  for (const input of tracks) {
    const track = record(input, 'material', ['id', 'target', 'frame', 'defaultFrame', 'banks', 'rotation', 'frameAttribute', 'modeAttribute', 'quoted', 'farBank', 'maximumZoom']);
    if (track.maximumZoom !== undefined) positive(track.maximumZoom, 'material maximum zoom');
