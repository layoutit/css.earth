...
Reproduce the added registration check with `python tools/objects/terrestrial-layers/verify-catalog-camera.py src/planets/ida/source OUTPUT --frame 202560500 --profile reference/registration-202560500.json`. The prepared observation report records each frame's share of the displayed surface from equal-area samples (`areaCoverage`), and the atlas observation index names the photograph behind every texel.
