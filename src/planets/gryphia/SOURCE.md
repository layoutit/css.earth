# (496) Gryphia: sources and interpretation

## Selected shape

[DAMIT model 6121](https://damit.cuni.cz/projects/damit/asteroid_models/view/6121), version 2022-02-14, from Hanuš et al. (2021), is a convex lightcurve inversion mesh. The original 1022 vertices and 2040 triangular faces are preserved as the source input. Published convex shape approximation assuming principal-axis rotation. Gryphia is a confirmed tumbler; its full changing orientation is not modeled. The displayed fixed attitude is illustrative.
...
DAMIT metadata report J2000 ecliptic pole λ=141°, β=26°, and rounded sidereal period 1049 h. The original IAUspin file uses equatorial pole α=154°, δ=39°, dW/dt=8.23659°/day, W0=295.4° at JD 2451545.0. These are different frame conventions. These values describe the source's principal-axis assumption, and are retained only as provenance for this body. IAUspin supplies an independent frame/rate consistency check; its period agrees within the printed precision of the model record. The presentation uses a fixed illustrative attitude, with zero propagated spin; it does not use that source pole or phase as a true current orientation. Model longitude zero is an inversion/display convention, not an observed landmark.

Gryphia is a confirmed tumbler. Pilcher, Franco & Pravec (2017), MPB 44, pp. 270–274, found a principal period of 1072 ± 2 h and an indeterminate second period. The 1049 h DAMIT model assumes principal-axis rotation. Preserve its geometry as a qualified approximation and its spin files as provenance; the presentation must use the existing fixed illustrative attitude with zero propagated spin. The full changing attitude is not reconstructed.
