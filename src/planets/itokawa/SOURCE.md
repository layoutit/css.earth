...
The navigation context image uses the same three-dimensional source-surface sampler as the Elevation atlas. Regenerate a candidate without changing pins using `node tools/objects/terrestrial-layers/prepare-source-surface-context.mjs itokawa output/itokawa-context.png`; review it, then update only the generated context image and its generated-intermediate digest. Ordinary object preparation reproduces and verifies those reviewed bytes.
