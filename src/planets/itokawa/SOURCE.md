...
The navigation context image uses the same three-dimensional source-surface sampler as the Elevation atlas. Regenerate a candidate without changing pins using `node tools/objects/terrestrial-layers/prepare-source-surface-context.mjs itokawa output/itokawa-context.png`; review it, then update the generated context image and its generated-intermediate digest. The navigation recipe also pins those same context bytes: update its source digest, refresh the navigation document digest in the source manifest and the navigation-source digest in `object.json`, then run `pnpm prepare:navigation` to regenerate the shared marker atlases and resolved context marker. Ordinary object preparation reproduces and verifies the reviewed context bytes.
