...
| --- | --- |
| Shape and spin | [DAMIT 12740](https://damit.cuni.cz/projects/damit/asteroid_models/view/12740) |
| Physical scale | [Calibration and uncertainty](source/reference/calibration.json) |

Selected [DAMIT model 12740](https://damit.cuni.cz/projects/damit/asteroid_models/view/12740), version 2022-11-29, from Ďurech & Hanuš.
...
The [iau validation record](../../../docs/asteroids-size-calibration-validation.json) contains source, scale, atlas, installation and browser results for its recorded files and revision.
...
- Diameter uncertainty: ±0.917 km. Published statistical/Monte Carlo fit error; excludes the additional systematic term below.
...
The [original vertex/facet table](https://damit.cuni.cz/projects/damit/stored_files/open/106228/shape.txt) contains 574 vertices and 1144 triangular facets. All current DAMIT matches for this number were inspected; none supplies a calibrated diameter. The selected file's numerical volume is 0.999999977699 source units cubed, consistent with the unit-volume normalization documented by DAMIT.
...
D_source = cbrt(6 * V / π)
metresPerSourceUnit = 1000 * 4.242 / D_source
                    = 3419.03495989
referenceRadiusKm = 4.242 / 2 = 2.121
```
...
The published diameter uses only W3 thermal detections and an assumed beaming parameter; its fit error is already about 22%.
...
[Original spin.txt](https://damit.cuni.cz/projects/damit/generated_files/open/AsteroidModel/12740/spin.txt) specifies the J2000 ecliptic pole λ = 31°, β = -38°, a sidereal period of 4.47862 hours, reference epoch JD 2457081, and reference angle φ₀ = 0°. The original DAMIT co-rotating Cartesian frame is preserved: +Z is its north pole and +X defines the model meridian. [DAMIT's frame and file documentation](https://damit.cuni.cz/pages/documentation) defines the body-to-ecliptic transform.
...
| Source | Disposition |
| --- | --- |
| [Current DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/12740) | Include original shape and published spin. The examined release supplies no registered optical map, geological map or resolved albedo field. |
| [Thermal sizing data](https://irsa.ipac.caltech.edu/TAP/sync?REQUEST=doQuery&LANG=ADQL&FORMAT=csv&QUERY=select+%2A+from+neowisesbpropv2+where+asteroid_number%3D5000) | Include a global size estimate and uncertainty only. Unresolved thermal flux/albedo measurements cannot supply spatial texels or a temperature map. |
| [Other DAMIT matches](https://damit.cuni.cz/?q=5000) | Inspected for calibrated replacements; retain the selected solution and disclose available alternatives. |
