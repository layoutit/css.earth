...
The published 2012 model gives three J2000 axes at 2010-11-04T13:59:47.7. Its rounded directions are orthogonalized once to a proper rotation. Hartley tumbles; the short observation sequence is not a full tumbling reconstruction, and the cartographic long axis is not a spin pole. Only pointing is fitted. Registration is bounded at 20 m RMS / 40 m maximum. The HRI pixel scale of 3.9-4.5 m is detector sampling, not a claim that restoration resolves terrain at that size. The combined mosaic retains the original acquisition shading; each image still passes its own camera and coverage checks.

## Combined EPOXI mosaic

The MRI and restored HRI products are clear-filter visible images, each labelled CLEAR1 with a 650 nm central wavelength and radiance units W/(m² sr µm). These labels do not establish identical spectral response between instruments; the overlap measurements below check their suitability for this grayscale display. Their short observation interval and separately checked cameras support one mixed-instrument photographic mosaic. This is a visual composite, not recovered albedo or a simultaneous exposure. HRI restoration can leave grain and ringing.

The existing finest-sampling selection gives priority to HRI only where its detector, shape and camera correspondence qualify. MRI extends the footprint. All ten image-overlap pairs pass the existing matching limits; fitted gains remain between 1.00 and 1.12. With the same 32 area-weighted samples per retained triangle, accepted coverage rises from MRI’s 43.34% and HRI’s 32.31% to 55.75% in the combined view. No acceptance limits or geometry were changed.
