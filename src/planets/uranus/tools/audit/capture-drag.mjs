...
if (![1, 2].includes(deviceScaleFactor)) {
  throw new RangeError("Uranus drag capture accepts DPR 1 or DPR 2.");
}
const timestamp = new Date().toISOString().replace(/[:.]/gu, "-");
const outputRoot = resolve(
  "output/playwright",
  `uranus-drag-${timestamp}-dpr${deviceScaleFactor}`,
);
...
try {
  ...
  async function capture(label) {
    const sample = await page.evaluate(() => ({
      ...
      cameraStats: window.__uranus.camera.stats(),
  ...
  await page.evaluate(() => window.__uranus.camera.setState({
    controlPitch: 34.23,
    zoom: 1,
  }));
  ...
  const report = Object.freeze({
    schema: "cssuranus-drag-capture@1",
  ...
  if (problems.length > 0 || externalRequests.length > 0) {
    throw new Error(`Uranus drag capture is not clean: ${JSON.stringify(report)}`);
  }
