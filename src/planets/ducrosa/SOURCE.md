# (400) Ducrosa: source and interpretation
...
Hanuš, Delbo, Ďurech and Alí-Lagoa (2018), Table A.3, report a **volume-equivalent diameter of 34.1 ± 0.5 km** from varied-shape thermophysical modelling of WISE observations. The reported spread accounts for variations in the optical shape and spin solution. The physical scale is transferred to the nominal archived model from the same published shape/pole family; this does not claim that the nominal mesh is identical to every thermophysical realization.
...
DAMIT 352 has pole (328°, 56°) and period 6.86788 h, matching the selected thermophysical row. The alternate mirror model 353 has its own 35.2 ± 1.1 km fit. Neither pole is declared uniquely correct and the two calibrations are not mixed. The nominal model 352 receives its own 34.1 ± 0.5 km size.
...
- [DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/352) — selected original mesh and spin metadata.
- [Original shape table](https://damit.cuni.cz/projects/damit/stored_files/open/1322/shape.txt) — counted XYZ vertices and one-based triangle indices.
...
- [Hanuš et al. (2011) ](https://damit.cuni.cz/projects/damit/references/view/141)
