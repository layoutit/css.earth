...
The [catalogue](https://pdssbn.astro.umd.edu/holdings/sdu-c-navcam-5-wild2-shape-model-v2.1/catalog/dataset.cat) reports roughly 50% coverage, 50 m horizontal resolution and 6 m vertical precision. The source contains eight edge-connected patches. Two plates need winding reversal to make their shared edges consistent; no positions change. Boundary-locked simplification retains 996 plates and all 348 source boundary edges. The library error estimate is about 49.46 m, not a guaranteed maximum distance or observation uncertainty.
