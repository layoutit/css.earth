...
const descriptor = { url: "/scenes/earth/earth-places.pack", bytes: packed.length, encoding: "gzip",
  sha256: createHash("sha256").update(packed).digest("hex"), decodedBytes: bytes.length,
  decodedSha256: createHash("sha256").update(bytes).digest("hex"), count: places.length,
...
await writeFile(`${EARTH_PUBLIC_ROOT}/earth-places.pack`, packed);
...
console.log(`Prepared ${places.length} place cards (${packed.length} transport bytes, ${bytes.length} decoded bytes).`);
