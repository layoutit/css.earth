...
import { assembleCityCoveragePlan, publishedCoveragePath } from "./city/published-coverage.mjs";

const root = resolve(import.meta.dirname,"../../../..");
const source = JSON.parse(await readFile(new URL("../source/city/manifest.json",import.meta.url),"utf8"));
const {pin,entries} = await readWorldCoverCatalog();
const expected = new Map(cityCoverageRoots().map(face => {
  const planned=expectedGlobalCityFace(face,[...planCityCoverage(PREPARED_EARTH_SCENE,entries,[face])]);
...
const cache=resolve(root,".local/earth-city-coverage-check");
...
await writeFile(publishedCoveragePath,bytes);
...
const plan=assembleCityCoveragePlan(PREPARED_EARTH_CITY_PAGES,snapshot);
await writeFile(new URL("../runtime/preparedCityPages.mjs",import.meta.url),
  `// Generated offline from pinned WorldCover sources and published coverage.\nexport const PREPARED_EARTH_CITY_PAGES = Object.freeze(${JSON.stringify(plan)});\n`);
