...
import { WORLDCOVER_BUCKET, WORLDCOVER_PREFIX, parseWorldCoverInventory,
  worldCoverTileBounds } from "./city/worldcover-catalog.mjs";

const root=resolve(import.meta.dirname,"../../../..");
const output=resolve(root,".local/earth-city-source/catalog");
const snapshotPath=resolve(output,"worldcover-rgbnir-2021.json.gz");
const sourceDirectory=resolve(import.meta.dirname,"../source/city");
const pin=JSON.parse(await readFile(resolve(sourceDirectory,"catalog-pin.json"),"utf8"));
...
const snapshot={schema:"cssearth-worldcover-inventory@1",dataset:"esa-worldcover-rgbnir-2021-v200",
...
if(process.argv.includes("--write-source-snapshot"))await writeFile(resolve(sourceDirectory,pin.path),bytes);
