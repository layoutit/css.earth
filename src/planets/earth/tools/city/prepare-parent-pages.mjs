...
import { CITY_PAGE_PIXELS, CITY_PAGE_GUTTER, pageKey, prepareCityPageGeometry } from "./page-geometry.mjs";
import { resamplePageRgba } from "./resample-page.mjs";
...
export function assembleParentCore(children, size = side) {
...
export function preparedCoverageCorners(children) {
...
export function copyCoreIntoGutter(output,core,dx,dy,size=side,padding=gutter) {
...
export async function writeCityCore(path, pixels, page) {
...
export async function prepareCityParentPages(seeds, directory, scene, publish) {
  ...
  for (let level = highestLevel - 1; level >= 0; level--) {
    ...
    const levelSources = new Map([...parents,...seeds.filter(page=>page.level===level).map(page=>[page.key,page])]);
