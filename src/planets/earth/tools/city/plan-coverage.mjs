...
import { childAddresses, pageBounds, pageKey, prepareCityPageGeometry,
  CITY_PAGE_LAST_LEVEL, CITY_POLAR_PAGE_LAST_LEVEL } from './page-geometry.mjs';
import { sourceTilesForBounds } from './worldcover-catalog.mjs';
import { citySourceWindow, CITY_SOURCE_WINDOW_MAX_PIXELS } from './worldcover-source.mjs';

export function cityCoverageRoots() {
  return Array.from({length:16},(_,y)=>Array.from({length:y===0||y===15?1:32},(_,x)=>({level:0,x,y}))).flat();
}

function outsideCap(bounds) {
  if(bounds.projection!=='polar')return false;
  const nearest=(lo,hi)=>lo>.5?2*lo-1:hi<.5?2*hi-1:0;
  return nearest(bounds.u0,bounds.u1)**2+nearest(bounds.v0,bounds.v1)**2>=1;
}

export function* planCityCoverage(scene,catalog,roots=cityCoverageRoots()) {
