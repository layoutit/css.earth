...
import { readWorldCoverCatalog, sourceTilesForBounds, worldCoverSourceEntry,
  worldCoverTileBounds } from './worldcover-catalog.mjs';
...
export async function openWorldCoverSource(entry, cacheRoot,
    ...
    async request({ headers, signal }) {
      ...
      try {
        [bytes, metadata] = await Promise.all([
          readFile(`${path}.bin`),
          readFile(`${path}.json`, "utf8").then(JSON.parse),
        ]);
        ...
        if (active >= 4) await new Promise((resume) => waiting.push(resume));
        active += 1;
        try {
  ...
  const tiff = await fromCustomClient(client, { blockSize: 65536, cacheSize: 64 });
  ...
  if (image.getWidth() !== 12000 || image.getHeight() !== 12000 ||
      image.getSamplesPerPixel() !== 4 || geo.GeographicTypeGeoKey !== 4326 ||
      ...
      !metadata.license.startsWith("CC-BY 4.0")) {
    await tiff.close();
...
async function fetchPinnedWorldCoverRange(entry, range, signal, maximumLength,
...
export function citySourceWindow(bounds) {
...
function tileWindow(tile,bounds,grid) {
...
export function validateWorldCoverRegionSources(region,bounds,catalog) {
  const coverage=sourceTilesForBounds(bounds,catalog);
  const entries=region.sources??[region];
  ...
  if(!Array.isArray(absent)||JSON.stringify([...absent].sort())!==JSON.stringify(coverage.unavailable.sort())) {
...
export async function readWorldCoverRegion(region, bounds, color, cacheRoot, options = {}) {
  ...
  const catalog=await (catalogPromise??=readWorldCoverCatalog());
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!region.sources) singleSourcePoint=(longitude,latitude)=>[
  ...
  const provenance = region.sources ? {...region,width,height,nodataPixels,sources:proofs} : proofs[0];
  ...
  return {rgba,width,height,provenance,receivedBytes,
    sourcePoint:singleSourcePoint??((longitude,latitude)=>[longitude*pixelsPerDegree-grid[0],-latitude*pixelsPerDegree-grid[1]])};
}
