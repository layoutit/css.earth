...
export async function readPublishedCoverage() {
  const bytes = await readFile(publishedCoveragePath).catch(error => error.code === "ENOENT" ? null : Promise.reject(error));
...
export function assembleCityCoveragePlan(base, snapshot) {
