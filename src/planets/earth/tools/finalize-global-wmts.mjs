...
export async function finalizeGlobalWmts(directory,{verify=true}={}){
  ...
  for(const [path,expected]of Object.entries(inputs.hashes))if(hashBytes(await readFile(new URL(`../${path}`,import.meta.url)))!==expected)throw new Error(`Preparation input changed: ${path}`);
  const hasTile=coverageLookup(levels),regions=[],stubs=new Map(),coarse=[],roots=[];
  for(const band of levels.find(l=>l.zoom===8).bands)for(let y=band.y0;y<band.y1;y++)for(const [a,b]of band.ranges)for(let x=a;x<b;x++){
    const name=`8-${x}-${y}`,saved=JSON.parse(await readFile(`${directory}/${name}.json`,"utf8"));
  ...
  for(const band of levels[0].bands)for(let y=band.y0;y<band.y1;y++)for(const [a,b]of band.ranges)for(let x=a;x<b;x++){
    ...
    try{section=prepareTreeSection(address,7,scene,hasTile,child=>stubs.get(tileKey(child)),dataset);}
      ...
      if(!error.message.startsWith("Empty prepared WMTS section"))throw error;
      const nodes=[],external=[];
      const visit=address=>{
    ...
    const filename=`5-${x}-${y}.pack`,ref={...section.ref,url:`/scenes/earth/wmts-${version}/${filename}`,offset:0};await writeFile(`${directory}/${filename}`,section.bytes);
...
if(process.argv[1]===new URL(import.meta.url).pathname){
  const directory=process.argv[2];if(!directory)throw new Error("A prepared global directory is required.");
  const result=await finalizeGlobalWmts(directory);console.log(JSON.stringify({...result,roots:result.roots.length,files:result.files.length}));
}
