...
if (pinned && !process.argv.includes("--reacquire")) {
  let bytes;
...
try {
  ...
  await mkdir(EARTH_STAGING_ROOT, { recursive: true });
  const temporary = new URL(path + ".partial", source);
  await writeFile(temporary, bytes); await rename(temporary, new URL(path, source));
  await writeFile(`${EARTH_STAGING_ROOT}/administrative-acquisition.json`, JSON.stringify(receipt, null, 2));
