...
export function mountEarthCityPages({ plan, carrier, system, scene, camera, stage }) {
  ...
  const slots = Array.from({ length: plan.poolSize }, () => {
    const leaf = createPreparedProjectiveTextureLeaf({
      tag: "s", className: "earth-city-page",
    ...
    if(plan.pageTemplate === "clipped-projective"){
      ...
      apiTexture.className="earth-api-texture";
    ...
    return { leaf, texture: leaf.firstElementChild, apiTexture, image: new Image(),
  ...
  let errors = [];
  const schedule = () => {
    if (!destroyed && pendingFrame === null) pendingFrame = requestAnimationFrame(refresh);
  };
  const index = createCityIndex(plan, schedule);
  const apiImages = createApiImageTransport();
  ...
  async function load(slot, page) {
    ...
    try {
      ...
      if(api){
        ...
        const localNoise=page.rasterSource === "prepared-noise@1" && plan.dataset === "buenos-aires-noise-2025" &&
          /^[a-f0-9]{64}$/u.test(page.sha256??"") && /^\/scenes\/earth\/earth-noise-day-\d-\d-[a-f0-9]{16}\.webp$/u.test(page.url) && page.url.endsWith(`-${page.sha256.slice(0,16)}.webp`);
        if (!localNoise && !isPreparedCityAssetUrl(plan, page.url, "page", page.sha256)) {
        ...
        if(localNoise){
          ...
          if(digest!==page.sha256)throw new Error("Prepared noise image hash mismatch.");
        }
  ...
  return Object.freeze({
    ...
    setLens(lens) {
      enabled = (plan.lensIds??["normal", "buenos-aires-noise"]).includes(lens.id);
      if (!enabled) {
        desired = [];
        desiredPages.clear();
        index.update([]);
