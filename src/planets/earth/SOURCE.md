...
The normal, topography and night-light lenses each use 49 bounded affine surface strips plus a pole atlas, selected once independently of DPR. Preparation folds the projective texture warp into the accepted seven-page encoding layout, then decodes those pages offline and losslessly encodes individual shelf rows. Every visible RGB and alpha byte is verified against that encoding. The full-width strips preserve source density, geometry and texel phase while bounding each Chrome image decode to at most 4096 × 712 pixels for the current scene. This trades larger encoded delivery for smaller decode work units; the atlas density and camera are unchanged. `prepare-assets.mjs --atlases-only` regenerates the three exterior and two cutaway atlas banks without preparing materials or global geometry. The noise lens reuses the normal surface bank and adds its own retained transparent pages. The city geometry keeps a separate prepared geographic frame for each accepted Earth face; all 450 frames remained byte-identical during the PR #2 integration. `source/city/geographic-rebind.json` records the migration and full pack-hash verification.
