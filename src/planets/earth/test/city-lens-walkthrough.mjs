...
try{
  ...
  const settle=async noise=>{
    ...
    await page.waitForFunction(noise=>{
      const s=(noise?window.__earth.noisePages:window.__earth.cityPages).stats();
  ...
  report.final=await page.evaluate(()=>({city:window.__earth.cityPages.stats(),noise:window.__earth.noisePages.stats(),
