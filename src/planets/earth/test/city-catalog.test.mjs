...
import { parseWorldCoverInventory, readWorldCoverCatalog, sourceTilesForBounds,
  worldCoverTileBounds, worldCoverSourceEntry } from "../tools/city/worldcover-catalog.mjs";
import { cityCoverageRoots, planCityCoverage } from '../tools/city/plan-coverage.mjs';
import { citySourceWindow, CITY_SOURCE_WINDOW_MAX_PIXELS, validateWorldCoverRegionSources } from '../tools/city/worldcover-source.mjs';
...
test("source lookup wraps the antimeridian and distinguishes absent tiles from imagery",async()=>{
  const {entries}=await readWorldCoverCatalog();
...
test('global face receipts bind resumable R2 publication to the pinned catalog',()=>{
  const source={dataset:'example-dataset',delivery:{bucket:'cssearth-assets',
    assetOrigin:'https://earth-assets.lowpoly.cc'}};
