...
export async function summarizeEntityTrace(events, metrics, { output, prefix, network = [] }) {
  ...
  const phases = (metrics?.phases ?? []).map(phase => {
    ...
    return { name: phase.name, wallMs: rounded(phase.duration), frameP95: rounded(percentile(frames, .95)), frameMax: rounded(Math.max(0, ...frames)), longTaskMax: rounded(Math.max(0, ...longTasks)),
...
export async function writeEntityPerformanceReview(report) {
  ...
  const row = phase => `<tr><td>${escape(phase.name)}</td><td>${phase.wallMs}</td><td>${phase.frameP95}</td><td class="${phase.frameMax > 100 ? "bad" : ""}">${phase.frameMax}</td><td>${phase.longTaskMax}</td><td>${phase.overlappingRequests.length}</td></tr>`;
  const runs = report.runs.filter(run => run.summary).map(run => `<section><h2>${escape(run.label)}</h2><table><thead><tr><th>Scenario</th><th>Total ms</th><th>Frame p95</th><th>Worst frame</th><th>Longest task</th><th>Requests overlapping</th></tr></thead><tbody>${run.summary.phases.map(row).join("")}</tbody></table>
    <video controls preload="none" src="${escape(run.video?.split("/").at(-1) ?? "")}"></video>
