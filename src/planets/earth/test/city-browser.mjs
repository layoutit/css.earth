...
try {
  for (const dpr of densities) {
    ...
    try {
      if(localFixture){
        await page.route(/\/runtime\/preparedCityPages\.mjs(?:\?.*)?$/,route=>route.fulfill({contentType:'text/javascript',
          body:`export const PREPARED_EARTH_CITY_PAGES=Object.freeze(${JSON.stringify(fixture.plan)});`}));
