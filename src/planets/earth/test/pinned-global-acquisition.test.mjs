...
import { acquirePinnedGlobalWmts } from "../tools/acquire-pinned-global-wmts.mjs";
...
const actualManifest = JSON.parse(await readFile(new URL("../source/manifest.json", import.meta.url)));
const catalogPin = await readFile(new URL("../source/city/catalog-pin.json", import.meta.url));
const catalogBytes = await readFile(new URL("../source/city/worldcover-rgbnir-2021.json.gz", import.meta.url));
const actualContent = JSON.parse(await readFile(new URL("../source/city/manifest.json", import.meta.url)));
...
test("an empty checkout acquires exact pinned packs with bounded concurrency and resumes without network", async t => {
  const f = await fixture(t), report = await acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, fetcher: f.fetcher, concurrency: 2 });
  ...
  for (const verifyOnly of [true, false]) {
    const reused = await acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, verifyOnly, fetcher: noNetwork });
...
test("verify-only rejects missing, corrupt, wrong-size and other-version packs without writes or requests", async t => {
  for (const kind of ["missing", "corrupt", "wrong-size", "other-version"]) {
    ...
    await assert.rejects(acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, verifyOnly: true, fetcher: f.fetcher }));
...
test("invalid downloads cannot replace a corrupt existing pack or leave a partial file", async t => {
  for (const kind of ["wrong-hash", "wrong-size", "oversized-body", "http-error", "redirect"]) {
    ...
    await assert.rejects(acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, fetcher: f.fetcher, concurrency: 2 }));
...
test("a corrupt pack remains intact until its complete valid replacement is ready", async t => {
  ...
  const pending = acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, fetcher: f.fetcher });
...
test("source drift, invalid versions and paths fail before any acquisition", async t => {
  for (const kind of ["untrusted-bytes", "invalid-version", "wrong-catalog", "wrong-dataset", "unsafe-prefix", "unsafe-origin"]) {
    ...
    await assert.rejects(acquirePinnedGlobalWmts({ projectRoot: f.projectRoot, fetcher: f.fetcher }));
    assert.equal(f.requests.length, 0);
