...
import { encodePreparedBlock, packWmtsRecords } from "../tools/city/encode-prepared-block.mjs";
import { createPreparedBlockDecoder, decodePreparedBlock, decodePreparedBlockAsync, restoreWmtsRecords } from "../runtime/prepared-block.mjs";
...
import { isPreparedBlockReference, readPreparedWmtsBlock } from "../runtime/prepared-block-transport.mjs";
import { createCityIndex } from "../runtime/city-index.mjs";
import { prepareWmtsCoverage } from "../tools/city/wmts-coverage.mjs";
import { wmtsLatitude } from "../tools/city/wmts-page-geometry.mjs";
import { worldCoverTileBounds } from "../tools/city/worldcover-catalog.mjs";
...
test("content-addressed spatial blocks verify both compressed and expanded bytes before restoring topology", async () => {
  ...
  const blocks = prepareWmtsBlocks(pages, "test-dataset");
  const recovered = [];
...
test("the shared index loads compressed blocks and budgets expanded metadata before fetching", async () => {
  ...
  const { roots, files } = prepareWmtsBlocks(pages, "test-dataset");
  let changed; const ready = new Promise(resolve => { changed = resolve; });
  const plan = { dataset: "test-dataset", roots, index: { maximumDirectories: 8, maximumBytes: 3 * 1024 * 1024, maximumDirectoryBytes: 131072, maximumConcurrentLoads: 3 } };
