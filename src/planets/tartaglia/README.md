...
| --- | --- |
| Shape and spin | [DAMIT 15683](https://damit.cuni.cz/projects/damit/asteroid_models/view/15683) |
| Physical scale | [Calibration and uncertainty](source/reference/calibration.json) |

Selected [DAMIT model 15683](https://damit.cuni.cz/projects/damit/asteroid_models/view/15683), version 2022-11-29, from Ďurech & Hanuš.

The adopted diameter is **13.584 km** from [Masiero et al. (2011), ApJ 741, 68](https://doi.org/10.1088/0004-637X/741/2/68); [original measurement data](https://irsa.ipac.caltech.edu/TAP/sync?REQUEST=doQuery&LANG=ADQL&FORMAT=csv&QUERY=select+%2A+from+neowisesbpropv2+where+asteroid_number%3D8888). Its quantity is **NEATM effective spherical diameter at the observing geometry**.

## Evidence

The [tartaglia validation record](https://github.com/layoutit/cssEarth/blob/cc01831f595e0b73ab6699d6235cf7b466f76cfc/docs/asteroids-size-calibration-validation.json) contains source, scale, atlas, installation and browser results for its recorded files and revision.
...
- Diameter uncertainty: ±0.067 km. Published statistical/Monte Carlo fit error; excludes the additional systematic term below.
...
The [original vertex/facet table](https://damit.cuni.cz/projects/damit/stored_files/open/123076/shape.txt) contains 574 vertices and 1144 triangular facets. All current DAMIT matches for this number were inspected; none supplies a calibrated diameter. The selected file's numerical volume is 1.0000001375 source units cubed, consistent with the unit-volume normalization documented by DAMIT.
...
D_source = cbrt(6 * V / π)
metresPerSourceUnit = 1000 * 13.584 / D_source
                    = 10948.6488499
referenceRadiusKm = 13.584 / 2 = 6.792
```
...
| --- | --- | --- | --- |
| Mas12 | 2455580.3281310 | 12.779 | 7.991 |
| Nug16 | 2457150.4707264 | 10.041 | 2.595 |
| Nug15 | 2456986.8843095 | 11.24 | 3.546 |
...
[Original spin.txt](https://damit.cuni.cz/projects/damit/generated_files/open/AsteroidModel/15683/spin.txt) specifies the J2000 ecliptic pole λ = 284°, β = 88°, a sidereal period of 7.2234 hours, reference epoch JD 2456922, and reference angle φ₀ = 0°. The original DAMIT co-rotating Cartesian frame is preserved: +Z is its north pole and +X defines the model meridian. [DAMIT's frame and file documentation](https://damit.cuni.cz/pages/documentation) defines the body-to-ecliptic transform.
...
| Source | Disposition |
| --- | --- |
| [Current DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/15683) | Include original shape and published spin. The examined release supplies no registered optical map, geological map or resolved albedo field. |
| [Thermal sizing data](https://irsa.ipac.caltech.edu/TAP/sync?REQUEST=doQuery&LANG=ADQL&FORMAT=csv&QUERY=select+%2A+from+neowisesbpropv2+where+asteroid_number%3D8888) | Include a global size estimate and uncertainty only. Unresolved thermal flux/albedo measurements cannot supply spatial texels or a temperature map. |
| [Other DAMIT matches](https://damit.cuni.cz/?q=8888) | Inspected for calibrated replacements; retain the selected solution and disclose available alternatives. |
