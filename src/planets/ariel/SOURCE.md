...
Ariel uses Paul Schenk's September 2020 [Uranian Satellites — Global Mosaics and DEMs](https://repository.hou.usra.edu/handle/20.500.11753/1687), based on Voyager 2 images and revised cartographic control. The original [author README](https://repository.hou.usra.edu/bitstreams/00528589-53e3-496b-ac5d-b6d86fe527c9/download) is retained in `source/observations/aaReadMe_uranian_MAP_DEM.txt`.
...
The DEM's numeric extrema are approximately −7.024 and +5.796 km. Its displayed scale is −8 to +6 km. Northwest cartographic lighting uses a unit direction (−0.5 east, +0.5 north, +0.7071 up), ambient 0.25, and the actual 1000 m/km height conversion without height exaggeration. The DEM contains sparse curved limb-profile tracks outside the denser image-derived terrain coverage; those are real source-valid samples, not continuous regional coverage. They remain visible without filling their surroundings. Kachina Chasmata's Gazetteer centre has mosaic imagery but no valid DEM sample, which the numeric regression preserves.
...
Physical/orbital values come from the vendored astronomy package: JPL satellite elements and IAU/NAIF Ariel rotation at the shared epoch. [NASA's Ariel overview](https://science.nasa.gov/uranus/moons/ariel/) supplies editorial and discovery facts. Ariel has its own standalone route and generic adapter; it does not add an embedded scene to Uranus or a body-specific controller. No atmospheric shell is supported.

Restore sources with `node tools/objects/dist/operations.js acquire ariel`; verify with `pnpm acquire:planets -- --verify-only --object=ariel`. Prepare with `node tools/objects/dist/prepare-authored.js ariel --write`, followed by shared navigation/world context. Runtime-only installation is `pnpm setup:assets --object=ariel`; original numeric cubes and unrelated body textures are not runtime dependencies.
