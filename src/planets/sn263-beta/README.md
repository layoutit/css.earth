...
| Source | Used for |
| --- | --- |
| [PDS SN263 V1.0](https://sbn.psi.edu/pds/resource/shape153591.html) | Original beta mesh, model limits, pole and rotation metadata |
...
The source mesh has 1,148 vertices and 2,292 faces, in kilometres. Its computed equivalent-volume radius is 0.394555 km. The paper's estimated diameter is **0.77 ± 0.12 km**; the exact mesh volume is a model property, not a more precise observation. Credits and terms are in [NOTICE.md](NOTICE.md).
...
The second integration brings in main at `085a404c1` ([PR #99](https://github.com/layoutit/cssEarth/pull/99)). This body's native inputs, prepared mesh and surface textures remain unchanged. Shared renderer and navigation code changed upstream, so the browser results and screenshots at `ea88f6feab53` do not qualify this integration. Historical reports and one-off scripts remain linked at their tested revision.

Navigation reconciliation combines the 444-object marker atlas, refreshes per-body marker bindings and updates the Sun's prepared world context. These binding changes also change the scene payload hashes. The [navigation reconciliation tool](../../../tools/audits/sn263/reconcile-navigation.mjs) rebuilds the atlas; the existing transport serializer updates the saved bindings. The [main integration receipt](../asteroid-2001-sn263/evidence/main-integration.json) records its completed checks and remaining gaps.

The merged astronomy suite passed 679/679 tests. Focused source, package, marker and router checks passed 91/92; the remaining failure is the pre-existing Phobos compiled-partition fixture. Astronomy, preparation and renderer builds passed.
