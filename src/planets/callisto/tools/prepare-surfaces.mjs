...
import { CALLISTO_PUBLIC_ROOT, CALLISTO_PREPARED_ROOT, CALLISTO_SOURCE_ROOT, ensureCallistoPreparationDirectories } from "./preparation-paths.mjs";

await ensureCallistoPreparationDirectories();
const [entry] = await validateCallistoSourceGroup("surfaces");
const path = resolve(CALLISTO_SOURCE_ROOT, entry.path);
const tiff = await fromFile(path);
const image = await tiff.getImage();
if (image.getWidth() !== entry.width || image.getHeight() !== entry.height || image.getGDALNoData() !== 0) {
  throw new Error("Callisto source dimensions or no-data value changed.");
}
...
if (resolution[0] <= 0 || resolution[1] >= 0 || keys.ProjCenterLongGeoKey !== 180 ||
  ...
  throw new Error("Callisto source coordinate mapping changed.");
}
await tiff.close();
const source = await sharp(path).greyscale().raw().toBuffer({ resolveWithObject: true });
if (source.info.channels !== 1) throw new Error("Callisto source must be monochrome.");
...
if (info.channels !== 4) throw new Error("Callisto resampling must retain its validity channel.");
...
await writeFile(resolve(CALLISTO_PREPARED_ROOT,"surfaces.json"),JSON.stringify({objectId:"callisto",surfaces:[normal]}));
console.log(`Prepared Callisto monochrome, with ${normal.missingPixels} no-data pixels marked.`);
...
async function prepareSurface(id, rgb, missing, metadata) {
  ...
  const map=await emit(`callisto-${id}-map.webp`,sharp(rgba,{raw:{width,height,channels:4}}));
  const surface=await emit(`callisto-${id}-surface@2x.webp`,sharp(packedPixels,{raw:{width:packed.packedWidth,height:packed.packedHeight,channels:4}}));
  const thumbnail=await emit(`callisto-${id}-thumbnail.webp`,sharp(rgba,{raw:{width,height,channels:4}}).resize(96,48));
...
async function emit(filename,pipeline) {
 ...
 await writeFile(resolve(CALLISTO_PUBLIC_ROOT,filename),bytes);
 const {width,height}=await sharp(bytes).metadata();
 return {url:`/scenes/callisto/${filename}`,width,height,bytes:bytes.length,sha256:createHash("sha256").update(bytes).digest("hex")};
}
