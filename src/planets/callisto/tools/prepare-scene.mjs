...
import { PREPARED_CALLISTO_STARFIELD } from "../runtime/preparedStarfield.mjs";
import { PREPARED_CALLISTO_SKY_SUN as sun } from "../runtime/preparedSkySun.mjs";
import { CALLISTO_CAMERA_POSE, CALLISTO_PRESENTATION_FRAME as frame } from "./scene-camera-pose.mjs";

const { BODIES } = await loadAstronomyPackage();
const radiusKm = BODIES.callisto.meanRadiusKm, radius = 230;
const registration = prepareAstrometricSkySceneRegistration("callisto");
const sky = { ...PREPARED_CALLISTO_STARFIELD,
...
const scene = {
  camera: preparePerspectiveCamera({ sky, radius, ...CALLISTO_CAMERA_POSE }), sky, sun,
  systemTransform: frame.cssTransform,
  // Mean-radius sphere; a resolved shape model is not claimed.
  bodyLeaves: prepareSolidBodySurface({ id: "callisto", radius,
    mapUrl: "/scenes/callisto/callisto-normal-surface@2x.webp", polesUrl: "/scenes/callisto/callisto-normal-poles@2x.webp" }),
  heliocentricView: prepareHeliocentricView({ bodyId: "callisto", presentationFrame: frame,
    ...
    system: await preparePlanetarySystem({ bodyId: "callisto", presentationFrame: frame, kilometersPerUnit: radiusKm / radius }),
  }),
};
await writeFile(new URL("../.prepared/scene.json", import.meta.url), JSON.stringify(scene));
console.log(`Prepared Callisto scene: ${scene.bodyLeaves.length} retained surface leaves.`);
