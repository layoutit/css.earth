...
import { CALLISTO_PUBLIC_ROOT } from "./preparation-paths.mjs";
...
const pointUrl = "/scenes/callisto/callisto-system-point.webp";
...
await sharp(point, { raw: { width: 32, height: 32, channels: 4 } }).webp({ lossless: true })
  .toFile(new URL("../../../../public/scenes/callisto/callisto-system-point.webp", import.meta.url).pathname);
const entries = [
...
const camera = b.mesh("polycss-camera callisto-camera planet-render-root");
const scene = b.mesh("polycss-scene callisto-scene"), system = b.mesh("callisto-system", `transform:${plan.systemTransform}`);
const body = b.mesh("callisto-body");
...
const materialRoot = b.element("div", "callisto-material-root planet-render-root");
const billboard = b.element("s", "callisto-billboard"), material = b.element("s", "callisto-material");
...
const track = { id: "lighting", target: index(material),
  ...
  rotation: { kind: "angle", source: "view-sun", reference: "prepared", baseDegrees: 0,
    zeroAtPole: false, property: "--callisto-light-roll" }, frameAttribute: null, modeAttribute: null, quoted: true };
const variants = surfaces.flatMap(s => [false, true].flatMap(shadows => [false, true].map(orbit => ({
  ...
  writes: [
    { kind: "texture", target: index(body), name: "--callisto-surface-image", resource: `surface:${s.id}`, quoted: true },
    { kind: "texture", target: index(body), name: "--callisto-poles-image", resource: `poles:${s.id}`, quoted: true },
    { kind: "style", target: index(materialRoot), name: "--callisto-billboard-color", value: s.billboardColor },
    { kind: "attribute", target: -1, name: "data-lens", value: s.id },
    { kind: "class", target: -1, name: "callisto-hide-orbit", value: !orbit },
  ],
...
const presentation = { schema: PREPARED_PRESENTATION_SCHEMA, camera: plan.camera, sky: plan.sky, sun: plan.sun,
  ...
  heliocentricView: { plan: plan.heliocentricView,
    bodyMarker: { url: atlasUrl, ...sprite("callisto"), size: 3 },
  ...
  viewBindings: [
    ...
    { kind: "view-property", target: index(materialRoot), property: "--callisto-billboard-opacity", source: "billboard-opacity", precision: 6 },
  ],
};
...
await prepareRuntimeAssetManifest({ planetId: "callisto", urls: [...new Set(urls)], publicRoot: CALLISTO_PUBLIC_ROOT,
  manifestPath: new URL("../runtime-assets.json", import.meta.url) });
console.log("Prepared Callisto runtime presentation and asset inventory.");
