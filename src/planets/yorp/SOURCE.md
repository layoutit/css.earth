...
- Unresolved: the [PDS radar-shape compilation](https://pds.nasa.gov/ds-view/pds/viewCollection.jsp?identifier=urn%3Anasa%3Apds%3Agbo.ast.jpl.radar.shape_models%3Adocument) lists YORP, but the archive directory returned HTTP403. The original JPL mesh and paper remain the acquired sources.
...
Every consumed source is pinned by byte length and SHA-256. Acquisition restores the unchanged OBJ, article and supplement, ESO panorama and Inter font through recorded URLs. Articles retain publisher copyright and are restoration inputs; their pages are not runtime assets. Runtime installation consumes only the published prepared package.
