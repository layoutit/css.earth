# (444) Gyptis: source and interpretation
...
AKARI supplies only one detection for Gyptis, so its formal ±6.66 km error is not the total scale error. Magri et al. (2007), using radar and other available constraints, tabulate a 163 ± 27 km effective diameter and adopted axes 189 × 164 × 137 km with 14% uncertainty. This supports a broad roughly 160 km scale, but does not calibrate the later ATLAS convex mesh. Neither those axes nor the observed occultation silhouette is substituted for the source mesh.
...
- [DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/5394) — selected original mesh and spin metadata.
- [Original shape table](https://damit.cuni.cz/projects/damit/stored_files/open/58886/shape.txt) — counted XYZ vertices and one-based triangle indices.
