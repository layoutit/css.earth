...
The full source mesh now supplies a 1,600-face native triangle presentation. The authored transfer limit is 100 m. Four barycentric samples per retained face gave a maximum distance of 69.40 m for Deimos; these are sampled rendering errors, not source measurement uncertainty or an exhaustive bound. Complete atlas transfer qualification remains a separate preparation check.
Relative albedo preserves the archive scale without an absolute-albedo claim. Slope is gravity-relative under the source authors’ rotation, uniform-density and Mars-distance assumptions. Deimos facets with missing albedo remain withheld in both new views.
