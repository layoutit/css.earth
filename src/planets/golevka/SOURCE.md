# Golevka sources and preparation
...
The PDS radar shape collection describes a center-of-mass origin and principal-axis frame, with positive Z as the spin pole. Its Golevka mesh and label were located, but both returned HTTP403 during this acquisition; an exact numeric comparison to that counterpart remains unresolved. This package binds the original JPL mesh to its own paper’s dimensions and pole.

The original mesh has 2048 vertices and 4092 triangles, closed volume 0.0779465269834 km³ and volume-equivalent radius 0.264994007693 km. The reference radius is 0.265 km. This datum does not rescale the shape. The published ecliptic pole (202°, -45°) is converted to equatorial J2000 using obliquity 23.439291111°. Body longitude is east-positive; display phase is arbitrary.
...
- Unresolved: [PDS Golevka counterpart](https://sbnarchive.psi.edu/pds4/non_mission/compil.ast.radar.shape-models/data/6489golevka.xml) was located but XML and TAB downloads returned HTTP403.
...
Every consumed source is pinned by byte length and SHA-256. Acquisition restores the unchanged OBJ, article, ESO panorama and Inter font through recorded URLs. Articles retain publisher copyright and are restoration inputs; their pages are not runtime assets. Runtime installation consumes only the published prepared package.
