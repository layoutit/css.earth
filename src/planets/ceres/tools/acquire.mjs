...
if (process.argv.slice(2).some(arg => arg !== "--verify-only")) {
  throw new Error("Usage: node src/planets/ceres/tools/acquire.mjs [--verify-only]");
}
if (!process.argv.includes("--verify-only")) {
  for (const entry of source.manifest.inputs) {
    ...
    for await (const chunk of response.body) {
      size += chunk.length;
      if (size > entry.expectedBytes) throw new Error(`Ceres source exceeds expected size: ${entry.path}`);
      chunks.push(chunk);
    }
    await publishSourceBytes({ destination, bytes: Buffer.concat(chunks), entry, planetName: "Ceres" });
  }
}
