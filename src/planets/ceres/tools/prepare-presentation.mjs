...
import { CERES_PUBLIC_ROOT } from "./preparation-paths.mjs";
...
const pointUrl = "/scenes/ceres/ceres-system-point.webp";
...
await sharp(point, { raw: { width: 32, height: 32, channels: 4 } }).webp({ lossless: true })
  .toFile(new URL("../../../../public/scenes/ceres/ceres-system-point.webp", import.meta.url).pathname);
const entries = [
...
const camera = b.mesh("polycss-camera ceres-camera planet-render-root");
const scene = b.mesh("polycss-scene ceres-scene"), system = b.mesh("ceres-system", `transform:${plan.systemTransform}`);
const body = b.mesh("ceres-body");
...
const materialRoot = b.element("div", "ceres-material-root planet-render-root");
const billboard = b.element("s", "ceres-billboard"), material = b.element("s", "ceres-material");
...
const track = { id: "lighting", target: index(material),
  ...
  rotation: { kind: "angle", source: "view-sun", reference: "prepared", baseDegrees: 0,
    zeroAtPole: false, property: "--ceres-light-roll" }, frameAttribute: null, modeAttribute: null, quoted: true };
const variants = surfaces.flatMap(s => [false, true].flatMap(shadows => [false, true].map(orbit => ({
  ...
  writes: [
    { kind: "texture", target: index(body), name: "--ceres-surface-image", resource: `surface:${s.id}`, quoted: true },
    { kind: "texture", target: index(body), name: "--ceres-poles-image", resource: `poles:${s.id}`, quoted: true },
    { kind: "style", target: index(materialRoot), name: "--ceres-billboard-color", value: s.billboardColor },
...
const presentation = { schema: PREPARED_PRESENTATION_SCHEMA, camera: plan.camera, sky: plan.sky, sun: plan.sun,
  ...
  heliocentricView: { plan: plan.heliocentricView,
    bodyMarker: { url: atlasUrl, ...sprite("ceres"), size: 3 },
...
console.log("Prepared Ceres runtime presentation and asset inventory.");
