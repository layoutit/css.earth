...
The authored neutral PNG and derived context PNG are checked in. Acquisition restores the original OBJ, four OSIRIS GEO/quality IMG pairs, NAVCAM reference, four VIRTIS table/label pairs and interpretation documents, the SHAP5 ambiguity reference, ESO panorama and Inter font and verifies their pinned hashes. `prepared/object.json` is generated at installation/build time; the prepared geometry and runtime inventory remain versioned. No other object is synthesized as a fallback.
