...
The authored neutral PNG and derived context PNG are checked in. Acquisition restores the original OBJ, four OSIRIS GEO/quality IMG pairs, NAVCAM reference, ESO panorama and Inter font and verifies their pinned hashes. `prepared/object.json` is generated at installation/build time; the prepared geometry and runtime inventory remain versioned. No other object is synthesized as a fallback.

Focused checks:

```sh
node --test tests/objects/unit/comet-67p/*.test.mjs tools/objects/terrestrial-layers/observation-mosaic.test.mjs
