...
const source = await createSourceManifest({ planetId: "europa", planetName: "Europa", sourceRoot });
if (process.argv.slice(2).some(arg => arg !== "--verify-only")) {
  throw new Error("Usage: node src/planets/europa/tools/acquire.mjs [--verify-only]");
}
if (!process.argv.includes("--verify-only")) {
  for (const entry of source.manifest.inputs) {
    ...
    if (!response.ok || !response.body) throw new Error(`Europa source HTTP ${response.status}: ${entry.origin}`);
    const chunks = [];
    let size = 0;
    for await (const chunk of response.body) {
      size += chunk.length;
      if (size > entry.expectedBytes) throw new Error(`Europa source exceeds expected size: ${entry.path}`);
      chunks.push(chunk);
    }
    await publishSourceBytes({ destination, bytes: Buffer.concat(chunks), entry, planetName: "Europa" });
  }
}
