...
import { ASTRONOMICAL_UNIT_KILOMETERS } from "../../../platform/solar-geometry.mjs";

test("Europa mounts through the shared prepared object contract", () => {
...
for (const [id, parent, radius, distanceKm] of [
  ["europa", "jupiter", 1560.8, 671000], ["moon", "earth", 1737.4, 384400],
]) {
  test(`${id}'s orbit surrounds ${parent}, while the Sun keeps its separate position`, async () => {
    ...
    assert.ok(view.sun.distanceAu > (id === "europa" ? 4.8 : 0.95));
