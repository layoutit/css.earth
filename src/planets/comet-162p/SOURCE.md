...
Reproduce with `node tools/objects/dist/prepare-authored.js comet-162p --write` after restoring sources through the acquisition recipe. Install published runtime assets with `pnpm setup:assets --object=comet-162p`. Original thesis bytes are linked and hashed, not redistributed.
