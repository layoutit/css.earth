...
Install prepared runtime assets with `pnpm setup:assets --object=kiviuq`. No source preparation is needed for this installation.

To rebuild from sources after building the preparation tools:

```sh
node tools/objects/dist/operations.js acquire kiviuq
node tools/objects/dist/prepare-authored.js kiviuq --write
```
