...
| --- | --- |
| Shape and spin | [DAMIT 523](https://damit.cuni.cz/projects/damit/asteroid_models/view/523) |
| Physical scale | [Calibration and uncertainty](source/reference/calibration.json) |

Checked 2026-09-08. The selected source is DAMIT model **523**, version/record date **2013-02-11**, credited to Hanuš et al. (2013). The unchanged source table contains **1022 vertices and 2040 triangles**.

Usui et al. (2011), the original ISAS/JAXA AcuA v1.0 catalog, report an effective thermal diameter of **52.82 ± 0.6 km**, from 10 detections. Its modified Standard Thermal Model fits a nonrotating sphere. This package explicitly uses that diameter as an **approximate volume scale** for the separate convex DAMIT mesh; it is not a published calibration of this mesh’s physical volume.
...
The [lucia validation record](https://github.com/layoutit/cssEarth/blob/cc01831f595e0b73ab6699d6235cf7b466f76cfc/docs/asteroids-size-calibration-validation.json) contains source, scale, atlas, installation and browser results for its recorded files and revision.
...
The selected pole is (293°, 49°), with alternate (106°, 50°). Hanuš et al. (2018) show Lucia as an example where the thermophysical solution has no satisfactory constrained minimum; their accepted diameter table does not supply a shape-aware calibration.

The AKARI effective diameter remains a qualified approximate scale.
...
Alternative archive models: Model 524: pole (106°, 50°), period 7.83671 h.
...
```text
Vsource = 1.0000000977828636 source units³
Deq(source) = 2 × (3 × abs(Vsource) / (4 × π))^(1/3)
scale = Dpublished / Deq(source) = 42.572706117947966 km per source unit
metersPerUnit = 42572.706117947964
reference radius = Dpublished / 2 = 26.41 km
```
...
The original co-rotating Cartesian frame is kept: positive Z is the rotation axis, and positive X defines the source reference meridian. The pole is ecliptic J2000 **(293°, 49°)** and the sidereal period is **7.8367 h**. The existing shared recipe converts that pole to equatorial J2000 with obliquity 23.439291111°.
...
- [DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/523) — selected original mesh and spin metadata.
- [Original shape table](https://damit.cuni.cz/projects/damit/stored_files/open/2060/shape.txt) — counted XYZ vertices and one-based triangle indices.
...
- [Hanuš et al. (2013), Asteroids' physical models from combined dense and sparse photometry and scaling of the YORP effect by the observed obliquity distribution](https://ui.adsabs.harvard.edu/abs/2013A%26A...551A..67H)
