# (147) Protogeneia: source and interpretation
...
The physical size is approximately calibrated by assigning the AKARI thermal effective diameter to the mesh volume-equivalent diameter. Neither source mesh nor spin is modified. Convex inversion supplies only broad outline; no resolved craters, concavities, reflectance or regolith texture can be inferred. Both 2013 convex poles (90,14) and (269,15) remain plausible. Model 557 is a representative solution; neither a smaller model ID nor the ordering in the paper establishes a preferred pole. Use the original unscaled coordinates with an explicitly approximate AKARI thermal-size scale.

The unmodified source has 1022 vertices and 2040 triangles. Its signed tetrahedral volume is 1.0000000565962182 source units³; an independent triangle-centroid divergence sum gives 1.0000000565962182. The existing recipe applies one uniform scale of 87.378022219040901 km per source unit so its volume-equivalent diameter is 108.41 km. No unit-volume assumption is made. Radius above a 54.205 km sphere is a shape-derived scalar, not gravitational height or measured geology.

The original +Z spin axis and +X reference meridian are retained. The source pole is ecliptic J2000 (90°, 14°), with sidereal period 7.85227 h. Conversion to equatorial J2000 uses obliquity 23.439291111°. Absolute phase is arbitrary; accelerated display spin is illustrative. Position uses JPL Horizons heliocentric ICRF elements at 2026-09-03 TT (TDB approximated as TT, under 2 ms).
...
Both 2013 convex poles (90,14) and (269,15) remain plausible. Model 557 is a representative solution; neither a smaller model ID nor the ordering in the paper establishes a preferred pole. Use the original unscaled coordinates with an explicitly approximate AKARI thermal-size scale.

No registered global image texture is supplied by the selected release. Shape diagrams, disk-integrated thermal estimates and AO comparison images cannot provide a regolith or albedo map; the shared gray grid is used. Alternative archive solutions: model 558, pole ['269', '15'], https://damit.cuni.cz/projects/damit/asteroid_models/view/558

## Preparation and qualification

The established source-meshoptimizer path retains source connectivity, reduces to at most 800 faces, and emits native PolyCSS `u` triangles with 128 px raster cells. The error allowance is 1084.1 m; sampled source-fit error is qualified separately from source accuracy. Elevation uses closest-source-surface sampling with the same physical scale. Shadows are off by default. Shared runtime, camera, navigation and shell remain generic. Delivery and browser evidence are recorded in the PR’s validation report.
