...
- **SPICE oracle:** `tools/oracles/spice/dart-draco.py` runs SpiceyPy 8.2.0 (CSPICE N0067) over the same fifteen pinned kernels and `tools/spice/oracle.test.mts` compares: leap seconds, TDB and the spacecraft clock agree to a microsecond over nine epochs; 202 states (geometric, `LT`, `LT+S`, `CN`, `CN+S`, in J2000 and in the body frame) agree to a millimetre, with velocities to 6 × 10⁻⁸ km/s inside the terminal type 13 segment and 10⁻¹⁰ km/s elsewhere; 21 frame rotations across every kernel frame class agree to a nanoradian; and 33 archived intercepts placed by CSPICE through `DART_DRACO` with `LT+S` land within 0.03 px of where the kernel camera puts them, because the camera folds one aberration rotation at the target and CSPICE aberrates each point.
