...
| --- | --- |
| Shape and spin | [DAMIT 1189](https://damit.cuni.cz/projects/damit/asteroid_models/view/1189) |
| Physical scale | [Calibration and uncertainty](source/reference/calibration.json) |

Checked 2026-09-08. The selected source is DAMIT model **1189**, version/record date **2016-01-04**, credited to Hanuš et al. (2016). The unchanged source table contains **1022 vertices and 2040 triangles**.

Usui et al. (2011), the original ISAS/JAXA AcuA v1.0 catalog, report an effective thermal diameter of **87.45 ± 0.95 km**, from 9 detections. Its modified Standard Thermal Model fits a nonrotating sphere. This package explicitly uses that diameter as an **approximate volume scale** for the separate convex DAMIT mesh; it is not a published calibration of this mesh’s physical volume.
...
The [proserpina validation record](https://github.com/layoutit/cssEarth/blob/cc01831f595e0b73ab6699d6235cf7b466f76cfc/docs/asteroids-size-calibration-validation.json) contains source, scale, atlas, installation and browser results for its recorded files and revision.
...
The archived model has one published pole solution. The occultation search also found predicted paths, but a prediction diameter is not a measured size and was not used.
...
```text
Vsource = 1.0000002492354696 source units³
Deq(source) = 2 × (3 × abs(Vsource) / (4 × π))^(1/3)
scale = Dpublished / Deq(source) = 70.484342333637912 km per source unit
metersPerUnit = 70484.342333637906
reference radius = Dpublished / 2 = 43.725 km
```
...
The original co-rotating Cartesian frame is kept: positive Z is the rotation axis, and positive X defines the source reference meridian. The pole is ecliptic J2000 **(88°, -52°)** and the sidereal period is **13.1098 h**. The existing shared recipe converts that pole to equatorial J2000 with obliquity 23.439291111°.
...
- [DAMIT model record](https://damit.cuni.cz/projects/damit/asteroid_models/view/1189) — selected original mesh and spin metadata.
- [Original shape table](https://damit.cuni.cz/projects/damit/stored_files/open/4308/shape.txt) — counted XYZ vertices and one-based triangle indices.
