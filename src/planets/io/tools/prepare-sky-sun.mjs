...
import {
  IO_CAMERA_POSE,
  IO_PRESENTATION_FRAME,
} from "./scene-camera-pose.mjs";
import {
  ensureIoPreparationDirectories,
  IO_PUBLIC_ROOT,
} from "./preparation-paths.mjs";

// Io's Sun direction is the observed one, not a presentation constant:
...
// fact stay standard.
const bodyFixedDirection = requireBodyFixedSunDirection("io");
const sceneDirection = IO_PRESENTATION_FRAME.sunDirection;
...
await preparePlanetDirectionalSun({
  objectId: "io",
  publicRoot: IO_PUBLIC_ROOT,
  preparedModulePath: new URL("../runtime/preparedSkySun.mjs", import.meta.url),
  ensureDirectories: ensureIoPreparationDirectories,
  meanHeliocentricDistanceAu: requireObject("io").distanceAu,
  presentation,
});
