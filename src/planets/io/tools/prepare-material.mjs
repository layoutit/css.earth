...
import { IO_PUBLIC_ROOT, IO_PREPARED_ROOT } from "./preparation-paths.mjs";
...
for (const surface of surfaces) {
  const { data, info } = await sharp(resolve(IO_PUBLIC_ROOT, surface.map.url.split("/").at(-1)))
    .ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const atlas = prepareSolidBodyPoleRaster(data, { width: info.width, height: info.height, tileSize: poleSize });
  surface.polesUrl = `/scenes/io/io-${surface.id}-poles@2x.webp`;
  await sharp(atlas, { raw: { width: poleSize * 2, height: poleSize, channels: 4 } })
    .webp({ lossless: true }).toFile(resolve(IO_PUBLIC_ROOT, surface.polesUrl.split("/").at(-1)));
...
const url = "/scenes/io/io-lighting.webp";
await sharp(pixels, { raw: { width: atlasWidth, height: atlasHeight, channels: 4 } })
  .webp({ lossless: true }).toFile(resolve(IO_PUBLIC_ROOT, "io-lighting.webp"));
...
await writeFile(resolve(IO_PREPARED_ROOT, "material.json"), JSON.stringify({ surfaces, lighting: { url, columns, rowCount: rows, frameCount, frames } }));
console.log("Prepared Io poles and lighting.");
