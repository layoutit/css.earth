...
import { PREPARED_IO_STARFIELD } from "../runtime/preparedStarfield.mjs";
import { PREPARED_IO_SKY_SUN as sun } from "../runtime/preparedSkySun.mjs";
import { IO_CAMERA_POSE, IO_PRESENTATION_FRAME as frame } from "./scene-camera-pose.mjs";

const { BODIES } = await loadAstronomyPackage();
const radiusKm = BODIES.io.meanRadiusKm, radius = 230;
const registration = prepareAstrometricSkySceneRegistration("io");
const sky = { ...PREPARED_IO_STARFIELD,
...
const scene = {
  camera: preparePerspectiveCamera({ sky, radius, ...IO_CAMERA_POSE }), sky, sun,
  systemTransform: frame.cssTransform,
  // Mean-radius sphere; a resolved shape model is not claimed.
  bodyLeaves: prepareSolidBodySurface({ id: "io", radius,
    mapUrl: "/scenes/io/io-normal-surface@2x.webp", polesUrl: "/scenes/io/io-normal-poles@2x.webp" }),
  heliocentricView: prepareHeliocentricView({ bodyId: "io", presentationFrame: frame,
    ...
    system: await preparePlanetarySystem({ bodyId: "io", presentationFrame: frame, kilometersPerUnit: radiusKm / radius }),
  }),
};
await writeFile(new URL("../.prepared/scene.json", import.meta.url), JSON.stringify(scene));
console.log(`Prepared Io scene: ${scene.bodyLeaves.length} retained surface leaves.`);
