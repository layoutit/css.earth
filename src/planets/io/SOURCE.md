...
`tools/prepare-surfaces.mjs` produces lossless 4096 × 2048 maps, projective strip
atlases and thumbnails; `prepare-material.mjs` produces registered pole tiles and
...
`tools/prepare-parent-marker.mjs` prepares an independent 1024-pixel Hubble
...
```sh
node src/planets/io/tools/acquire.mjs
node src/planets/io/tools/prepare.mjs
node --test src/planets/io/test/*.test.mjs
