...
export async function prepareSurfaces(config: RasterRecipe, sourceDirectory: string, publicDirectory: string, interpret?: ObservationInterpretation) {
    ...
    for (const surface of config.surfaces) {
        ...
        } else pixels = source ?? await readRgba(resolve(sourceDirectory, surface.source), width, height, true, surface.sharpen);
        if (surface.exposure)
        ...
        if (config.thumbnail.crop) {
            ...
            let layout = raster(packed.data, packed.packedWidth, packed.packedHeight);
