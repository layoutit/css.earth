...
export const outputName = (template: string, density = 1, id = '') => template.replaceAll('{density}', String(density)).replaceAll('{suffix}', density === 1 ? '' : '@2x').replaceAll('{id}', id);
...
export const assetPath = (directory: string, template: string, density = 1, id = '') => resolve(directory, outputName(template, density, id));
