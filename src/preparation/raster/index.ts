...
export async function prepareRasterAssets({ sourceDirectory, publicDirectory, outputDirectory, config, interpret }: {
    ...
    const surfaces = Object.fromEntries(config.surfaces.map(surface => [surface.id, { id: surface.id, falseColor: surface.falseColor, ...(surface.resolutionScale ? { dimensions: { width: config.width * surface.resolutionScale, height: config.height * surface.resolutionScale } } : {}), url: config.publicBase + outputName(surface.output, RASTER_DENSITY, surface.id), url2x: config.publicBase + outputName(surface.output, RASTER_DENSITY, surface.id), ...(config.emission ? { coronaUrl: config.publicBase + outputName(config.emission.offLimbOutput, RASTER_DENSITY, surface.id), coronaUrl2x: config.publicBase + outputName(config.emission.offLimbOutput, RASTER_DENSITY, surface.id), limbUrl: config.publicBase + outputName(config.emission.limbOutput, RASTER_DENSITY, surface.id), limbUrl2x: config.publicBase + outputName(config.emission.limbOutput, RASTER_DENSITY, surface.id) } : {}), ...(config.surfaceMetadata.sourcePositionVariable ? { sourcePositionVariable: config.surfaceMetadata.sourcePositionVariable } : {}), ...(metadata[surface.id] ? { coverageCompletion: metadata[surface.id] } : {}), ...(interpretations[surface.id] ? { interpretation: interpretations[surface.id] } : {}) }]));
