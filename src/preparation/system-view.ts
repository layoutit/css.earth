...
export function prepareSystemView(parent: Pick<PreparedWorldContext['focus'], 'id' | 'positionM' | 'radiusM'>,
  ...
  const members = bodies.filter(body => body.orbit.centerBodyId === parent.id)
    .sort((a, b) => b.radiusM - a.radiusM || states[a.id]!.semiMajorAxisM - states[b.id]!.semiMajorAxisM
  ...
  const candidates: PreparedSystemViewCandidate[] = [];
  for (const degrees of policy.elevationsDegrees) for (let azimuth = 0; azimuth < 360; azimuth += policy.azimuthStepDegrees) {
    ...
    const project = (position: Vector3): Vector3 => {
    ...
    for (const member of main) for (const vertex of member.orbit.verticesM) {
    ...
    candidates.push(Object.freeze({
      cameraToReference: Object.freeze([right[0], down[0], back[0], right[1], down[1], back[1], right[2], down[2], back[2]]),
      minimumM: Object.freeze(minimum) as unknown as Vector3,
      maximumM: Object.freeze(maximum) as unknown as Vector3,
      memberPositionsM: Object.freeze(main.map(member => Object.freeze(project(member.positionM)))),
    }));
  }
  return Object.freeze({ memberIds: Object.freeze(main.map(member => member.id)),
    memberRadiiM: Object.freeze(main.map(member => member.radiusM)), candidates: Object.freeze(candidates) });
}
