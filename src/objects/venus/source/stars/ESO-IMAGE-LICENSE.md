...
The Venus diffuse starfield uses the 6000 x 3000 public equirectangular image
`eso0932a`, "The Milky Way panorama", released by the European Southern
...
single-epoch Venus sky. The prepared Venus scene therefore uses the image as a
source-qualified visual background, not as an observer-epoch or ephemeris
claim.

The preparation pipeline adapts the published equirectangular panorama into
six retained CSS cubemap faces, applies documented luminance levels, and bakes
the separately qualified Sun presentation into those faces. The complete ESO
credit is exposed in the Venus source resources.
