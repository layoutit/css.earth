...
Shape uses the shared missing-imagery grid. DAMIT's viewer illustrations are not source surface maps; no albedo, photographic color, regolith or composition is inferred. Elevation shows the radius of the original model minus a 93.5 km sphere, on a -20 to 30 km legend.
...
DAMIT 298 at the original occultation-calibrated 187 km scale.

- [Alternative pole and 198 km calibrated shape.](https://damit.cuni.cz/projects/damit/asteroid_models/view/299) — excluded alternative solution. The package preserves model 298 and its own spin/scale rather than mixing solutions.
...
[Original counted triangle table](https://damit.cuni.cz/projects/damit/stored_files/open/1044/shape.txt) has 1022 vertices and 2040 triangles. It is consumed without conversion by the existing PDS plate-table reader because its count/XYZ/one-based-triangle layout is identical. This is a DAMIT dataset, not a PDS release.
...
The record gives diameter 187 km, period 8.58189 h and ecliptic J2000 pole (221°, -47°). The original mesh has measured volume-equivalent radius 93.500005 km and Cartesian extents 211.794 × 203.420 × 172.690 km. The rounded catalog diameter is used as the reference-sphere scale; the original coordinates are not rescaled.
