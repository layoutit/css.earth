...
HR 8799 c is one of the three planets Marois et al. ([2008](https://arxiv.org/abs/0811.2606)) imaged with Keck and Gemini. It orbits [HR 8799](../hr-8799/README.md) about 41 au out.

## Sources

**Orbit.** Zurlo et al. (2022, A&A 666, A133; [arXiv:2207.10684](https://arxiv.org/abs/2207.10684)), Table 3 model 1: the coplanar four-planet fit to over 20 years of astrometry, at stellar mass 1.47 solar masses and parallax 24.525662 mas, osculating elements at epoch 1998.83. Its semi-major axis, 1005.21 mas, is kept as an angle and placed at the Gaia DR3 distance: 41.09 au. Eccentricity 0.0538, inclination 26.87° and ascending node 62.19° east of north, shared by all four planets. The paper prints no period; it is derived from Kepler's third law with the fitted masses, about 217 years. Every element is in [`hr-8799-c.json`](../../../packages/astronomy/data/bodies/hr-8799-c.json).

**Checked against later astrometry.** On 5 November 2023 JWST measured the planet's offset from the star (Balmer et al. 2025, AJ; [arXiv:2503.13608](https://arxiv.org/abs/2503.13608), Table 2), after the fit was made. The recorded orbit puts the planet 10.3 mas from that position ([hostedOrbits.test.ts](../../../packages/astronomy/src/hostedOrbits.test.ts)). It misses in declination by 10 mas, 3.4 of the paper's 3 mas sigmas, which are the scatter between filters.

**Radius, temperature and mass.** Nasedkin et al. (2024, A&A 687, A298; [arXiv:2404.03776](https://arxiv.org/abs/2404.03776)), Table 2: the Bayesian model average of retrievals on GRAVITY and archival spectra, radius 1.10 ± 0.01 Jupiter radii and effective temperature 1,159 +11/−12 K. The radius is the one a model atmosphere needs for the measured spectrum, constrained by the dynamical mass as a prior; the planet itself is a point in every image. Mass 7.80 Jupiter masses, dynamical (Zurlo et al. 2022, Table 3 model 1).
...
**NIRCam colour lens.** The planet's flux densities in JWST/NIRCam F460M, F430M and F410M, 139.4, 159.7 and 238.2 µJy (Balmer et al. 2025, Table 2), drive red, green and blue. Balmer et al. detect all four planets in these three filters (section III.3), and the longest wavelength is red. The four planets share one range, from zero to the largest of their twelve values, planet d in F410M (410.5 µJy), so their band ratios and their brightness against each other survive ([colour preparation](../../../docs/color-preparation.md)). The result, #9ea8c8, is infrared false colour, not what an eye would see, and one colour for the whole disc ([photometry record](source/photometry/jwst-nircam-band-color.json)).
