...
- [`hammond-offset.test.mts`](../../../tests/objects/unit/wasp-43b/hammond-offset.test.mts) explains why the MIRI offset differs from Hammond et al.'s, as the known problem below describes. [`transit-timing.mts`](../../../tools/objects/eclipse-map/transit-timing.mts) fits a limb-darkened transit on the package orbit. On the same window it agrees with batman-package 2.5.3, the reduction toolchain's pin, to 0.5 s and to 0.1 in χ².
...
- **The MIRI offset is 1.4° west of Hammond et al.'s, and the difference is explained.** Hammond et al. (2024) report 7.5 ± 0.5° for the same observation; this map gives 6.05°. An eclipse map's longitude trades against two things the fit assumes ([`hammond-offset.test.mts`](../../../tests/objects/unit/wasp-43b/hammond-offset.test.mts)):
  - *Eclipse timing.* Moving the eclipses 1 s earlier moves the offset about 0.04° east. The transit in this light curve comes 19.5 s (18.5 to 20.0) before the package ephemeris predicts. Eclipse light arrives 15 s later than transit light, the time light takes to cross the orbit, so the eclipses sit near −4.5 s, where the map gives 6.2°. Hammond et al. give t0 = 55934.292283 BMJD_TDB with the period held at 0.813474 d. For this visit, that ephemeris puts the transit at −39.5 s, which the transit rules out (χ² worse by 579). With it the map gives 7.45°, inside their interval. Whether their code used exactly this period cannot be checked: with Bell et al.'s fitted period, 0.81347406 d, the same t0 gives −13 s and 6.4–6.6°.
  - *The ramp.* The detector ramp's time constant is poorly constrained, and the offset follows it: at the measured timing, 0.08 d gives 5.8° and 0.16 d gives 6.95°, with χ² within 1 of each other. The lens profiles the time constant, which settles at 0.103 d. The first release of these lenses stopped at the 0.08 d grid value and showed 5.7°. It also compared the map's hottest point (5.3°) with Hammond et al.'s offset, which is a different quantity.
  - *The reduction.* On Bell et al.'s deposited light curve, the same fits give offsets about 0.7° smaller at every timing, for example 5.5° at the measured timing.
