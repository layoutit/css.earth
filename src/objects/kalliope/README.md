...
Kalliope is a main-belt asteroid observed in the ESO/VLT/SPHERE survey. Its published reconstruction combines resolved telescope images with an ADAM starting shape.

- [Vernazza et al. (2021), final VLT/SPHERE survey](https://doi.org/10.1051/0004-6361/202141781), Table 1 and Table A.1: volume-equivalent diameter 150 km, ecliptic J2000 pole (195°, 4°), sidereal period 4.1482 h. The original article is pinned and restorable.

- [Original MPCD mesh](https://observations.lam.fr/astero/3Dshape/22_Kalliope_mpcd.obj): 1286 vertices, 2568 triangles, unmodified Cartesian coordinates in kilometers. Its measured volume-equivalent radius is 76.369742 km. The survey's diameter averages ADAM and MPCD; the original coordinates are not rescaled to that average. Maximum Cartesian extents are 149.069 × 214.254 × 119.080 km; these are not best-fit ellipsoid axes.
...
Source and output are each one closed component with Euler characteristic 2. Meshoptimizer estimates 1353.0 m error; the authored stopping threshold is 1400 m. This estimate is not a Hausdorff bound. Independent nearest-triangle sampling (8192 area-stratified samples each way) measured p95 607.7 m and maximum 1572.3 m.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `zimpol` | 16 | 16 | 7.91° | 6.45° | 4.58° | its other 16 frames | 1 of 16 | — | 8 of 16, 1.00° |
...
Shape uses the shared neutral-gray material. It is not photographed color, reflectance, regolith or inferred composition. Elevation samples the original mesh radius minus a 75 km reference sphere, with a -30 to 60 km legend. This includes global shape, not height above a gravitational equipotential.
...
- [Alternative released mesh](https://observations.lam.fr/astero/3Dshape/22_Kalliope_adam.obj): radius 76.411653 km. The ADAM model is an alternative reconstruction of the same shape. Excluded as a second lens. The selected MPCD refinement uses resolved SPHERE detail; see survey section 3 and Appendix B.

- [Released SPHERE images](https://observations.lam.fr/astero/Data/22Kalliope/): individual, illuminated, resolved telescope images. Excluded as a globe texture in this PR: they are not a registered global reflectance mosaic. They remain the observational constraints behind the selected reconstruction.
