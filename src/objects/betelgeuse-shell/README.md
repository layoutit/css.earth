...
**Placement.** The cube's centre is Betelgeuse's own scene origin, the point its sphere is drawn at: the prepared bank's frame origin and the star's world frame agree to 794 metres, one and a half billionths of a stellar radius, which is float round-trip, and one volume unit is exactly one stellar radius. The star therefore sits at the centre of the cube, and the dark middle of the map sits on the star: measured on the released products, the centroid of the sub-quarter-scale region within three radii is 0.10 radii from the measured stellar centre. What is off-centre is the light, not the star. The centroid of the drawn brightness between one and 4.5 radii lies 1.24 radii from the star, because the envelope is lopsided; one quadrant of the one-to-three-radius ring carries about a third of the polarised light of the other three. The cube is anchored on the Betelgeuse object's own scene origin, not on the FITS header centre, which is the catalogue position and sits 0.8 arcseconds, 38 stellar radii, from the star at the scene epoch. One volume unit is one stellar radius (531,514,800 km, Joyce et al. 2020), so the cube spans six radii each way. The star's centre in the image is the intensity centroid, at pixel (513.0, 429.1).
