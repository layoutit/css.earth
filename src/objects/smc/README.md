...
Five published images colour one shared three-dimensional emission model; **Horálek optical is the default**. Each lens carries the same 272 directional slices (80 x, 64 y, 128 z) and the same 1,803 catalogue stars. All five lenses total 5,606,878 bytes of cloud textures. Nothing here measures gas or dust depth.
...
3. **Lenses.** [`finite-lenses-ellipsoid.json`](source/evidence/smc/constrained/finite-lenses-ellipsoid.json) bakes each registered image onto that one geometry, and the [promotion recipe](source/lenses.json) installs the five publisher-image lenses here with identity appearance controls ([receipt](source/lens-settings-evidence.json)).
...
- **App inspection.** [The inspection record](evidence/promotion-2026-09-17/inspection.json) and its captures hold this promotion's Earth view, an oblique view, both exact 90° side views, all five lens switches and stars on and off, taken from the running application by [the object's browser inspection](../../../tests/objects/browser/smc/promotion-inspection-browser.mts). Every lens decoded 128 textures at the Earth framing, a lens switch left the world camera unchanged, all 1,803 stars mounted and rendered visible, and the route raised no page error and no failing request. The side views show line-of-sight extent rather than a flat sheet.
- **Delivery.** The [prepared bank](prepared/lenses.json) and [object descriptor](object.json) identify the accepted delivery; [delivery byte pins](source/lens-manifest.json) define the reproducibility target. Startup verification reads every one of the 1,360 slice textures against those pins before the view is offered.
...
- Opening `/sun/?focus=smc` directly throws "Unknown physical host: lmc" before the scene mounts. This is a shell defect that predates the promotion: the same failure reproduces on the untouched `carina_2` and `reticulum_2`, and 461 catalogue objects name a positioned host. The sidebar search also returned no result rows in this checkout. The object is reached by its label in the shared scene.
- This object ships no compact bake inputs and no atlas delivery. A clean checkout verifies the committed bank but cannot regenerate it, and the runtime loads 272 slice images per lens where the LMC loads three axis atlases.
