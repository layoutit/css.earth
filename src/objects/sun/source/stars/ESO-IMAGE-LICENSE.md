...
The Sun photographic starfield uses the 6000 x 3000 public equirectangular
...
single-epoch sky. The prepared Sun scene therefore uses the image as a
source-qualified visual background, not as an observer-epoch claim.

The preparation pipeline adapts the published equirectangular panorama into
six retained CSS cubemap faces and applies documented luminance levels. It
does not add or bake a second Sun. The complete ESO credit is exposed in the
Sun's source resources.
