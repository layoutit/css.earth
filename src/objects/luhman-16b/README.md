...
- [`healpix-map.test.mts`](../../../tools/objects/terrestrial-layers/healpix-map.test.mts) checks the reader against HEALPix pixel centres in RING and NESTED order, and that the deposited map's dark region and bright northern region lie where the paper's figure shows them.
