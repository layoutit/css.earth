...
Run of 2026-09-16 (this version): `node tools/prepare-object.mts eris` prepared the package with the Color lens. The prepared surface, pole and thumbnail WebPs decode to 255, 250, 234 on every opaque pixel. `node --test tools/objects/observation/disc-integrated-color.test.mts site/test/object-discovery.test.mts tests/objects/unit/eris/runtime-contract.test.mts tools/object-package-consistency.test.mts tools/investigation-ledger.test.mts` passes. The page at `/eris/` renders the Color lens with no console errors; the disc centre in the [default-view capture](source/reference/rendered-default-view.png) is 255, 250, 234.
