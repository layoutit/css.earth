...
Its surface leaves overlap nothing; slices of its own mesh fill the cracks between them ([method and limits](../../../docs/surface-preparation.md#internal-fill-for-globe-seams)). Against main (elevation lens, pixelmatch, threshold 0.1): 0.39–0.64% at default zoom across five poses, 5–68% at maximum zoom.

![Main, interior slices and their pixelmatch difference at two default-zoom poses and one maximum-zoom pose](evidence/interior-slices-pixelmatch.webp)

## Known problems

At maximum zoom Chrome's 3D sort can draw the interior slice over surface leaves at some poses (up to 68% of the view at yaw 60°).
...
[Original counted triangle table](https://damit.cuni.cz/projects/damit/stored_files/open/1068/shape.txt) has 1020 vertices and 2036 triangles. It is consumed without conversion by the existing PDS plate-table reader because its count/XYZ/one-based-triangle layout is identical. This is a DAMIT dataset, not a PDS release.
...
The record gives diameter 58 km, period 7.87754 h and ecliptic J2000 pole (294°, 41°). The original mesh has measured volume-equivalent radius 29.000000 km and Cartesian extents 70.287 × 73.835 × 45.694 km. The rounded catalog diameter is used as the reference-sphere scale; the original coordinates are not rescaled.
...
- [Hanuš (2011), A study of asteroid pole-latitude distribution based on an extended set of shape models derived by the lightcurve inversion method](https://ui.adsabs.harvard.edu/abs/2011A%26A...530A.134H) — selected model publication.
