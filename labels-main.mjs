...
// src/renderers/css/labels/surface-feature-labels.oracle-main.ts
...
function mountSurfaceFeatureLabels({ host, plan, objectId, target, scene, zoomRange, navigation, flightLimits, onFlight, onSelect, lifetime, pickingHost, inputSurface, onError, transport }) {
  ...
  let destroyed = false, loadStarted = false, loaded = false, error = null, playing = false, enabled = false, frames = 0, zoomGate = false, outlinePieces = 0;
  let view = null;
  let matrix = null, local = null, catalog = null, flight = null;
  ...
  let pendingFrame = null, loopFrame = null, populateFrame = null;
  ...
  function populate(loadedValue, start) {
    populateFrame = null;
    if (destroyed) return;
  ...
  function refresh() {
    ...
    if (!labelsEnabled() || !loaded || !enabled || !projection || !zoomGate && pinnedIndex === null || view !== null && view.levelOfDetail.stage !== "geometry") {
      hideAll();
      return;
    }
