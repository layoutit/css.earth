...
const results = ['earth', 'tree', 'search'].map(view => {
  const beforeBytes = fs.readFileSync(`output/navigation-direct/before-${view}.png`);
  const afterBytes = fs.readFileSync(`output/navigation-direct/after-${view}.png`);
  ...
  fs.writeFileSync(`output/navigation-direct/diff-${view}.png`, PNG.sync.write(diff));
  ...
  fs.writeFileSync(`output/navigation-direct/diff-${view}-exact.png`, PNG.sync.write(exact));
...
const sourceFiles = git('diff','--name-only','--diff-filter=ACMRT','c3f222a').split('\n').filter(Boolean);
const report = {
  ...
  navigationPayload:{identical:fs.readFileSync('output/navigation-direct/before-navigation-tree.json').equals(fs.readFileSync('output/navigation-direct/after-navigation-tree.json')),sha256:sha(fs.readFileSync('output/navigation-direct/after-navigation-tree.json'))},results,
};
fs.writeFileSync('output/navigation-direct/pixelmatch.json',JSON.stringify(report,null,2)+'\n');
