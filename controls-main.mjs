...
// src/renderers/css/navigation/camera-input.oracle-main.ts
...
function createUnboundedMatrixDragControls({
  ...
  const applyPointerSamples = (event) => {
    ...
    for (const sampleEvent of sampleEvents) {
      if (Math.abs(sampleEvent.clientX - previousX) <= POINTER_POSITION_EPSILON && Math.abs(sampleEvent.clientY - previousY) <= POINTER_POSITION_EPSILON) {
        continue;
      }
      if (!pointerDragging) {
        const wasInteractionActive = interactionActive;
      ...
      const fittedPitch = projected.pitchDegrees * (skyGesture ? 1 : activeTrackball.pitchResponse ?? TRACKBALL_DRAG_INERTIA.directPitchResponse);
  ...
  const endPointer = (event) => {
    ...
    const throwState = wasDragging && event.type === "pointerup" && freshRelease ? estimateDragThrow({
      history,
      releaseTimestamp: event.timeStamp,
      trackball: activeTrackball,
      frameMilliseconds,
      projectRotation: skyGesture ? projectSkyRotation : void 0
    }) : null;
  ...
  return Object.freeze({
    ...
    stats() {
      return dragControlDiagnostics({
        skyGesture,
        pointerActive: pointerId !== null,
        inertiaActive: inertiaFrame !== null,
        flyToActive: surfaceFlightActive(),
        surfaceFlyToEnabled: surfaceFlyToState !== null,
        activeMode: surfaceFlightActive() ? "fly-to" : activeMode,
        pointerDragging,
        inertiaStarts,
        inertiaFrames,
        inertiaCancels,
