...
const output = "output/playwright/rhea";
await mkdir(output,{recursive:true});
const browser = await chromium.launch({headless:true,channel:process.env.PLAYWRIGHT_CHANNEL ?? "chrome"});
const scene = JSON.parse(await readFile(new URL("../../../../src/planets/rhea/prepared/scene.json",import.meta.url)));
...
const views = [["initial",{controlPitch:40,controlYaw:0,zoom:1.3}],
  ...
  ["tirawa",face(34.2,208.3,1.3)],["inktomi",face(-14.1,247.9,1.3)],
  ["eastern-source-edge",face(0,359.5,1.3)],["southern-source-edge",face(-89.5,90,1.3)],["close-limb",face(-14.1,247.9,2.5)]];
...
try {
  for (const dpr of [1,2]) {
    ...
    await page.goto(new URL("/rhea/",baseUrl).href,{waitUntil:"networkidle"});
    await page.waitForFunction(() => window.__rhea?.ready === true);
    ...
    assert.deepEqual(await page.evaluate(() => ({id:window.__cssEarth.activeObjectId,mounted:window.__cssEarth.mountedObjectCount,
      leaves:document.querySelectorAll(".rhea-body > s").length,forbidden:document.querySelectorAll(".planet-stage canvas,.planet-stage svg").length})),
      {id:"rhea",mounted:1,leaves:452,forbidden:0});
    const paint = () => page.evaluate(() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve))));
    for (const lens of ["normal", "enhanced", "elevation"]) {
    await page.evaluate(id => window.__rhea.lenses.select(id), lens);
    assert.equal(await page.locator('.planet-stage').getAttribute('data-lens'), lens);
    for (const [name,state] of views) {
      await page.evaluate(state => window.__rhea.camera.setState(state),state);
      await paint();
      assert.equal(await page.evaluate(() => window.__rhea.assertStableDomIdentity()),true);
      const path=`${output}/${lens}-${name}-dpr${dpr}.png`;
      await page.screenshot({path});
      captures.push({path,lens,dpr,camera:await page.evaluate(() => window.__rhea.camera.state())});
    }
    assert.notEqual(await page.locator(".rhea-material").evaluate(el => getComputedStyle(el).backgroundImage), "none");
    const before = await page.locator(".rhea-material").evaluate(el => getComputedStyle(el).backgroundPosition);
    await page.locator('input[name="shadows"]').evaluate(el => el.click());
    await page.waitForFunction(() => window.__rhea.material.state().lighting.rotationEnabled);
    await paint();
    assert.notEqual(await page.locator(".rhea-material").evaluate(el => getComputedStyle(el).backgroundPosition),before);
    assert.notEqual(await page.locator(".rhea-material").evaluate(el => getComputedStyle(el).backgroundImage),"none");
    await page.screenshot({path:`${output}/${lens}-shadows-dpr${dpr}.png`});
    await page.locator('input[name="shadows"]').evaluate(el => el.click());
    await page.waitForFunction(() => !window.__rhea.material.state().lighting.rotationEnabled);
    }
    await page.evaluate(() => window.__rhea.camera.setState({zoom:window.__rhea.camera.stats().minimumZoom}));
    await paint();
    assert.equal(await page.evaluate(() => window.__rhea.camera.state().levelOfDetail.stage),"marker");
    assert.equal(await page.evaluate(() => window.__rhea.assertStableDomIdentity()),true);
    await context.close();
  }
  assert.deepEqual(problems,[]);
  const manifest=await readFile(new URL("../../../../src/planets/rhea/runtime-assets.json",import.meta.url));
