...
// Usage: node tests/objects/browser/asteroid-photographic-coverage.mts <itokawa|lutetia|donaldjohanson> <before|after>
...
if(!['itokawa','lutetia','donaldjohanson'].includes(id)||!['before','after'].includes(phase))throw Error('Choose itokawa, lutetia or donaldjohanson and before or after');
...
try{
 ...
 const lens=id==='itokawa'?'amica':id==='lutetia'?'osiris':'llorri';await page.locator(`button[name="lens"][value="${lens}"]`).click();await page.waitForFunction(({id,lens})=>window.__cssearthTest.object(id).runtime.selection().committed?.lensId===lens,{id,lens});
 ...
 if(phase==='after'){
  ...
  if(id==='donaldjohanson'){
   await page.evaluate(id=>window.__cssearthTest.object(id).camera.setState({controlPitch:150,controlYaw:180,zoom:1.3}),id);
   await page.waitForTimeout(300);await page.screenshot({path:resolve(out,'hero.png')});interaction.heroCamera=await page.evaluate(id=>window.__cssearthTest.object(id).camera.state(),id);
   await page.evaluate(({id,yaw})=>window.__cssearthTest.object(id).camera.setState({controlPitch:100,controlYaw:yaw,zoom:1.7}),{id,yaw});
  }
  ...
  await page.setViewportSize({width:390,height:844});await page.evaluate(id=>{window.__cssearthTest.input('input[name="shadows"]').click();window.__cssearthTest.object(id).camera.setState({controlPitch:100,controlYaw:180,zoom:.7});},id);await page.waitForTimeout(300);await page.screenshot({path:resolve(out,'mobile.png')});
  const hi=await createTestPage(browser,{viewport:{width:1440,height:1000},deviceScaleFactor:2});hi.on('pageerror',e=>errors.push(e.message));await hi.goto(`http://127.0.0.1:4278/${id}/`,{waitUntil:'networkidle'});await hi.waitForFunction(id=>window.__cssEarth?.object(id)?.ready,id);await hi.evaluate(()=>{const m=window.__cssearthTest.input('input[name="motion"]');if(m.checked)m.click();});await hi.locator(`button[name="lens"][value="${lens}"]`).click();await hi.waitForFunction(({id,lens})=>window.__cssearthTest.object(id).runtime.selection().committed?.lensId===lens,{id,lens});await hi.evaluate(({id,yaw})=>window.__cssearthTest.object(id).camera.setState({controlPitch:100,controlYaw:yaw,zoom:1.7}),{id,yaw});await hi.waitForTimeout(300);await hi.screenshot({path:resolve(out,'dpr2.png')});interaction.dpr2Selection=await hi.evaluate(id=>window.__cssearthTest.object(id).runtime.selection(),id);await hi.close();
 }
 if(errors.length)throw Error(errors.join('\n'));
 await writeFile(resolve(out,'report.json'),JSON.stringify({id,phase,overrides,viewport:{width:1440,height:1000},dpr:1,browser:browser.version(),manifestHash:createHash('sha256').update(await readFile(`src/planets/${id}/runtime-assets.json`)).digest('hex'),runtimeManifest:JSON.parse(await readFile(`src/planets/${id}/runtime-assets.json`,'utf8')),reports,errors,interaction,selection:await page.evaluate(id=>window.__cssearthTest.object(id).runtime.selection(),id)},null,2));await page.close();
