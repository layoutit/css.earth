...
try{
 ...
 const lens=id==='itokawa'?'amica':id==='lutetia'?'osiris':id==='mathilde'?'near-msi':'llorri';await page.locator(`button[name="lens"][value="${lens}"]`).click();await page.waitForFunction(({id,lens})=>window.__cssearthTest.object(id).runtime.selection().committed?.lensId===lens,{id,lens});
