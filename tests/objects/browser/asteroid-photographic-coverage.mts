...
// Usage: node tests/objects/browser/asteroid-photographic-coverage.mts <itokawa|lutetia> <before|after>
...
if(!['itokawa','lutetia'].includes(id)||!['before','after'].includes(phase))throw Error('Choose itokawa or lutetia and before or after');
...
try{
 ...
 const lens=id==='itokawa'?'amica':id==='lutetia'?'osiris':'calibrated';await page.locator(`button[name="lens"][value="${lens}"]`).click();await page.waitForFunction(({id,lens})=>window.__cssearthTest.object(id).runtime.selection().committed?.lensId===lens,{id,lens});
