import {createObjectBrowserProfile} from '../../../../site/test/object-browser-profile.mts';
import controls from '../../../../src/objects/aethra/prepared/controls.json' with {type:'json'};
export const browserProfile=createObjectBrowserProfile({id:'aethra',controls,audit:{
  "preparedAssetPairs": [
    {
      "one": "/scenes/aethra/aethra-directional-sun.webp",
      "two": "/scenes/aethra/aethra-directional-sun@2x.webp"
    }
  ],
  "canonicalPreparedAssets": [
    "/scenes/aethra/aethra-shape-surface@2x.webp"
  ],
  "retained": {
    "lensIds": [
      "shape",
      "elevation"
    ],
    "speedClicks": 5,
    "allowedMountSelectors": []
  },
  "lensRace": {
    "defaultId": "shape",
    "slowId": "elevation",
    "winnerId": "shape",
    "slowAsset": "/scenes/aethra/aethra-elevation-surface@2x.webp",
    "preReadyDisabled": true
  }
}});
