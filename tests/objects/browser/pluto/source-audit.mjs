...
import { decodeElevationGrid, elevationRaster } from "../../unit/pluto/preparation-fixture.mjs";
import { verifyPlutoSourceManifest } from "../../unit/pluto/preparation-fixture.mjs";
...
const source = JSON.parse(await readFile(new URL("../../../../src/planets/pluto/source/manifest.json", import.meta.url)));
const report = { qualification: "SOURCE-BOUND BROWSER PRESENTATION; not native camera or pixel parity", baseUrl, capturedAt: new Date().toISOString(), channel: "chrome", headless: true, runtimeManifestSha256: sha(manifestBytes), preparedSceneSha256: sha(await readFile(new URL("../../../../src/planets/pluto/prepared/scene.json", import.meta.url))), sourceInputs: source.inputs, views: [], runtime: [] };
