...
if(input.checked!==checked)input.click();},shadows);
...
return Number(getComputedStyle(requiredElement(document.querySelector(`[data-context-label="${id}"]`))).opacity)>0.1; },id);
  const cue=await group.evaluate(group=>{
    function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
return ({placement:group.dataset.contextPlacement,label:requiredElement(group.querySelector('[data-context-label]')).textContent,
   labelOpacity:getComputedStyle(requiredElement(group.querySelector('[data-context-label]'))).opacity,
   indicatorRadius:getComputedStyle(requiredElement(group.querySelector('[data-context-indicator]'))).borderRadius,
   circleStroke:getComputedStyle(requiredElement(group.querySelector('[data-context-indicator]'))).boxShadow,
   standardCircleStroke:getComputedStyle(requiredElement(requiredElement(group.parentElement).querySelector('[data-context-group]:not([data-context-placement]):not([data-context-selected="true"]) > [data-context-indicator]'))).boxShadow,
   orbitStroke:getComputedStyle(requiredElement(group.querySelector('.context-orbit s:nth-child(odd)'))).height,
   gapColors:[...new Set([...group.querySelectorAll<HTMLElement>('.context-orbit s:nth-child(even)')].map(s=>getComputedStyle(s).backgroundColor))],
   dashColors:[...new Set([...group.querySelectorAll<HTMLElement>('.context-orbit s:nth-child(odd)')].map(s=>getComputedStyle(s).backgroundColor))]}); });
  assert.equal(cue.placement,'approximate');assert.match(cue.label,/\(approx\)/);assert.equal(cue.indicatorRadius,'50%');
  const strokeGeometry=(value: string)=>value.replace(/^rgba?\([^)]*\)\s*/, '');
  assert.equal(strokeGeometry(cue.circleStroke),strokeGeometry(cue.standardCircleStroke));assert.equal(cue.orbitStroke,'1px');assert.match(cue.circleStroke,/ 0px 0px 0px 1px inset$/);
