import {createObjectBrowserProfile} from '../../../../site/test/object-browser-profile.mjs';
import controls from '../../../../src/planets/lucia/prepared/controls.json' with {type:'json'};
export const browserProfile=createObjectBrowserProfile({id:'lucia',controls,audit:{
  "preparedAssetPairs": [
    {
      "one": "/scenes/lucia/lucia-directional-sun.webp",
      "two": "/scenes/lucia/lucia-directional-sun@2x.webp"
    }
  ],
  "canonicalPreparedAssets": [
    "/scenes/lucia/lucia-shape-surface@2x.webp"
  ],
  "retained": {
    "lensIds": [
      "shape",
      "elevation"
    ],
    "speedClicks": 5,
    "allowedMountSelectors": []
  },
  "lensRace": {
    "defaultId": "shape",
    "slowId": "elevation",
    "winnerId": "shape",
    "slowAsset": "/scenes/lucia/lucia-elevation-surface@2x.webp",
    "preReadyDisabled": true
  }
}});
