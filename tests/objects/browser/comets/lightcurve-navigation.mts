...
try{
 const page=await browser.newPage({viewport:{width:1440,height:900}}),errors=[],reports=[];
 page.on('pageerror',e=>errors.push(e.message));
 await page.goto(`${origin}/comet-137p/`,{waitUntil:'networkidle'});
 await page.waitForFunction(()=>document.documentElement.dataset.ready==='true');
 await page.evaluate(()=>{window.__cometNavigationShell=document.querySelector('.planet-sidebar')});
 for(const [id,query] of [['comet-143p','Kowal'],['comet-162p','Siding Spring'],['comet-137p','Shoemaker']]){
  ...
  await page.waitForFunction(id=>location.pathname===`/${id}/`&&document.querySelector('.planet-stage')?.dataset.objectId===id&&document.documentElement.dataset.ready==='true',id);
  ...
  assert.equal(await page.evaluate(()=>window.__cometNavigationShell===document.querySelector('.planet-sidebar')),true);
