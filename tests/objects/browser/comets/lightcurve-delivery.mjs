...
const proxy=createServer(async(req,res)=>{
 try{
  const path=new URL(req.url,'http://localhost').pathname,name=path.split('/').at(-1);
...
const freshOrigin=`http://127.0.0.1:${proxy.address().port}`;
...
try{
 for(const dpr of [1,2])for(const id of ids){
  ...
  const page=await context.newPage(),errors=[],loaded=[],pending=[];page.setDefaultTimeout(25000);
  ...
  try{
   const payload=await readFile(`src/planets/${id}/prepared/object.json`),content=JSON.parse(await readFile(`src/planets/${id}/source/content/object.json`));
   ...
   assert.equal(await page.evaluate(id=>!!window[`__${id}`],id),false,'Use production public observables');
   ...
   const input=await page.locator('.planet-input-surface').boundingBox(),x=input.x+input.width*.5,y=input.y+input.height*.5;
