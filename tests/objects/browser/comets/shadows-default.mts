...
declare global { interface Window { __cometDefaultNodes: Element[]; } }
...
try{
 for(const {id} of OBJECTS.filter(o=>o.classification==='comet')){
  ...
  try{
   const payload=await readFile(`src/objects/${id}/prepared/object.json`),definition=requireObjectRuntimeDefinition(await loadObjectTestDefinition(id));
   ...
   const lenses = definition.controls.lenses; assert.ok(lenses, 'Comet lenses are prepared');
   const lensIds=lenses.controls.map(l=>l.id),views=[];
   const finalLens = lensIds.at(-1); assert.ok(finalLens, 'Comet has at least one lens');
   const atlas=async (lens: string,lit: boolean)=>{
    const suffix=`${id}-${lens}-${lit?'shadow':'surface'}@2x.webp`;
    await page.waitForFunction(({id,suffix})=>[...document.querySelectorAll<HTMLElement>(`.${id}-body > u`)].filter(n=>getComputedStyle(n).display!=='none').every(n=>getComputedStyle(n).backgroundImage.includes(suffix)),{id,suffix});
...
return requiredElement(document.querySelector('.planet-stage')).dataset.lens===lens; },lens);
   ...
   assert.equal(await shadows.isChecked(),true);await atlas(finalLens,true);
   await page.getByRole('button',{name:'Settings',exact:true}).click();
   await page.locator('label').filter({hasText:'Shadows'}).click();await page.keyboard.press('Escape');
   assert.equal(await shadows.isChecked(),false);await atlas(finalLens,false);
