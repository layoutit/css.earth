...
export function traceDurationEvents(events: readonly TraceEvent[], start: number, end: number): TraceDurationEvent[] {
  ...
  for (const event of [...events].sort((a, b) => a.ts - b.ts)) {
    ...
    if (event.ph === 'X' && duration !== undefined && Number.isFinite(duration)) {
      if (typeof event.name !== 'string') throw new TypeError('Complete trace span requires a name');
      spans.push({...event, name:event.name, dur: duration});
    }
    else if (event.ph === 'B') {
      ...
      if (begin) {
        if (typeof begin.name !== 'string') throw new TypeError('Begin trace span requires a name');
        spans.push({ ...begin, name:begin.name, ph: 'X', dur: event.ts - begin.ts });
      }
    }
  }
