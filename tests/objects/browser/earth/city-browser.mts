...
import { parseCityFixtureBounds } from "../../unit/earth/city-fixture-schema.mts";
import { readCityFixture } from "../../unit/earth/city-fixture.mts";
import { PREPARED_EARTH_SCENE, preparePagingDiagnostic, routePagingDiagnostic } from "../../unit/earth/prepared-fixture.mts";
...
type CitySnapshot = Awaited<ReturnType<typeof snapshot>>;
type CameraState = CitySnapshot['camera'];
type Transition = {root:string;zoom:number;pose:CameraState;state:CitySnapshot};
type Boundary = {edge:{key:string;prefix:string;longitude:number;latitude?:number};zoom:number;pose:CameraState;state:CitySnapshot};
type CityReport = {dpr:number;urls?:string[];indexUrls?:string[];transitions?:Transition[];boundaries?:Boundary[];[key:string]:unknown};
type HandoffProof = {priorView:(string|null)[];handoffPending?:CitySnapshot;handoffReady?:CitySnapshot;priorPageBounds?:{key?:string;bounds:{left:number;right:number;top:number;bottom:number};intersects:boolean;painted:boolean}[];cancelled?:CitySnapshot;failed?:CitySnapshot};
declare global { interface Window {
  __cityPlan: {pages:Awaited<ReturnType<typeof readCityFixture>>['pages'];roots:string[]};
  __cityNodes: readonly Element[]; __cityLongTasks:number[]; __coarseVisibility:[HTMLElement,string][];
  __cityMotionRecords:{time:number;playing:boolean;matrix:number[];scale:number;viewport:{width:number;height:number;originX:number;originY:number};city:PageLayerStats}[];
  __cityMotionStop:boolean; __handoffLeaves:{key?:string;leaf:HTMLElement}[];
} }
...
try {
  for (const dpr of densities) {
    ...
    const urls = new Set<string>(), indexUrls = new Set<string>(), errors: string[] = [], navigations: {url:string;at:number}[] = [];
    ...
    try {
      ...
      await page.evaluate(({pages,roots}) => {
        ...
        window.__cityNodes = [...requiredElement(document.querySelector('.planet-stage')).querySelectorAll<HTMLElement>('*')];
      ...
      if(motionOnly) {
        reports.push(await measureCityMotion(page,selectedRoot??required(fixture.roots[0])));
        continue;
      }
      if(handoffOnly) {
        reports.push({dpr,browser:browser.version(),delivery:localFixture?'Pinned local proof bytes; remote delivery not tested':'Remote R2',faultProof:await checkCityHandoff(page,required(fixture.roots[0]),dpr)});
        continue;
      }
      const roots = (await page.evaluate(() => window.__cityPlan.roots)).filter(root=>!selectedRoot||root===selectedRoot);
      const memory: Record<string,Awaited<ReturnType<typeof processMemory>>> | null = dpr === 2 && !visualOnly ? {baseline:await processMemory('baseline')} : null;
...
return requiredDiagnostics(window.__earth).camera.setState({zoom}); }, zoom);
          await settle(page);
      ...
      for(const edge of selectedRoot?[]:[
        ...
        const m=required(prepareCityPageGeometry(boundary,PREPARED_EARTH_SCENE).geographicMatrix, "Boundary page must have its prepared geographic projection");
...
return requiredDiagnostics(window.__earth).camera.setState({zoom}); },zoom);
          ...
          if(boundaryOnly) {
            await page.evaluate(()=>{
              window.__coarseVisibility=[...document.querySelectorAll<HTMLElement>('.earth-surface-leaf')].map(n=>[n,n.style.visibility]);
        ...
        for(const collapsed of mobile||edge.prefix!=='face-boundary' ? [] : [true,false]) {
          const before=await page.evaluate(()=>{
            function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).runtime.pages().city.selectionRuns; });
          await page.locator('.planet-sidebar-toggle').click();
          await page.waitForFunction(before=>{
            function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).runtime.pages().city.selectionRuns>before; },before);
          await settle(page);
      ...
      await aim(page, required(roots[1]??roots[0]));
      await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:2048}); });
      await settle(page);
      ...
      if(dpr===2&&process.argv.includes('--endurance')) {
        for(let cycle=0;cycle<8;cycle++) {
          for(const root of roots) {
            ...
            await page.evaluate(()=>{
              function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:4096}); });
            await settle(page);
            const state=await snapshot(page);
            assert.ok(state.city.retained.length<=state.city.poolSize);
            assert.ok(state.city.reservedDecodedBytes<=state.city.decodedPageByteBound);
...
return requiredDiagnostics(window.__earth).camera.setState({zoom:1.1}); });
        ...
        await aim(page,required(roots[1]));
        await page.evaluate(()=>{
          function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:2048}); });
        await settle(page);
      ...
      const inputBefore=await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return ({stats:requiredDiagnostics(window.__earth).camera.stats(),scroll:scrollY}); });
      ...
      assert.ok(input && sidebar, 'Input surface and sidebar must be mounted');
...
return ({stats:requiredDiagnostics(window.__earth).camera.stats(),scroll:scrollY}); });
      ...
      const complete = new Promise<string>((resolve,reject)=>cdp.once('Tracing.tracingComplete',event=>{if(!event.stream)reject(new Error('Chrome trace stream is missing'));else resolve(event.stream);}));
      ...
      const trace = parseChromeTrace(JSON.parse(json));
      const frameStates: Record<string,number> = {};
      let missing=0,checkerboarded=0;
      for(const event of trace.traceEvents){const f=event.args?.frame_reporter;if(!f)continue;if(!f.state)throw new Error('Chrome frame reporter requires state');frameStates[f.state]=(frameStates[f.state]??0)+1;missing+=Number(Boolean(f.has_missing_content));checkerboarded+=Number(Boolean(f.checkerboarded_needs_raster||f.checkerboarded_needs_record));}
      ...
      for (const [shadows,atmosphere] of [[false,false],[true,false],[true,true],[false,true]]) {
        await page.evaluate(({shadows,atmosphere})=>{
          ...
          for(const [name,value] of Object.entries({shadows,atmosphere})) {
            const input=requiredInput(document.querySelector(`input[name="${name}"]`));
            input.checked=value;input.dispatchEvent(new Event('change',{bubbles:true}));
          }
        },{shadows,atmosphere});
        await page.waitForFunction(()=>{
          function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
          return requiredDiagnostics(window.__earth).runtime.resources().pools
            .filter(pool=>["lighting","atmosphere"].includes(pool.id)).every(pool=>pool.pending===0);
        });
        await settle(page);
        assert.deepEqual(await page.evaluate(()=>{
          function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).features.state(); }),{shadows,atmosphere});
      ...
      for(const lens of ['topography','night-lights','cross-section']) {
        await page.evaluate(lens=>{
          function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).lenses.select(lens); },lens);
        await page.evaluate(()=>{
          function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:4096}); });
        await settle(page);
      ...
      await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).lenses.select('normal'); });
      await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:1.1}); });
      await settle(page);
      ...
      const faultProof=await checkCityHandoff(page,required(roots[0]),dpr);
      await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:1.1}); });
      await settle(page);
...
return requiredDiagnostics(window.__earth).camera.setState({zoom:2048}); });
      await page.waitForFunction(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).runtime.pages().city.activeLoads>0; });
      await page.evaluate(()=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:1.1}); });
      await settle(page);
      ...
      await page.evaluate(zoom=>{
        function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom}); },mobile?1024:256);
      await settle(page);
...
return requiredDiagnostics(window.__earth).camera.setState({zoom}); },mobile?1024:256);
      ...
      await writeFile(new URL(`failure-dpr${dpr}.json`,output),JSON.stringify({error:error instanceof Error ? error.message : String(error),
        state:await snapshot(page).catch(failure=>({unavailable:failure instanceof Error ? failure.message : String(failure)})),pageErrors:errors,navigations},null,2));
  ...
  if(reports.length===2){
    ...
    for(const report of reports) {
      assert.ok(report.urls && report.transitions && report.boundaries, "DPR comparison requires complete captures");
      ...
      for(const transition of report.transitions)for(const key of transition.state.city.desired)
        assert.ok(report.urls.includes(required(canonical.get(key))),'Each selected prepared page must have been requested');
    }
    ...
    const matched=(report: CityReport)=>required(report.transitions).map(({root,zoom,state})=>({root,zoom,
    ...
    const edges=(report: CityReport)=>required(report.boundaries).map(({edge,zoom,state})=>({edge,zoom,urls:state.city.desired.map(key=>canonical.get(key)).sort()}));
...
async function measureCityMotion(page: Page,root: string) {
  ...
  const captureView=()=>page.evaluate(()=>{
    function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
    const camera=requiredElement(document.querySelector('.polycss-camera')),stage=requiredElement(document.querySelector('.planet-stage'));
    const m=new DOMMatrix(getComputedStyle(requiredElement(document.querySelector('.polycss-scene'))).transform)
      .multiply(new DOMMatrix(getComputedStyle(requiredElement(document.querySelector('.earth-system'))).transform))
      .multiply(new DOMMatrix(getComputedStyle(requiredElement(document.querySelector('.earth-body:not(.earth-body-polar)'))).transform));
...
return requiredDiagnostics(window.__earth).camera.setState({zoom}); },zoom);
      ...
      const visible=availableRoots.filter(p=>{const q=projectCityPage(parseCityFixtureBounds(p),view.matrix,view.scale,view.viewport);return q.visible&&q.span>=96;});
...
return requiredDiagnostics(window.__earth).camera.setState({zoom:4096}); });
  ...
  await page.evaluate(()=>{
    ...
    const camera=requiredElement(document.querySelector('.polycss-camera')),stage=requiredElement(document.querySelector('.planet-stage'));
    const scene=requiredElement(document.querySelector('.polycss-scene')),system=requiredElement(document.querySelector('.earth-system')),carrier=requiredElement(document.querySelector('.earth-body:not(.earth-body-polar)'));
    const c=camera.getBoundingClientRect(),s=stage.getBoundingClientRect();
    ...
    const collect=()=>{
      ...
      window.__cityMotionRecords.push({time:performance.now(),playing:document.documentElement.dataset.playing==='true',
        matrix:Array.from(m.toFloat64Array()),scale:parseFloat(getComputedStyle(camera).scale),viewport,city:requiredDiagnostics(window.__earth).runtime.pages().city});
  ...
  await page.evaluate(()=>{
    function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
    function requiredInput(value: Element | null): HTMLInputElement { if (!(value instanceof HTMLInputElement)) throw new Error("Expected required HTMLInputElement"); return value; }
window.__cityMotionStop=true;{ const motion = requiredInput(document.querySelector('input[name="motion"]')); if (motion.checked) motion.click(); }});
  await page.locator('.planet-settings-action').click();
  await settle(page);
  ...
  const prepared=required(fixture.pages.find(p=>p.key===root)),start=required(records.find(r=>r.playing)?.time);
  ...
  assert.notDeepEqual(samples[0].matrix,required(samples.at(-1)).matrix,'The actual Earth carrier must rotate');
...
async function processMemory(label: string) {
...
async function checkCityHandoff(page: Page,root: string,dpr: number) {
  await aim(page,root);
  await page.evaluate(()=>{
    function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
return requiredDiagnostics(window.__earth).camera.setState({zoom:16}); });
  await settle(page);
  ...
  await page.evaluate(()=>{window.__handoffLeaves=[...document.querySelectorAll<HTMLElement>('.earth-city-page')]
    .filter(leaf=>leaf.style.visibility==='visible').map(leaf=>({key:leaf.dataset.cityPage,leaf}));});
  const proof: HandoffProof={priorView};
  let releaseHandoff:()=>void=()=>{throw new Error('Handoff gate was not initialized');};
  const handoffGate=new Promise<void>(resolve=>{releaseHandoff=resolve;});
  const pauseHandoff=async (route: Route)=>{await handoffGate;await route.fallback().catch(()=>{});};
...
return requiredDiagnostics(window.__earth).runtime.pages().city.activeLoads>0; });
    ...
    proof.priorPageBounds=await page.evaluate(()=>{
      function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
      const stage=requiredElement(document.querySelector('.planet-stage')).getBoundingClientRect();
...
async function settle(page: Page) {
  ...
  await page.waitForFunction(()=>{
    function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
    const s=requiredDiagnostics(window.__earth).runtime.pages().city;return !s.pendingSelection && s.activeLoads===0 && s.index.activeLoads===0;
  });
}
...
async function snapshot(page: Page) {
  return page.evaluate(()=>{
    function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
    function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
return ({city:requiredDiagnostics(window.__earth).runtime.pages().city,camera:requiredDiagnostics(window.__earth).camera.state(),
    stable:requiredDiagnostics(window.__earth).assertStableDomIdentity(),nodes:requiredElement(document.querySelector('.planet-stage')).querySelectorAll<HTMLElement>('*').length,
    identical:[...requiredElement(document.querySelector('.planet-stage')).querySelectorAll<HTMLElement>('*')].every((n,i)=>n===window.__cityNodes[i]),
...
async function aim(page: Page,key: string,targetPoint?: readonly number[]) {
  return page.evaluate(({key,targetPoint})=>{
    function requiredDiagnostics<T>(value: T | undefined): T { if (value === undefined) throw new Error("Expected mounted development diagnostics"); return value; }
    function requiredElement(value: Element | null): HTMLElement { if (!(value instanceof HTMLElement)) throw new Error("Expected required HTML observation element"); return value; }
    const page=window.__cityPlan.pages.find(p=>p.key===key);
    if(!page)throw new Error('Aiming requires a known prepared city page');
    const point=targetPoint??page.corners.reduce<number[]>((v,p)=>v.map((x,i)=>x+p[i]/4),[0,0,0]);
    const scene=requiredElement(document.querySelector('.polycss-scene')),system=requiredElement(document.querySelector('.earth-system')),carrier=requiredElement(document.querySelector('.earth-body:not(.earth-body-polar)'));
    ...
    const move=(pitch: number,yaw: number)=>{const c=requiredDiagnostics(window.__earth).camera.state();requiredDiagnostics(window.__earth).camera.setState({controlPitch:c.controlPitch+pitch,controlYaw:c.controlYaw+yaw});};
    requiredDiagnostics(window.__earth).camera.setState({zoom:1.1});
    let best={score:Infinity,pitch:0,yaw:0};
    for(let pitch=0;pitch<360;pitch+=30)for(let yaw=0;yaw<360;yaw+=30){requiredDiagnostics(window.__earth).camera.setState({controlPitch:pitch,controlYaw:yaw});const p=error();const score=p[2]>0?Math.hypot(p[0],p[1]):Infinity;if(score<best.score)best={score,pitch,yaw};}
    requiredDiagnostics(window.__earth).camera.setState({controlPitch:best.pitch,controlYaw:best.yaw});
    for(let i=0;i<12;i++){
