 ...
 * objects through the sidebar search. This inspection opens the resulting native
 * `/sun/?focus=smc` route directly, so the server fragment must retain the complete
 * SMC → LMC → MW physical-host chain before the lens bank can mount.
 *
...
const directory = resolve(process.argv[3] ?? `src/objects/${OBJECT_ID}/evidence/promotion`);
...
const report: {
  ...
  inputPins, navigation: 'direct native /sun/?focus=smc route', lenses: [],
...
async function capture(name: string, view: string, lens: string, textures?: number) {
  ...
  await write();
  console.log(`SMC_INSPECTION_CAPTURE ${name} ${view}`);
}
...
try {
  ...
  await write();
  await browser.close();
  console.log(`SMC_INSPECTION_${report.result === 'passed' ? 'PASSED' : 'FAILED'} ${resolve(directory, 'inspection.json')}`);
}
