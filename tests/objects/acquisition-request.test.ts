...
test('solid observation packages retain complete pinned download refresh recipes',async()=>{
 for(const id of ['ceres','io','europa','ganymede','callisto']){
  ...
  const scientific=manifest.inputs.filter(entry=>!entry.origin.startsWith('Repository-authored'));
  assert.deepEqual(new Set(paths),new Set(scientific.map(entry=>entry.path)),`${id} refresh covers every pinned scientific input`);
  for(const entry of manifest.inputs.filter(entry=>!scientific.includes(entry)))assert.match(entry.path,/\.json$/,'Repository-authored presentation data stays in the checked source package');
 }
});
