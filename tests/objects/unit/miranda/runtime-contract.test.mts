// Draft replacement for the retired terrestrial-lane unit tests of Miranda
...
test("Miranda's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "miranda") });
...
test("Miranda is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 3 lenses", () => {
  assert.equal(scene.schema, "cssmiranda-prepared-runtime-scene@1");
  ...
  assert.equal(scene.body.sourceMapSize[0], 3200);
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("miranda-material-composite")));
...
test("Miranda lenses keep their prepared legends and false-colour declarations", () => {
...
test("Miranda publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`miranda-hide-stars`), !input.checked);
...
test("Miranda lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
