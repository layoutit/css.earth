...
test('Miranda mosaic and kilometre DEM register despite their different longitude origins', async () => {
  ...
  const entry=manifest.inputs.find((x: { lensId: string; })=>x.lensId==='normal');
  ...
  for(const [lon,lat,dn,km] of [[257.1,-24.8,486.825195,.446178168],[73.7,-29.1,1870.390747,-.233635366],[325.7,-66.9,1564.252319,1.164340019]] as const) {
    assert.ok(Math.abs(required(mosaic.sample(lon,lat))-dn)<.001);
    assert.ok(Math.abs(required(dem.sample(lon,lat))-km)<.000001);
  }
  ...
  const value=required(mosaic.sample(325.5,-66.5)),index=156*360+325;
