...
test('Thalassa uses the measured flattened figure rather than the older PCK sphere',async()=>{
  ...
  for(const [lon,lat,radius] of [[0,0,54000],[90,0,50000],[180,0,54000],[270,0,50000],[0,90,26000],[0,-90,26000]])
  ...
  const old=pck.match(/BODY804_RADII\s*=\s*\(([^)]+)\)/)[1].trim().split(/\s+/).map(Number);
