...
const root = new URL('../../../../src/objects/fornjot/source/', import.meta.url);
...
test('Fornjot preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3108.946990339562], [180,0,3108.946990339562], [90,0,2800.8531444500554], [270,0,2800.8531444500554], [0,90,2800.8531444500554], [0,-90,2800.8531444500554]] as const) {
...
test('fornjot stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('fornjot');
