...
import {loadStlShape} from '../../../../tools/objects/terrestrial-layers/obj-shape.mjs';
...
test('Nix retains the released source mesh scale and independent source anchors',async()=>{
 const recipe=JSON.parse(await readFile(new URL('preparation/terrestrial.json',source)));
 ...
 for(const [lon,lat,radius] of [[0,0,22654.277801515],[90,0,13741.85562134],[180,0,24838.7298584],[270,0,16925.86326599],[0,90,16357.99598694],[0,-90,12884.452819825]])
  assert.ok(Math.abs(mesh.sample(lon,lat)-radius)<.001,`source anchor ${lon},${lat}`);
 ...
 const release=JSON.parse(await readFile(new URL('survey/figshare-v1.json',source)));
...
test('Nix withholds unregistered photographic coverage before shared lighting',async()=>{
 ...
 const rotation=JSON.parse(await readFile(new URL('preparation/rotation.json',source)));
 assert.equal(rotation.phase,'arbitrary-display-phase');assert.equal(rotation.periodHours,43.896);
 const content=JSON.parse(await readFile(new URL('content/object.json',source)));
 assert.ok(content.settings.controls.some(control=>control.name==='shadows'));
});
