...
test('the default camera looks at the Earth-facing hemisphere, straight at the sub-Earth point', async () => {
  ...
  const view = defaultViewGeometry('antares', requireRecord(runtime.camera) as never, requireRecord(requireRecord(descriptor.properties).worldFrame) as never);
...
test('the display axis stands up on screen, with celestial north up and the Earth toward the viewer', async () => {
  ...
  const star = requireRecord(requireRecord(JSON.parse(await readFile(resolve(BODY, '../../../packages/astronomy/data/bodies/antares.json'), 'utf8')) as unknown).star);
