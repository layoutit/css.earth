...
const root=resolve(import.meta.dirname,'../../../../src/planets/astraea/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Astraea retains source identity and restoration closure',async()=>{
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,1816);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2017-05-26');
});
test('Astraea preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[-152.872,-9029.638,59584.850000000006]);assert.deepEqual(mesh.indices[0],[0,1,2]);assert.deepEqual([t.vertices,t.faces,t.components,t.eulerCharacteristic],[402,800,1,2]);
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-742757.3821436274)<.00001);const r=await read('preparation/rotation.json');assert.equal(r.periodHours,16.80059);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(config.raster.scientific[0].valueTransform.offset,-56);
...
test('Astraea keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'astraea',planetName:'Astraea',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
