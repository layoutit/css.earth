...
test('Astraea retains source identity and restoration closure',async()=>{
 ...
 const model=requireRecord(await read('reference/damit-model.json'));const fields=requireRecord(model.fields);assert.equal(model.modelId,1816);assert.equal(fields['Calibrated Size'],'1 (Yes)');assert.equal(fields.Version,'2017-05-26');
});
test('Astraea preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-742757.3821436274)<.00001);const r=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(r.periodHours,16.80059);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(lens.valueTransform.offset,-56);
...
test('Astraea keeps closed source connectivity at 800 native raster leaves',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json'));const source=await createSourceManifest({planetId:'astraea',planetName:'Astraea',sourceRoot:root}),r=requireClosedRadialTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
