// Draft replacement for the retired terrestrial-lane unit tests of Europa
...
test("Europa is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 5 lenses", () => {
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("europa-material-composite")));
...
test("Europa lenses keep their prepared legends and false-colour declarations", () => {
...
test("Europa publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`europa-hide-stars`), !input.checked);
...
test("Europa lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
