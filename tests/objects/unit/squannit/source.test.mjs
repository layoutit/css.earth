...
test('Squannit retains original JPL Beta mesh scale and topology',async()=>{
 ...
 for(const [axis,extent] of [[0,571.130],[1,463.039],[2,348.909]]) {const values=shape.positions.map(vertex=>vertex[axis]);assert.ok(Math.abs(Math.max(...values)-Math.min(...values)-extent)<.001);}
});
