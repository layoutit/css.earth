...
const root = new URL('../../../../src/objects/tarqeq/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Tarqeq preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3609.9723501129347], [180,0,3609.9723501129347], [90,0,2734.827537964344], [270,0,2734.827537964344], [0,90,2734.827537964344], [0,-90,2734.827537964344]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.32) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 3000) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 3000);
  ...
  const expectedVolume = 4/3 * Math.PI * 3000 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Tarqeq supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('tarqeq stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('tarqeq');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('tarqeq', row.jdTdb), expected = row.expectedPositionKm;
...
test('Tarqeq preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '652');
  assert.equal(evidence.identity.designation, 'S/2007 S1');
  ...
  assert.equal(rotationFact.value, "76.13 ± 0.04 h");
});
