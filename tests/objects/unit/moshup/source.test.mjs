...
test('Moshup preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,2.7645);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,0.6585);assert.equal(config.raster.scientific[0].valueTransform.offset,-0.6585);
...
test('Moshup keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'moshup',planetName:'Moshup',sourceRoot:root});
