...
const root=resolve(import.meta.dirname,'../../../../src/planets/davida/source');
const read=async path=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Davida retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'davida',planetName:'Davida',sourceRoot:root});await source.verify();
...
test('Davida preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,5.129365);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,149);assert.equal(config.raster.scientific[0].valueTransform.offset,-149);
...
test('Davida keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'davida',planetName:'Davida',sourceRoot:root});
