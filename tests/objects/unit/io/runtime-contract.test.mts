...
import assets from "../../../../src/objects/io/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/io/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/io/prepared/lenses.json" with { type: "json" };
import text from "../../../../src/objects/io/prepared/text.json" with { type: "json" };
import controls from "../../../../src/objects/io/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal","enhanced","geology","spectral-slope","visible-absorption","volcanic-heat"];

objectRuntimePackageTests(runtimeDefinition);

test("Io's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "io") });
...
test("Io is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 6 lenses", () => {
