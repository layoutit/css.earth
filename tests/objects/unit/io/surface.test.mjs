...
const verifyIoSourceManifest = () => source.verify();
const ioSourceInputsFor = consumer => source.inputsFor(consumer);
...
test('Io binds its independent source and prepared runtime closure',async()=>{
 assert.equal((await verifyIoSourceManifest()).inputCount,6); // Includes the authored surface minimap frame.
