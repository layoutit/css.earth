...
const root = new URL('../../../../src/objects/tarvos/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Tarvos preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,7684.318616023413], [180,0,7684.318616023413], [90,0,7115.109829651308], [270,0,7115.109829651308], [0,90,7115.109829651308], [0,-90,7115.109829651308]] as const) {
  ...
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 7300) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 7300);
  ...
  const expectedVolume = 4/3 * Math.PI * 7300 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Tarvos supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('tarvos stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('tarvos');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('tarvos', row.jdTdb), expected = row.expectedPositionKm;
...
test('Tarvos preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '621');
  assert.equal(evidence.identity.designation, 'S/2000 S4');
  ...
  assert.equal(rotationFact.value, "10.691 ± 0.001 h");
});
