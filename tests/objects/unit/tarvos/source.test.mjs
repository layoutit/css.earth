...
const root = new URL('../../../../src/planets/tarvos/source/', import.meta.url);
...
test('Tarvos preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,7684.318616023413], [180,0,7684.318616023413], [90,0,7115.109829651308], [270,0,7115.109829651308], [0,90,7115.109829651308], [0,-90,7115.109829651308]]) {
...
test('tarvos stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('tarvos');
...
test('Tarvos preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "10.691 ± 0.001 h");
});
