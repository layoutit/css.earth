...
const root = new URL('../../../../src/objects/thrymr/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Thrymr preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,4314.930882607614], [180,0,4314.930882607614], [90,0,3566.05858066745], [270,0,3566.05858066745], [0,90,3566.05858066745], [0,-90,3566.05858066745]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.21) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 3800) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 3800);
  ...
  const expectedVolume = 4/3 * Math.PI * 3800 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Thrymr supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('thrymr stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('thrymr');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('thrymr', row.jdTdb), expected = row.expectedPositionKm;
...
test('Thrymr preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '630');
  assert.equal(evidence.identity.designation, 'S/2000 S7');
  ...
  assert.equal(rotationFact.value, "38.79 ± 0.25 h · tentative");
});
