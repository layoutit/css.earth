...
const root = new URL('../../../../src/planets/thrymr/source/', import.meta.url);
...
test('Thrymr preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,4314.930882607614], [180,0,4314.930882607614], [90,0,3566.05858066745], [270,0,3566.05858066745], [0,90,3566.05858066745], [0,-90,3566.05858066745]]) {
...
test('thrymr stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('thrymr');
...
test('Thrymr preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "38.79 ± 0.25 h · tentative");
});
