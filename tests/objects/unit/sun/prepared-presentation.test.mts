...
function textures(f: Awaited<ReturnType<typeof preparedSelectionFixture>>) {
  ...
  return [required(nodes.find((n: { className: string; })=>n.className==="polycss-mesh sun-body")).style.getPropertyValue("--sun-surface-image"),
    required(nodes.find((n: { className: string; })=>n.className==="polycss-mesh sun-body")).style.getPropertyValue("--sun-poles-image"),
...
test("all four sourced solar layers wait for the complete replacement group", async()=>{
  ...
  try {
    ...
    f.presentation.publishFrame({selection:required(f.selection.state().committed),resources:f.residency.resources,view:{...f.view,zoom:2.25,body:{visible:true,
...
test("failed or retired solar replacement preserves complete committed layers",async()=>{
  for(const retired of [false,true]){
    const f=await preparedSelectionFixture(runtimeDefinition);
    try{
      const before=textures(f),lens=f.stage.dataset.lens,next=required(runtimeDefinition.controls.lenses).controls[1].id;
      const action=f.selection.dispatch({kind:"lens",id:next});await f.flush();
      const jobs=f.jobs.filter(job=>!job.done);assert.equal(jobs.length,4);
