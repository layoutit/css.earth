...
for (const id of ['moon', 'pluto', 'sun']) {
  test(`${id}: authored inputs regenerate the exact retained geometry and runtime`, async () => {
    ...
    const profile = await read(`src/planets/${id}/source/preparation/geometry.json`);
...
test('scientific FITS colors preserve signed polarity and authored log intensity endpoints', async () => {
  ...
  const signed = recipe.variants.find(variant => variant.fits?.color.kind === 'signed-asinh').fits.color;
  ...
  for (const variant of recipe.variants.filter(variant => variant.fits?.color.kind === 'positive-log')) {
...
test('FITS decoding rejects incomplete data and preserves signed floating observations', () => {
  ...
  const recipe = { bitpix: -32, width: 2, height: 2, latitude: 'sine-latitude', reverseLongitude: true, nearestLatitudeLimit: 1, positiveOnly: false, color: { kind: 'signed-asinh', softening: 8, maximum: 250, palette: [[28,95,190],[95,32,11],[255,224,110]] } };
