...
const root=resolve(import.meta.dirname,'../../../../src/planets/diotima/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Diotima retains source identity and restoration closure',async()=>{
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,1836);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2017-07-11');
});
test('Diotima preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-4765749.061234409)<.00001);const r=await read('preparation/rotation.json');assert.equal(r.periodHours,4.775377);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(config.raster.scientific[0].valueTransform.offset,-104.5);
...
test('Diotima keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'diotima',planetName:'Diotima',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
