...
test('the MIRI 15 µm lens, fitted to ten visits this project reduced from raw, is centred on the star-facing point', async () => {
  ...
  assert.equal(map.fit.basis.lmax, 2); assert.equal(map.fit.fit.ncurves, 1); assert.equal(map.fit.samples, 6737);
  near(map.fit.fit.chiSquared, 7015.99, 0.5, 'chi2');
  near(map.sample(0, 0)!, 519.0, 1, 'substellar temperature'); near(map.sample(180, 0)!, 44.1, 1, 'antistellar temperature');
  const day = hemisphere(0), night = hemisphere(180);
  near(day, 463.3, 1, 'dayside hemisphere');
  near(night, 242.6, 1, 'nightside hemisphere');
});
