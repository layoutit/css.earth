...
test('Despina preserves the published triaxial axes rather than using the PCK sphere as shape',async()=>{
  const cfg=JSON.parse(await readFile(new URL('source/preparation/terrestrial.json',root)));
  ...
  for(const [lon,lat,radius] of [[0,0,90000],[90,0,74000],[180,0,90000],[270,0,74000],[0,90,64000],[0,-90,64000]])
  ...
  assert.deepEqual(pck.match(/BODY805_RADII\s*=\s*\(([^)]+)\)/)[1].trim().split(/\s+/).map(Number),[74,74,74]);
