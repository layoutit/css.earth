...
test('Caliban radius table preserves independently derived physical-axis anchors',async()=>{
 ...
 for(const [lon,lat,metres] of [[0, 0, 23167.862200725947], [90, 0, 19993.368076847764], [0, 90, 19993.368076847764], [0, -90, 19993.368076847764]]) assert.ok(Math.abs(shape.sample(lon,lat)-metres)<.001,`${lon},${lat}`);
