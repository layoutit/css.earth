...
const test = sourceTest('rosalind');
import assert from 'node:assert/strict';
import { readFile } from 'node:fs/promises';
import { parsePdsRadiusTable } from '../../../../tools/objects/terrestrial-layers/obj-shape.mts';
import sharp from 'sharp';
const root = new URL('../../../../src/objects/rosalind/source/', import.meta.url);
const read = async (path: string|URL) => JSON.parse((await readFile(new URL(path, root))).toString('utf8'));

test('Rosalind retains the published axes without confusing projected and volume radii', async () => {
  ...
  for (const [lon, lat, radius] of [[0,0,36000], [90,0,36000], [180,0,36000], [270,0,36000], [0,90,36000], [0,-90,36000]] as const) {
    assert.ok(Math.abs(required(shape.sample(lon,lat))-radius)<.001, `${lon},${lat}`);
  }
  assert.ok(Math.abs(recipe.geometry.radiusKm-Math.cbrt(36*36*36))<1e-10);
  const measurements = await read('measurements.json');
  assert.deepEqual(measurements.semiAxesKm, [36,36,36]);
});

test('Rosalind grid remains explicitly unavailable surface coverage', async () => {
  const recipe = await read('preparation/terrestrial.json');
