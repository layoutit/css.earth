...
const root=resolve(import.meta.dirname,'../../../../src/planets/hermione/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Hermione retains source identity and restoration closure',async()=>{
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,156);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2017-09-21');
});
test('Hermione preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-4205891.322728729)<.00001);const r=await read('preparation/rotation.json');assert.equal(r.periodHours,5.550877);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(config.raster.scientific[0].valueTransform.offset,-100);
...
test('Hermione keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'hermione',planetName:'Hermione',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
