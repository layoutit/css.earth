...
test('Parthenope retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'parthenope',planetName:'Parthenope',sourceRoot:root});await source.verify();
...
test('Parthenope preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,13.72204);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,74.5);assert.equal(config.raster.scientific[0].valueTransform.offset,-74.5);
...
test('Parthenope keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'parthenope',planetName:'Parthenope',sourceRoot:root});
