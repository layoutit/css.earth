...
test('Fides keeps closed source connectivity at 800 native raster leaves',async()=>{
 ...
 assert.equal(r.faces.length,800);assert.equal(r.simplification.sourceFaces,2014);assert.equal(r.simplification.removedOppositeFaces,0);assert.equal(r.simplification.topology.eulerCharacteristic,2);assert.ok(r.leaves.every(l=>l.tag==='u'&&l.attributes['data-polycss-texture-leaf-sizing']==='raster'));assert.ok(r.plans.reduce((sum,p)=>sum+p.rect.width*p.rect.height,0)<=16384*r.faces.length);
