...
import {controlledShapeCamera, decodeCalibratedCamera} from '../../../../tools/objects/terrestrial-layers/shape-camera-mosaic.mjs';
...
test('Methone shape preserves the published semi-axes in physical metres',async()=>{
 ...
 for(const[lon,lat,metres]of[[0,0,1940],[90,0,1290],[180,0,1940],[270,0,1290],[0,90,1210],[0,-90,1210]])assert.ok(Math.abs(shape.sample(lon,lat)-metres)<.001);
});

test('Methone camera follows source north orientation and decodes calibrated I/F',async()=>{
 const config=JSON.parse(await readFile(new URL('preparation/terrestrial.json',root))),frame=config.raster.mosaics[0].frames[0];
 const record=JSON.parse(await readFile(new URL('survey/source-camera.json',root)));
