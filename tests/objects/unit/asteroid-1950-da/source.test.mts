...
const test = sourceTest('asteroid-1950-da');
...
const root=resolve(import.meta.dirname,'../../../../src/objects/asteroid-1950-da/source');
const read=createSourceFixtureReader(root);
test('1950 DA preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[0,0,589.121]);
 ...
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-1.1456495421769999)<.00001);
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,2.1216);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,0.65);assert.equal(config.raster.scientific[0].valueTransform.offset,-0.65);
