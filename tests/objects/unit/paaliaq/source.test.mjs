...
const root = new URL('../../../../src/planets/paaliaq/source/', import.meta.url);
...
test('Paaliaq preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,13119.881737660637], [180,0,13119.881737660637], [90,0,12495.1254644387], [270,0,12495.1254644387], [0,90,12495.1254644387], [0,-90,12495.1254644387]]) {
...
test('paaliaq stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('paaliaq');
...
test('Paaliaq preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "18.79 ± 0.09 h");
});
