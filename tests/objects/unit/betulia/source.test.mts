...
const root=resolve(import.meta.dirname,'../../../../src/objects/betulia/source');
const read=createSourceFixtureReader(root);
test('Betulia retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'betulia',planetName:'Betulia',sourceRoot:root});await source.verify();
...
test('Betulia preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[0,0,2077.743]);
 ...
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-81.9824803165423)<.00001);
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,6.13836);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,2.695);assert.equal(config.raster.scientific[0].valueTransform.offset,-2.695);
...
test('Betulia keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'betulia',planetName:'Betulia',sourceRoot:root});
 ...
 assert.equal(radial.simplification.topology.eulerCharacteristic,2);assert.ok(radial.simplification.estimatedErrorMeters<=46);
