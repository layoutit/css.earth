...
test('source-derived normalized Minnaert material reproduces every accepted light phase row',async()=>{
 ...
 const result=await preparePhotometricDisc({sourceDirectory,config}),manifest=JSON.parse(await readFile(new URL('../../../../src/objects/jupiter/runtime-assets.json',import.meta.url),'utf8'));
