...
test('source-derived normalized Minnaert material reproduces every accepted light phase row',async()=>{
 ...
 for(const{filename,bytes,sha256,data}of result.assets){assert.deepEqual({filename,bytes,sha256},manifest.assets.find(asset=>asset.filename===filename));assert.ok(data.equals(await readFile(new URL(`../../../../public/scenes/jupiter/${filename}`,import.meta.url))));}
});
