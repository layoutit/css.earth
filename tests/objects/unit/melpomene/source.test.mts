...
const test = sourceTest('melpomene');
...
test('Melpomene preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[3876.9424,4681.5166,66037.224]);
 assert.deepEqual(mesh.indices[0],[3,542,543]);
 assert.deepEqual([topology.vertices,topology.faces,topology.components,topology.eulerCharacteristic],[2162,4320,1,2]);
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-1470595.272773534)<.00001);
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,11.570306);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,70.5);assert.equal(config.raster.scientific[0].valueTransform.offset,-70.5);
...
test('Melpomene keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'melpomene',planetName:'Melpomene',sourceRoot:root});
 const radial=requireClosedTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
 assert.equal(radial.faces.length,800);assert.equal(radial.simplification.sourceFaces,4320);assert.equal(radial.simplification.removedOppositeFaces,0);
