...
test('1996 HW1 keeps a closed source-connected mesh within the retained raster budget',async()=>{
 ...
 assert.equal(radial.faces.length,800);assert.equal(radial.simplification.sourceFaces,2780);assert.equal(radial.simplification.removedOppositeFaces,0);
 assert.equal(radial.simplification.topology.eulerCharacteristic,2);assert.ok(radial.simplification.estimatedErrorMeters<=26);
...
test('1996 HW1 radius colors match independent full-source projections',async()=>{
 ...
 assert.ok(anchors.checks.filter(c=>c.kind==='radial-ambiguity').some(c=>Math.abs(c.expectedValue-c.oldFirstRayValue)>1));
});
