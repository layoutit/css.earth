...
const root = new URL('../../../../src/objects/loge/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Loge preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  assert.ok(Math.abs(major/minor - 1.04) < 1e-10);
...
test('Loge supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('loge stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('loge');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('loge', row.jdTdb), expected = row.expectedPositionKm;
...
test('Loge preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '646');
  assert.equal(evidence.identity.designation, 'S/2006 S5');
  ...
  assert.equal(rotationFact.value, "6.9 ± 0.1 h · tentative");
});
