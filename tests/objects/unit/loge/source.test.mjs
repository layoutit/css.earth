...
const root = new URL('../../../../src/planets/loge/source/', import.meta.url);
...
test('loge stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('loge');
...
test('Loge preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "6.9 ± 0.1 h · tentative");
});
