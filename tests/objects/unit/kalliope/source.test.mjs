...
test('Kalliope preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,4.1482);assert.equal(rotation.phase,'arbitrary-display-phase');
...
test('Kalliope keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'kalliope',planetName:'Kalliope',sourceRoot:root});
