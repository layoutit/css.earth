...
test("Triton's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "triton") });
...
test("Triton is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 2 lenses", () => {
  ...
  assert.equal(scene.body.sourceMapSize[0], 7168);
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("triton-material-composite")));
  assert.ok(runtimeDefinition.viewBindings.some(binding => binding.kind === "silhouette-fit"));
  assert.equal(runtimeDefinition.heliocentricView.plan.orbit.centerBodyId, "neptune", "the orbit still surrounds the parent while the Sun keeps its own position");
});

test("Triton lenses keep their prepared legends and false-colour declarations", () => {
...
test("Triton publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`triton-hide-stars`), !input.checked);
...
test("Triton lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
