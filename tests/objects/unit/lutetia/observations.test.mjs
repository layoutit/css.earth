...
import {project,diskGain} from '../../../../tools/objects/terrestrial-layers/osiris-geo.mjs';
...
test('Lutetia distinguishes archive pointing from image-controlled fit and holdout evidence',()=>{
  ...
  assert.equal(r.windows.filter(w=>w.role==='fit').length,2);assert.equal(r.windows.filter(w=>w.role==='holdout').length,2);
  for (const w of r.windows.filter(w=>w.role==='holdout')) assert.ok(Math.hypot(...w.offsetPixels.map((n,i)=>n-r.offsetPixels[i]))<12 && w.correlation>.7);
  // A half disk's photometry has an independently simple result: at i=e=60,
  // D=.5^(2k-1). Phase 30 degrees gives k=.7005.
  const gain=diskGain(Math.PI/3,Math.PI/3,{model:'minnaert',coefficient:.5505,phaseCoefficientPerDegree:.005,maximumIncidenceDegrees:70,maximumEmissionDegrees:70,maximumGain:2.5},Math.PI/6);
