...
async function sourceColorAtAtlasTexel(image:GeoTIFFImage,layout:{width:number;height:number},location:{longitude:number;latitude:number}){
 ...
 const origin=image.getOrigin(),resolution=image.getResolution(),radius=required(image.getGeoKeys()).GeogSemiMajorAxisGeoKey;
 for(let dy=0;dy<2;dy++)for(let dx=0;dx<2;dx++){
  ...
  const cells=numericRaster(await image.readRasters({window:[x,y,x+2,y+2],interleave:true}));
...
test('the actual retained Galileo-color atlas preserves original color landmarks and partial coverage',async()=>{
 ...
 try{
  ...
  for(const [longitude,latitude]of [[145.2,0],[154.4,-24.2],[145.7,-3.6],[137.3,-9.9],[113,-1.3],[144.1,32.6]] as const){
 ...
 for(const [longitude,latitude]of [[300,0],[0,0],[180,85],[180,-85]] as const){
 ...
 assert.equal(material.surfaces.find((s: { id: string; })=>s.id==='enhanced').falseColor,true);
 const lens=runtime.controls.lenses.controls.find((l: { id: string; })=>l.id==='enhanced');
 assert.equal(lens.label,'Galileo color');
 const lenses=JSON.parse(await readFile(new URL('prepared/lenses.json',root),'utf8'));
 assert.equal(lenses.controls.find((l: { id: string; })=>l.id==='enhanced').falseColor,true);
 assert.equal(lens.legend.items.length,2);assert.match(lens.description,/not calibrated I\/F or albedo/);
 const variants=runtime.variants.filter((v: { when: { lensId: string; }; })=>v.when.lensId==='enhanced');assert.ok(variants.length>0);
