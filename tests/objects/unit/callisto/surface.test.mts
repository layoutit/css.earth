...
import {publishedObservation,expectedMonochromeTexel,assertDisplayClose,countInteriorPixels} from "../observed-atlas-proof.mts";
...
const callistoSourceInputsFor = (consumer: string) => source.inputsFor(consumer).map(parseObservedSource);
...
test("source coordinates and explicit no-data agree with the actual retained atlas", async () => {
  ...
  try {
    ...
    const radius = required(source.getGeoKeys()).GeogSemiMajorAxisGeoKey;
  ...
  try{
    ...
    for(const [longitude,latitude]of [[66,24],[132,0],[211,11],[304,14.7]] as const){
  ...
  assert.ok(countInteriorPixels(surface,(r: number,g: number,b: number)=>r>0&&r<20&&Math.abs(r-g)<3&&Math.abs(r-b)<3)>100,'Nonzero dark observations survive the validity mask');
});

test("runtime closure includes Callisto's parent image and canonical surface geometry", async () => {
  const manifest = JSON.parse((await readFile(new URL("../../../../src/planets/callisto/runtime-assets.json", import.meta.url))).toString('utf8'));
  await verifyRuntimeAssetClosure({planetId:"callisto",manifest,root:publicRoot});
  const { surfaces: [surface] } = JSON.parse((await readFile(new URL("../../../../src/planets/callisto/prepared/surfaces.json", import.meta.url))).toString('utf8'));
