...
const entry=manifest.inputs.find(e=>e.id==='galileo-pia03456-registered-color');
...
function cameraPoint(longitude,latitude){
...
test('Callisto color source, frozen camera, generator and declared derivative have exact closure',async()=>{
 ...
 assert.deepEqual(registration.fit.holdouts.map(h=>h.diagnosticOffsetPixels),[[0,0],[0,0]]);
 assert.ok(registration.fit.holdouts.every(h=>!h.searchBoundary));
 assert.equal(entry.falseColor,true);assert.equal(recipe.maximumEmissionDegrees,65);
 assert.deepEqual(geo.keys.GeogSemiMajorAxisGeoKey,2410300);assert.equal(geo.keys.GeogSemiMinorAxisGeoKey,2410300);
 assert.equal(geo.keys.GTRasterTypeGeoKey,1);assert.equal(geo.keys.ProjCenterLongGeoKey,180);assert.equal(geo.noData,0);
...
test('independent Gazetteer landmarks constrain the accepted photo orientation without refitting',()=>{
 ...
 assert.ok(registration.independentReview.namedFeatures.every(f=>f.unshiftedLocalNcc>.6&&!f.diagnosticSearchBoundary));
});
...
test('every derived texel obeys the 65-degree camera footprint and original opaque RGB contributors',()=>{
 ...
 for(const [x,y]of [[0,360],[1439,360],[580,0],[580,719],[1300,360]])assert.equal(rgba[(y*1440+x)*4+3],0);
});

test('the shared observation decoder honors this actual GeoTIFF footprint and geographic pixel centers',async()=>{
 const config=await json('preparation/terrestrial.json');const lens=config.raster.observations.find(o=>o.id==='enhanced');
