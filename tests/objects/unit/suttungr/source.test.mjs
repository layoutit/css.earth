...
const root = new URL('../../../../src/planets/suttungr/source/', import.meta.url);
...
test('Suttungr preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3908.3134079823762], [180,0,3908.3134079823762], [90,0,3312.1300067647253], [270,0,3312.1300067647253], [0,90,3312.1300067647253], [0,-90,3312.1300067647253]]) {
...
test('suttungr stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('suttungr');
...
test('Suttungr preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "7.67 ± 0.02 h");
});
