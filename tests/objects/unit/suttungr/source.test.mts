...
const test = sourceTest('suttungr');
...
const root = new URL('../../../../src/objects/suttungr/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Suttungr preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3908.3134079823762], [180,0,3908.3134079823762], [90,0,3312.1300067647253], [270,0,3312.1300067647253], [0,90,3312.1300067647253], [0,-90,3312.1300067647253]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.18) < 1e-10);
...
test('Suttungr supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('suttungr stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('suttungr');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('suttungr', row.jdTdb), expected = row.expectedPositionKm;
...
test('Suttungr preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '623');
  assert.equal(evidence.identity.designation, 'S/2000 S12');
  ...
  assert.equal(rotationFact.value, "7.67 ± 0.02 h");
});
