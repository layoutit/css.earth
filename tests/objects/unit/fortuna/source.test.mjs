...
const root=resolve(import.meta.dirname,'../../../../src/planets/fortuna/source');
const read=async path=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Fortuna retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'fortuna',planetName:'Fortuna',sourceRoot:root});await source.verify();
