...
const near=(a,b)=>assert.ok(Math.abs(a-b)<=Math.max(1e-12,Math.abs(b)*1e-10),`${a} versus ${b}`);
...
test('Aemilia source preserves original ISAM OBJ identity and acquisition closure',async()=>{
 ...
 const plan=await read('source/preparation/acquisition.json');for(const i of source.manifest.inputs)assert.ok(plan.operations.some(s=>s.path===i.path),i.path);
 const input=source.manifest.inputs.find(i=>i.path==='shape/isam-sage-102.obj');assert.equal(input.expectedSha256,expected.sha256);
 assert.equal(input.origin,'http://isam.astro.amu.edu.pl/model.php?nr_planet=159&nr_modelu=102');assert.ok(plan.operations.some(s=>s.path===input.path&&s.kind==='download'&&s.url===input.origin));
...
test('Aemilia scale matches independent OBJ volume and published SAGE pole',async()=>{
 const{vertices,faces}=await original(),edges=new Map(),adjacent=Array.from({length:vertices.length},()=>new Set());let volume=0,divergence=0;
 ...
 const seen=new Set([0]),pending=[0];while(pending.length)for(const n of adjacent[pending.pop()])if(!seen.has(n)){seen.add(n);pending.push(n);}assert.equal(seen.size,vertices.length);
...
test('Aemilia prepared output retains closed native raster triangles from original OBJ',async()=>{
 ...
 const positions=[],indices=[],lookup=new Map();for(const[i,f]of terrain.faces.entries()){
  ...
  for(const v of f.vertices){const key=v.join(',');if(!lookup.has(key)){lookup.set(key,positions.length);positions.push(v);}indices.push(lookup.get(key));}
 }
 ...
 const bound=config.raster.scientific.find(s=>s.id==='elevation').surfaceSampling.maximumDistanceMeters;
 for(const[i,f]of terrain.faces.entries()){
  const centroid=[0,1,2].map(axis=>f.vertices.reduce((sum,v)=>sum+v[axis],0)*scale/3);
