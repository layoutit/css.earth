...
for(const id of ['topography']) test(`Moon ${id} matches independent original-DN anchors and source coverage`,async()=>{
  const lens=recipe.lenses.find((item: { id: string; })=>item.id===id), policy=lens.scientific;
  ...
  for(const key of ['sourcePixels','validPixels','missingPixels','rejectedRangePixels','zeroPixels'] as const)
    assert.equal(map.report[key],expected[key],key);
  for(const key of ['minimum','maximum'] as const)assert.ok(Math.abs(required(map.report[key])-expected[key])<1e-9,key);
  for(const anchor of expected.anchors){
    ...
    else assert.ok(Math.abs(required(value)-anchor.value)<1e-9,`${anchor.longitude},${anchor.latitude}`);
  }
...
for(const id of ['midnight-temperature','heat-anomalies','rock-abundance']) test(`Moon ${id} preserves compact numeric values, longitude registration and polar gaps`,async()=>{
  ...
  try {
    ...
    for(const [x,y] of [[0,1024],[4095,1024],[1536,768],[2560,1280],[2048,300]] as const){
