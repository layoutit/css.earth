...
import assets from "../../../../src/planets/moon/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/planets/moon/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/planets/moon/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/planets/moon/prepared/controls.json" with { type: "json" };
...
test("Moon is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 8 lenses", () => {
  assert.equal(scene.schema, "cssmoon-prepared-runtime-scene@1");
  ...
  assert.equal(scene.body.axialTiltDegrees, 6.68);
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("moon-material-composite")));
  assert.ok(runtimeDefinition.viewBindings.some(binding => binding.kind === "silhouette-fit"));
});

test("Moon science lenses keep their prepared legends and false-colour declarations", () => {
...
test("Moon publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["stars", "shadows"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`moon-hide-stars`), !input.checked);
...
test("Moon lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
