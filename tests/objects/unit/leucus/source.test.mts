...
const root = new URL('../../../../src/planets/leucus/source/', import.meta.url);
test('Leucus preserves the published approximation extents, not the full axis lengths as radii', async () => {
  ...
  for (const [lon, lat, metres] of [[0, 0, 30400], [90, 0, 19550], [0, 90, 13900], [0, -90, 13900]]) assert.ok(Math.abs(shape.sample(lon, lat) - metres) < .001);
