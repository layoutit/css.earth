...
import {publishedObservation,canonicalPoint,expectedMonochromeTexel,assertDisplayClose,countInteriorPixels} from '../observed-atlas-proof.mts';
...
let pending: Promise<{normal:Awaited<ReturnType<typeof publishedObservation>>;enhanced:Awaited<ReturnType<typeof publishedObservation>>}> | undefined;
function observations(){return pending??=Promise.all([publishedObservation('ganymede','normal'),publishedObservation('ganymede','enhanced')]).then(([normal,enhanced])=>({normal,enhanced}));}

test('source maps retain georeference and valid observed terrain',async()=>{
 await source.verify();const {normal,enhanced}=await observations();
 ...
 for(const entry of source.inputsFor('surfaces').map(parseObservedSource)){
  const tiff=await fromFile(new URL(entry.path,sourceRoot).pathname);
  try{const image=await tiff.getImage(),expected=entry.lensId==='normal'?anchors.normal:anchors.enhanced;assert.equal(image.getGDALNoData(),0);
   assert.equal(required(image.getGeoKeys()).ProjCenterLongGeoKey,180);
   ...
   assert.deepEqual(required((entry.lensId==='normal'?normal:enhanced).record.sourceGeoreference).origin.slice(0,2),expected.origin);
   if(entry.lensId==='normal')for(const [x,y]of [[1500,1500],[4000,2048],[6000,2000],[7200,2800]] as const){
...
test('enhanced color preserves monochrome across synthesized and missing source coverage',async()=>{
 const {normal,enhanced}=await observations();
 ...
 assert.ok(countInteriorPixels(enhanced,(r: number,g: number,b: number)=>Math.max(r,g,b)-Math.min(r,g,b)>12)>8192*4096/4,'The actual runtime atlas retains real color outside withheld coverage');
 assert.ok(required(enhanced.record.withheldSyntheticPixels)>0);assert.ok(required(enhanced.record.monochromePixels)>8192*4096/10);
