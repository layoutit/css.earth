// Draft: keeps the unchanged Ganymede nomenclature recipe proven on the generic lane
...
import descriptor from "../../../../src/objects/ganymede/prepared/features.json" with { type: "json" };
import { parsePreparedSurfaceFeatureCatalog } from "../../../../src/renderers/css/dist/index.js";

const plan = runtimeDefinition.features;
const bytes = await readFile(new URL("../../../../public/scenes/ganymede/ganymede-features.json", import.meta.url));
const catalog = parsePreparedSurfaceFeatureCatalog(JSON.parse(bytes.toString("utf8")), plan, "ganymede");

test("the prepared Ganymede nomenclature catalogue is pinned by the runtime plan and the provenance descriptor", () => {
  ...
  assert.equal(plan.catalog.count, 196, "the unchanged features recipe keeps the terrestrial-lane count");
