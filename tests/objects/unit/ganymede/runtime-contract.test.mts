...
import assets from "../../../../src/planets/ganymede/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/planets/ganymede/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/planets/ganymede/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/planets/ganymede/prepared/controls.json" with { type: "json" };
...
test("Ganymede publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`ganymede-hide-stars`), !input.checked);
...
test("Ganymede lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
