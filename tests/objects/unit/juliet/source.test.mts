...
test('Juliet source preserves the published prolate semiaxes in metres', async () => {
  ...
  for (const [lon, lat, metres] of [[0,0,75000],[90,0,37000],[180,0,75000],[270,0,37000],[0,90,37000],[0,-90,37000]] as const) {
    assert.ok(Math.abs(required(shape.sample(lon, lat)) - metres) < 0.001, `${lon},${lat}`);
  }
});

test('Juliet shape model has no invented observed texels', async () => {
