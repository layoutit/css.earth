...
const test = sourceTest('asteroid-1998-wt24');
...
test('1998 WT24 preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(rotation.periodHours,3.697);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,0.2075);assert.equal(lens.valueTransform.offset,-0.2075);
...
test('1998 WT24 keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json')),source=await createSourceManifest({planetId:'asteroid-1998-wt24',planetName:'1998 WT24',sourceRoot:root});
