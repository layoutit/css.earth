...
test('666 Desdemona retains source identity and restoration closure',async()=>{
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,6218);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2022-10-18');
});
