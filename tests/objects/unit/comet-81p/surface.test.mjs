...
test('Wild 2 retains original observed positions, open boundaries and all source patches',async()=>{
 ...
 assert.equal(terrain.faces.length,1494);assert.equal(after.boundaryEdges,348);assert.equal(after.edgeComponents,8);assert.equal(after.eulerCharacteristic,1);
 const runtime=await json('prepared/runtime.json');assert.equal(runtime.surfaceHit.frontFace,'clockwise');
 assert.equal(runtime.surfaceHit.triangles.length,1494);assert.equal((await json('source/preparation/rotation.json')).schema,'cssearth-display-orientation@1');
});
