...
test('Charon MVIC bands and missing masks match independent NumPy source anchors', async () => {
  ...
  const policy = config.raster.observations.find(x => x.id === 'enhanced-color').validity;
  const entry = manifest.inputs.find(x => x.lensId === 'enhanced-color');
  ...
  const anchors = JSON.parse(await readFile(new URL('validation/color-source-inspection.json', root)));
