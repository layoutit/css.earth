...
const root = new URL('../../../../src/objects/skoll/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Skoll preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,2509.946147847534], [180,0,2509.946147847534], [90,0,2201.707147234679], [270,0,2201.707147234679], [0,90,2201.707147234679], [0,-90,2201.707147234679]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.14) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 2300) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 2300);
  ...
  const expectedVolume = 4/3 * Math.PI * 2300 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Skoll supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('skoll stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('skoll');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('skoll', row.jdTdb), expected = row.expectedPositionKm;
...
test('Skoll preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '647');
  assert.equal(evidence.identity.designation, 'S/2006 S8');
  ...
  assert.equal(rotationFact.value, "7.26 ± 0.09 h · tentative");
});
