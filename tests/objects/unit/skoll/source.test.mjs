...
const root = new URL('../../../../src/planets/skoll/source/', import.meta.url);
...
test('Skoll preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,2509.946147847534], [180,0,2509.946147847534], [90,0,2201.707147234679], [270,0,2201.707147234679], [0,90,2201.707147234679], [0,-90,2201.707147234679]]) {
...
test('skoll stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('skoll');
...
test('Skoll preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "7.26 ± 0.09 h · tentative");
});
