...
import { writeSurfaceDelivery } from "../../../../tools/objects/paged-ellipsoid/surface-page-delivery.mjs";

const plan = {
  atlas: { density: 8, maximumDeliveryHeight: 1024 },
...
for (const density of [2, 4, 8]) test(`delivery preserves accepted colors, alpha and texel phase at density ${density}`, async () => {
  ...
  const receipt = await writeSurfaceDelivery({ plan, density,
  ...
  assert.deepEqual(await writeSurfaceDelivery({ plan, density,
...
test("delivery rejects incompatible density, source dimensions and out-of-bounds strips", async () => {
  ...
  const run = (overrides) => writeSurfaceDelivery({ plan, density: 8,
    encodeSourcePage: async () => encoded, writePage: async () => {}, ...overrides });
  await assert.rejects(run({ density: 3 }), /incompatible/);
  await assert.rejects(run({ plan: { ...plan, atlas: undefined } }), /incompatible/);
  await assert.rejects(run({ plan: { ...plan, atlas: { ...plan.atlas, maximumDeliveryHeight: 4 } } }), /bounds/);
