...
test("archived city noise source remains pinned but is absent from the globe MVP", async () => {
  const pin=shape({file:text,decodedSha256:text})(JSON.parse((await readFile(new URL("../../../../src/planets/earth/source/noise/manifest.json",import.meta.url))).toString('utf8')));
  ...
  const assets=shape({assets:array(shape({filename:text}))})(JSON.parse((await readFile(new URL("../../../../src/planets/earth/runtime-assets.json",import.meta.url))).toString('utf8')));
