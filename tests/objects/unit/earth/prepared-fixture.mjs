...
export const PREPARED_EARTH_NOISE=(await read('prepared/observation-buenos-aires-noise.json')).plan,PREPARED_EARTH_CITY_PAGES=await read('prepared/pages.json'),PREPARED_EARTH_PLACES=await read('prepared/places.json');
...
export {config as earthPreparationConfig,sourceDirectory as earthSourceDirectory};
const geographicLenses=await read('prepared/geographic-lenses.json');
export const PREPARED_GEOGRAPHIC_LENSES=geographicLenses.inventory;
export const PREPARED_ROOT_GEOGRAPHIC_LENSES=geographicLenses.rootLenses;
export const PREPARED_EARTH_LAND_COVER_DESCRIPTOR=(await read('prepared/observation-worldcover-land-cover.json')).descriptor;
