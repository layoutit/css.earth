...
const snapshot = await readPublishedCoverage(publishedCoveragePath);
...
test("published coverage is pinned and matches the reproducible source-window plans", async () => {
  ...
  const manifest = JSON.parse(await readFile(new URL("../../../../src/planets/earth/source/manifest.json", import.meta.url)));
