...
import { PREPARED_EARTH_NOISE as plan } from "../../unit/earth/prepared-fixture.mjs";
import { PREPARED_EARTH_LENSES } from "../../unit/earth/prepared-fixture.mjs";
const pin=JSON.parse(await readFile(new URL("../../../../src/planets/earth/source/noise/manifest.json",import.meta.url))),source=gunzipSync(await readFile(new URL(`../../../../src/planets/earth/source/noise/${pin.file}`,import.meta.url)));
const data=JSON.parse(source);
const hash=b=>createHash("sha256").update(b).digest("hex");
test("noise overlay is bound to the official 2025 daytime source and explicit units",()=>{
  ...
  const lens=PREPARED_EARTH_LENSES.controls.find(l=>l.id==="buenos-aires-noise");assert.ok(lens);assert.match(lens.qualification,/not live measurements/);
  assert.deepEqual(lens.legend.map(l=>l.label),plan.legend.map(l=>l.label));
});
...
test("prepared noise texels agree with independent source polygon classification",async()=>{
  ...
  for(const [lon,lat] of [[-58.382,-34.604],[-58.393,-34.588],[-58.44,-34.61],[-58.48,-34.65],[-58.42,-34.58]]){
    ...
    const page=plan.roots.find(p=>p.x===Math.floor(px/1024)&&p.y===Math.floor(py/1024));
