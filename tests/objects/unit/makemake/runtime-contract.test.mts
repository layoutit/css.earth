// Draft replacement for the retired shape-model unit tests of Makemake
...
import runtimeDefinition from "../../../../src/objects/makemake/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/objects/makemake/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/makemake/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/makemake/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/objects/makemake/prepared/controls.json" with { type: "json" };
...
test("Makemake's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "makemake") });
...
test("Makemake lenses keep their prepared legends and false-colour declarations", () => {
...
test("Makemake publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`makemake-hide-stars`), !input.checked);
...
test("Makemake lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
