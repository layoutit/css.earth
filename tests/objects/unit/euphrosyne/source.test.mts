...
const test = sourceTest('euphrosyne');
...
const root=resolve(import.meta.dirname,'../../../../src/objects/euphrosyne/source');
const read=createSourceFixtureReader(root);
test('Euphrosyne preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[-11314.220000000001,-9070.7521,124792.94]);
 assert.deepEqual(mesh.indices[0],[0,802,803]);
 assert.deepEqual([topology.vertices,topology.faces,topology.components,topology.eulerCharacteristic],[3202,6400,1,2]);
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-10101918.116371565)<.00001);
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,5.529595);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,134);assert.equal(config.raster.scientific[0].valueTransform.offset,-134);
