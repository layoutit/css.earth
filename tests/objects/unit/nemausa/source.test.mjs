...
const root=resolve(import.meta.dirname,'../../../../src/planets/nemausa/source');
const read=async path=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Nemausa retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'nemausa',planetName:'Nemausa',sourceRoot:root});await source.verify();
...
test('Nemausa preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,7.78484);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,75);assert.equal(config.raster.scientific[0].valueTransform.offset,-75);
...
test('Nemausa keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'nemausa',planetName:'Nemausa',sourceRoot:root});
