...
export function testLightcurveModel(id,ab,bc,radius,longitude,latitude) {
 test(`${id}: delivered approximation retains published proportions and closes one surface`,async()=>{
  ...
  const config=await json(`${root}/source/preparation/terrestrial.json`),terrain=await json(`${root}/prepared/terrain.json`);
  assert.equal(terrain.faces.length,800);
  const surfaces=await json(`${root}/prepared/surfaces.json`);
 ...
 test(`${id}: fixed nominal pole recovers the published J2000 ecliptic coordinates`,async()=>{
  const root=resolve('src/planets',id),d=await json(`${root}/object.json`),ref=d.properties.recipe.sources.find(r=>r.id==='rotation');
