...
const root=resolve(import.meta.dirname,'../../../../src/planets/hektor/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Hektor retains source identity and restoration closure',async()=>{
 const source=await createSourceManifest({planetId:'hektor',planetName:'Hektor',sourceRoot:root});await source.verify();
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,232);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2016-06-27');
});
...
test('Hektor keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'hektor',planetName:'Hektor',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
