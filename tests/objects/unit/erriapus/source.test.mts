...
const test = sourceTest('erriapus');
...
const root = new URL('../../../../src/objects/erriapus/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Erriapus preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,6712.559386509957], [180,0,6712.559386509957], [90,0,4445.403567225137], [270,0,4445.403567225137], [0,90,4445.403567225137], [0,-90,4445.403567225137]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.51) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 5100) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 5100);
  ...
  const expectedVolume = 4/3 * Math.PI * 5100 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Erriapus supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('erriapus stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('erriapus');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('erriapus', row.jdTdb), expected = row.expectedPositionKm;
...
test('Erriapus preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '628');
  assert.equal(evidence.identity.designation, 'S/2000 S10');
  ...
  assert.equal(rotationFact.value, "28.15 ± 0.25 h");
});
