...
const root = new URL('../../../../src/planets/erriapus/source/', import.meta.url);
...
test('Erriapus preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,6712.559386509957], [180,0,6712.559386509957], [90,0,4445.403567225137], [270,0,4445.403567225137], [0,90,4445.403567225137], [0,-90,4445.403567225137]]) {
...
test('erriapus stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('erriapus');
...
test('Erriapus preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "28.15 ± 0.25 h");
});
