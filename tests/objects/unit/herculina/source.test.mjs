...
const root=resolve(import.meta.dirname,'../../../../src/planets/herculina/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Herculina retains source identity and restoration closure',async()=>{
 const source=await createSourceManifest({planetId:'herculina',planetName:'Herculina',sourceRoot:root});await source.verify();
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,1814);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2017-06-19');
});
test('Herculina preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-3514536.3589627864)<.00001);const r=await read('preparation/rotation.json');assert.equal(r.periodHours,9.404937);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(config.raster.scientific[0].valueTransform.offset,-94.5);
...
test('Herculina keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'herculina',planetName:'Herculina',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
