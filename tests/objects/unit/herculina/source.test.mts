...
const root=resolve(import.meta.dirname,'../../../../src/planets/herculina/source'),read=async (p:string):Promise<unknown>=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Herculina retains source identity and restoration closure',async()=>{
 ...
 const model=requireRecord(await read('reference/damit-model.json'));const fields=requireRecord(model.fields);assert.equal(model.modelId,1814);assert.equal(fields['Calibrated Size'],'1 (Yes)');assert.equal(fields.Version,'2017-06-19');
});
