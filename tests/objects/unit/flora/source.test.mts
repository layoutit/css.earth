...
const root=resolve(import.meta.dirname,'../../../../src/objects/flora/source');
const read=async (path:string):Promise<unknown>=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Flora retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'flora',planetName:'Flora',sourceRoot:root});await source.verify();
...
test('Flora preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[117.07805,281.97267999999997,62424.511000000006]);
 assert.deepEqual(mesh.indices[0],[0,1002,1003]);
 assert.deepEqual([topology.vertices,topology.faces,topology.components,topology.eulerCharacteristic],[4002,8000,1,2]);
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-1498117.6037469828)<.00001);
 const rotation=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(rotation.periodHours,12.86667);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,73);assert.equal(lens.valueTransform.offset,-73);
...
test('Flora keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json')),source=await createSourceManifest({planetId:'flora',planetName:'Flora',sourceRoot:root});
 const radial=requireClosedRadialTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
 assert.equal(radial.faces.length,800);assert.equal(radial.simplification.sourceFaces,8000);assert.equal(radial.simplification.removedOppositeFaces,0);
