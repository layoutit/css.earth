...
test('Steins source mesh preserves released kilometer coordinates and independent principal rays',async()=>{
 ...
 for(const [lon,lat,r]of [[0,0,3165.583232014827],[90,0,2905.4732933897985],[180,0,3574.3718085389255],[270,0,2626.214531407711],[0,90,2155.1841971449626],[0,-90,1958.590202996904]])assert.ok(Math.abs(mesh.sample(lon,lat)-r)<1e-6);
});

test('Steins cylindrical map retains eastward geography, dark crater pixels and connected coverage',async()=>{
 const c=await read('preparation/terrestrial.json'),manifest=await read('manifest.json'),entry=manifest.inputs.find(x=>x.id==='steins-normal');
 ...
 for(const[x,y,v]of [[0,900,203],[3150,600,20],[400,1000,174]]){const i=y*3600+x;assert.deepEqual([...map.rgb.subarray(i*3,i*3+3)],[v,v,v]);assert.equal(map.missing[i],0)}
 ...
 assert.equal(map.sourceMissingPixels,3567208);
});
...
test('Steins rotation uses the revised PCK model that matches the released pole',async()=>{
 ...
 const values=key=>pck.match(new RegExp(`BODY2002867_${key}\\s*=\\s*\\(\\s*([-\\d.]+)\\s+([-\\d.]+)`)).slice(1).map(Number);
...
test('Steins reduced mesh retains bounded source fit over independent equal-area rays',async()=>{
 ...
 const text=faces.flatMap(f=>f.vertices.map(v=>'v '+v.map(n=>n*scale).join(' '))).join('\n')+'\n'+faces.map((_,i)=>`f ${i*3+1} ${i*3+2} ${i*3+3}`).join('\n');
 const mesh=parseObjShape(text,{metersPerUnit:1,expectedVertices:faces.length*3,expectedFaces:faces.length}),errors=[];
 for(let y=0;y<40;y++)for(let x=0;x<80;x++){const lon=(x+.37)/80*360,lat=Math.asin(-1+2*(y+.5)/40)*180/Math.PI,a=source.sample(lon,lat),b=mesh.sample(lon,lat);assert.notEqual(b,null);errors.push(Math.abs(a-b));}
 errors.sort((a,b)=>a-b);assert.ok(errors[3040]<35);assert.ok(errors.at(-1)<65);
});
