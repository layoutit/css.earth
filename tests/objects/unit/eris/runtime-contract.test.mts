// Draft replacement for the retired shape-model unit tests of Eris
...
import runtimeDefinition from "../../../../src/objects/eris/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/objects/eris/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/eris/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/eris/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/objects/eris/prepared/controls.json" with { type: "json" };
...
test("Eris's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "eris") });
...
test("Eris is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 1 lens", () => {
  assert.equal(scene.schema, "csseris-prepared-runtime-scene@1");
...
test("Eris lenses keep their prepared legends and false-colour declarations", () => {
...
test("Eris publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`eris-hide-stars`), !input.checked);
...
test("Eris lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
