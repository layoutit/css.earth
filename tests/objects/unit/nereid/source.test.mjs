...
test('Nereid preserves the source dimensions as a closed physical surface', async () => {
  ...
  for (const [lon, lat, radius] of [[0,0,170000], [90,0,170000], [180,0,170000], [270,0,170000], [0,90,170000], [0,-90,170000]]) {
    assert.ok(Math.abs(shape.sample(lon,lat)-radius)<.001, `${lon},${lat}`);
  }
  assert.ok(Math.abs(recipe.geometry.radiusKm-Math.cbrt(170*170*170))<1e-10);
