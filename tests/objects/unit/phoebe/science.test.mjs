...
import {decodeIsis3Raster} from '../../../../tools/objects/terrestrial-layers/isis3-raster.mjs';
...
test('Phoebe paired cubes retain every native value and actual missing-maplet sentinel',async()=>{
 const proof=JSON.parse(await readFile(resolve(root,'validation/2023-source-statistics.json')));
 for(const [key,p] of Object.entries(proof.products)){
  ...
  let low=Infinity,high=-Infinity,finite=0;const counts={};
...
test('Phoebe support policy rejects filled albedo at zero/few images or missing maplet; thresholds remain exact',async()=>{
 const config=JSON.parse(await readFile(resolve(root,'preparation/terrestrial.json'))),proof=JSON.parse(await readFile(resolve(root,'validation/2023-source-statistics.json')));
 const lens=config.raster.scientific.find(x=>x.id==='normal'),albedo=await loadScienceSurface(root,lens),raw=await loadScienceSurface(root,{...lens,qualityMasks:undefined});
 const count=await loadScienceSurface(root,config.raster.scientific.find(x=>x.id==='image-count'));
 const maplet=await loadScienceSurface(root,config.raster.scientific.find(x=>x.id==='maplet-resolution'));
 ...
 for(let y=0;y<180;y++)for(let x=0;x<360;x++){
  const lon=x+.5,lat=89.5-y,c=count.sample(lon,lat),m=maplet.sample(lon,lat),v=albedo.sample(lon,lat),pass=c>=5&&m!==null&&m<=1500;
