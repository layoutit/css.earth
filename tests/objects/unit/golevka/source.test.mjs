...
const root=resolve(import.meta.dirname,'../../../../src/planets/golevka/source');
const read=async path=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Golevka retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'golevka',planetName:'Golevka',sourceRoot:root});await source.verify();
