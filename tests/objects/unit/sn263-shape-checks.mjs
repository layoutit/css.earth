...
export function checkSn263Shape(id,component){
 ...
 test(`${id}: PDS component, units, input closure and shape agree`,async()=>{
  ...
  const entry=source.manifest.inputs.find(x=>x.id==='radar-shape');
  ...
  assert.ok(label.includes(entry.product.split('::')[0]),'native product identity, not a fabricated LID');
 ...
 test(`${id}: prepared mesh preserves closed shape and bounded source fit`,async()=>{
  ...
  assert.ok(faces.length<=800);assert.equal(faces.simplification.topology.components,1);
  ...
  for(let lat=-85;lat<=85;lat+=10)for(let lon=0;lon<360;lon+=10){
   const before=mesh.sample(lon,lat),after=reduced.sample(lon,lat);
 ...
 if(component!=='alpha')test(`${id}: the parent context has source-backed point photometry`,async()=>{
  ...
  const parent=system.bodies.find(body=>body.id==='asteroid-2001-sn263');
  assert.ok(parent.pointPresentation.samples.every(Number.isFinite));assert.equal(parent.illumination.geometricAlbedo,.048);
 });
