...
const LENS_IDS = ["normal","enhanced","elevation"];
...
test("Ceres is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 3 lenses", () => {
...
test("Ceres publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`ceres-hide-stars`), !input.checked);
