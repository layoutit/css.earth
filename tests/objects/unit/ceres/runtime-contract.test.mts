// Draft replacement for the retired terrestrial-lane unit tests of Ceres
...
import assets from "../../../../src/objects/ceres/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/ceres/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/ceres/prepared/lenses.json" with { type: "json" };
import text from "../../../../src/objects/ceres/prepared/text.json" with { type: "json" };
import controls from "../../../../src/objects/ceres/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal","enhanced","elevation","clay-band","ammonium-band"];

objectRuntimePackageTests(runtimeDefinition);

test("Ceres's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "ceres") });
...
test("Ceres is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 5 lenses", () => {
  assert.equal(scene.schema, "cssceres-prepared-runtime-scene@1");
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("ceres-material-composite")));
  assert.ok(runtimeDefinition.viewBindings.some(binding => binding.kind === "silhouette-fit"));
});

test("Ceres lenses keep their prepared legends and false-colour declarations", () => {
...
test("Ceres publishes every declared toggle through the shared controls and selection owner", async () => {
...
test("Ceres lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
