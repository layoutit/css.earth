...
test('the default camera looks at the substellar point with the orbit normal up', async () => {
  ...
  const view = defaultViewGeometry('hd-189733b', requireRecord(runtime.camera) as never, requireRecord(requireRecord(descriptor.properties).worldFrame) as never);
  ...
  const m = requireBodyFixedToIcrf('hd-189733b');
