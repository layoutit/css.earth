...
const root=resolve(import.meta.dirname,'../../../../src/objects/irene/source'),read=async (p:string):Promise<unknown>=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Irene retains source identity and restoration closure',async()=>{
 ...
 const model=requireRecord(await read('reference/damit-model.json'));const fields=requireRecord(model.fields);assert.equal(model.modelId,1842);assert.equal(fields['Calibrated Size'],'1 (Yes)');assert.equal(fields.Version,'2017-09-21');
});
test('Irene preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-1874517.2568236503)<.00001);const r=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(r.periodHours,15.02987);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(lens.valueTransform.offset,-76.5);
...
test('Irene keeps closed source connectivity at 800 native raster leaves',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json'));const source=await createSourceManifest({planetId:'irene',planetName:'Irene',sourceRoot:root}),r=requireClosedRadialTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
