// Draft replacement for the retired terrestrial-lane unit tests of Iapetus
...
import runtimeDefinition from "../../../../src/objects/iapetus/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/objects/iapetus/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/iapetus/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/iapetus/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/objects/iapetus/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal","enhanced","infrared","ice-absorption"];

objectRuntimePackageTests(runtimeDefinition);

test("Iapetus's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "iapetus") });
...
test("Iapetus is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 4 lenses", () => {
  assert.equal(scene.schema, "cssiapetus-prepared-runtime-scene@1");
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("iapetus-material-composite")));
  assert.ok(runtimeDefinition.viewBindings.some(binding => binding.kind === "silhouette-fit"));
  assert.equal(runtimeDefinition.heliocentricView.plan.orbit.centerBodyId, "saturn", "the orbit still surrounds the parent while the Sun keeps its own position");
});

test("Iapetus lenses keep their prepared legends and false-colour declarations", () => {
...
test("Iapetus publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`iapetus-hide-stars`), !input.checked);
...
test("Iapetus lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
