// Draft: keeps the unchanged Iapetus nomenclature recipe proven on the generic lane
...
import descriptor from "../../../../src/objects/iapetus/prepared/features.json" with { type: "json" };
import { parsePreparedSurfaceFeatureCatalog } from "../../../../src/renderers/css/dist/index.js";

const plan = runtimeDefinition.features;
const bytes = await readFile(new URL("../../../../public/scenes/iapetus/iapetus-features.json", import.meta.url));
const catalog = parsePreparedSurfaceFeatureCatalog(JSON.parse(bytes.toString("utf8")), plan, "iapetus");

test("the prepared Iapetus nomenclature catalogue is pinned by the runtime plan and the provenance descriptor", () => {
...
test("the plan anchors labels to the single body mesh for every surface lens", () => {
  ...
  assert.match(node.className ?? "", /(^|\s)iapetus-body(\s|$)/u);
  assert.equal(runtimeDefinition.tree.nodes.filter(n => /(^|\s)iapetus-body(\s|$)/u.test(n.className ?? "")).length, 1);
