...
test('Gaspra radial model keeps independent west-longitude source anchors',async()=>{
 ...
 for(const[lon,lat,radius]of [[0,0,10564.5],[90,0,5679.4],[180,0,7628.6],[270,0,4959.2],[0,90,4463.6],[0,-90,4466.7]])assert.ok(Math.abs(grid.sample(lon,lat)-radius)<1e-8);
...
test('Gaspra monochrome registration preserves source samples and zero coverage',async()=>{
 const config=await read('preparation/terrestrial.json'),manifest=await read('manifest.json'),entry=manifest.inputs.find(e=>e.id==='gaspra-normal'),policy=config.raster.observations[0].validity;
...
test('Gaspra observation geometry rejects the misleading generic FITS display orientation',async()=>{
 const report=await evaluateRegistration(),accepted=report.candidates.find(c=>c.rows==='north-to-south'&&c.samplesIncrease==='east'),literal=report.candidates.find(c=>c.rows==='south-to-north'&&c.samplesIncrease==='west');
...
test('Gaspra rotation binds the pinned NAIF pole and spin',async()=>{
 ...
 const value=key=>pck.match(new RegExp(`BODY9511010_${key}\\s*=\\s*\\(\\s*([\\d.]+)\\s+([\\d.]+)`)).slice(1).map(Number);
...
test('Gaspra prepared silhouette keeps bounded sampled error against the source radii',async()=>{
 ...
 const text=faces.flatMap(f=>f.vertices.map(v=>'v '+v.map(n=>n*metersPerUnit).join(' '))).join('\n')+'\n'+faces.map((_,i)=>`f ${i*3+1} ${i*3+2} ${i*3+3}`).join('\n');
 ...
 for(let y=0;y<40;y++)for(let x=0;x<80;x++){
  const lon=(x+.37)/80*360,lat=Math.asin(-1+2*(y+.5)/40)*180/Math.PI,a=source.sample(lon,lat),b=reduced.sample(lon,lat);
  assert.notEqual(b,null,'Every source ray must still intersect the reduced surface');errors.push(Math.abs(a-b));
 }
 errors.sort((a,b)=>a-b);assert.ok(errors[3040]<100);assert.ok(errors.at(-1)<250);
});
