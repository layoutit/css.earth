...
const root = new URL('../../../../src/planets/skathi/source/', import.meta.url);
...
test('skathi stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('skathi');
...
test('Skathi preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "11.1 ± 0.02 h");
});
