...
const root = new URL('../../../../src/objects/skathi/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Skathi preserves the selected lower-limit elongation and explicit volume scale', async () => {
...
test('Skathi supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('skathi stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('skathi');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('skathi', row.jdTdb), expected = row.expectedPositionKm;
...
test('Skathi preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '627');
  assert.equal(evidence.identity.designation, 'S/2000 S8');
  ...
  assert.equal(rotationFact.value, "11.1 ± 0.02 h");
});
