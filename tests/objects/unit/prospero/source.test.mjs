...
const root=new URL('../../../../src/planets/prospero/source/',import.meta.url);
test('Prospero radius table preserves independently derived physical-axis anchors',async()=>{
 ...
 for(const [lon,lat,metres] of [[0, 0, 32156.83831473364], [90, 0, 22043.134064691756], [0, 90, 22043.134064691756], [0, -90, 22043.134064691756]]) assert.ok(Math.abs(shape.sample(lon,lat)-metres)<.001,`${lon},${lat}`);
