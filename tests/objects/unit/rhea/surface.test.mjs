...
test('Rhea elevation uses measured radii, actual geotransform and source bounds', async () => {
  ...
  try {
    ...
    for(const [x,y] of [[0,0],[100,100],[1287,344],[1535,642],[2221,1110]]){
      ...
      assert.ok(Math.abs(source.sample(longitude,latitude)-(values[0]-763500)/1000)<1e-8);
    }
...
test('GeoTIFF surface alignment follows Rhea origin easting rather than projection center alone', async () => {
  ...
  const entry=manifest.inputs.find(e=>e.lensId==='normal');
  ...
  for(const [lat,lon] of [[34.2,208.3],[-14.1,247.9],[20,90],[-55,50]]){
