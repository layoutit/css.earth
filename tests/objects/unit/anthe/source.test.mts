...
const test = sourceTest('anthe');
import assert from 'node:assert/strict';
import { readFile } from 'node:fs/promises';
import { parsePdsRadiusTable } from '../../../../tools/objects/terrestrial-layers/obj-shape.mts';
import { validateClosedMesh } from '../../../../tools/objects/terrestrial-layers/radial-terrain.mts';
const root = new URL('../../../../src/objects/anthe/source/', import.meta.url);
const read = async (path: string|URL) => JSON.parse((await readFile(new URL(path, root))).toString('utf8'));

test('Anthe preserves the source dimensions as a closed physical surface', async () => {
  ...
  for (const [lon, lat, radius] of [[0,0,500.0], [90,0,500.0], [180,0,500.0], [270,0,500.0], [0,90,500.0], [0,-90,500.0]] as const) {
    assert.ok(Math.abs(required(shape.sample(lon,lat))-radius)<.001, `${lon},${lat}`);
  }
  assert.ok(Math.abs(recipe.geometry.radiusKm-Math.cbrt(0.5*0.5*0.5))<1e-10);
