...
test('Ida measured radii preserve independent pole, equatorial and extremity anchors',async()=>{
 const config=await read('preparation/terrestrial.json'),surface=await loadScienceSurface(root,config.raster.scientific.find(x=>x.id==='elevation'));
 for(const [longitude,latitude,radiusKm]of [[0,90,8.4529],[0,-90,6.2869],[0,0,25.6316],[90,0,14.4770],[180,0,28.3430],[270,0,9.4459],[228,-48,3.2963],[190,-6,31.0466]]){
  assert.ok(Math.abs(surface.sample(longitude,latitude)-(radiusKm-16))<1e-6,`Ida source anchor ${longitude},${latitude}`);
 }
...
test('Ida FITS samples retain verified north-up rows and are recentered, preserving only label-defined gaps',async()=>{
 const manifest=await read('manifest.json'),config=await read('preparation/terrestrial.json'),entry=manifest.inputs.find(x=>x.id==='ida-normal');
 ...
 for(const [x,y]of [[100,200],[640,500],[1400,700],[2000,1000]]){
