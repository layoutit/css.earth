...
test('retains the 293 authoritative moons and eight named major moon orbits',()=>{
 ...
 assert.equal(new Set(catalog.moons.map(moon=>moon.id)).size,293);
 for(const id of expectedMoons){
  const moon=catalog.moons.find(moon=>moon.id===id);
 ...
 const titan=catalog.moons.find(moon=>moon.id==='titan');
...
test('retains every pinned moon source image and catalog byte',async()=>{
 const sources=manifest.inputs.filter(entry=>entry.path.startsWith('moons/'));
...
test('keeps dormant moon billboards, orbit guides and shadow banks outside the parent runtime',async()=>{
 const [definition,scene]=await Promise.all(['runtime','scene'].map(name=>readPreparedFixture('saturn',name)));
 ...
 assert.ok(assets.assets.every(asset=>!asset.filename.includes('moon')));
