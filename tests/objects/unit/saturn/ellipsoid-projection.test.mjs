...
test("the cutaway's lower-density frame bank is expanded during preparation without a private runtime mapper", () => {
  ...
  for (const bank of track.banks) for (let frame = 0; frame < 256; frame++) {
    ...
    const [x,y] = original.backgroundPosition.split(' ').map(Number.parseFloat);
    assert.equal(bank.frames[frame].backgroundPosition, `${x}px ${y + original.rowIndex * 1040}px`);
    assert.equal(bank.frames[frame].backgroundSize, '16640px 1040px');
    assert.equal(bank.frames[frame].row, original.rowIndex);
    assert.equal(bank.frames[frame].resource, `interior-material:${bank.id}:row:${original.rowIndex}`);
