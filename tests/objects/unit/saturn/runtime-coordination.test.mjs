...
test("Saturn commits one coherent latest exclusive lens with ring and shadow settings", async () => {
  ...
  try {
    ...
    assert.ok(runtimeDefinition.assets.entries.some(({key})=>key.startsWith("exterior:normal-ringless:row:") && f.residency.resources.has(key)));
    assert.ok(runtimeDefinition.assets.entries.some(({key})=>key.startsWith("interior-material:normal-ringless:row:") && f.residency.resources.has(key)));
...
test("Saturn repeated cross-section selection stays selected while replacing a pending request", async () => {
  ...
  try {
    ...
    assert.ok(pool(f, "interior").resident > 0);
    assert.equal(pool(f, "interior-material").resident, 2, 'current material row and its warm neighbor');
