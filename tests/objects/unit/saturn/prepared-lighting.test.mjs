...
test('ships every active material bank with exact pinned bytes and prepared addresses', async () => {
  ...
  for (const bank of exterior.banks) {
    assert.equal(bank.default, null, 'material selection uses bounded rows');
    const digest = createHash('sha256');
    assert.equal(bank.rows.length, 16);
    for (const row of bank.rows) {
      const resource = runtime.assets.entries.find(entry => entry.key === row.resource);
      assert.ok(resource);
      assert.equal(resource.url, variants[bank.id].runtimeAtlas.assetUrl.replace(/\.webp$/, `-stream-row-${String(row.row).padStart(2,'0')}.webp`));
      const bytes = await verifiedAsset(resource.url); digest.update(bytes);
      const metadata = await sharp(bytes).metadata();
      assert.equal(metadata.width, 4160); assert.equal(metadata.height, 260);
      assert.equal(metadata.hasAlpha, true);
    }
    distinctHashes.add(digest.digest('hex'));
    ...
    for (const [index, frame] of bank.frames.entries()) {
      ...
      assert.equal(frame.resource, `exterior:${bank.id}:row:${prepared.rowIndex}`);
      const [x,y] = prepared.backgroundPosition.split(' ').map(Number.parseFloat);
      assert.equal(frame.backgroundPosition, `${x}px ${y + prepared.rowIndex * 1040}px`);
      assert.equal(frame.backgroundSize, '16640px 1040px');
    }
  }
...
test('keeps fixed startup resources separate from view-selected rows and the shared sky', () => {
  for (const key of ['ring-shadow', 'directional-sun']) assert.ok(runtime.assets.startup.includes(key));
  assert.equal(runtime.assets.startup.some(key=>key.includes(':row:')), false);
  const sky = runtime.assets.entries.filter(entry => entry.key.startsWith('sky:'));
  assert.equal(sky.length, 0);
