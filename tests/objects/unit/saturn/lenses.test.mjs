...
test('ships five prepared retained-DOM Saturn lenses',async()=>{
 ...
 for(const lens of lenses.controls){
  const expected=lens.view==='interior'?[[lens.thumbnailUrl,112,64]]:[
 ...
 const interior=lenses.controls.find(lens=>lens.view==='interior');
 const meta=await sharp(await readFile(publicPath(interior.interiorMaterialUrl))).metadata();
...
test('keeps every canonical prepared ring alpha texel unchanged across lenses',async()=>{
 const alpha=async url=>sharp(await readFile(publicPath(url))).ensureAlpha().extractChannel(3).raw().toBuffer();
 const normal=await alpha('/scenes/saturn/saturn-rings@2x.webp');
 for(const lens of lenses.controls.filter(lens=>lens.falseColor))assert.equal(Buffer.compare(await alpha(lens.ring2xUrl),normal),0,lens.id);
});
test('declares canonical prepared lens resources and exclusive cross-section selection',()=>{
 ...
 const view={controlPitch:definition.camera.defaultControlPitchDegrees,controlYaw:definition.camera.defaultControlYawDegrees,sunViewDirection:viewSunDirectionToPreparedLightDirection(definition.sun.referenceViewDirection)};view.reference=view;
 for(const lens of lenses.controls.filter(lens=>lens.view!=='interior')){
  ...
  const resources=plan.required.map(key=>definition.assets.entries.find(entry=>entry.key===key));assert.ok(resources.every(Boolean));
  ...
  assert.deepEqual(cutawayPlan.pressedLenses,['cross-section']);assert.ok(cutawayPlan.required.includes('surface:normal'));
  assert.ok(cutawayPlan.required.some(key=>key.startsWith('interior-material:normal-no-shadows:row:')));
 }
});
