...
const root=new URL('../../../../src/planets/galatea/',import.meta.url);
test('Galatea ellipsoid retains Karkoschka published axes at independent cardinal anchors',async()=>{
 const recipe=JSON.parse(await readFile(new URL('source/preparation/terrestrial.json',root)));
 ...
 for(const [lon,lat,radius] of [[0,0,102000],[90,0,92000],[180,0,102000],[270,0,92000],[0,90,72000],[0,-90,72000]])
  assert.ok(Math.abs(shape.sample(lon,lat)-radius)<0.001,`${lon},${lat}`);
...
test('Galatea unavailable imagery is represented by the existing no-data grid recipe',async()=>{
 ...
 const inventory=JSON.parse(await readFile(new URL('source/survey/opus-finest.json',root)));
 assert.equal(inventory.page[0][0],'vg-iss-2-n-c1135055');
 assert.equal(Number(inventory.page[0][3]),18.66229);
 const metadata=JSON.parse(await readFile(new URL('source/survey/1135055-metadata.json',root)));
