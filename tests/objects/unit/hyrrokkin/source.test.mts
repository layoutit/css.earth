...
const test = sourceTest('hyrrokkin');
...
const root = new URL('../../../../src/objects/hyrrokkin/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Hyrrokkin preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  assert.ok(Math.abs(major/minor - 1.27) < 1e-10);
...
test('Hyrrokkin supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('hyrrokkin stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('hyrrokkin');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('hyrrokkin', row.jdTdb), expected = row.expectedPositionKm;
...
test('Hyrrokkin preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '644');
  assert.equal(evidence.identity.designation, 'S/2004 S19');
  ...
  assert.equal(rotationFact.value, "12.76 ± 0.03 h");
});
