...
const root = new URL('../../../../src/planets/hyrrokkin/source/', import.meta.url);
...
test('hyrrokkin stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('hyrrokkin');
...
test('Hyrrokkin preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "12.76 ± 0.03 h");
});
