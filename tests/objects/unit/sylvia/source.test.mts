...
const root=resolve(import.meta.dirname,'../../../../src/planets/sylvia/source');
...
test('Sylvia preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[12999.788,2231.3828,98592.636]);
 assert.deepEqual(mesh.indices[0],[0,726,727]);
 assert.deepEqual([topology.vertices,topology.faces,topology.components,topology.eulerCharacteristic],[2898,5792,1,2]);
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-10446886.455372147)<.00001);
 const rotation=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(rotation.periodHours,5.18364);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,137);assert.equal(lens.valueTransform.offset,-137);
...
test('Sylvia keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json')),source=await createSourceManifest({planetId:'sylvia',planetName:'Sylvia',sourceRoot:root});
 const radial=requireClosedRadialTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
 assert.equal(radial.faces.length,800);assert.equal(radial.simplification.sourceFaces,5792);assert.equal(radial.simplification.removedOppositeFaces,0);
