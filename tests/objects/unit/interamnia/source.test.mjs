...
const root=resolve(import.meta.dirname,'../../../../src/planets/interamnia/source');
const read=async path=>JSON.parse(await readFile(resolve(root,path),'utf8'));
test('Interamnia retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'interamnia',planetName:'Interamnia',sourceRoot:root});await source.verify();
