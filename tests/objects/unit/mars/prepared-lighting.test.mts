...
import { PREPARED_MARS_SKY_SUN } from "../../unit/mars/prepared-fixture.mts";
...
} from "../../unit/mars/prepared-fixture.mts";
import acceptedDelivery from '../../../../src/planets/mars/source/preparation/material-delivery.json' with {type:'json'};

test('immutable material delivery identity is independently byte-verified from generator provenance',()=>{
  assert.equal(PREPARED_MARS_LIGHTING.bankFingerprint,acceptedDelivery.bankId);
  assert.match(PREPARED_MARS_LIGHTING.generatorFingerprint,/^[a-f0-9]{12}$/);
  assert.notEqual(PREPARED_MARS_LIGHTING.generatorFingerprint,acceptedDelivery.bankId);
  for(const expected of acceptedDelivery.rows){const row=PREPARED_MARS_LIGHTING.banks[expected.density].rows[expected.row];assert.equal(row.sha256,expected.sha256);assert.equal(row.bytes,expected.bytes);assert.ok(row.url.includes(`material-${acceptedDelivery.bankId}-`));}
});
...
test("keeps unattended playback on compositor animations", async () => {
  ...
  assert.doesNotMatch(client, /setInterval|setTimeout|canvas|getContext/);
