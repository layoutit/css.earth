...
import runtimeDefinition from "../../../../src/planets/oberon/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/planets/oberon/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/planets/oberon/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/planets/oberon/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/planets/oberon/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal"];

objectRuntimePackageTests(runtimeDefinition);

test("Oberon's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: OBJECTS.filter(object => object.id === "oberon") });
...
test("Oberon is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 1 lens", () => {
  assert.equal(scene.schema, "cssoberon-prepared-runtime-scene@1");
...
test("Oberon publishes every declared toggle through the shared controls and selection owner", async () => {
...
test("Oberon lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
