// Draft: keeps the unchanged Oberon nomenclature recipe proven on the generic lane
...
import descriptor from "../../../../src/planets/oberon/prepared/features.json" with { type: "json" };
import { parsePreparedSurfaceFeatureCatalog } from "../../../../src/renderers/css/dist/index.js";

const plan = runtimeDefinition.features;
const bytes = await readFile(new URL("../../../../public/scenes/oberon/oberon-features.json", import.meta.url));
const catalog = parsePreparedSurfaceFeatureCatalog(JSON.parse(bytes.toString("utf8")), plan, "oberon");

test("the prepared Oberon nomenclature catalogue is pinned by the runtime plan and the provenance descriptor", () => {
  ...
  assert.equal(plan.catalog.count, 10, "the unchanged features recipe keeps the terrestrial-lane count");
...
test("the plan anchors labels to the single body mesh for every surface lens", () => {
  ...
  assert.match(node.className ?? "", /(^|\s)oberon-body(\s|$)/u);
  assert.equal(runtimeDefinition.tree.nodes.filter(n => /(^|\s)oberon-body(\s|$)/u.test(n.className ?? "")).length, 1);
