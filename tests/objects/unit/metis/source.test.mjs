...
test('Metis reference shape reproduces the PCK dimensions without invented terrain',async()=>{
 ...
 const axes=pck.match(/BODY516_RADII\s*=\s*\(([^)]+)\)/)[1].trim().split(/\s+/).map(Number);
 ...
 for(const [lon,lat,expected] of [[0,0,30000],[90,0,20000],[0,90,17000],[0,-90,17000]])assert.ok(Math.abs(grid.sample(lon,lat)-expected)<.001);
});
test('Metis camera directions agree with source phase, scale and raw image north',async()=>{
 ...
 for(const frame of recipe.raster.mosaics[0].frames){
  const metadata=JSON.parse(await readFile(new URL(`geometry/${frame.id}-opus.json`,source)))['Metis Surface Geometry Constraints'];
  ...
  const label=await readFile(new URL(frame.labelPath,source),'utf8'),north=Number(label.match(/^NORTH_AZIMUTH\s*=\s*([-+.\d]+)/m)[1]);
