...
const root = new URL('../../../../src/planets/hati/source/', import.meta.url);
const read = async path => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Hati preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3032.0531129669675], [180,0,3032.0531129669675], [90,0,2135.2486711034985], [270,0,2135.2486711034985], [0,90,2135.2486711034985], [0,-90,2135.2486711034985]]) {
  ...
  assert.ok(Math.abs(major/minor - 1.42) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 2400) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 2400);
  ...
  const expectedVolume = 4/3 * Math.PI * 2400 ** 3;
  assert.ok(Math.abs(faces.simplification.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Hati supplies only missing coverage to the surface preparer', async () => {
  const config = await read('preparation/terrestrial.json');
...
test('hati stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('hati');
  for (const row of evidence.runs.find(run => run.label === 'holdouts').comparisons) {
    ...
    const actual = satellitePositionKm('hati', row.jdTdb), expected = row.expectedPositionKm;
...
test('Hati preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '643');
  assert.equal(evidence.identity.designation, 'S/2004 S14');
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "5.45 ± 0.04 h");
});
