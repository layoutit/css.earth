...
const root = new URL('../../../../src/objects/hati/source/', import.meta.url);
...
test('Hati preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3032.0531129669675], [180,0,3032.0531129669675], [90,0,2135.2486711034985], [270,0,2135.2486711034985], [0,90,2135.2486711034985], [0,-90,2135.2486711034985]] as const) {
...
test('hati stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('hati');
...
test('Hati preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(rotationFact.value, "5.45 ± 0.04 h");
});
