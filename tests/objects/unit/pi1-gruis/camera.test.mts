/**
 * Every controlled-shape camera field in the PIONIER lens is recomputed from the pinned inputs and compared against the recipe,
 * through the same observer-camera transform the asteroid photographs use. The star has no measured axis: the display axis is
 * celestial north in the plane of the sky, so the sub-observer point is on the equator and the image's north is the pole.
 */
...
test('the lens recipe states the camera the pinned astrometry and display axis imply', async () => {
  const recipe = requireRecord(await readJson(resolve(BODY, 'source/preparation/raster.json')));
  const surface = requireRecord(requireArray(recipe.surfaces).find(entry => requireRecord(entry).id === 'pionier'));
  ...
  assert.equal(rotation.schema, 'cssearth-display-orientation@1', 'no measured axis: a display orientation');
  assert.match(requireString(rotation.qualification), /not a measurement/u);

  // The display axis: the sky-north direction at the star, in the plane of the sky.
  ...
  assert.ok(Math.abs(pole[0] * toStar[0] + pole[1] * toStar[1] + pole[2] * toStar[2]) < 1e-9, 'the axis lies in the plane of the sky');
  ...
  assert.ok(Math.abs(positionAngle) < 1e-6, `the axis is celestial north, position angle 0, got ${positionAngle}`);

  // The mesh frame: +z on the axis, +x toward the observer at the epoch (the sub-Earth point is the display meridian).
  ...
  const camera = observerCamera({ epochJd: 2456926.5, targetRightAscensionDegrees: ra, targetDeclinationDegrees: dec,
  ...
  assert.ok(inside / sw > 0.8 && inside / sw < 1, `${(inside / sw * 100).toFixed(1)}% of the reconstructed flux lies within the fitted disc`);
});
