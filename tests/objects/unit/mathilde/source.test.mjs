...
const read=async p=>JSON.parse(await readFile(resolve(base,p),'utf8'));
test('Mathilde Stooke column order, east longitude and meter scale preserve independent release anchors',async()=>{
 ...
 for(const[longitude,latitude,radiusMeters]of[[0,0,27080.9],[90,0,25248],[180,0,25712.7],[270,0,24467.2],[0,-90,22126.3],[0,90,23668.1],[210,30,30680.4]]){
  assert.ok(Math.abs(grid.sample(longitude,latitude)-radiusMeters)<1e-6,`${longitude}E ${latitude}N source anchor`);
 }
});
test('Mathilde elevation rejects source placeholders before interpolation and reverses the Thomas west longitudes',async()=>{
 ...
 assert.ok(Math.abs(grid.sample(210,30)-30680.4)<1e-6);
 assert.ok(Math.abs(grid.sample(270,0)-24383.9)<1e-6);
...
test('Mathilde arbitrary orientation does not produce a measured spin rate',async()=>{
 const descriptor=await read('object.json'),ref=descriptor.properties.recipe.sources.find(s=>s.id==='rotation');
