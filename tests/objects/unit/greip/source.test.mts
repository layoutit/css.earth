...
const root = new URL('../../../../src/objects/greip/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Greip preserves the selected lower-limit elongation and explicit volume scale', async () => {
...
test('Greip supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('greip stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('greip');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('greip', row.jdTdb), expected = row.expectedPositionKm;
...
test('Greip preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '651');
  assert.equal(evidence.identity.designation, 'S/2006 S4');
  ...
  assert.equal(rotationFact.value, "12.75 ± 0.35 h · tentative");
});
