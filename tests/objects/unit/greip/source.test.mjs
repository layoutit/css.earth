...
const root = new URL('../../../../src/planets/greip/source/', import.meta.url);
...
test('greip stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('greip');
...
test('Greip preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "12.75 ± 0.35 h · tentative");
});
