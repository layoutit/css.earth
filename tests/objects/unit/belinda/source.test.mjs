...
const j=async p=>JSON.parse(await readFile(new URL(p,source)));
...
test('Belinda preserves the published prolate axes and volume scale',async()=>{
 ...
 for(const [lon,lat,r] of [[0,0,64000],[180,0,64000],[90,0,32000],[270,0,32000],[0,90,32000],[0,-90,32000]])assert.ok(Math.abs(mesh.sample(lon,lat)-r)<.001);
...
test('Belinda has no photographic coverage and uses the body714 rotation source',async()=>{
 ...
 const pole=r.evaluation.bodyToJ2000.map(row=>row[2]);assert.ok(Math.abs(Math.atan2(pole[1],pole[0])*180/Math.PI+360-r.rightAscensionDegrees)<1e-10);assert.ok(Math.abs(Math.asin(pole[2])*180/Math.PI-r.declinationDegrees)<1e-10);
});
