...
const test = sourceTest('mundilfari');
...
const root = new URL('../../../../src/objects/mundilfari/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Mundilfari preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,4442.479195657919], [180,0,4442.479195657919], [90,0,3106.628808152391], [270,0,3106.628808152391], [0,90,3106.628808152391], [0,-90,3106.628808152391]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.43) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 3500) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 3500);
  ...
  const expectedVolume = 4/3 * Math.PI * 3500 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Mundilfari supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('mundilfari stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('mundilfari');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('mundilfari', row.jdTdb), expected = row.expectedPositionKm;
...
test('Mundilfari preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '625');
  assert.equal(evidence.identity.designation, 'S/2000 S9');
  ...
  assert.equal(rotationFact.value, "6.74 ± 0.08 h");
});
