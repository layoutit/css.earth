...
const root = new URL('../../../../src/planets/mundilfari/source/', import.meta.url);
...
test('Mundilfari preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,4442.479195657919], [180,0,4442.479195657919], [90,0,3106.628808152391], [270,0,3106.628808152391], [0,90,3106.628808152391], [0,-90,3106.628808152391]]) {
...
test('mundilfari stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('mundilfari');
...
test('Mundilfari preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "6.74 ± 0.08 h");
});
