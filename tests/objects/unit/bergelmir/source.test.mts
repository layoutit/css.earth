...
const test = sourceTest('bergelmir');
...
test('Bergelmir preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,2712.2245184493067], [180,0,2712.2245184493067], [90,0,2400.198688893192], [270,0,2400.198688893192], [0,90,2400.198688893192], [0,-90,2400.198688893192]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.13) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 2500) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 2500);
  ...
  const expectedVolume = 4/3 * Math.PI * 2500 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Bergelmir supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('bergelmir stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('bergelmir');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('bergelmir', row.jdTdb), expected = row.expectedPositionKm;
...
test('Bergelmir preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '638');
  assert.equal(evidence.identity.designation, 'S/2004 S15');
  ...
  assert.equal(rotationFact.value, "8.13 ± 0.09 h");
});
