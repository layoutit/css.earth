...
test('Bergelmir preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,2712.2245184493067], [180,0,2712.2245184493067], [90,0,2400.198688893192], [270,0,2400.198688893192], [0,90,2400.198688893192], [0,-90,2400.198688893192]]) {
...
test('bergelmir stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('bergelmir');
...
test('Bergelmir preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "8.13 ± 0.09 h");
});
