...
const test = sourceTest('menoetius');
import assert from 'node:assert/strict';
import {readFile} from 'node:fs/promises';
import {parsePdsRadiusTable} from '../../../../tools/objects/terrestrial-layers/obj-shape.mts';
import {validateClosedMesh} from '../../../../tools/objects/terrestrial-layers/radial-terrain.mts';
const root=new URL('../../../../src/objects/menoetius/source/',import.meta.url);
test('Menoetius radius table preserves independently derived physical-axis anchors',async()=>{
 ...
 for(const [lon,lat,metres] of [[0, 0, 58500.0], [90, 0, 54000], [0, 90, 45000], [0, -90, 45000]] as const) assert.ok(Math.abs(required(shape.sample(lon,lat))-metres)<.001,`${lon},${lat}`);
