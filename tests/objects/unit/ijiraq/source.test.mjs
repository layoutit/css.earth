...
const root = new URL('../../../../src/planets/ijiraq/source/', import.meta.url);
...
test('Ijiraq preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,6736.936868842444], [180,0,6736.936868842444], [90,0,6237.904508187448], [270,0,6237.904508187448], [0,90,6237.904508187448], [0,-90,6237.904508187448]]) {
...
test('ijiraq stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('ijiraq');
...
test('Ijiraq preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "13.03 ± 0.14 h");
});
