...
const root = new URL('../../../../src/planets/bebhionn/source/', import.meta.url);
...
test('Bebhionn preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3520.76825517781], [180,0,3520.76825517781], [90,0,2496.9987625374542], [270,0,2496.9987625374542], [0,90,2496.9987625374542], [0,-90,2496.9987625374542]]) {
...
test('bebhionn stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('bebhionn');
...
test('Bebhionn preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "16.33 ± 0.03 h");
});
