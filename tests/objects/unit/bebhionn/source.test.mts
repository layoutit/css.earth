...
const test = sourceTest('bebhionn');
...
const root = new URL('../../../../src/objects/bebhionn/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Bebhionn preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,3520.76825517781], [180,0,3520.76825517781], [90,0,2496.9987625374542], [270,0,2496.9987625374542], [0,90,2496.9987625374542], [0,-90,2496.9987625374542]] as const) {
  ...
  assert.ok(Math.abs(major/minor - 1.41) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 2800) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 2800);
  ...
  const expectedVolume = 4/3 * Math.PI * 2800 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Bebhionn supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('bebhionn stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('bebhionn');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('bebhionn', row.jdTdb), expected = row.expectedPositionKm;
...
test('Bebhionn preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(evidence.identity.targetNaif, '637');
  assert.equal(evidence.identity.designation, 'S/2004 S11');
  ...
  assert.equal(rotationFact.value, "16.33 ± 0.03 h");
});
