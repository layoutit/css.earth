...
const root=resolve(import.meta.dirname,'../../../../src/objects/thule/source'),read=createSourceFixtureReader(root);
...
test('Thule keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'thule',planetName:'Thule',sourceRoot:root}),r=requireClosedTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
