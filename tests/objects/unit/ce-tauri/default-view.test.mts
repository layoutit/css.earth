 ...
 * degree from the sub-Earth point, the photographed hemisphere. */
...
test('the default camera looks at the photographed hemisphere, one degree off the sub-Earth point', async () => {
  ...
  const view = defaultViewGeometry('ce-tauri', requireRecord(runtime.camera) as never, requireRecord(requireRecord(descriptor.properties).worldFrame) as never);
...
test('the display axis stands up on screen, with celestial north up and the Earth toward the viewer', async () => {
  ...
  const star = requireRecord(requireRecord(JSON.parse(await readFile(resolve(BODY, '../../../packages/astronomy/data/bodies/ce-tauri.json'), 'utf8')) as unknown).star);
