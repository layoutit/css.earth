// Draft: keeps the unchanged Umbriel nomenclature recipe proven on the generic lane
...
import runtimeDefinition from "../../../../src/planets/umbriel/prepared/runtime.json" with { type: "json" };
import descriptor from "../../../../src/planets/umbriel/prepared/features.json" with { type: "json" };
import { parsePreparedSurfaceFeatureCatalog } from "../../../../src/renderers/css/dist/index.js";

const plan = runtimeDefinition.features;
const bytes = await readFile(new URL("../../../../public/scenes/umbriel/umbriel-features.json", import.meta.url));
const catalog = parsePreparedSurfaceFeatureCatalog(JSON.parse(bytes.toString("utf8")), plan, "umbriel");

test("the prepared Umbriel nomenclature catalogue is pinned by the runtime plan and the provenance descriptor", () => {
  ...
  assert.equal(plan.catalog.count, 13, "the unchanged features recipe keeps the terrestrial-lane count");
...
test("the plan anchors labels to the single body mesh for every surface lens", () => {
  ...
  assert.match(node.className ?? "", /(^|\s)umbriel-body(\s|$)/u);
  assert.equal(runtimeDefinition.tree.nodes.filter(n => /(^|\s)umbriel-body(\s|$)/u.test(n.className ?? "")).length, 1);
  assert.deepEqual(plan.lensIds, ["normal"]);
