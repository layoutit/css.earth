...
import assets from "../../../../src/planets/umbriel/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/planets/umbriel/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/planets/umbriel/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/planets/umbriel/prepared/controls.json" with { type: "json" };
...
test("Umbriel's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: OBJECTS.filter(object => object.id === "umbriel") });
...
test("Umbriel publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`umbriel-hide-stars`), !input.checked);
