...
const root = new URL('../../../../src/planets/bestla/source/', import.meta.url);
...
test('Bestla preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  for (const [lon, lat, expected] of [[0,0,4266.373073534494], [180,0,4266.373073534494], [90,0,2902.294607846595], [270,0,2902.294607846595], [0,90,2902.294607846595], [0,-90,2902.294607846595]]) {
...
test('bestla stays within the independent vector position guards', async () => {
  ...
  const evidence = await read('validation/orbit-checks.json'), record = satelliteRecord('bestla');
...
test('Bestla preserves the source identity and honest rotation/shape interpretation', async () => {
  ...
  assert.equal(content.panel.facts.find(fact => fact.id === 'rotation').value, "14.6238 ± 0.0001 h");
});
