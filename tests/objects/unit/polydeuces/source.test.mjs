...
test('Polydeuces preserves the source dimensions as a closed physical surface', async () => {
  ...
  for (const [lon, lat, radius] of [[0,0,1750.0], [90,0,1550.0], [180,0,1750.0], [270,0,1550.0], [0,90,1310.0], [0,-90,1310.0]]) {
...
test('Polydeuces retains calibrated native pixels and withholds the unseen hemisphere', async () => {
  ...
  const map = await prepareShapeCameraMosaic(root.pathname,manifest.inputs.filter(e=>e.consumers.includes(chosen.consumer)),chosen,64,32,recipe.geometry.radialTerrain);
