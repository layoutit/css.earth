...
const root=resolve(import.meta.dirname,'../../../../src/objects/elektra/source');
const read=createSourceFixtureReader(root);
test('Elektra retains original source pins and acquisition closure',async()=>{
 const source=await createSourceManifest({planetId:'elektra',planetName:'Elektra',sourceRoot:root});await source.verify();
...
test('Elektra preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 assert.deepEqual(mesh.positions[0],[5910.9249,-8482.213,72471.588]);
 assert.deepEqual(mesh.indices[0],[0,662,663]);
 assert.deepEqual([topology.vertices,topology.faces,topology.components,topology.eulerCharacteristic],[2642,5280,1,2]);
 assert.ok(Math.abs(topology.signedVolumeCubicMeters/1e9-4100886.242414155)<.00001);
 const rotation=await read('preparation/rotation.json');assert.equal(rotation.periodHours,5.224663);assert.equal(rotation.phase,'arbitrary-display-phase');
...
test('Elektra keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'elektra',planetName:'Elektra',sourceRoot:root});
