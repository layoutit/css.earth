// Draft: keeps the unchanged Ariel nomenclature recipe proven on the generic lane
...
import descriptor from "../../../../src/objects/ariel/prepared/features.json" with { type: "json" };
import { parsePreparedSurfaceFeatureCatalog } from "../../../../src/renderers/css/dist/index.js";

const plan = runtimeDefinition.features;
const bytes = await readFile(new URL("../../../../public/scenes/ariel/ariel-features.json", import.meta.url));
const catalog = parsePreparedSurfaceFeatureCatalog(JSON.parse(bytes.toString("utf8")), plan, "ariel");

test("the prepared Ariel nomenclature catalogue is pinned by the runtime plan and the provenance descriptor", () => {
  ...
  assert.equal(plan.catalog.count, 26, "the unchanged features recipe keeps the terrestrial-lane count");
...
test("the plan anchors labels to the single body mesh for every surface lens", () => {
  ...
  assert.match(node.className ?? "", /(^|\s)ariel-body(\s|$)/u);
  assert.equal(runtimeDefinition.tree.nodes.filter(n => /(^|\s)ariel-body(\s|$)/u.test(n.className ?? "")).length, 1);
  assert.deepEqual(plan.lensIds, ["normal","elevation"]);
