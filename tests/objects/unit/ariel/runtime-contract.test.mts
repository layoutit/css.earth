// Draft replacement for the retired terrestrial-lane unit tests of Ariel
...
test("Ariel's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "ariel") });
...
test("Ariel is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 2 lenses", () => {
  assert.equal(scene.schema, "cssariel-prepared-runtime-scene@1");
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("ariel-material-composite")));
...
test("Ariel lenses keep their prepared legends and false-colour declarations", () => {
...
test("Ariel publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`ariel-hide-stars`), !input.checked);
...
test("Ariel lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
