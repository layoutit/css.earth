...
test('source-derived oblate material preparation reproduces all accepted fixed and orbit products',async()=>{
  ...
  for(const [id,lens]of Object.entries(requireRecord(accepted.lenses)).map(([id,value])=>[id,requireRecord(value)] as const))for(const field of['frameRawSha256','frameForegroundRingTexelCounts','frameRingShadowTexelCounts'] as const)assert.deepEqual(result.lenses[id].bank[field],lens[field],`${id} source frame ${field}`);
});
