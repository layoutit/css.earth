...
const rowPool = (f: Awaited<ReturnType<typeof preparedSelectionFixture>>) => required(f.residency.stats().pools.find(pool => pool.id === "rows"));
const capacity=required(runtimeDefinition.assets.pools.find(pool=>pool.id==='rows')).capacity;
const requiredRows=(f: Awaited<ReturnType<typeof preparedSelectionFixture>>)=>required(f.selection.state().plan).required.filter((key: string)=>key.startsWith('row:'));
async function directional(f: Awaited<ReturnType<typeof preparedSelectionFixture>>){const request=f.selection.dispatch({kind:'toggle',name:'shadows',value:true});await f.settle();assert.equal(await request,true);f.selection.setView({...f.view,sunViewDirection:[1,0,0],revision:2});await f.settle();}
...
test("Uranus pending lens preserves the active neighborhood and a failed row remains retryable", async () => {
  ...
  try {
    ...
    const requestedRow = required(runtimeDefinition.assets.entries.find(entry => entry.key===active[0].replace('row:normal:','row:methane:')));
    ...
    assert.equal(required(f.selection.state().committed).lensId, "normal");
    ...
    assert.ok(requiredRows(f).every((key: string)=>key.startsWith('row:methane:')&&rowPool(f).keys.includes(key)));
...
test("Uranus camera changes during pending selection revalidate the current neighborhood", async () => {
  ...
  try {
    ...
    const expected = resolvePreparedPresentation(runtimeDefinition, { selection: required(f.selection.state().committed), view, previousPlan: f.selection.state().plan });
    assert.ok(expected.required.filter(key => key.startsWith("row:")).every(key=>rowPool(f).keys.includes(key)));
    assert.ok(rowPool(f).resident <= required(runtimeDefinition.assets.pools.find(pool=>pool.id==='rows')).capacity); assert.deepEqual(f.errors, []);
  } finally { f.restore(); }
});
...
test("Uranus hidden lighting keeps its committed neighborhood and publisher row through camera movement", async () => {
  ...
  try {
    ...
    for (const z of [-1, 1, -.5]) {
      f.selection.setView({ ...f.view, controlPitch: 89, sunViewDirection: [0, 0, z], revision: required(f.view.revision) + 1 });
      await f.settle();
...
test("Uranus native row release failure is fatal and still releases sibling ownership", async () => {
  ...
  try {
    await directional(f);
    const key = rowPool(f).keys[0], url = required(runtimeDefinition.assets.entries.find(entry => entry.key === key)).url;
    const native = required(f.jobs.find(job => job.url === url)).image;
