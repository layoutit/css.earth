...
const root=resolve(import.meta.dirname,'../../../../src/planets/alphonsina/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Alphonsina retains source identity and restoration closure',async()=>{
 const source=await createSourceManifest({planetId:'alphonsina',planetName:'Alphonsina',sourceRoot:root});await source.verify();
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,301);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2011-02-18');
});
...
test('Alphonsina keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'alphonsina',planetName:'Alphonsina',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
 assert.equal(r.faces.length,800);assert.equal(r.simplification.sourceFaces,2036);assert.equal(r.simplification.removedOppositeFaces,0);assert.equal(r.simplification.topology.eulerCharacteristic,2);assert.ok(r.leaves.every(l=>l.tag==='u'&&l.attributes['data-polycss-texture-leaf-sizing']==='raster'));assert.deepEqual([r.tileSize,r.width,r.height],[128,2048,6400]);
