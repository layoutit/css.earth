...
test('Kerberos keeps the published 2025 semi-axes independently of its scene reference radius',async()=>{
 ...
 for(const [lon,lat,radius] of [[0,0,7250],[90,0,4100],[180,0,7250],[270,0,4100],[0,90,3600],[0,-90,3600]])
...
test('Kerberos unregistered few-pixel imagery remains no-data, including both resolved epochs',async()=>{
 ...
 const qualified=JSON.parse(await readFile(new URL('source/survey/qualification.json',root)));
 assert.deepEqual(qualified.inspectedObservations.map(x=>x.opusId),['nh-lorri-lor_0299153805','nh-lorri-lor_0299136735']);
});
