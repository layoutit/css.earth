...
const test = sourceTest('victoria');
...
const root=resolve(import.meta.dirname,'../../../../src/objects/victoria/source');
...
test('Victoria preserves the original kilometer mesh and published spin interpretation',async()=>{
 ...
 const rotation=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(rotation.periodHours,8.660345);assert.equal(rotation.phase,'arbitrary-display-phase');
 assert.equal(config.geometry.radiusKm,58);assert.equal(lens.valueTransform.offset,-58);
...
test('Victoria keeps a closed source-connected mesh within the retained raster budget',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json')),source=await createSourceManifest({planetId:'victoria',planetName:'Victoria',sourceRoot:root});
