...
test('Portia preserves published prolate radii and separates projected from volume radius', async () => {
  ...
  for (const [lon, lat, radius] of [[0, 0, 78000], [90, 0, 63000], [180, 0, 78000], [270, 0, 63000], [0, 90, 63000], [0, -90, 63000]]) {
  ...
  const published = JSON.parse(await readFile(new URL('source/survey/published-shape.json', root)));
