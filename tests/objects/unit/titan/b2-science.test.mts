...
test('Titan scalar views preserve observed-versus-interpolated support',async()=>{const c=JSON.parse((await readFile(sourceRoot+'preparation/terrestrial.json')).toString('utf8'));const r=JSON.parse((await readFile(sourceRoot+'validation/b2-scalar-anchors.json')).toString('utf8'));const measured=await loadScienceSurface(sourceRoot,c.raster.scientific.find((l: { id: string; })=>l.id==='topography'));const interpolated=await loadScienceSurface(sourceRoot,c.raster.scientific.find((l: { id: string; })=>l.id==='interpolated'));const missing=r.sources.find((s: { path: string|string[]; })=>s.path.includes('GTFED00N270')).anchors.find((a: { value: null; })=>a.value===null);assert.equal(measured.sample(missing.longitudeEastDegrees,missing.latitudeDegrees),null);assert.ok(Number.isFinite(interpolated.sample(missing.longitudeEastDegrees,missing.latitudeDegrees)));assert.equal(measured.sample(0,-90),null,'Do not extrapolate beyond the last raster row');});
