// Draft replacement for the retired terrestrial-lane unit tests of Titan
...
import runtimeDefinition from "../../../../src/objects/titan/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/objects/titan/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/titan/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/titan/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/objects/titan/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal","radar","topography","interpolated","coverage-distance","geology"];

objectRuntimePackageTests(runtimeDefinition);

test("Titan's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "titan") });
...
test("Titan is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 6 lenses", () => {
  assert.equal(scene.schema, "csstitan-prepared-runtime-scene@1");
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("titan-material-composite")));
...
test("Titan lenses keep their prepared legends and false-colour declarations", () => {
...
test("Titan publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`titan-hide-stars`), !input.checked);
...
test("Titan lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
