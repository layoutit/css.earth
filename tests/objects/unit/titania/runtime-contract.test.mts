// Draft replacement for the retired terrestrial-lane unit tests of Titania
...
import runtimeDefinition from "../../../../src/objects/titania/prepared/runtime.json" with { type: "json" };
import assets from "../../../../src/objects/titania/prepared/assets.json" with { type: "json" };
import scene from "../../../../src/objects/titania/prepared/scene.json" with { type: "json" };
import lenses from "../../../../src/objects/titania/prepared/lenses.json" with { type: "json" };
import controls from "../../../../src/objects/titania/prepared/controls.json" with { type: "json" };
...
const LENS_IDS = ["normal","elevation","geology"];

objectRuntimePackageTests(runtimeDefinition);

test("Titania's actual import closure has only shared runtime owners", async () => {
  const audit = await auditObjectRuntimeOwnership({ objects: SCENE_OBJECTS.filter(object => object.id === "titania") });
...
test("Titania is prepared by the generic raster lane with the source-radius sphere, the Lambert lighting bank and its 3 lenses", () => {
  assert.equal(scene.schema, "csstitania-prepared-runtime-scene@1");
  ...
  assert.ok(runtimeDefinition.tree.nodes.some(node => node.className?.includes("titania-material-composite")));
  assert.ok(runtimeDefinition.viewBindings.some(binding => binding.kind === "silhouette-fit"));
  assert.equal(runtimeDefinition.heliocentricView.plan.orbit.centerBodyId, "uranus", "the orbit still surrounds the parent while the Sun keeps its own position");
});

test("Titania lenses keep their prepared legends and false-colour declarations", () => {
...
test("Titania publishes every declared toggle through the shared controls and selection owner", async () => {
  ...
  try {
    ...
    for (const name of ["shadows","stars"]) {
      ...
      if (name === "stars") assert.equal(f.stage.classList.contains(`titania-hide-stars`), !input.checked);
...
test("Titania lens selection keeps the retained tree and switches only the surface textures", async () => {
  const f = await preparedSelectionFixture(runtimeDefinition);
  try {
