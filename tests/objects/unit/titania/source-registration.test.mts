...
test('Titania mosaic and stereo-limb DEM retain their original geographic values and masks', async () => {
  ...
  const inspection = JSON.parse(await readFile(root + 'observations/source-inspection.json'));
  const entry = manifest.inputs.find(source => source.lensId === 'normal');
  ...
  for (const sample of inspection.samples) {
    for (const [id, source, tolerance] of [['normal', mosaic, 0.00001], ['elevation', dem, 0.00000001]]) {
  ...
  const value = mosaic.sample(351.5, -55.5);
