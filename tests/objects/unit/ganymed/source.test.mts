...
const root=resolve(import.meta.dirname,'../../../../src/objects/ganymed/source'),read=async (p:string):Promise<unknown>=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Ganymed retains source identity and restoration closure',async()=>{
 ...
 const model=requireRecord(await read('reference/damit-model.json'));const fields=requireRecord(model.fields);assert.equal(model.modelId,1849);assert.equal(fields['Calibrated Size'],'1 (Yes)');assert.equal(fields.Version,'2017-09-21');
});
test('Ganymed preserves calibrated original coordinates and the paired spin model',async()=>{
 ...
 assert.ok(Math.abs(t.signedVolumeCubicMeters/1e9-31105.990054071677)<.00001);const r=requireRotation(await read('preparation/rotation.json'),['periodHours','phase']);assert.equal(r.periodHours,10.31304);assert.equal(r.phase,'arbitrary-display-phase');
 assert.equal(lens.valueTransform.offset,-19.5);
...
test('Ganymed keeps closed source connectivity at 800 native raster leaves',async()=>{
 const {config}=requireRadialTestConfig(await read('preparation/terrestrial.json'));const source=await createSourceManifest({planetId:'ganymed',planetName:'Ganymed',sourceRoot:root}),r=requireClosedRadialTerrain(await loadRadialTerrain({config,sourceDirectory:root,source}));
