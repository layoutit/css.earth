...
const root = new URL('../../../../src/objects/kiviuq/source/', import.meta.url);
const read = async (path:string):Promise<unknown> => JSON.parse(await readFile(new URL(path, root), 'utf8'));

test('Kiviuq preserves the selected lower-limit elongation and explicit volume scale', async () => {
  ...
  assert.ok(Math.abs(major/minor - 2.32) < 1e-10);
  assert.ok(Math.abs(Math.cbrt(major*minor*polar) - 8400) < .001);
  ...
  const faces = await simplifyRadialShape(shape, config.geometry.radialTerrain, config.geometry.radius / 8400);
  ...
  const expectedVolume = 4/3 * Math.PI * 8400 ** 3;
  assert.ok(Math.abs(report.topology.signedVolumeCubicMeters / expectedVolume - 1) < .04);
});

test('Kiviuq supplies only missing coverage to the surface preparer', async () => {
  const config = irregularSatelliteConfig(await read('preparation/terrestrial.json'));
...
test('kiviuq stays within the independent vector position guards', async () => {
  ...
  const evidence = satelliteOrbitEvidence(await read('validation/orbit-checks.json')), record = satelliteRecord('kiviuq');
  ...
  for (const row of holdouts.comparisons) {
    ...
    const actual = satellitePositionKm('kiviuq', row.jdTdb), expected = row.expectedPositionKm;
