...
test('MICAS original XYZ cubes register every reviewed USGS terrain post independently', async () => {
  ...
  const surface = await loadOrthographicObservation({ sourceDirectory: root, source, recipe, radial: { grid: mesh }, config });
  assert.equal(surface.report.registration.coordinatePixels, 62879);
  assert.ok(surface.report.registration.maximumCoordinateErrorMeters < .000001);
  const changed = structuredClone(recipe); changed.grid.pixelToSource[1] += 16;
  await assert.rejects(loadOrthographicObservation({ sourceDirectory: root, source, recipe: changed, radial: { grid: mesh }, config }), /registration/);
