...
import { loadPdsPlanetocentricShape } from '../../../../tools/objects/terrestrial-layers/obj-shape.mjs';
...
const root = resolve(import.meta.dirname, '../../../../src/planets/comet-9p');
const json = async path => JSON.parse(await readFile(resolve(root, path)));

test('Tempel 1 retains published geometry, source constraint flags and fixed non-spin orientation', async () => {
  ...
  assert.deepEqual(source.coverage.vertexFlags, { 1: 11104, 2: 1450, 3: 3468 });
