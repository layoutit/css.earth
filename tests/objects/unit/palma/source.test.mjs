...
const root=resolve(import.meta.dirname,'../../../../src/planets/palma/source'),read=async p=>JSON.parse(await readFile(resolve(root,p),'utf8'));
test('Palma retains source identity and restoration closure',async()=>{
 const source=await createSourceManifest({planetId:'palma',planetName:'Palma',sourceRoot:root});await source.verify();
 ...
 const model=await read('reference/damit-model.json');assert.equal(model.modelId,298);assert.equal(model.fields['Calibrated Size'],'1 (Yes)');assert.equal(model.fields.Version,'2011-02-18');
});
...
test('Palma keeps closed source connectivity at 800 native raster leaves',async()=>{
 const config=await read('preparation/terrestrial.json'),source=await createSourceManifest({planetId:'palma',planetName:'Palma',sourceRoot:root}),r=await loadRadialTerrain({config,sourceDirectory:root,source});
