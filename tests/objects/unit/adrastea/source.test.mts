...
test('Adrastea model preserves the independently published triaxial dimensions in metres',async()=>{
  ...
  const radii=source.match(/BODY515_RADII\s*=\s*\(([^)]+)\)/)[1].trim().split(/\s+/).map(Number);
  ...
  for(const [lon,lat,radius] of [[0,0,10000],[90,0,8000],[180,0,10000],[270,0,8000],[0,90,7000],[0,-90,7000]])
