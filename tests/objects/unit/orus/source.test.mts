...
test('Orus preserves the published approximation extents, not the full axis lengths as radii', async () => {
  ...
  for (const [lon, lat, metres] of [[0, 0, 35350], [90, 0, 31500], [0, 90, 25700], [0, -90, 25700]] as const) assert.ok(Math.abs(required(shape.sample(lon, lat)) - metres) < .001);
