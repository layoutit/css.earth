...
import { parseCalibrationSampleIds } from "./input-validation.mts";
...
const sampleIds = parseCalibrationSampleIds(report);
...
for (const [sampleIndex, id] of sampleIds.entries()) {
  ...
  const blurredBySigma = new Map<number, Float32Array>();
  ...
  for (const candidate of candidates) {
    ...
    if (!blurred) throw new Error("Missing prepared blur for candidate sigma.");
    let browserCount = 0;
    let intersection = 0;
...
async function transformFullResolution(localBytes: Buffer, referenceAnalysis: Analysis, browserAnalysis: Analysis, candidate: Candidate) {
...
function excludedPixel(x: number, y: number, referenceAnalysis: Analysis, browserAnalysis: Analysis) {
...
function deduplicateCandidates(values: readonly (Candidate | undefined)[]) {
  const seen = new Set();
  return values.filter((candidate): candidate is Candidate => {
...
async function readRgb(bytes: Buffer) {
...
function luminances(rgb: Uint8Array) {
...
function inclusionMask(referenceAnalysis: Analysis, browserAnalysis: Analysis) {
...
function thresholdMask(values: Float32Array, included: Uint8Array) {
...
function maskedMean(values: Float32Array, included: Uint8Array) {
  let sum = 0;
  let count = 0;
  for (let index = 0; index < values.length; index += 1) {
    if (!included[index]) continue;
