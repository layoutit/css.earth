...
import { prepareMarsProjectedSilhouette } from "../../../unit/mars/prepared-fixture.mjs";
import {
  MARS_LIMB_CONTOUR_THRESHOLDS,
  measureMarsLimbContour,
  type LimbContourMeasurement,
} from "./limb-contour-metric.mts";
...
const samples: Array<{ readonly density: number; readonly pitch: number; readonly measurement: Readonly<{ readonly passes: boolean }>; readonly smoothControl: Readonly<{ readonly passes: boolean }> }> = [];
try {
  for (const density of DENSITIES) {
    ...
    for (const pitch of PITCHES) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        if (!(leaf instanceof HTMLElement)) throw new Error("Mars material leaf is unavailable.");
      ...
      const captures: Partial<CapturePaths> = {};
      for (const mode of ["composite", "body", "material"] as const) {
      ...
      const compositePath = captures.composite;
      if (!compositePath) throw new Error("Composite capture path is unavailable.");
      const composite = PNG.sync.read(await import("node:fs/promises").then(
        ({ readFile }) => readFile(compositePath),
      ));
      ...
      samples.push(Object.freeze({
        ...
        clip,
        captures: requireCapturePaths(captures),
        diffPath,
...
async function installCaptureMode(page: Page, mode: CaptureMode): Promise<void> {
...
async function writeContourDiff(path: string, source: PNG, outward: Uint8Array, inward: Uint8Array): Promise<void> {
...
function reportableMeasurement(measurement: LimbContourMeasurement): Readonly<Omit<LimbContourMeasurement, "outward" | "inward">> {
...
function requireCapturePaths(paths: Partial<CapturePaths>): CapturePaths {
  const { composite, body, material } = paths;
  if (!composite || !body || !material) throw new Error("All Mars limb capture paths are required.");
  return Object.freeze({ composite, body, material });
}
