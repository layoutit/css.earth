...
interface DensityMeasurement {
  ...
  readonly bodyBytes: number;
  readonly coldAssetBytes: number;
  readonly bodyBytesByType: Readonly<Record<string, number>>;
  readonly resources: readonly ResourceMeasurement[];
}
...
const captures: DensityMeasurement[] = [];
try {
...
async function measureDensity(browserInstance: Browser, deviceScaleFactor: number): Promise<DensityMeasurement> {
...
function localBaseUrl(value: string | undefined): string {
  if (!value) {
