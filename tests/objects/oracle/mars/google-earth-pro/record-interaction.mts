import { object, array, parseJson } from "./oracle-values.mts";
...
async function compareDecodedImages(leftPath: string, rightPath: string) {
...
async function normalizedPng(path: string) {
  return sharp(path).rotate().toColorspace("srgb").ensureAlpha().png().toBuffer();
}

function numericDelta(requested: ViewInfo, observed: ViewInfo) {
...
async function readInstrumentationEvents(path: string) {
  const source = await readFile(path, "utf8");
  return Object.freeze(source.trim().split("\n").filter(Boolean).map((line) =>
    Object.freeze(compactInstrumentationEvent(parseJson(line)))));
}

function compactInstrumentationEvent(value: unknown) {
  const event = object(value, "instrumentation event");
  ...
  return {
    ...
    menuItemCount: event.menus === undefined ? 0 : array(event.menus, "menus").length,
    windows: array(event.windows ?? [], "windows").map(value => {
      const { title, visible, level, frame } = object(value, "window");
      return { title, visible, level, frame };
    }),
    qtActions: array(event.qtActions ?? [], "qtActions").map(value => object(value, "Qt action")).filter(({ objectName }) =>
      typeof objectName === "string" && trackedObjectNames.has(objectName)),
  };
}
