...
function input(value: unknown, label: string): InputEvent { const source = record(value, label); const kind = text(source.kind, `${label}.kind`); if (kind !== "down" && kind !== "drag" && kind !== "up" && kind !== "wheel" && kind !== "move") throw new TypeError(`${label}.kind is invalid.`); return { id: text(source.id, `${label}.id`), kind, x: number(source.x, `${label}.x`), y: number(source.y, `${label}.y`), event: source.event === undefined ? undefined : text(source.event, `${label}.event`), qtButtons: source.qtButtons === undefined ? undefined : number(source.qtButtons, `${label}.qtButtons`), acceptedMonotonicSeconds: source.acceptedMonotonicSeconds === undefined ? undefined : number(source.acceptedMonotonicSeconds, `${label}.acceptedMonotonicSeconds`) }; }
function parseReport(value: unknown): Report { const source = record(value, "native report"); const viewport = record(source.viewport, "native report.viewport"); const capture = source.captureConfiguration === undefined ? undefined : record(source.captureConfiguration, "native report.captureConfiguration"); return { scenario: text(source.scenario, "native report.scenario"), inputHistoryPath: source.inputHistoryPath === undefined ? undefined : text(source.inputHistoryPath, "native report.inputHistoryPath"), frameTimingPath: source.frameTimingPath === undefined ? undefined : text(source.frameTimingPath, "native report.frameTimingPath"), inputs: array(source.inputs, "native report.inputs").map((entry, index) => input(entry, `native report.inputs[${index}]`)), gesture: array(source.gesture, "native report.gesture").map((entry, index) => input(entry, `native report.gesture[${index}]`)), frames: array(source.frames, "native report.frames").map((entry, index) => ({ monotonicSeconds: number(record(entry, `native report.frames[${index}]`).monotonicSeconds, `native report.frames[${index}].monotonicSeconds`) })), captureConfiguration: capture === undefined ? undefined : { captureUntilRest: capture.captureUntilRest === undefined ? undefined : capture.captureUntilRest === false }, viewport: { width: number(viewport.width, "native report.viewport.width"), height: number(viewport.height, "native report.viewport.height"), sceneLeft: number(viewport.sceneLeft, "native report.viewport.sceneLeft"), contentWidth: number(viewport.contentWidth, "native report.viewport.contentWidth") }, nativeMotionTrace: source.nativeMotionTrace === undefined ? undefined : text(source.nativeMotionTrace, "native report.nativeMotionTrace") }; }
