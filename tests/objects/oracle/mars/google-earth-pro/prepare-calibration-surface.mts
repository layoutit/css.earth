...
if (process.argv.slice(2).some((argument) => argument !== "--verify")) {
  throw new Error("Usage: prepare-calibration-surface.mts [--verify]");
}
...
if (verifyOnly) {
  ...
  for (const [relativePath, expected] of artifacts) {
    ...
    try {
      ...
      failures.push(`${relativePath}: ${error instanceof Error ? ('code' in error ? error.code : error.message) : String(error)}`);
    }
  }
...
async function prepareGoogleTilePyramid() {
  const tileArtifacts: [string, Buffer][] = [];
  const tiles = [];
...
async function prepareCssEarthAtlases() {
  const entries = [];
  const atlasArtifacts: [string, Buffer][] = [];
...
function drawNorthPoleMarker(rgba: Buffer) {
...
function drawSouthPoleMarker(rgba: Buffer) {
...
function drawRegistrationDots(rgba: Buffer) {
  const dots: [number, number, number, readonly number[]][] = [
...
async function encodeLosslessPng(rgba: Uint8Array, { width, height }: {width: number; height: number}) {
...
function derivativeDescriptor({
  ...
  preparation,
}: {id: string; path: string; bytes: Uint8Array; rgba: Uint8Array; width: number; height: number; role: string; sourceDecodedRgbaSha256: string; preparation: Record<string, unknown>}) {
  return Object.freeze({
    id,
    path,
    role,
...
function extractRgba(source: Buffer, {
  sourceWidth,
  left,
  top,
  width,
  height,
}: {sourceWidth: number; left: number; top: number; width: number; height: number}) {
...
function setPixel(rgba: Buffer, x: number, y: number, color: readonly number[]) {
...
function fillRect(rgba: Buffer, left: number, top: number, width: number, height: number, color: readonly number[]) {
...
function drawVerticalLine(rgba: Buffer, centerX: number, width: number, color: readonly number[]) {
  fillRect(rgba, Math.round(centerX - width / 2), 0, width, SOURCE_HEIGHT,
    color);
}

function drawHorizontalLine(rgba: Buffer, centerY: number, height: number, color: readonly number[]) {
  fillRect(rgba, 0, Math.round(centerY - height / 2), SOURCE_WIDTH, height,
    color);
}

function drawDot(rgba: Buffer, centerX: number, centerY: number, radius: number, color: readonly number[]) {
...
function drawRing(rgba: Buffer, centerX: number, centerY: number, radius: number, color: readonly number[]) {
...
function drawCross(rgba: Buffer, centerX: number, centerY: number, radius: number, color: readonly number[]) {
...
function cellIdentityColor(identity: number) {
  return [
    72 + identity * 47 % 184,
    72 + identity * 83 % 184,
    72 + identity * 131 % 184,
