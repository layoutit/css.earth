import { appBundleForMember, parseWindowAudit, parseEventLine, type WindowAudit } from "./native-evidence.mts";
import { object, integer, parseJson, isNodeError } from "./oracle-values.mts";
import type { ViewInfo } from "./controller.mts";
interface InputEvent { id: string; kind: string; atMilliseconds: number; x: number; y: number; deltaY?: number; clickCount?: number }
...
try {
  ...
  const launch = object(parseJson(launchResult.stdout), "headless launch");
  oraclePid = integer(launch.pid, "launch.pid");
...
async function runScenario({ revision, id, events }: { revision: number; id: string; events: InputEvent[] }) {
  await setViewInfo({ ...camera, appName });
...
function dragEvents({ from, to, steps = 8, interval = 16 }: { from: readonly [number, number]; to: readonly [number, number]; steps?: number; interval?: number }): InputEvent[] {
  const events = [{
    id: "drag-move-start",
    kind: "move",
    atMilliseconds: 0,
...
function doubleClickEvents([x, y]: readonly [number, number]): InputEvent[] {
  return ([
    ["double-down-1", "down", 0, 1],
    ["double-up-1", "up", 40, 1],
    ["double-down-2", "down", 90, 2],
    ["double-up-2", "up", 130, 2],
  ] as const).map(([id, kind, atMilliseconds, clickCount]) => ({
    id,
    kind,
    atMilliseconds,
    x,
...
async function writeControl(revision: number, events: readonly InputEvent[], { terminate = false } = {}) {
...
async function waitForEvent(predicate: (event: Record<string, unknown>) => boolean, timeoutMilliseconds = 20_000) {
...
async function terminateOracle(pid: number) {
