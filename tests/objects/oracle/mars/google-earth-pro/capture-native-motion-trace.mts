import { object, integer, parseJson, isNodeError } from "./oracle-values.mts";
import { appBundleForMember, parseWindowAudit, type WindowAudit } from "./native-evidence.mts";
import { parseCaptureEvent, eventNumber, eventString, type EventRecord } from "./native-capture-values.mts";
...
type CaptureRun = Awaited<ReturnType<typeof captureRun>>;
type TracedRun = CaptureRun & { trace: NonNullable<CaptureRun["trace"]> };
...
const appPath = appBundleForMember(executablePath);
...
const timingControls: TracedRun[] = [];
const fullRuns: TracedRun[] = [];
for (let index = 0; index < options.repeat; index += 1) {
  timingControls.push(await captureTracedRun({
    id: `repeat-${String(index + 1).padStart(2, "0")}-present-control`,
    mode: "present-only",
  }));
  fullRuns.push(await captureTracedRun({
...
const gates = Object.freeze({
  ...
  noInstrumentationLongTask: fullRuns.every((run) =>
    (run.trace.instrumentation.maximumMilliseconds ?? Infinity) < 50),
...
async function captureRun({ id, mode }: { id: string; mode: "off" | "present-only" | "full" }) {
  ...
  try {
    ...
    const scenarioEvents = (await readEvents(launchRoot)).filter((event) =>
      typeof event.revision === "number" && event.revision >= 1 && event.revision <= inputEvents.length);
...
async function inspectTrace({ tracePath, events }: { tracePath: string; events: readonly EventRecord[] }) {
  const bytes = await readFile(tracePath);
  const decoded = decodeMotionTrace(bytes);
  const accepted = events.filter(({ event }) => event === "native-input-accepted").map((event) => ({
    ...event, kind: eventString(event, "kind"), acceptedInputSerial: eventNumber(event, "acceptedInputSerial"), acceptedMonotonicSeconds: eventNumber(event, "acceptedMonotonicSeconds"),
  }));
  if (accepted.length === 0) throw new Error("Motion trace has no accepted input evidence.");
  ...
  if (!lastAccepted) throw new Error("Motion trace has no final accepted input.");
  const lastAcceptedNanoseconds = lastAccepted.acceptedMonotonicSeconds * 1e9;
...
function decodeMotionTrace(bytes: Buffer) {
  ...
  const phases: readonly NativeMotionFrame["inputPhase"][] = [
    "idle",
    "move",
    "drag-held",
...
function cadenceStatistics(frames: readonly NativeMotionFrame[]) {
...
function instrumentationStatistics(frames: readonly NativeMotionFrame[]) {
...
async function waitForTraceInputSerial(tracePath: string, inputSerial: number) {
...
function compareFullRuns(runs: readonly TracedRun[]) {
...
function summarizeRun(run: CaptureRun) {
  return Object.freeze({
    id: run.id,
    mode: run.mode,
    runRoot: run.runRoot,
...
function closestFrame(frames: readonly NativeMotionFrame[], targetNanoseconds: number) {
  let selected: NativeMotionFrame | null = null;
...
async function writeControl(path: string, revision: number, events: readonly (NativeGesture & { sequenceOffsetMilliseconds?: number })[], { terminate = false }: { terminate?: boolean } = {}) {
...
async function waitForEvent(launchRoot: string, predicate: (event: EventRecord) => boolean, timeoutMilliseconds = 20_000) {
  const started = Date.now();
...
function dragEvents({ from, to, steps, interval }: { from: readonly [number, number]; to: readonly [number, number]; steps: number; interval: number }) {
  const events = [{
    id: "drag-move-start",
    kind: "move",
    atMilliseconds: 0,
...
function maximumInputDispatchLag(events: readonly EventRecord[]) {
  ...
  return maximum(posted.map((event) => {
    ...
    if (!batch) throw new Error("Missing input batch for posted event.");
    return (eventNumber(event, "postedMonotonicSeconds") - eventNumber(batch, "acceptedMonotonicSeconds")) *
      1e3 - eventNumber(event, "requestedMilliseconds");
  }));
}

function numericDelta(before: ViewInfo, after: ViewInfo) {
  return Object.freeze(Object.fromEntries(
    (["latitude", "longitude", "distance", "tilt", "azimuth"] as const).map((key) =>
      [key, after[key] - before[key]]),
  ));
}

function matrixMaximumError(left: readonly number[], right: readonly number[]) {
...
function readCpuMilliseconds(pid: number) {
...
function formatProcess({ pid, state, command }: { pid: number; state: string; command: string }) {
...
async function captureTracedRun(options: { id: string; mode: "present-only" | "full" }): Promise<TracedRun> {
  const run = await captureRun(options);
  if (!run.trace) throw new Error("Instrumented capture did not produce a trace.");
