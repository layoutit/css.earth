import { parseSuiteConfiguration, parseSuiteManifest, type SuiteManifest } from "./interaction-suite-manifest.mts";
import { parseJson, object, array } from "./oracle-values.mts";
...
import { interactionSuiteCases, repeatabilityCases } from "./interaction-suite-cases.mts";
...
const config = parseSuiteConfiguration(parseJson(await readFile(configPath, "utf8")));
...
let manifest: SuiteManifest = { schema: "cssearth-oracle-interaction-suite@1", startedAt: new Date().toISOString(),
...
if (await exists(manifestPath)) {
  ...
  manifest = parseSuiteManifest(parseJson(await readFile(manifestPath, "utf8")));
...
try {
  ...
  if (!nativeOnly) {
    assert.ok(browserUrl, "Browser URL was not resolved");
  ...
  for (const entry of cases) {
    ...
    for (let repeat = 0; repeat < count; repeat++) {
      ...
      if (resume && await exists(existingNative)) {
        const existing = object(parseJson(await readFile(existingNative, "utf8")), "native report");
        let heldButtons = 0;
        reusableNative = array(existing.gesture, "native gesture").map(value => object(value, "native gesture event")).every(e => {
      ...
      try {
        ...
        if (!reusableNative) {
          await command("capture-rendered-motion.mts", [resolve(config.appRoot), resolve(runRoot, "native"), "1", scenarioPath], resolve(runRoot, "native.log"), { CSS_EARTH_EVIDENCE_ROOT:evidenceRoot });
        }
        ...
        if (repeat === 0 && !nativeOnly) {
          ...
          if (!(resume && await exists(comparisonPath))) {
            await command("compare-rendered-motion.mts", [nativePath, browserPath, dirname(comparisonPath)], resolve(browserRoot, "comparison.log"));
          }
          ...
          if (!(resume && await exists(frameComparison))) await command("compare-rendered-motion.mts",
        ...
        run.status = "invalid"; run.error = error instanceof Error ? error.message : String(error);
      }
      await save();
  ...
  await save();
  if (!nativeOnly) await command("publish-interaction-suite.mts", [manifestPath], resolve(out, "publish.log"));
...
async function command(script: string, argv: readonly string[], log: string, env: NodeJS.ProcessEnv = {}) {
