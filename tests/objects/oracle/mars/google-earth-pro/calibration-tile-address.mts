export interface FlatTileAddress { readonly level: number; readonly row: number; readonly col: number; }
export interface DecodedRgba { readonly data: Buffer; readonly info: { readonly width: number; readonly height: number; readonly channels: number; }; }

// Flat tile space is square: both axes span 360 degrees. Polar rows are unused.
export function flatTileBounds(level: number, row: number, col: number) {
...
export function sampleCalibrationTile(source: DecodedRgba, address: FlatTileAddress, size = 256) {
  if (!Number.isSafeInteger(size) || size < 1) throw new RangeError("Invalid calibration tile size.");
  const bounds = flatTileBounds(address.level, address.row, address.col);
  const { width, height, channels } = source.info;
  if (channels !== 4 || width !== height * 2 || source.data.length !== width * height * channels) throw new TypeError("Expected equirectangular RGBA source.");
