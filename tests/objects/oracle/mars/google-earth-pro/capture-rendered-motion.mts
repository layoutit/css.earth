import { parseRenderedMotionScenario, parseNativeCaptureEvent, parseRenderedFrame, type NativeGesture, type RenderedFrame } from "./rendered-motion-inputs.mts";
import { object, text, finite, boolean, integer, parseJson, isNodeError } from "./oracle-values.mts";
import { appBundleForMember, parseWindowAudit } from "./native-evidence.mts";
import type { ViewInfo } from "./controller.mts";
import type { InteractionGeometry } from "./interaction-corpus.mts";
import type { NativeMotionFrame } from "./native-motion-trace-reader.mts";
interface StartVerification {
  method: string; requested: ViewInfo; resolved: ViewInfo; prior: NativeMotionFrame; held: NativeMotionFrame;
  maximumMatrixElementDelta: number; maximumViewElementDelta: number; matrixRoundingAllowance: number; scope: string;
  captureStart?: { method: string; firstPresentIndex: number; prePressFrames: number; initializationFrameCount: number; rawFrameCount: number; settledModelView: readonly number[]; settledProjection: readonly number[] };
}
...
import { getViewInfo, setViewInfo } from "./controller.mts";
import { decodeNativeMotionTrace } from "./native-motion-trace-reader.mts";
import { resolveScenarioEvents } from "./interaction-corpus.mts";
...
const scenario = parseRenderedMotionScenario(scenarioArgument ? parseJson(await readFile(resolve(scenarioArgument), "utf8")) : {});
...
const build = object(parseJson(await readFile(resolve(appRoot, "headless-oracle-manifest.json"), "utf8")), "headless manifest");
const app = appBundleForMember(text(build.embeddedDylibPath, "embeddedDylibPath"));
...
const provenMap = object(parseJson(await readFile(resolve(evidenceRoot,
  "output/playwright/google-earth-pro-mars-interaction-video-v1/native-registration/mapping/texture-map.json"), "utf8")), "texture map");
const cacheTransform = object(provenMap.cacheTransform, "cacheTransform");
const mappingKey = await readFile(text(cacheTransform.keyPath, "keyPath"));
assert.equal(hash(mappingKey), text(cacheTransform.keySha256, "keySha256"));
...
const calibration = object(parseJson(await readFile(resolve(evidenceRoot,
  ".local/oracles/google-earth-pro/calibration/manifest.json"), "utf8")), "calibration manifest");
const calibrationSource = object(calibration.source, "calibration source");
const rgba = await sharp(resolve(evidenceRoot, ".local/oracles/google-earth-pro/calibration",
  text(calibrationSource.path, "source.path"))).toColorspace("srgb").ensureAlpha().raw().toBuffer();
assert.equal(hash(rgba), text(calibrationSource.decodedRgbaSha256, "source.decodedRgbaSha256"));
...
const child = spawn(executablePath, ["-multiple"], {
  ...
  env: { ...process.env,
    ...
    ...([30,60].includes(scenario.presentHz ?? 0) ? {CSSEARTH_ORACLE_PRESENT_HZ:String(scenario.presentHz),
...
assert.ok(child.pid, "Native capture process did not start");
...
const audit = async () => {
  const result = parseWindowAudit(parseJson((await exec(resolve(appRoot, "window-audit"), [String(child.pid)])).stdout));
...
try {
  ...
  let startVerification: StartVerification | null = null;
  let gesture: NativeGesture[] = scenario.gesture ?? [
  ...
  let inputGeometry: (InteractionGeometry & { method: string; modelViewMatrix: readonly number[]; projectionMatrix: readonly number[] }) | null = null;
  if (scenario.events) {
    ...
    let buttons = 0;
    gesture = resolveScenarioEvents({ events: scenario.events }, inputGeometry).map(event => {
  ...
  const mapping = await exec(process.execPath, [resolve(import.meta.dirname,
    "prepare-google-calibration-map.mts"), ...seedArguments, "--audit", resolve(out, "draws.jsonl"),
  ...
  const capturedFrames = records.filter(e => e.event === "rendered-frame" && captureRevisions.includes(e.revision ?? -1)).map(parseRenderedFrame).filter(e =>
    (!stopObservation || e.presentIndex <= stopObservation.lastPresentedSequence));
  let frames = capturedFrames;
  let initializationFrames: RenderedFrame[] = [];
  ...
  const bindings = (await readFile(resolve(out, "bindings.jsonl"), "utf8")).trim().split("\n").filter(Boolean).map(line => {
      const binding = object(parseJson(line), "texture binding");
      return { ...binding, revision: integer(binding.revision), mapped: boolean(binding.mapped) };
    })
  ...
  if (scenario.normalizeStart) {
    ...
    assert.ok(press && startVerification, "Normalized capture requires its press and verified start state");
    ...
    const prePress=frames.filter(f=>f.monotonicSeconds<finite(press.acceptedMonotonicSeconds));
    assert.ok(prePress.length,"The capture must contain a real frame before press");
    const settled=poses.get(prePress[prePress.length - 1].presentIndex);
    assert.ok(settled, "Initial rendered frame lacks its camera matrices");
    const initial=[...settled.modelViewMatrix,...settled.projectionMatrix];
    assert.ok(settled.modelViewMatrix.slice(0,12).every((v,i)=>
      Math.abs(v-verification.prior.modelViewMatrix[i])<=verification.matrixRoundingAllowance),
    ...
    for(let i=prePress.length-1;i>=0;i--) {
      ...
      assert.ok(pose, "Rendered frame lacks its camera matrices");
  ...
  for (const entry of extraCaptures) {
    ...
    await writeFile(resolve(caseRoot, "report.json"), JSON.stringify({
      ...
      frames:records.filter(e => e.event === "rendered-frame" && e.revision === entry.revision).map(parseRenderedFrame).filter(e =>
...
async function observeRest(revision: number, scheduled: readonly NativeGesture[]) {
  const started = Date.now();
  let previous: number[] | null = null;
  let lastChange = Date.now(), matrixChanges = 0;
  const finalEvent = scheduled.at(-1);
  assert.ok(finalEvent, "Rest observation requires a scheduled event");
  const finalId = finalEvent.id;
  let apiView: Readonly<ViewInfo> | null = null;
  let apiStableSince: number | null = null;
  for (;;) {
    await delay(100);
    ...
    if (finalInput &&
        Date.now()-started > finalEvent.atMilliseconds+500 && Date.now()-lastChange>=500) {
      ...
      const fields = ["latitude", "longitude", "distance", "tilt", "azimuth"] as const;
      if (!apiView || fields.some(key => view[key] !== apiView?.[key])) {
        apiView = view; apiStableSince = Date.now();
      }
      if (apiStableSince === null || Date.now() - apiStableSince < 500) continue;
...
async function events() {
  try { const text = await readFile(log, "utf8"); return text.slice(0, text.lastIndexOf("\n")).split("\n").filter(Boolean).map(line => parseNativeCaptureEvent(parseJson(line))); }
  catch (e) { if (isNodeError(e, "ENOENT")) return []; throw e; }
}
async function control(value: { revision: number; events: readonly NativeGesture[]; captureMilliseconds?: number; terminate?: boolean }) {
...
async function until(test: () => boolean | Promise<boolean>, milliseconds: number) {
