...
export async function createPixelmatchTriptych({
  ...
  outputPath,
  threshold = 0.1,
}: {referencePath: string; candidatePath: string; outputPath: string; threshold?: number}) {
...
function outputStem(path: string) {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  ...
  if (!referencePath || !candidatePath || !outputPath) {
    throw new Error(
      "Usage: node triptych.mts <native-image> <browser-image> <output.png>",
    );
  }
