...
import { parseJson } from './oracle-values.mts';
import { parseInteractionCorpus, type InteractionScenario } from './interaction-corpus.mts';
import { parseNativeVideoReport, parseBrowserVideoReport, parseVideoTrace,
  type NativeVideoScenario, type BrowserVideoScenario, type VideoTrace } from './interaction-video-records.mts';

interface MotionPoint { milliseconds: number; matrix: readonly number[]; zoomRatio: number; phase: string; inputSerial: number | null; }
type CumulativePoint = MotionPoint & { pathDegrees: number };
interface InterpolatedPoint { milliseconds: number; pathDegrees: number; zoomRatio: number; phase: string; inputSerial: number | null; velocityDegreesPerSecond: number; accelerationDegreesPerSecondSquared: number; }
interface Sample { milliseconds: number; native: InterpolatedPoint; browser: InterpolatedPoint; angularPathResidualDegrees: number; zoomRatioResidual: number; velocityResidualDegreesPerSecond: number; accelerationResidualDegreesPerSecondSquared: number; }
interface Row { frame: number; milliseconds: number; nativeAngularPathDegrees: number; browserAngularPathDegrees: number; angularPathResidualDegrees: number; nativeZoomRatio: number; browserZoomRatio: number; zoomRatioResidual: number; nativeVelocityDegreesPerSecond: number; browserVelocityDegreesPerSecond: number; velocityResidualDegreesPerSecond: number; nativeAccelerationDegreesPerSecondSquared: number; browserAccelerationDegreesPerSecondSquared: number; accelerationResidualDegreesPerSecondSquared: number; }
type EventTiming = ReturnType<typeof compareEventTiming>;
type Run = Awaited<ReturnType<typeof compareRun>>;
type Aggregate = ReturnType<typeof summarizeRuns>;
...
async function main() {
const [nativeValue, browserValue, corpusValue] = await Promise.all([
...
const nativeReport = parseNativeVideoReport(nativeValue), browserReport = parseBrowserVideoReport(browserValue), corpus = parseInteractionCorpus(corpusValue);
assert.equal(
...
for (const id of includedIds) {
  ...
  assert.ok(nativeQualification, `${id}: missing native qualification`);
  const runs: Run[] = [];
  ...
  assert.ok(browserVisual, `${id}: missing browser screenshots`);
...
async function compareRun({
  ...
  nativeScenario,
  browserScenario,
}: { repeat: number; scenario: InteractionScenario; nativeScenario: NativeVideoScenario; browserScenario: BrowserVideoScenario }) {
  const nativeTrace = parseVideoTrace(await readJson(nativeScenario.trace.path));
...
function nativeMotionPoints(trace: VideoTrace, scenario: NativeVideoScenario) {
...
function browserMotionPoints(scenario: BrowserVideoScenario) {
...
function cumulativeMotion(points: readonly MotionPoint[]) {
...
function resamplePair({ nativePoints, browserPoints, endMilliseconds }: { nativePoints: readonly CumulativePoint[]; browserPoints: readonly CumulativePoint[]; endMilliseconds: number }): Sample[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      velocityResidualDegreesPerSecond: 0, accelerationResidualDegreesPerSecondSquared: 0,
      milliseconds,
      native,
      browser,
      angularPathResidualDegrees:
...
function interpolateMotion(points: readonly CumulativePoint[], milliseconds: number): InterpolatedPoint {
  const derivatives = { velocityDegreesPerSecond: 0, accelerationDegreesPerSecondSquared: 0 };
  if (milliseconds <= points[0].milliseconds) return { ...points[0], ...derivatives };
  if (milliseconds >= points.at(-1)!.milliseconds) return { ...points.at(-1)!, ...derivatives };
  let low = 0;
...
function addDerivatives(samples: Sample[]) {
  for (let index = 0; index < samples.length; index += 1) {
    for (const owner of ["native", "browser"] as const) {
...
function compareEventTiming({ scenario, nativeTrace, browserScenario }: { scenario: InteractionScenario; nativeTrace: VideoTrace; browserScenario: BrowserVideoScenario }) {
...
function responsePair({
  ...
  browserPoints,
  sampledOnly = false,
}: { eventId: string | null; eventTiming: EventTiming; nativePoints: readonly CumulativePoint[]; browserPoints: readonly CumulativePoint[]; sampledOnly?: boolean }) {
...
function firstMotionResponse(points: readonly CumulativePoint[], milliseconds: number) {
...
function nextSample(points: readonly CumulativePoint[], milliseconds: number) {
  return points.find((point) => point.milliseconds >= milliseconds) ??
    points.at(-1)!;
}

function firstResponseEventFor(scenario: InteractionScenario) {
...
function interruptionEventFor(scenario: InteractionScenario) {
...
function summarizeSamples(rows: readonly Row[]) {
  return Object.freeze({
...
function summarizeRuns(runs: readonly Run[]) {
  const metric = (key: keyof Run["metrics"]) => runs.map((run) => run.metrics[key]);
  ...
  const drivenWindows = runs.map(({ drivenInputWindow }) =>
    drivenInputWindow).filter(value => value !== null);
  return Object.freeze({
...
function summarizeDrivenInputWindow({
  ...
  browserPoints,
  rows,
}: { scenario: InteractionScenario; eventTiming: EventTiming; nativePoints: readonly CumulativePoint[]; browserPoints: readonly CumulativePoint[]; rows: readonly Row[] }) {
...
function qualifyTrainingScenario(id: string, aggregate: Aggregate) {
...
async function renderTelemetryEvidence({ id, scenario, run, scenarioRoot }: { id: string; scenario: InteractionScenario; run: Run; scenarioRoot: string }) {
...
function drawPanel({ kind, source, sampleIndex, maxPath, eventTimes }: { kind: string; source: readonly Row[]; sampleIndex: number; maxPath: number; eventTimes: readonly number[] }) {
  ...
  const points = source.slice(0, sampleIndex + 1).map((sample): [number, number] => {
...
function joinPanels(panels: readonly Buffer[], id: string) {
...
async function makeContactSheet(comparisons: readonly { visual: { timelinePath: string } }[], path: string) {
  const rows = [];
...
async function encodeVideo(frames: string, output: string) {
...
async function writeRgbPng(path: string, data: Buffer, { width, height }: { width: number; height: number }) {
  await sharp(data, { raw: { width, height, channels: 3 } })
    .png({ compressionLevel: 6 })
    .toFile(path);
}

function rotationDifferenceDegrees(first: readonly number[], second: readonly number[]) {
...
function rotation3(matrix: readonly number[]) {
  const rows = [
...
function multiply3(first: readonly (readonly number[])[], second: readonly (readonly number[])[]) {
  return first.map((row) => row.map((_, column) =>
    row.reduce((sum, value, index) =>
      sum + value * second[index][column], 0)));
}

function transpose3(matrix: readonly (readonly number[])[]) {
  return matrix[0].map((_, column) => matrix.map((row) => row[column]));
}

function rowsToCsv(rows: readonly Row[]) {
  const headers = Object.keys(rows[0]);
  return `${headers.join(",")}\n${rows.map((row) => headers.map((header) =>
    Object.entries(row).find(([key]) => key === header)?.[1]).join(",")).join("\n")}\n`;
}

function fill(buffer: Buffer, width: number, height: number, color: Color) {
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) pixel(buffer, width, x, y, color);
  }
}

function rect(buffer: Buffer, width: number, x: number, y: number, rectangleWidth: number, rectangleHeight: number, color: Color) {
...
function line(buffer: Buffer, width: number, x0: number, y0: number, x1: number, y1: number, color: Color) {
  let currentX = x0;
  let currentY = y0;
...
function circle(buffer: Buffer, width: number, centerX: number, centerY: number, radius: number, color: Color) {
  let x = radius;
  let y = 0;
  let error = 0;
  while (x >= y) {
...
function pixel(buffer: Buffer, width: number, x: number, y: number, color: Color) {
...
function drawText(buffer: Buffer, width: number, x: number, y: number, text: string, color: Color, scale: number) {
  let cursor = x;
...
function clamp(value: number, minimum: number, maximumValue: number) {
...
function parseArguments(args: readonly string[]) {
  const parsed: { native?: string; browser?: string; output?: string } = {};
  ...
  return Object.freeze({ native: parsed.native, browser: parsed.browser, output: parsed.output });
}
