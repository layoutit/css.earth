...
import { parseGranularContract, parseGranularSample, parseSourceIndex, parseSkyboxContract, parseSunContract, parseSequenceManifest, type GranularSample, type DrawnGranularSample } from "./granular-contract-values.mts";
type Vector = readonly number[];
type Matrix = readonly Vector[];
type CameraBasis = ReturnType<typeof cameraBasis>;
type Projection = ReturnType<typeof extractProjection>;
interface SunTimeModel { referenceTimeUtc: string; azimuthAtReferenceDegrees: number; angularRateDegreesPerSecond: number; bodyY: number; }
interface AngularPoint { time: number; angle: number; }
...
const rawContract = parseGranularContract(parseJson(await readFile(rawContractPath, "utf8")));
const samples = (await readFile(rawContract.rawSamples, "utf8")).trim()
  .split("\n").filter(Boolean).map((line) => parseGranularSample(parseJson(line)));
...
const [sourceIndex, sourceSkybox, sourceSun] = await Promise.all([
  readJson(resolve(authoritativeRoot, "contract-index.json")).then(parseSourceIndex),
  readJson(resolve(authoritativeRoot, "skybox-contract.json")).then(parseSkyboxContract),
  readJson(resolve(authoritativeRoot, "sun-presentation-contract.json")).then(parseSunContract),
]);
...
const drawnSun = samples.flatMap((sample, index): { sample: DrawnGranularSample; index: number; direction: Vector }[] => sample.sun === null
  ? []
...
function cameraBasis({ latitude, longitude, heading, tilt }: { latitude: number; longitude: number; heading: number; tilt: number }) {
...
function captureBasis(sample: GranularSample) {
  return Object.freeze({
...
function basisMatrix({ right, up, direction }: CameraBasis) {
  return Object.freeze([
...
function extractProjection(mvp: Vector) {
  const ratios = [];
...
function extractCatalogueRotation(mvp: Vector, projection: Projection) {
...
function catalogueMvp(rotation: Matrix, projection: Projection) {
  const scale = projection.measuredScale;
  const modelView = [
...
function fitSkyRotation(allSamples: readonly GranularSample[]) {
...
function recoverSunDirection(sample: DrawnGranularSample, basis: CameraBasis) {
...
function fitSunDirection(sunSamples: readonly { sample: DrawnGranularSample; direction: Vector }[]) {
...
function sunDirectionAt(model: SunTimeModel, timeSeconds: number) {
...
function maximumRequestedDistance(cullingSamples: readonly { family: string; drawState: string; requested: { distance: number } }[], drawState: string) {
...
function minimumRequestedDistance(cullingSamples: readonly { family: string; drawState: string; requested: { distance: number } }[], drawState: string) {
  const distances = cullingSamples.filter(({ family, drawState: state }) =>
    family === "log-zoom" && state === drawState)
    .map(({ requested }) => requested.distance);
...
async function createContactSheet(allSamples: readonly GranularSample[]) {
...
async function collectSequencePackages() {
  return Object.freeze(await Promise.all(Object.entries(
    ...
    const manifest = parseSequenceManifest(await readJson(path));
...
function multiply4(left: Matrix, right: Matrix) {
  return left.map((row) => right[0].map((_, column) => row.reduce(
    (sum, value, index) => sum + value * right[index][column],
    0,
  )));
...
function multiplyVector3(matrix: Matrix, vector: Vector) {
  return matrix.map((row) => dot(row, vector));
}

function transpose3(matrix: Matrix) {
...
function averageMatrices(matrices: readonly Matrix[]) {
  return matrices[0].map((row, rowIndex) => row.map((_, columnIndex) =>
    mean(matrices.map((matrix) => matrix[rowIndex][columnIndex]))));
}

function matrixResidual(matrices: readonly Matrix[], reference: Matrix) {
...
function linearFit(points: readonly AngularPoint[]) {
...
function unwrapAngles(points: AngularPoint[]) {
...
function project4(matrix: Vector, vector: Vector) {
...
function flatten3(matrix: Matrix) {
  return matrix.flat();
}

function maximumAbsoluteDifference(left: Vector, right: Vector) {
  return Math.max(...left.map((value, index) => Math.abs(value - right[index])));
}

function vectorAngleDegrees(left: Vector, right: Vector) {
  return Math.acos(Math.max(-1, Math.min(1,
    dot(normalize3(left), normalize3(right)),
  ))) * 180 / Math.PI;
}

function add3(left: Vector, right: Vector) {
  return left.map((value, index) => value + right[index]);
}

function scale3(vector: Vector, scale: number) {
  return vector.map((value) => value * scale);
}

function normalize3(vector: Vector) {
...
function mean(values: Vector) {
  return values.reduce((sum, value) => sum + value, 0) / values.length;
}

function numericRange(values: Vector) {
  return Object.freeze({
