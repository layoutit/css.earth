import { object, text, finite, integer, boolean, parseJson, isNodeError } from "./oracle-values.mts";
import { parseWindowAudit, type WindowAudit } from "./native-evidence.mts";
import { parseLayerManifest, parseCameraCalibration, type CameraCalibration } from "./native-capture-values.mts";
import type { ViewInfo, ViewRequest } from "./controller.mts";
import type { Region } from "sharp";
type StatePlan = ReturnType<typeof statePlan>;
type Headless = Awaited<ReturnType<typeof ensureLiveOracle>> & { variantId: string; eventLogPath: string; layerAuditLogPath: string; controlFilePath: string; controlLogPath: string; foregroundAfter: WindowAudit["frontmostApplication"]; visibleWindowCount: number; oracleOwnsForeground: boolean; windows: WindowAudit["windows"] };
...
import { createPixelmatchTriptych } from "./triptych.mts";
...
const layerOracleManifest = parseLayerManifest(parseJson(await readFile(layerOracleManifestPath, "utf8")));
...
const captures: Record<string, Awaited<ReturnType<typeof captureState>>> = {};
for (const plan of statePlans) {
  captures[plan.id] = await captureState(plan, camera);
}

const comparisonPaths: Record<string, { outputPath: string; metricsPath: string; metrics: Awaited<ReturnType<typeof createPixelmatchTriptych>> }> = {};
...
function statePlan(id: string, variantId: string, atmosphere: boolean, sun: boolean, description: string, textureMode = "off") {
  return Object.freeze({
    id,
    variantId,
    atmosphere,
...
async function getCameraCalibration(): Promise<CameraCalibration> {
  ...
  const calibration = parseCameraCalibration(parseJson(sourceBytes.toString("utf8")));
...
async function captureState(plan: StatePlan, requestedCamera: ViewRequest) {
...
async function withOracleProcess<Result>(plan: StatePlan, stateRoot: string, operation: (context: { appName: string; headless: Headless }) => Promise<Result>): Promise<Result> {
  await setLayerControl({
...
async function ensureLiveOracle() {
  ...
  if (!processRow) throw new Error("Canonical native process was not found.");
  const { pid, state } = processRow;
...
async function initializeControlFile({ mode, atmosphere, sun, revision }: { mode: string; atmosphere: boolean; sun: boolean; revision: number }) {
...
async function setLayerControl({ mode, atmosphere, sun }: { mode: string; atmosphere: boolean; sun: boolean }) {
...
async function compareDecodedImages(leftPath: string, rightPath: string, tolerance: number, crop: Region | null = null) {
...
async function absoluteDifference(subjectPath: string, baselinePath: string, outputPath: string) {
  return imageDifference(subjectPath, baselinePath, outputPath, false);
}

async function positiveDifference(subjectPath: string, baselinePath: string, outputPath: string) {
  return imageDifference(subjectPath, baselinePath, outputPath, true);
}

async function imageDifference(subjectPath: string, baselinePath: string, outputPath: string, positiveOnly: boolean) {
...
async function normalizedRgba(path: string, crop: Region | null = null) {
...
async function largestBrightComponent(path: string, crop: Region, minimumChannel: number) {
  ...
  let best: { pixelCount: number; minX?: number; minY?: number; maxX?: number; maxY?: number; width?: number; height?: number; centerX?: number; centerY?: number } = { pixelCount: 0 };
...
async function centerDiskMean(path: string) {
...
async function createContactSheet({ panels, outputPath }: { panels: readonly (readonly [string, string])[]; outputPath: string }) {
...
async function readLayerAudit(path: string | null) {
  if (path === null) return [];
  try {
    return (await readFile(path, "utf8")).trim().split("\n")
      ...
      .map((line) => {
        ...
        return Object.freeze({
          program: integer(Number(program), "program"),
          groundTextureLocation: integer(Number(groundTextureLocation), "ground texture location"),
          skyMapTextureLocation: integer(Number(skyMapTextureLocation), "sky texture location"),
...
async function readControlLog(path: string) {
  try {
    return (await readFile(path, "utf8")).trim().split("\n")
      .filter(Boolean)
      .map((line) => parseControlEntry(Object.fromEntries(
...
function parseControlEntry(value: unknown) {
  const entry = object(value, "layer control");
  return Object.freeze({ ...entry, revision: integer(entry.revision), atmosphere: boolean(entry.atmosphere), sun: boolean(entry.sun) });
}
