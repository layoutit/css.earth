...
interface CacheSource {directory: string; requestKeyBase64: string; levelDbKeyBase64: string; cacheValueSha256: string}
interface CacheEntry {cachePayloadSha256: string; cachePayloadByteCount: number; cachePayloadPrefixBase64: string; cachePayloadPath: string; storage: string; requestKind: number; level: number; path: string; row: number; col: number; longitudeRangeDegrees: readonly number[]; latitudeRangeDegrees: readonly number[]; cacheSources: CacheSource[]}
...
const payloadRoot = resolve(dirname(outputPath), "google-cache-payloads");
...
try {
  ...
  const entriesByIdentity = new Map<string, CacheEntry>();
  const snapshots = [];
  ...
  for (let index = 0; index < cacheDirectories.length; index += 1) {
    ...
    const database = new ClassicLevel<Buffer, Buffer>(snapshotPath, {
      keyEncoding: "buffer",
  ...
  const byCachePayloadSha256 = new Map<string, CacheEntry[]>();
...
function decodeQuadtreePath(request: Buffer, level: number) {
...
function pathToAddress(path: string) {
  let row = 0;
  let col = 0;
...
function parseArguments(arguments_: string[]) {
  const parsed: {cacheRoot?: string; output?: string} = {};
  ...
  if (!parsed.cacheRoot || !parsed.output) {
    throw new Error(
      "Usage: export-google-imagery-cache.mts --cache-root <path> --output <json>",
    );
  }
  return Object.freeze({cacheRoot: parsed.cacheRoot, output: parsed.output});
}
