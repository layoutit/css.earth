...
export function decodeNativeMotionTrace(bytes: Buffer): NativeMotionTrace {
  ...
  const phases: readonly NativeInputPhase[] = [
    "idle",
    "move",
    "drag-held",
  ...
  const toNanoseconds = (ticks: bigint): number => Number(
...
export function nativeScenarioTrace({
...
}: { readonly decoded: NativeMotionTrace; readonly events: readonly NativeInputEvent[]; readonly revision: number; readonly revisions?: readonly number[]; readonly tailMs?: number; readonly measurementEndNanoseconds?: number | null }) {
  ...
  const endNanoseconds = measurementEndNanoseconds !== null && Number.isFinite(measurementEndNanoseconds)
...
export function nativeCadenceStatistics(frames: readonly NativeMotionFrame[]) {
...
export function nativeInstrumentationStatistics(frames: readonly NativeMotionFrame[]) {
...
export function closestNativeFrame(frames: readonly NativeMotionFrame[], targetNanoseconds: number) {
...
export function matrixMaximumError(reference: readonly number[] | null | undefined, candidate: readonly number[] | null | undefined): number {
