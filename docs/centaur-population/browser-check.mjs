...
try{
 for(const dpr of navigationOnly?[1]:[1,2]){
  ...
  for(const id of navigationOnly?[]:ids){
   ...
   assert.equal(ringCount,id==='chariklo'?16:0);
   if(id==='chariklo') {
    assert.ok(loads.slice(start).some(load=>load.filename==='chariklo-rings.webp'),'Ring uses the freshly installed source-pinned asset');
    ...
    assert.ok(rings.every(r=>r.width>0&&r.height>0&&r.transform!=='none'&&r.background.includes('chariklo-rings.webp')));
