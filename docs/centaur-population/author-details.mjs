...
const { bodies } = await read('docs/centaur-population/inputs.json');
const ringSource = await read('docs/centaur-population/chariklo-rings.json');
const text = await readFile('packages/astronomy/src/data/asteroidElements-4.data.ts', 'utf8');
const elements = JSON.parse(text.split('export const ASTEROID_ELEMENTS_4 = ')[1].split(' satisfies')[0]);
for (const { id } of bodies) {
  ...
  const descriptor = await read(`${pkg}/object.json`);
  if (id === 'chariklo') {
    ...
    descriptor.properties.recipe.rings = { source: 'terrestrial' };
    const content = await read(`${source}/content/object.json`);
