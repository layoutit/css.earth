...
export async function checkShape(id) {
  ...
  assert.ok(scene.bodyLeaves.every(leaf => leaf.tag === 'u'));
  const ratio = config.geometry.radiusKm / (config.geometry.radius * BASE_TILE);
  const km = point => [point[1] * ratio, point[0] * ratio, point[2] * ratio];
  const points = scene.surfaceTriangles.flatMap(triangle => {
    const v = triangle.map(km);
    return [...v, [0, 1, 2].map(axis => v.reduce((sum, p) => sum + p[axis], 0) / 3)];
  });
  const residuals = points.map(p => Math.abs(Math.sqrt(p.reduce((sum, x, axis) => sum + (x / axes[axis]) ** 2, 0)) - 1));
  ...
  const extentsKm = [0, 1, 2].map(axis => Math.max(...points.map(p => Math.abs(p[axis]))));
  extentsKm.forEach((value, axis) => assert.ok(Math.abs(value / axes[axis] - 1) < .035));
  for (const name of ['shadows', 'orbit']) assert.equal(controls.settings.controls.find(c => c.name === name).checked, false);
  if (id === 'chariklo') {
    ...
    const ringRadiiKm = scene.rings.coverage.sourceFaces.flatMap(face => face.vertices.map(v => Math.hypot(v[0], v[1]) * ratio));
