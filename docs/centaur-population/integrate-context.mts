...
import {parseAuthoringManifest} from '../../tools/source-authoring-templates.mts';
const parseContext=shape({bodies:array(shape({id:text,name:text,color:text}))});
const parseDescriptor=shape({properties:shape({recipe:shape({sources:array(shape({id:text,sha256:text}))})})});
...
const path='src/planets/sun/source/navigation/universe.json',source=parseContext(await read(path));
...
const manifestPath='src/planets/sun/source/manifest.json',manifest=parseAuthoringManifest(await read(manifestPath));
...
const descriptorPath='src/planets/sun/object.json',descriptor=parseDescriptor(await read(descriptorPath));
...
await prepareSpatialContext({sourcePath:resolve(path),outputPath:resolve('src/planets/sun/prepared/world-context.json'),solarGeometryPath:resolve('src/platform/solar-geometry.mts')});
