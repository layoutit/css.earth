...
import {parseSolidReplayScene} from '../../tools/prepared-replay-source.mts';
...
import {refreshSolidSceneEpoch} from '../../tools/objects/terrestrial-layers/solid-scene.mts';
...
for(const id of ids){
 ...
 const config=parseSolidPreparationSource(await json(`${root}/source/preparation/terrestrial.json`));
 const scene=parseSolidReplayScene(await json(`${out}/scene.json`)),definition=requireObjectRuntimeDefinition(await json(`${out}/runtime.json`));
