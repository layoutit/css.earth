...
const ids=['ivar','toro','cerberus','tantalus'],origin='http://127.0.0.1:4278';
const output=resolve('output/playwright/near-earth-population');await mkdir(output,{recursive:true});
...
try{
 for(const dpr of navigationOnly?[1]:[1,2]){
  ...
  await page.route(/\/scenes\/(ivar|toro|cerberus|tantalus)\//,async route=>{
   const [,,id,filename]=new URL(route.request().url()).pathname.split('/');
   const bytes=await readFile(resolve('output/near-earth-population/fresh-runtime',id,filename));
  ...
  for(const id of navigationOnly?[]:ids){
   ...
   assert.equal(await leaves.evaluateAll(nodes=>nodes.every((n,i)=>n===globalThis.__neaLeaves[i])),true);
   ...
   results.push({id,dpr,leaves:800,shadowsDefault:false,orbitDefault:false,sourceLimitationVisible:true,forbiddenLeaves:forbidden,loadedFreshAssets:loads.slice(start)});
  }
  if(dpr===1){
