...
const parseBodies=array(shape({id:text,displayName:text,shapeUrl:text}));
...
async function files(root:string,prefix=''):Promise<string[]>{const list=[];for(const e of await readdir(root,{withFileTypes:true})){const name=prefix+e.name;list.push(...e.isDirectory()?await files(resolve(root,e.name),name+'/'):[name]);}return list;}
async function seal(root:string){
 const s=resolve(root,'source'),m=parseAuthoringManifest(await json(resolve(s,'manifest.json')));
 ...
 const d=parseAuthoringDescriptor(await json(resolve(root,'object.json')));
 for(const ref of d.properties.recipe.sources){try{ref.sha256=(await pin(resolve(root,ref.path))).expectedSha256}catch(e){if(!hasErrorCode(e,'ENOENT'))throw e;}}
 await write(resolve(root,'object.json'),d);
}
const args=process.argv.slice(2);
const bodies=parseBodies(await json(args.find(a=>a.endsWith('.json')) ?? 'docs/near-earth-population/inputs.json')).filter(b=>!args.some(a=>a.startsWith('--object='))||args.includes('--object='+b.id));
const fontSource=fontkit.openSync(PLANET_TITLE_RECIPE.checkedFontPath);
if(!('getVariation' in fontSource))throw new TypeError('Title preparation requires a variable font.');
const font=fontSource.getVariation({wght:PLANET_TITLE_RECIPE.weight,opsz:PLANET_TITLE_RECIPE.opticalSize});
for(const b of bodies){
 ...
 const content=requireRecord(await json(resolve(s,'content/object.json')));content.title=title;await write(resolve(s,'content/object.json'),content);await seal(root);
}
for(const b of bodies){
 const root=resolve('src/planets',b.id),s=resolve(root,'source'),m=parseAuthoringManifest(await json(resolve(s,'manifest.json')));
 if(m.generatedIntermediates.length&&!args.includes('--refresh-context')){console.log(b.id,'already sealed');continue;}
 const config=parseSolidPreparationSource(await json(resolve(s,'preparation/terrestrial.json')));
 ...
 if(!radial)throw new TypeError('Source finalization requires its published shape.');
 const stage=resolve('output/near-earth-population/context',b.id);await mkdir(stage,{recursive:true});
 ...
 const recipe={generator:'tools/objects/terrestrial-layers/radial-snapshot.mts',inputs:[b.id+'-shape'],size:512,longitudeDegrees:0,latitudeDegrees:35,ambient:.45,diffuse:.55,lensId:'shape'};
 const png=await renderRadialSnapshot({...recipe,faces:radial.faces,map:resolve(stage,requireString(surfaces.find(x=>x.id==='shape')?.map.url.split('/').at(-1)))});
 await writeFile(resolve(s,'presentation/context.png'),png);
 const shape=m.inputs.find(e=>e.id===b.id+'-shape');
 if(!shape)throw new TypeError('Shape source is missing from the manifest.');
 ...
 const navigation=requireRecord(await json('src/planets/dike/source/preparation/navigation.json'));navigation.planetId=b.id;navigation.source={...generated};await write(resolve(s,'preparation/navigation.json'),navigation);
 const props=requireRecord(await json(resolve(s,'reference/model-properties.json')));props.simplification=radial.simplification;await write(resolve(s,'reference/model-properties.json'),props);
 await seal(root);console.log(b.id,radial.faces.length,'faces',requireRecord(radial.simplification).estimatedErrorMeters,'m estimated error');
}
