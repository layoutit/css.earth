...
A local preparation trial preserved all 348 boundary edges while reducing the mesh to 1,000 faces, with a 49.51 m simplifier error estimate. This trial is **not qualified**: its topology and winding still require investigation, and views from the unobserved side expose the open boundary. Those gaps must remain explicit; a closed nucleus cannot be supplied by filling them. The original model, labels, hashes and trial outputs are retained under `output/comet-intake/wild2/`. No Wild 2 route, geometry or fallback is shipped.
