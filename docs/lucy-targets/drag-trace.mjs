...
const id = process.argv[5] ?? 'polymele';
assert.ok(OBJECTS.some(object => object.id === id && object.classification === 'asteroid'));
...
try {
  ...
  const initial = await page.evaluate(id => {
    ...
    window.__asteroidTraceNodes = [root, ...root.querySelectorAll('*')];
    ...
    return { sceneTransform: getComputedStyle(document.querySelector('.polycss-scene')).transform,
      nodes: window.__asteroidTraceNodes.length, bodyLeaves: document.querySelectorAll(`.${id}-body > u`).length,
  ...
  await page.evaluate(() => performance.mark('asteroid-trace-start'));
  await page.waitForTimeout(2000);
  const requestOffset = requests.length;
  await page.evaluate(() => {
    performance.mark('asteroid-trace-interaction-start');
    window.__asteroidRaf = []; window.__asteroidRafActive = true;
    const tick = t => { if (window.__asteroidRafActive) { window.__asteroidRaf.push(t); requestAnimationFrame(tick); } }; requestAnimationFrame(tick);
  });
  ...
  const raf = await page.evaluate(() => { window.__asteroidRafActive = false; performance.mark('asteroid-trace-interaction-end'); return window.__asteroidRaf; });
  await page.waitForTimeout(2000);
  await page.evaluate(() => performance.mark('asteroid-trace-end'));
  ...
  const final = await page.evaluate(id => {
    ...
    return { sceneTransform: getComputedStyle(document.querySelector('.polycss-scene')).transform,
      nodes: nodes.length, retainedIdentity: nodes.length === window.__asteroidTraceNodes.length && nodes.every((node, i) => node === window.__asteroidTraceNodes[i]),
  ...
  const trace = JSON.parse(text), marks = new Map(trace.traceEvents.filter(e => e.name.startsWith('asteroid-trace-')).map(e => [e.name, e]));
  const start = marks.get('asteroid-trace-interaction-start'), end = marks.get('asteroid-trace-interaction-end');
  ...
  const report = { schema: 'cssearth-asteroid-drag-trace@1', capturedAt: new Date().toISOString(),
