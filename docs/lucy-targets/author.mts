#!/usr/bin/env node
import {parseAuthoringManifest,parseAuthoringDescriptor,parseAuthoringContent,parseAuthoringNavigation,parseAuthoringSolid,parseLucyAuthoringInputs} from '../../tools/source-authoring-templates.mts';
import {requireRecord} from '../../tools/source-values.mts';
type LucyBody=ReturnType<typeof parseLucyAuthoringInputs>['bodies'][number];
...
const { bodies, checkedOn } = parseLucyAuthoringInputs(JSON.parse(await readFile(inputsPath,'utf8')));
const reproduceCommand = `docs/lucy-targets/author.mts${inputsArg ? ' ' + inputsArg : ''}`;
...
const readCommon = (path:string) => execFileSync('git', ['show', `${commonCommit}:${common}${path}`]);
const commonJson = (path:string):unknown => JSON.parse(readCommon(path).toString('utf8'));
...
const replace = (text:string, b:Pick<LucyBody,'id'|'name'>) => text.replaceAll('menoetius', b.id).replaceAll('Menoetius', b.name);
const configFor = (path:string, b:LucyBody):unknown => JSON.parse(replace(readCommon(path).toString(), b));
async function repin(b:LucyBody) {
  const pkg = resolve(root, 'src/planets', b.id), src = resolve(pkg, 'source');
  const manifest = parseAuthoringManifest(JSON.parse(await readFile(resolve(src, 'manifest.json'),'utf8')));
  ...
  const descriptorPath = resolve(pkg, 'object.json'), descriptor = parseAuthoringDescriptor(JSON.parse(await readFile(descriptorPath,'utf8')));
...
for (const b of bodies) {
  ...
  if (!pole) {
    if(!b.poleEclipticDegrees)throw new TypeError(`Missing authored pole: ${b.id}`);
  ...
  const config = parseAuthoringSolid(configFor('source/preparation/terrestrial.json', b));
  ...
  requireRecord(config.raster.observations[0].metadata).coverage = coverage;
  ...
  const content = parseAuthoringContent(configFor('source/content/object.json', b));
  ...
  for(const name of ['shadows','orbit']){const setting=content.settings.controls.find(c=>c.name===name);if(!setting)throw new TypeError(`Missing existing setting: ${name}`);setting.checked=false;}
  const stars=content.resources.find(r=>r.role==='stars');if(!stars)throw new TypeError('Template lacks its star source.');
  content.resources = [{ label: 'Shape and dimensions', role: 'surface', description: b.credit, href: b.source }, ...b.papers.map(href => ({ label: 'Scientific source', role: 'facts', description: 'Published model and interpretation', href })), stars];
  ...
  const snapshotRecipe = { generator: 'tools/objects/terrestrial-layers/radial-snapshot.mts', inputs: ['occultation-approximation', 'model-surface'], size: 512, longitudeDegrees: 55, latitudeDegrees: 20, ambient: .45, diffuse: .55, lensId: 'model' };
  ...
  const navigation = parseAuthoringNavigation(configFor('source/preparation/navigation.json', b));
  ...
  const manifest = parseAuthoringManifest(configFor('source/manifest.json', b));
  for (const entry of manifest.inputs.filter(e => ['occultation-approximation', 'model-surface'].includes(e.id ?? ''))) {
    ...
    requireRecord(entry.projection).referenceRadiusMeters = radius * 1000;
  }
  manifest.generatedIntermediates = [{ ...navigation.source }];
  await write(resolve(src, 'manifest.json'), manifest);
  const descriptor = parseAuthoringDescriptor(configFor('object.json', b));
