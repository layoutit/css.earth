...
- The zero test counts one resolution element as the 150 km the paper gives at the sub-observer point. That is the best the data reach, so it counts the most independent elements and gives the smallest error the median can honestly claim; away from that point the elements are larger and so is the error.
