...
The image carries a real sky WCS about where the target stood at the middle of the exposure, and the stage reads its own product back with [`@cssearth/fits`](../packages/fits/README.md)'s `skyImageAxes` before it finishes: a grid this stage could not state as a sky image never leaves the run. Beside the counts it writes the exposure-normalised image, the background it fitted, the model it compared against and the per-pixel significance, a [product record](../packages/telescope/src/product-record.ts) pinning what went in and what each check establishes, and a reproduction receipt. The arithmetic is in [`timetag-reduction.mts`](../tools/objects/hst/timetag-reduction.mts), covered by `timetag-frame.test.mts` on synthetic event lists.
