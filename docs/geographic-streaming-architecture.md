...
3. Load admitted images whose metadata is available. An unrelated directory request must not stop them. Rank the final selected image groups by distance from the viewport centre, keeping each image's prepared pieces together. Admit metadata in increasing pyramid level, then by distance from the viewport centre, so a deep central branch cannot starve covering metadata for its neighbours. View changes reorder waiting metadata without restarting active requests or discarding loaded entries. Preserve bounded retry, timeout, source validation and cancellation.
