...
The final production build generated 814 HTML pages and all scene endpoints. Assembly passed for all 406 objects after restoring missing baseline asset copies by manifest hash. All 16,136 production asset files passed size/SHA256 verification. Production-mode browser checks at DPR 1 and 2 verified a coarse saved-URL start, finer atlases through real wheel input, explicit sidebar loading, cached return, retained scene nodes and no request/page errors. These checks use public DOM and network observations because normal production disables diagnostic APIs. Evidence: `output/playwright/earth-texture-levels-production-closeout-20260909/` at `4e88adebe`.

CI restores the pinned scene JSON before the runtime/dataset tests; its first integration run exposed the reversed step order, which was corrected without changing runtime behavior.

Main subsequently advanced to `19ef50abf` with spectral moon maps and four asteroid mosaics. Their seven scene payloads are preserved byte-for-byte; only their scene-bound page metadata was refreshed from verified transports. All 406 payload pins and 74 shell/router/page checks pass after integration. The preparation writer already emits scene and page metadata together, so future preparation uses that shared path.
