...
ROOT=Path('docs/mars-crossing-population'); CACHE=Path('output/population-prs/mars-crossers/research'); BASE=Path('src/planets/dike')
...
for b in bodies:
 ...
 for file in ['documentation.html','AcuA_V1.txt.gz','ReadMe.AcuA.txt','jaxa-data-policy.html']:
 ...
 copyto(CACHE/b['populationSource']['file'],s/'reference/sbdb.json')
 ...
 content['panel']['introduction']=f'{name} is a Mars-crossing asteroid. '+kind+'. The grid marks unavailable surface imagery.'
 facts=content['panel']['facts'];facts[0].update(value=f'{radius:g} km ({"approximate thermal scale" if not b["calibrated"] else "size-calibrated model"})');facts[0]['source'].update(url=calibration['sourceUrl'],label=calibration['sourceLabel'],checked='2026-09-09')
 ...
 restore_entries += [('reference/AcuA_V1.txt.gz','https://data.darts.isas.jaxa.jp/pub/akari/AKARI-IRC_Catalogue_AllSky_AcuA_1.0/AcuA_V1.txt.gz'),('reference/ReadMe.AcuA.txt','https://data.darts.isas.jaxa.jp/pub/akari/AKARI-IRC_Catalogue_AllSky_AcuA_1.0/ReadMe.AcuA.txt'),('reference/jaxa-data-policy.html','https://www.isas.jaxa.jp/en/researchers/data-policy/'),('reference/sbdb.json',b['populationSource']['url'])]
 ...
 descriptor=replace(read(BASE/'object.json'),id,name);descriptor['properties']['recipe']['shape']['radiusKm']=radius;descriptor['properties'].pop('worldFrame',None);descriptor.pop('prepared',None);descriptor['properties']['page']=dict(stylesheets=[f'src/renderers/css/styles/{id}-surfaces.css']);write(root/'object.json',descriptor)
...
- [AKARI AcuA catalog]({calibration['sourceUrl']}) and [Usui et al. (2011)](https://arxiv.org/abs/1106.1948) — retained original catalog and field definitions. Row number {num}, {b['akari']['detections']} detections. The quoted error is the catalog thermal-model error, not total shape-scale uncertainty.
- [JPL SBDB]({b['populationSource']['url']}) — retained independent MCA population classification, checked 2026-09-09.
{refs}
{paper_notes}
