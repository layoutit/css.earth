...
const bodies=await read('docs/mars-crossing-population/inputs.json');
for(const {id,displayName:name} of bodies) {
 const body={id,name,color:'#aaaaaa'};
...
if(!process.argv.includes('--source-only'))await prepareSpatialContext({sourcePath:resolve(path),outputPath:resolve('src/planets/sun/prepared/world-context.json'),solarGeometryPath:resolve('src/platform/solar-geometry.mjs')});
console.log('Mars-crossers added to shared authored context; manifest and recipe source pins agree:',sha256);
