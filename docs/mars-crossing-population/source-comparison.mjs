...
import {shadeRadialFaces} from '../../tools/objects/terrestrial-layers/radial-terrain.mjs';
...
for (const id of ids) {
  ...
  const mapFilename = `${id}-shape-map.webp`, map = resolve(root, 'output/mars-crossing-population/context', id, mapFilename);
  ...
  const report = {
    ...
    mapIdentity:{role:'Preparation intermediate pinned by prepared/surfaces.json; not a runtime request',path:`output/mars-crossing-population/context/${id}/${mapFilename}`,sha256:sha256(mapBytes),bytes:mapBytes.length},
