...
p=Path('packages/astronomy/src/body-types.ts');s=p.read_text()
...
p.write_text(s)
p=Path('packages/astronomy/tools/generate-asteroids.mts');s=p.read_text();match=re.search(r'const bodies = (\[.*?\]);',s);values=json.loads(match[1]);existing={v[0] for v in values}
