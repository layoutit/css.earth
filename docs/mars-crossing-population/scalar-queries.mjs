...
import {BODY_FIXED_SUN_DIRECTIONS} from '../../src/platform/solar-geometry.mts';
const base='output/mars-crossing-population';
const ids=process.argv.slice(2).length?process.argv.slice(2):JSON.parse(await readFile('docs/mars-crossing-population/inputs.json')).map(x=>x.id);
for(const id of ids){
