...
DataLink previews are not science choices. Multiple science links remain separate. Nested responses have a visited set and bounds. Fixed and referenced IDs are resolved by PyVO and retained unchanged. The ESO compatibility rule accepts its documented fixed-ID `meta.id;meta.dataset` declaration only for its advertised service. ALMA's captured malformed nine-character MIME field is retained and refused; no protocol is guessed from its hostname.
...
Top-level coordinate/time systems are preserved; unresolved or ambiguous explicit time references remain unknown. Transfer errors expose typed `authentication`, `no-content`, `byte-limit`, `protocol`, `transport`, `interrupted`, `identity` or `local-io` codes.
...
Validation includes 22 VO boundary/product-record tests, 11 package-extraction tests, 5 VO session/identity tests, 2 mixed-service/ref-binding tests and 44 target/query tests. Legacy session checks and the affected native export/handoff checks also pass; scoped Telescope TypeScript passes. The new catalogue alias-loader assertion passed, while its containing object-schema suite retains one unrelated discovery-path expectation failure (6/7). The full tools typecheck also remains blocked by missing generated modules; these are not reported as full-suite passes.
