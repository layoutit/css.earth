...
const fullCohort = ["moon", "phobos", "deimos", "dimorphos", "io", "europa", "ganymede", "enceladus", "tethys", "dione", "rhea", "titan", "charon"];
const plan = Object.fromEntries(["io", "europa", "ganymede", "tethys", "dione", "rhea"].map(id => [id, ['enhanced']]));
...
const out = resolve(parentOut, `europa-color-footprint-${stamp}`);
...
const report = {schema:'cssearth-b2-visual-capture@1', status:'RUNNING', startedAt:new Date().toISOString(),
  ...
  fullCohort, coverageScope:'enhanced-supplement', plannedViews:plan, outputBudgetBytes:maxOutputBytes,
  derivedFrom:{path:"/tmp/b2-visual-capture-proposal/capture-b2-integrated.mjs",sha256:'2753d81db2aebf11235591eebb4f242e02f0f5a3966d88a2a0ec3adb8725ff84'},
  qualification:'Supplemental enhanced-lens coverage; combines with the separate full-cohort integrated run. Actual Chrome captures and runtime checks; visual review pending. No baseline/native parity or compositor-FPS claim.',
...
try {
  ...
  assert.deepEqual([...closure.bodies.map(row => row.body)].sort(), [...fullCohort].sort());
...
async function captureBody(id, dpr, inputs) {
  ...
  page.on('response', response => {
    ...
    responseQueue = Promise.all([responseQueue, (async () => {
      ...
      } catch (error) { entry.errors.push({kind:'response-read', url:response.url(), status:response.status(), message:error.message}); }
    })()]);
  });
  try {
    ...
    for (const lensId of plan[id]) {
      ...
      await page.evaluate(id => { window.__b2CaptureOwner = window[`__${id}`]; }, id);
      ...
      const flightBefore = id === 'europa' && lensId === 'elevation' ? await snapshot(page,id) : null;
      await page.locator(`button[name="lens"][value="${lensId}"]`).click();
      const target = {controlPitch:130.78769189390778,controlYaw:-164.70299854691237,zoom:1.2396521739130437};
      await page.evaluate(({id,target})=>window[`__${id}`].camera.setState(target),{id,target});
      if (id === 'europa' && lensId === 'elevation') assert.ok(target, 'Europa regional lens must carry a prepared camera destination.');
      ...
      if (flightBefore) {
        ...
        assert.equal(finished.starts,before.starts+1,'Real elevation button starts the prepared destination flight');
        ...
        regionalFlight={trigger:'Real elevation lens button',target,before:flightBefore,after};
      }
