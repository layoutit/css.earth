...
const root=process.cwd(), destination='/tmp/moons-b2-fresh-install-BuW5IT';
...
const save=()=>writeFile(resolve(destination,'post-build-route-receipt.json'),JSON.stringify(report,null,2)+'\n');
...
finally{await browser.close();report.browserClosed=true;report.finishedAt=new Date().toISOString();await save();console.log(resolve(destination,'post-build-route-receipt.json'));}
