#!/usr/bin/env node
// One-off B2 delivery check. Uses the repository's existing acquisition and pin APIs.
...
if (args.some(arg => !arg.startsWith('--project=') && !arg.startsWith('--destination=') && arg !== '--restore')) {
  throw new Error('Usage: node b2-fresh-source-restore.mjs --project=/absolute/repo --destination=/tmp/fresh-dir [--restore]');
}
const option = name => {
...
if (unmerged) throw new Error('B2 source paths still have merge conflicts; freeze them before restoration.');
const bodies = [];
...
const report = {
  schema: 'cssearth-b2-fresh-source-restoration@1', status: restore ? 'PREFLIGHT' : 'PLAN_ONLY_NOT_RUN',
  ...
  scope: 'All declared ignored sources for the 13 B2 packages; complete source closure with individually copied, pinned tracked entries. No runtime assets, cache links, copied ignored files or new checkout.',
...
const python = process.env.CSSEARTH_SPICE_PYTHON;
if (!python) throw new Error('Set CSSEARTH_SPICE_PYTHON to the existing pinned DSK interpreter before the 13-body run.');
const pythonCheck = await run(python, ['-c', 'import json,spiceypy,numpy; print(json.dumps({"spiceypy":spiceypy.__version__,"cspice":spiceypy.tkvrsn("TOOLKIT"),"numpy":numpy.__version__}))']);
report.dskToolchain = JSON.parse(pythonCheck.stdout);
if (report.dskToolchain.spiceypy !== '6.0.3' || report.dskToolchain.cspice !== 'CSPICE_N0067' || report.dskToolchain.numpy !== '2.3.5') throw new Error('DSK interpreter differs from the existing reproduction receipt. No packages were installed.');
...
await mkdir(resolve(destination, 'transform-tmp'));
process.env.TMPDIR = resolve(destination, 'transform-tmp');
