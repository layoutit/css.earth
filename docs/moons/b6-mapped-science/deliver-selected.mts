...
// The existing publisher and setup protocol, serialized for this four-body batch.
...
const root=process.cwd(),ids=['moon','europa','callisto','charon'];
const base='b3a0410f742501a1a1552dea14d9a3730fce7484';
...
assert.equal(changed.length,24,'Review the exact B6 publication inventory if it changes');
await mkdir('output/b6-delivery',{recursive:true});
const directory=await mkdtemp(resolve('output/b6-delivery/run-'));
