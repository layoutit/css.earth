...
assert.ok(ids.length && new Set(ids).size === ids.length && ids.every(id => plan[id]), 'Select unique B6 cohort IDs.');
...
const parentOut = resolve(root, 'output/playwright/b6-surfaces');
...
const report = {schema:'cssearth-b6-visual-capture@1', status:'RUNNING', startedAt:new Date().toISOString(),
...
try {
  ...
  for (const id of ids) {
    ...
    assert.ok(plan[id].every(lens => declared.includes(lens)), `${id}: a planned B6 lens is not prepared yet.`);
