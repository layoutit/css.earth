 ...
 * Run from the existing B1 checkout: node docs/moons/b1-preparation/author-saturn-packages.mjs
...
const data=JSON.parse(await readFile(new URL('./saturn-inputs.json',import.meta.url),'utf8'));
...
const git=path=>execFileSync('git',['show',`${SHA}:${path}`],{cwd:root});
const json=path=>JSON.parse(git(path).toString('utf8'));
const stringify=x=>JSON.stringify(x,null,2)+'\n';
const write=async(path,value)=>{await mkdir(dirname(path),{recursive:true});await writeFile(path,typeof value==='string'||Buffer.isBuffer(value)?value:stringify(value));};
const exists=path=>access(path).then(()=>true,()=>false);
const files=async path=>(await Promise.all((await readdir(path,{withFileTypes:true})).map(async e=>e.isDirectory()?files(resolve(path,e.name)):[resolve(path,e.name)]))).flat();
const baseline='src/planets/kiviuq/';
const commonManifest=json(baseline+'source/manifest.json');
const replace=(s,id,name)=>s.replaceAll('kiviuq',id).replaceAll('Kiviuq',name);
const parseCopy=(value,id,name)=>JSON.parse(replace(JSON.stringify(value),id,name));
async function repin(id){
 const pkg=resolve(root,'src/planets',id),src=resolve(pkg,'source');
 const man=JSON.parse(await readFile(resolve(src,'manifest.json'),'utf8'));
 ...
 const descriptor=JSON.parse(await readFile(resolve(pkg,'object.json'),'utf8'));
...
const {createPlanetTitleSource}=await import(pathToFileURL(resolve(root,'tools/prepare-planet-title-sources.mjs')));
const {PLANET_TITLE_RECIPE}=await import(pathToFileURL(resolve(root,'src/platform/planet-title-recipe.mjs')));
const {parsePdsRadiusTable}=await import(pathToFileURL(resolve(root,'tools/objects/terrestrial-layers/obj-shape.mjs')));
const {simplifyRadialShape}=await import(pathToFileURL(resolve(root,'tools/objects/terrestrial-layers/radial-terrain.mjs')));
const {renderRadialSnapshot}=await import(pathToFileURL(resolve(root,'tools/objects/terrestrial-layers/radial-snapshot.mjs')));
const {paintMissingCoverage}=await import(pathToFileURL(resolve(root,'src/platform/prepare-missing-coverage.mjs')));
...
for(const body of data.bodies){
 ...
 const measurements=structuredClone(body.measurementDefinition);measurements.sampling={stepDegrees:5,longitudeDirection:'east-positive',radiusUnit:'km'};
 ...
 const config=parseCopy(json(baseline+'source/preparation/terrestrial.json'),id,name);
 ...
 config.raster.observations[0].metadata.coverage=body.contentDefinition.lensCoverage;
 ...
 const content=parseCopy(json(baseline+'source/content/object.json'),id,name);
 content.panel.introduction=body.contentDefinition.introduction;
 content.panel.facts=[{id:'diameter',...body.contentDefinition.diameterFact},{id:'parent',label:'Orbits',value:'Saturn'},{id:'shape',...body.contentDefinition.shapeFact},{id:'rotation',...body.contentDefinition.periodFact},{id:'discovery',label:'Discovery',value:/S\/(\d{4})/.exec(designation)[1]}];
 ...
 const recipe={generator:'tools/objects/terrestrial-layers/radial-snapshot.mjs',inputs:['lightcurve-approximation','model-surface'],size:512,longitudeDegrees:55,latitudeDegrees:20,ambient:.45,diffuse:.55,lensId:'model'};
 ...
 const navigation=parseCopy(json(baseline+'source/preparation/navigation.json'),id,name);
 ...
 const manifest=parseCopy(commonManifest,id,name);
 for(const entry of manifest.inputs){
  if(['lightcurve-approximation','model-surface'].includes(entry.id)){
   const bytes=await readFile(resolve(src,entry.path));entry.expectedBytes=bytes.length;entry.expectedSha256=hash(bytes);entry.credit=attribution;entry.coverage=body.contentDefinition.lensCoverage;entry.projection.referenceRadiusMeters=R*1000;
 ...
 const descriptor=parseCopy(json(baseline+'object.json'),id,name);delete descriptor.properties.worldFrame;descriptor.properties.recipe.shape.radiusKm=R;
 // A fail-closed scaffold sentinel: no prepared artifact is copied or asserted.
 descriptor.prepared.sha256='0'.repeat(64);descriptor.properties.preparation.state='awaiting-source-restoration-orbit-fit-and-preparation';
 await write(resolve(pkg,'object.json'),descriptor);
 await repin(id);
