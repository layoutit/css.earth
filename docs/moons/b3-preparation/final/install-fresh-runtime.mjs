...
assert.equal(JSON.parse(await readFile(resolve(root,'package.json'))).name,'cssearth');
const ids='callisto hyperion phoebe proteus titania miranda'.split(' ');
const {runtimeAssets}=await import(pathToFileURL(resolve(root,'tools/runtime-assets.mjs')));
const {installRuntimeAssets}=await import(pathToFileURL(resolve(root,'tools/setup.mjs')));
const parent=resolve(root,'output/b3-fresh-runtime');
await mkdir(parent,{recursive:true});
const dest=await mkdtemp(resolve(parent,'install-'));
...
try {
 receipt.result=await installRuntimeAssets(assets,{concurrency:1,onProgress:p=>{if(p.completed%25===0)console.log(JSON.stringify(p));}});
