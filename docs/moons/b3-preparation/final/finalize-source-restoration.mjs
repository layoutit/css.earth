...
const reports=new Map(await Promise.all([first,uranus].map(async dir=>[dir,JSON.parse(await readFile(resolve(dir,'report.json')))])));
...
for(const id of ids){
 ...
 const manifest=parseSourceManifest(JSON.parse(bytes),id),row={id,sourceRoot,priorReport:resolve(base,'report.json'),manifestSha256:hash(bytes),trackedDocumentRefresh:[]};
 for(const collection of ['inputs','generatedIntermediates','documents'])for(const entry of manifest[collection]){
  let existing;try{existing=await readFile(resolve(sourceRoot,entry.path));}catch(e){if(e.code!=='ENOENT')throw e;}
