...
const plan = {
  titania: ['normal','elevation','geology'],
  miranda: ['normal','elevation','geology'],
  hyperion: ['normal','elevation'],
  proteus: ['normal','elevation','filter-color'],
  callisto: ['normal','enhanced'],
  phoebe: ['normal','elevation','maplet-resolution','image-count'],
};
...
try {
  ...
  for (const id of ids) {
    ...
    assert.ok(plan[id].every(lens => declared.includes(lens)), `${id}: a planned B3 lens is not prepared yet.`);
...
async function captureBody(id, dpr, inputs) {
  ...
  entry.responseCaptureLimits = {maxTotalBufferSize:256*1024**2,maxResourceBufferSize:Math.max(16*1024**2,2*maximumBodyBytes)};
  ...
  try {
    ...
    for (const lensId of plan[id]) {
      ...
      await page.evaluate(id => { window.__b3CaptureOwner = window[`__${id}`]; }, id);
...
async function settled(page, id, lensId, target) {
  await page.waitForFunction(({id,lensId,target}) => {
    ...
    if (runtime !== window.__b3CaptureOwner || app?.activeObjectId !== id) throw new Error('Runtime owner changed during capture');
...
async function snapshot(page, id) {
  return page.evaluate(id => {
    ...
    return {at:performance.now(), url:location.href, activeObjectId:app?.activeObjectId, selectedObjectId:app?.selectedObjectId,
      ready:app?.ready, appError:app?.error == null ? null : String(app.error.message ?? app.error), ownerSame:runtime === window.__b3CaptureOwner,
...
async function drag(page, id) {
  ...
  await page.evaluate(id => {
    ...
    window.__b3DragEnd=()=>{cancelAnimationFrame(frameId);for(const type of ['pointerdown','pointermove','pointerup'])document.removeEventListener(type,pointer,true);
  ...
  try {
    ...
    try {await page.mouse.up();} finally {result=await page.evaluate(()=>{const result=window.__b3DragEnd();delete window.__b3DragEnd;return result;});}
  }
