...
const hash=async p=>{const h=createHash('sha256');for await(const c of createReadStream(p))h.update(c);return h.digest('hex');};
...
for(const v of views){
 ...
 const transform={scale:plan.encoding.scale*(v.id==='rock-abundance'?100:1),offset:plan.encoding.offset*(v.id==='rock-abundance'?100:1)};
 ...
 const i=raster.lenses.findIndex(x=>x.id===v.id);if(i>=0)raster.lenses[i]=entry;else raster.lenses.push(entry);
 ...
 const ci=content.controls.lenses.controls.findIndex(x=>x.id===v.id);if(ci>=0)content.controls.lenses.controls[ci]=ui;else content.controls.lenses.controls.push(ui);
 ...
 const li=content.lenses.controls.findIndex(x=>x.id===v.id);if(li>=0)content.lenses.controls[li]=lens;else content.lenses.controls.push(lens);
 content.lenses.provenance[v.id]='Powell et al. (2023), LRO Diviner team/UCLA; NASA PDS GHRM v1.0; 2009–2022';
}
...
for(const array of [raster.lenses,content.controls.lenses.controls,content.lenses.controls])array.sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id));
content.provenance.thermal='Powell et al. (2023), DOI 10.1029/2022JE007532; PDS GHRM float32 mosaics; author data CC0 DOI 10.25346/S6/LFAVXU';
content.resources=content.resources.filter(x=>x.description!=='Modeled surface rock abundance');
if(!content.resources.some(x=>x.href===sourceUrl))content.resources.push({label:'LRO Diviner',role:'science',description:'Nighttime temperature and rock maps',href:sourceUrl});
...
for(const v of views){
 ...
 for(const name of [stem+'.img',plan.output]){
  ...
  const i=manifest.inputs.findIndex(x=>x.path===p);if(i>=0)manifest.inputs[i]=e;else manifest.inputs.push(e);
 }
 for(const ext of ['img','xml'])if(!acquisition.operations.some(x=>x.path===prefix+stem+'.'+ext))acquisition.operations.push({kind:'download',groups:['diviner-ghrm'],path:prefix+stem+'.'+ext,url:baseUrl+stem+'.'+ext});
}
await write(resolve(root,'preparation/acquisition.json'),acquisition);
for(const name of await readdir(dir)){
 const p='science/diviner-ghrm/'+name;
 if(manifest.inputs.some(x=>x.path===p))continue;
 ...
 const i=manifest.documents.findIndex(x=>x.path===p);if(i>=0)manifest.documents[i]=e;else manifest.documents.push(e);
}
...
for(const p of ['preparation/raster.json','content/static.json','preparation/acquisition.json']){
 const e=manifest.documents.find(x=>x.path===p)??manifest.inputs.find(x=>x.path===p);
...
for(const ref of descriptor.properties.recipe.sources)ref.sha256=await hash(resolve(object,ref.path));
descriptor.properties.recipe.surfaces[0].lenses=content.lenses.controls.map(x=>({id:x.id,source:'content',material:'curvature'}));
