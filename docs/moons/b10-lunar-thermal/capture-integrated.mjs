...
const baseUrl = process.argv[2] ?? 'http://127.0.0.1:4292';
const selectedIds = process.argv.slice(3);
const plan = Object.fromEntries(Object.entries(JSON.parse(await readFile(new URL('./selected-views.json',import.meta.url)))).map(([id,lenses])=>[id,['surface',...lenses]]));
...
const dprs = process.env.B10_DPR ? [Number(process.env.B10_DPR)] : [1,2];
assert.ok(dprs.every(dpr => [1,2].includes(dpr)), 'B10_DPR must be 1 or 2.');
const lensFilter = process.env.B10_LENS;
if (lensFilter) {
...
const parentOut = resolve(root, 'output/playwright/b10-surfaces');
const out = resolve(parentOut, `integrated-${stamp}`);
const viewport = process.env.B10_MOBILE ? {width:390,height:844} : {width:1440,height:1000};
...
const report = {schema:'cssearth-b10-visual-capture@1', status:'RUNNING', startedAt:new Date().toISOString(),
...
async function captureBody(id, dpr, inputs) {
  ...
  try {
    ...
    for (const lensId of plan[id]) {
      ...
      await page.evaluate(id => { window.__b10CaptureOwner = window[`__${id}`]; }, id);
      ...
      for (const shadows of supportsShadows ? [false,true] : [null]) {
        ...
        view.selectorLabels = await page.locator('button[name="lens"]').evaluateAll(buttons => buttons.filter(button=>['midnight-temperature','heat-anomalies','rock-abundance'].includes(button.value)).map(button=>{
          const label=button.querySelector('.planet-lens-label'),detail=button.querySelector('.planet-lens-detail');
          return {id:button.value,label:label.textContent,detail:detail.textContent,labelWidth:label.clientWidth,labelScrollWidth:label.scrollWidth,detailWidth:detail.clientWidth,detailScrollWidth:detail.scrollWidth};
        }));
        for(const row of view.selectorLabels){assert.ok(row.labelScrollWidth<=row.labelWidth+1,'Main selector label is truncated: '+row.id);assert.ok(row.detailScrollWidth<=row.detailWidth+1,'Right selector label is truncated: '+row.id);}
        ...
        if (shadows === true || shadows === null) {
          if(process.env.B10_MOBILE){
            await page.locator('.planet-input-surface').scrollIntoViewIfNeeded();
            await screenshot(page,caseOut,`${lensId}-mobile-globe.png`,view.screenshots);
          }
          ...
          if(!process.env.B10_MOBILE && lensId!=='surface'){
            const box=await page.locator('.polycss-camera').boundingBox();
            await page.mouse.move(box.x+box.width/2,box.y+box.height/2);
            await page.mouse.wheel(0,-500);
            await settled(page,id,lensId,null);
            view.closeZoom=await snapshot(page,id);
            assert.ok(view.closeZoom.camera.zoom>view.afterDrag.camera.zoom,'Real wheel must zoom closer');
            assert.ok(view.closeZoom.ownerSame&&view.closeZoom.retainedStable);
            await screenshot(page,caseOut,`${lensId}-close-zoom.png`,view.screenshots);
          }
        }
        await responseQueue;
...
async function settled(page, id, lensId, target) {
  await page.waitForFunction(({id,lensId,target}) => {
    ...
    if (runtime !== window.__b10CaptureOwner || app?.activeObjectId !== id) throw new Error('Runtime owner changed during capture');
...
async function snapshot(page, id) {
  return page.evaluate(id => {
    ...
    return {at:performance.now(), url:location.href, activeObjectId:app?.activeObjectId, selectedObjectId:app?.selectedObjectId,
      ready:app?.ready, appError:app?.error == null ? null : String(app.error.message ?? app.error), ownerSame:runtime === window.__b10CaptureOwner,
...
async function drag(page, id) {
  ...
  await page.evaluate(id => {
    ...
    window.__b9DragEnd=()=>{cancelAnimationFrame(frameId);for(const type of ['pointerdown','pointermove','pointerup'])document.removeEventListener(type,pointer,true);
  ...
  try {
    ...
    try {await page.mouse.up();} finally {result=await page.evaluate(()=>{const result=window.__b9DragEnd();delete window.__b9DragEnd;return result;});}
  }
...
async function selectInformationTab(page, name) {
  const id=await page.evaluate(()=>window.__cssEarth.activeObjectId);
  const tab=page.locator(`#${id}-${name}-tab`);
  assert.equal(await tab.count(),1,'Exactly one active object card owns the tab');
  if(await tab.getAttribute('aria-selected')!=='true')await tab.click();
  await page.locator(`#${id}-${name}-content`).waitFor({state:'visible'});
  assert.equal(await tab.getAttribute('aria-selected'),'true');
  assert.deepEqual(await tab.evaluate(selected=>
    [...selected.closest('[role="tablist"]').querySelectorAll('[data-information-tab]')].filter(tab=>tab.getAttribute('aria-selected')==='true').map(tab=>tab.dataset.informationTab)),[name]);
}
...
async function exerciseInformationTabs(page,id,directory) {
  ...
  for(const name of ['factsheet','sources','dataset']){
    ...
    const panel=page.locator(`#${id}-${name}-content`);
