...
const root=process.cwd(),ids=['moon'];
const base='eccb0e1b20b6d4da42b504e48ecec751f7d64786';
const all=await runtimeAssets(root,ids),changed=[];
for(const id of ids){
 const old=parseRuntimeManifest(JSON.parse(execFileSync('git',['show',`${base}:src/planets/${id}/runtime-assets.json`],{encoding:'utf8'})),id);
 for(const asset of all.filter(a=>a.id===id)){
  const previous=old.assets.find(a=>a.filename===asset.filename);
  if(previous?.sha256===asset.sha256){assert.equal(asset.bytes,previous.bytes);continue;}
  assert.ok(['midnight-temperature','heat-anomalies','rock-abundance'].some(lens=>asset.filename.startsWith('moon-'+lens)), 'Unselected image changed');
  changed.push(asset);
 }
}
...
await mkdir('output/b10-delivery',{recursive:true});
const directory=await mkdtemp(resolve('output/b10-delivery/run-'));
...
assert.equal(process.env.B10_PUBLICATION_INVENTORY,inventorySha256,'Publication must match the previously reviewed image inventory');
const missing=[];
for(const asset of changed){
