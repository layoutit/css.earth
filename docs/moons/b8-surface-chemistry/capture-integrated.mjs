...
const dprs = process.env.B8_DPR ? [Number(process.env.B8_DPR)] : [1,2];
assert.ok(dprs.every(dpr => [1,2].includes(dpr)), 'B8_DPR must be 1 or 2.');
const lensFilter = process.env.B8_LENS;
if (lensFilter) {
...
const parentOut = resolve(root, 'output/playwright/b8-surfaces');
...
const report = {schema:'cssearth-b8-visual-capture@1', status:'RUNNING', startedAt:new Date().toISOString(),
...
async function captureBody(id, dpr, inputs) {
  ...
  try {
    ...
    for (const lensId of plan[id]) {
      ...
      await page.evaluate(id => { window.__b8CaptureOwner = window[`__${id}`]; }, id);
...
async function settled(page, id, lensId, target) {
  await page.waitForFunction(({id,lensId,target}) => {
    ...
    if (runtime !== window.__b8CaptureOwner || app?.activeObjectId !== id) throw new Error('Runtime owner changed during capture');
...
async function snapshot(page, id) {
  return page.evaluate(id => {
    ...
    return {at:performance.now(), url:location.href, activeObjectId:app?.activeObjectId, selectedObjectId:app?.selectedObjectId,
      ready:app?.ready, appError:app?.error == null ? null : String(app.error.message ?? app.error), ownerSame:runtime === window.__b8CaptureOwner,
...
async function drag(page, id) {
  ...
  await page.evaluate(id => {
    ...
    window.__b8DragEnd=()=>{cancelAnimationFrame(frameId);for(const type of ['pointerdown','pointermove','pointerup'])document.removeEventListener(type,pointer,true);
  ...
  try {
    ...
    try {await page.mouse.up();} finally {result=await page.evaluate(()=>{const result=window.__b8DragEnd();delete window.__b8DragEnd;return result;});}
  }
