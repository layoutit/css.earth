...
selections=read(Path('docs/moons/b8-surface-chemistry/selected-views.json'))
bodies=sys.argv[1:] or list(selections)
if any(body not in selections for body in bodies):raise ValueError('Select a B8 body')
for body in bodies:
 ...
 for p in s.rglob('*'):
  ...
  m['documents'].append({'path':rel,'purpose':'Surface chemistry preparation recipe, source review or conversion receipt.'})
