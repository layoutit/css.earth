...
const selections=await read(new URL('./selected-views.json',import.meta.url));
const id=process.argv[2];if(!selections[id])throw new Error('Specify one B9 body.');
...
const savedRuntime=await read(resolve(objectDirectory,'prepared/runtime.json'));
const frozen=await read(new URL('./source-review/baseline.json',import.meta.url));
for(const name of ['prepared/scene.json','prepared/terrain.json']){const pin=frozen.bodies[id][name];if(pin&&hash(await readFile(resolve(objectDirectory,name)))!==pin.sha256)throw new Error('Fixed geometry differs from approved B9 baseline: '+name);}
...
for(const name of ['scene.json','terrain.json']){const pin=frozen.bodies[id]['prepared/'+name];if(pin&&hash(await readFile(resolve(outputDirectory,name)))!==pin.sha256)throw new Error('Material preparation changed fixed geometry: '+name);}
...
console.log('Asset stage complete; finalize transport in a fresh process: node docs/moons/b9-cassini-ice-surfaces/finalize-one.mjs '+id);
