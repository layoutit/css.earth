...
const root=process.cwd(),ids=['tethys','iapetus','phoebe'];
const base='80e19c51349f713c9a9a64b8ef1cbb78917f0fc7';
...
await mkdir('output/b9-delivery',{recursive:true});
const directory=await mkdtemp(resolve('output/b9-delivery/run-'));
...
assert.equal(process.env.B9_PUBLICATION_INVENTORY,inventorySha256,'Publication must match the previously reviewed image inventory');
const missing=[];
for(const asset of changed){
