...
const server = await dev({root:process.cwd(), server:{host:'127.0.0.1',port:4292}, vite:{cacheDir:'.astro/b9-vite-cache',server:{strictPort:true,hmr:false,watch:{ignored:['**/output/**','**/docs/**']}}}});
let child;
try {
  child=spawn(process.execPath,['docs/moons/b9-cassini-ice-surfaces/capture-integrated.mjs','http://127.0.0.1:4292',ids[0]],
