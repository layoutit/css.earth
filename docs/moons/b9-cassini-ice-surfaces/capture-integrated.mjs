...
const plan = Object.fromEntries(Object.entries(JSON.parse(await readFile(new URL('./selected-views.json',import.meta.url)))).map(([id,lenses])=>[id,['normal',...lenses]]));
...
const dprs = process.env.B9_DPR ? [Number(process.env.B9_DPR)] : [1,2];
assert.ok(dprs.every(dpr => [1,2].includes(dpr)), 'B9_DPR must be 1 or 2.');
const lensFilter = process.env.B9_LENS;
if (lensFilter) {
...
const parentOut = resolve(root, 'output/playwright/b9-surfaces');
...
const report = {schema:'cssearth-b9-visual-capture@1', status:'RUNNING', startedAt:new Date().toISOString(),
...
async function captureBody(id, dpr, inputs) {
  ...
  try {
    ...
    for (const lensId of plan[id]) {
      ...
      await page.evaluate(id => { window.__b9CaptureOwner = window[`__${id}`]; }, id);
...
async function settled(page, id, lensId, target) {
  await page.waitForFunction(({id,lensId,target}) => {
    ...
    if (runtime !== window.__b9CaptureOwner || app?.activeObjectId !== id) throw new Error('Runtime owner changed during capture');
...
async function snapshot(page, id) {
  return page.evaluate(id => {
    ...
    return {at:performance.now(), url:location.href, activeObjectId:app?.activeObjectId, selectedObjectId:app?.selectedObjectId,
      ready:app?.ready, appError:app?.error == null ? null : String(app.error.message ?? app.error), ownerSame:runtime === window.__b9CaptureOwner,
