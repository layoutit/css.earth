...
// B9 incremental replay: use existing preparers for selected lenses, retaining
...
const id=Object.keys(selections).find(id=>id===process.argv[2]);if(!id)throw new Error('Specify one B9 body.');
...
const savedRuntime=requireObjectRuntimeDefinition(await read(resolve(objectDirectory,'prepared/runtime.json')));
const frozen=await read(new URL('./source-review/baseline.json',import.meta.url));
for(const name of ['prepared/scene.json','prepared/terrain.json']){const pin=requireRecord(requireRecord(requireRecord(frozen).bodies)[id])[name];if(pin&&hash(await readFile(resolve(objectDirectory,name)))!==requireRecord(pin).sha256)throw new Error('Fixed geometry differs from approved B9 baseline: '+name);}
...
const stage=await mkdtemp(resolve('output/b9-preparation',`${id}-`)),publicDirectory=resolve(stage,'public'),outputDirectory=resolve(stage,'prepared');
...
// This replay owns surfaces. The shared catalogue may have grown since the
// body's navigation was prepared; retain that accepted navigation verbatim.
definition.heliocentricView=savedRuntime.heliocentricView;
...
for(const name of ['scene.json','terrain.json']){const pin=requireRecord(requireRecord(requireRecord(frozen).bodies)[id])['prepared/'+name];if(pin&&hash(await readFile(resolve(outputDirectory,name)))!==requireRecord(pin).sha256)throw new Error('Material preparation changed fixed geometry: '+name);}
...
console.log('Asset stage complete; finalize transport in a fresh process: node docs/moons/b9-cassini-ice-surfaces/finalize-one.mts '+id);
