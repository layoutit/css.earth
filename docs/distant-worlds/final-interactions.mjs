...
try {
 ...
 const ready=id=>p.waitForFunction(id=>document.documentElement.dataset.ready==='true'&&document.querySelector('.planet-stage')?.dataset.objectId===id,id);
 ...
 await p.locator('input[name="shadows"]').evaluate(n=>n.click());await p.waitForTimeout(300);
 ...
 await p.locator('input[name="orbit"]').evaluate(n=>n.click());await p.waitForTimeout(100);
 ...
 for(const id of ['oumuamua','gkunhomdima']){
  ...
  const layout=await p.evaluate(()=>{const v=document.querySelector('.planet-viewport'),s=document.querySelector('.planet-sidebar'),r=v.getBoundingClientRect();return{position:getComputedStyle(v).position,viewport:r.toJSON(),cardTop:s.getBoundingClientRect().top,overflow:document.documentElement.scrollWidth>innerWidth,diagnostics:window.__cssEarth!==undefined};});
