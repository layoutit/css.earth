...
def generator(s):
    match=re.search(r'const bodies = (\[.*?\]);',s); rows=json.loads(match[1]); ids={x[0] for x in rows}
...
update('packages/astronomy/tools/generate-asteroids.mjs',generator)
def registry(s):
    for b in bodies:
    ...
    return s
update('site/objects.mjs',registry)
