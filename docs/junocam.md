...
The kernels live in the shared bank [`src/spice/juno`](../src/spice/juno/manifest.json): text kernels are committed, and `node tools/spice/kernel-bank.mts acquire juno` restores the trajectory and attitude kernels by their pins. The reconstructed trajectory `spk_rec_220909_221019_221027.bsp` carries its own Jupiter system, including the Europa ephemeris the navigation team updated from the flyby, so no separate satellite ephemeris is loaded.
