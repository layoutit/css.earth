...
import type {CheckedObjectRuntimeDefinition} from '../../tools/object-runtime-contract.mts';
...
for(const {id} of OBJECTS){
 const root=`src/planets/${id}`,descriptor=parseObjectDescriptor(await readFile(`${root}/object.json`,'utf8'));
 ...
 const withoutMarkers=({heliocentricView,...rest}:CheckedObjectRuntimeDefinition)=>rest;
