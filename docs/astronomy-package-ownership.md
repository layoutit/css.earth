...
WWT's catalog does not certify tile availability, a prepared asset's reuse terms, or a science observation's identity, units and masks. Its [service client](https://wwt-api-client.readthedocs.io/en/latest/) may be useful for hosted collection operations, but the examined API does not establish Telescope's delivery and qualification semantics. A future actual-image consumer must check each image and its terms independently.
