...
| A ground-based photograph lens from a survey release | `tools/objects/scaffold-frame-lens.mts` | Elektra `zimpol` (registers by relief); Daphne and Eugenia measured, unwired, ledgered | Inputs pinned, acquisition, manifest, derivation record, lens with relief turn and silhouette tilt, control, text, README markers, ledger entry, cameras derived, source records bound, lens measured; a lens outside three degrees on every measurement is unwired | The scaffolded body's `observer-cameras.test.mts` case, `registration.test.mts` (tilt on a wrong pole, the kept rule), `cameras.test.mts` (tilted camera) |
