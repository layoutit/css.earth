...
| Ground-based telescope frames with no archived geometry | `controlled-shape-camera` with `fits-zimpol-intensity` | Kleopatra `zimpol`: deconvolved VLT/SPHERE/ZIMPOL frames | Frame pins with cameras computed by `observer-camera.mts` from the release's spin record, Horizons geometry and each frame's header; the mesh the spin record describes, as a `radialTerrainAlternatives` entry | `observer-camera.test.mts`, checked against the IAU elements DAMIT publishes for the same model |
