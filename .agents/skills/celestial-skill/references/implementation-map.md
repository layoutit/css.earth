...
| A star with no spectrum or catalogue temperature of its own | `stellar-photometric-color` with `spectrum: planck` and `temperature.published` (kelvin, bounds, citation with URL) instead of a catalogue row (`tools/objects/observation/stellar/stellar-photometric-color.mts`) | The six stars of VHS 1256-1257, GQ Lup, DH Tau and ROXs 42B | `stellar-photometric-color.test.mts` | No limb darkening unless a law is cited |
...
| Prepare one authored object end to end, resumable by step | `node tools/prepare/prepare-object.mts <id> [--from <step>] [--reuse-images]` (the last keeps a paged-ellipsoid or raster body's published images and rebuilds only scene, presentation and content): stale builds, catalogue, title, geometry, write mode, discovery, source records, page, text, markers, world context, provenance for this object only |
...
| Subtract a star from JWST NIRCam coronagraphy the way a paper did | `node tools/objects/jwst/toolchain.mts install klip` (spaceKLIP on the paper's jwst version and CRDS context, `klip/toolchain.json`), then `node tools/objects/jwst/klip/reduce.mts <program> <band> <work> --raw <dir>` over a pinned `klip/programs/<id>.json` naming the raw exposures, science and reference roles and every published setting |
...
| Measure a ground-based lens against its paper's comparison figure | `node tools/objects/published-comparison.mts <id> [--write]`: reads the figure from the pinned PDF (`tools/sources/pdf-image.mts`), writes `evidence/published-comparison.json` and its image; a new record's zero pixel digest is adopted on the first `--write` |
