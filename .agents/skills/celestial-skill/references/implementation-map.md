...
| Select channels, nights, error floors or a wavelength scale before a reconstruction | `node tools/objects/interferometry/oifits-select.mts <in> <out> [--window min:max] [--mjd min:max] [--half even\|odd] [--error-floor <fraction>:<degrees>] [--wavelength-scale f]` |
