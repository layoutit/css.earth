...
| An unresolved body with published whole-disc colours and albedo | `disc-integrated-color` science kind on the raster route: cited colour record plus CIE 1931 and D65 tables, painted uniformly; discovery does not count it as imagery | Makemake `color`, Eris `color` | `tools/objects/observation/disc-integrated-color.test.mts`, `site/test/object-discovery.test.mts` |
