...
- the prepared outputs the recipe produces: `object.json`'s pin and the refreshed `prepared-assets.json`
  inventory (written automatically by the preparation tools). Nothing under `prepared/` is committed: publish
  the baked bytes with `node tools/publish-runtime-assets.mts --object=<id>` before opening the PR.
