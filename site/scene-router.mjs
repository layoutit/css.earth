...
export function createSceneRouter({
  ...
  async function mountApplication({ factory, content, handoff, request } = {}) {
    ...
    try {
      ...
      if (mount.sharedView && windowTarget.location?.href) {
        ...
        if (!interrupted && !mount.destinations) await session.lifetime.wait(session.viewUrl.restore());
        if (active !== session || session.lifetime.disposed) return;
      }
      const saveView = () => {
        // Navigation checkpoints the departure before moving the world camera.
        // Late entity/loading notifications must not replace it mid-flight.
        if (active === session && !session.lifetime.disposed && !pending) {
          session.viewUrl?.flush();
          historyOwner?.remember();
        }
      };
      if (mount.destinations) await session.lifetime.wait(shell.setDestinations?.({
        ...mount.destinations,
        saveView,
        listenHistory: !historyOwner,
        ...(historyOwner ? { writeRoute(url, { replace }) {
          if (active !== session || session.lifetime.disposed) return;
          // Entity/lens entries use the same identities and departed snapshots
          // as object navigation. Keep the mounted session's route current.
          historyOwner.commit(url, { history: replace ? 'replace' : 'push' });
          session.url = new URL(url, windowTarget.location.href).href;
        } } : {}),
  ...
  function navigate(id, options = {}) {
    ...
    if (!options.url) {
      ...
      if (object.id !== objectId) {
        const entity = new URLSearchParams(url.hash.slice(1));
        if (entity.has('place') || entity.has('lens')) {
          entity.delete('place'); entity.delete('lens'); url.hash = entity.toString();
        }
      }
    }
  ...
  async function bindSessionView(session, { restore = true } = {}) {
    ...
    if (restore) await session.lifetime.wait(session.mount.destinations && session.shell.restoreDestinations
      ? session.shell.restoreDestinations() : owner.restore());
  }
