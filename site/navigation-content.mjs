      ...
      try {
        ...
        return Object.freeze({
          ...
          apply() {
            ...
            const destinationResults = documentTarget.querySelector('.planet-destination-results');
            const incomingDestinations = source.querySelector('.planet-destination-results');
            if (incomingDestinations) {
              const content = documentTarget.importNode(incomingDestinations, true);
              if (destinationResults) destinationResults.replaceWith(content);
              else documentTarget.querySelector('.planet-object-browser').append(content);
            } else destinationResults?.remove();
