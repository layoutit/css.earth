...
export function orderFacts<T extends { id: string; label?: string }>(facts: readonly T[] = [], moreFacts: readonly T[] = []) {
  ...
  const rank = (id: string) => id === "distance-from-sun" && ids.has("distance-from-parent")
    ? (FACT_RANK.get("rotation-period") ?? FACT_IDS.length) + 0.5
