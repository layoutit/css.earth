...
export function createDestinationBrowser({ documentTarget, onSelected, onReset }: {
  ...
  let selected = false, open = false;
  back.addEventListener('click', onReset, { signal: events.signal });
  return Object.freeze({
    present(value: DestinationPresentation | null) {
      if (events.signal.aborted) return;
      const opening = value !== null && !selected;
      selected = value !== null;
    ...
    setOpen(value: boolean) { open = value; panel.hidden = open || !selected; },
    destroy() { events.abort(); selected = false; panel.hidden = true; },
  });
}
