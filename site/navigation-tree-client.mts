...
export function createNavigationTreeController(root: HTMLElement, windowTarget: BrowserWindow) {
  ...
  const markerAnchor = (node: NavigationTreeRecord, branch: boolean) => {
    ...
    anchor.className = [branch ? 'atlas-tree-body' : 'atlas-row', node.marker?.className].filter(Boolean).join(' ');
    if (node.marker?.style) anchor.setAttribute('style', node.marker.style);
    anchor.href = `/${node.objectId}/`;
    anchor.dataset.atlasObject = node.objectId!;
    if (node.objectId === selected) anchor.setAttribute('aria-current', 'page');
    ...
    async select(objectId: string) {
      ...
      if (!target && pin) {
        try {
          ...
          const path: string[] = [];
          for (let key: string | undefined = objectId; key; key = parent.get(key)) path.unshift(key);
