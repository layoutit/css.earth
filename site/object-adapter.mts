...
export const objectAdapter = Object.freeze({
  async load(objectId: string, descriptor?: ObjectDescriptor, objects = SCENE_OBJECTS) {
    ...
    if (descriptor) {
      ...
      mount = await loadPackagedObject(preparedDescriptor);
    } else mount = await objectRecord.loadScene();
