...
export interface ObjectDiscovery { featured: boolean; imagery: boolean; illustration: boolean; }

export function parseObjectDiscovery(value: unknown): Readonly<ObjectDiscovery> {
  if (!record(value) || Object.keys(value).some(key => !['featured', 'imagery', 'illustration'].includes(key)) ||
...
export function discoveryVisibility(objects: readonly { id: string; classification: string; discovery: ObjectDiscovery }[],
  ...
  const hiddenBodies: string[] = [], hiddenLabels: string[] = [];
  for (const object of objects) {
    ...
    const highlighted = object.classification === options.highlighted;
  ...
  return { hiddenBodies, hiddenLabels };
}
