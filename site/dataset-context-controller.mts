...
export function createDatasetContextController(drawer: HTMLElement, _document: Document, windowTarget: BrowserWindow, lifetime: SceneLifetime) {
  // Focus banks arrive after mount (`focus-fragment.mts`), so each render reads the current hosts.
  ...
  const hosted = hosts().length > 0 || drawer.querySelector('[data-focus-banks-slot]') !== null;
  const observer = hosted ? new windowTarget.MutationObserver(render) : null;
