...
interface SourceLink { id: string; href: string; label: string; }
interface SourceGroup { id: string; sourceIds: string[]; links: SourceLink[]; description: string; usedFor: string[]; role: string; }
import { productSourceIds, validateObjectProvenance } from '../src/platform/object-provenance.mts';

const external = (url: unknown): url is string => {
...
export function objectSources(provenance: ProvenanceDocument | null | undefined) {
  ...
  for (const source of document.sources.filter(source => source.kind === 'source-input' && uses.has(source.id))) {
    ...
    const link = { id: source.id, href,
      label: source.title ?? source.label ?? source.path.split('/').at(-1) ?? source.id };
