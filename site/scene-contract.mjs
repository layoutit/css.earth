...
export const OBJECT_BEHAVIOR = Object.freeze({
  ...
  shell: Object.freeze({
    required: true,
    // Settings here means object-specific extras. The shared Settings panel,
    // Motion and high contrast are unconditional shell controls.
...
export function requireObjectControls(content, objectId = "unknown") {
  ...
  if (names.some((name) => typeof name !== "string" || !name ||
      ["motion", "skyContrast"].includes(name)) || new Set(names).size !== names.length ||
