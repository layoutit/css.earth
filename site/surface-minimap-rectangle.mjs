...
import { wrapMapU } from './surface-minimap-math.mjs';
...
export function minimapCamera({ eye, rotation, view, axes }) {
  const toMap = v => new Cartesian3(dot(v, axes.prime), dot(v, axes.east), dot(v, axes.north));
  const positionWC = toMap(eye);
  const direction = v => toMap(rotateWorldPosition(rotation, v));
  const planes = [[1, 0, view.left], [-1, 0, -view.right], [0, 1, view.top], [0, -1, -view.bottom], [0, 0, -1]]
    .map(v => {
  ...
  return {
    ...
    pickEllipsoid({ x, y }, ellipsoid, result = new Cartesian3()) {
...
export function rectangleOnMap(rectangle) {
...
export function surfaceViewRectangle(state) {
