import {validatePreparedCssVolume,parsePreparedWorldContext} from "../../src/renderers/css/dist/index.js";
import {requireRecord} from "../../tools/source-values.mts";
import type {WorldCameraPose} from "../../src/renderers/css/navigation/world-camera.js";
declare global {interface Window {__cullingNodes:Element[];}}
...
try {
  ...
  await page.evaluate(async()=>{
    ...
    await Promise.all([...urls].map(url=>new Promise<void>((resolve,reject)=>{const image=new Image();image.onload=()=>resolve();image.onerror=()=>reject(new Error(`Missing volume image ${url}`));image.src=url;})));
  });
  ...
  for(const pose of poses){
    const physical=cameraPoseToReferenceFrame({positionM:[pose.eye[0]*volume.frame.metersPerUnit,pose.eye[1]*volume.frame.metersPerUnit,pose.eye[2]*volume.frame.metersPerUnit],orientationXyzw:pose.orientation},volume.frame);
    const world={referenceFrame:volume.frame.referenceFrame,epochJdTt:volume.frame.epochJdTt,pose:physical};
    await page.evaluate(({world,frame})=>window.__cssearthTest.object('sun').camera.applyWorldCamera(world,frame),{world,frame:context.frame});
...
async function checkCase(name:string,world:WorldCameraPose|null,minimumRatio:number){
  ...
  await page.evaluate(()=>{for(const root of document.querySelectorAll('.css-volume-projection'))window.__cssearthTest.htmlElement(root).style.transformStyle='preserve-3d';});
  let mutant;
  try{mutant=await measure(`${name}-mutant`);}finally{await page.evaluate(()=>{for(const root of document.querySelectorAll('.css-volume-projection'))window.__cssearthTest.htmlElement(root).style.removeProperty('transform-style');});}
