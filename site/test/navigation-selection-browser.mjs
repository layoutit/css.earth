  ...
  for (const dpr of [1, 2]) {
    ...
    await page.evaluate(() => {
      ...
      document.addEventListener('click', () => {
        window.__clickCard = {
          selected: window.__cssEarth.selectedObjectId, active: window.__cssEarth.activeObjectId,
    ...
    assert.equal(immediate.title, 'Ceres'); assert.ok(immediate.introduction?.length > 20);
    ...
    assert.equal(await page.locator('.planet-information-panel').evaluate(node => node.inert), false);
