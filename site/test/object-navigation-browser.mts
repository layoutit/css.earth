...
try {
  for (const width of [390, 1440]) {
    ...
    for (const object of browserObjects()) {
      ...
      for (const entry of SCENE_OBJECTS) {
        ...
        assert.equal(label.replace(/\s+/gu, " ").trim(), `${Number(entry.distance.value.toFixed(3))} AU`);
      }
      assert.deepEqual(new Set(await page.locator(".planet-object-link").evaluateAll((links) => links.map((link) => link.dataset.objectId).filter((id) => id !== undefined))), new Set(SCENE_OBJECTS.map(({ id }) => id)));
