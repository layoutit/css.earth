...
const descriptor = JSON.parse(await readFile('src/planets/venus/object.json', 'utf8'));
...
try {
  for (const dpr of [1, 2]) {
    ...
    const result = await page.evaluate(async descriptor => {
      ...
      const failure = async (descriptor, bytes) => {
      ...
      const invalid = JSON.parse(new TextDecoder().decode(source));
      invalid.data.camera.defaultZoom = 0;
      ...
      const malformed = await failure({ ...descriptor, prepared: { ...descriptor.prepared, sha256 } }, invalidBytes);
      ...
      await new Promise(resolve => setTimeout(() => { controller.abort(); resolve(); }, 0));
