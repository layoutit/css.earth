...
function publisher(target: Window & typeof globalThis, id: string) {
  ...
  const options: ObjectDiagnosticsOptions = {
    ...
    definition: boundary<ObjectDiagnosticsOptions['definition']>({ id,
      ...
      sun: undefined, heliocentricView: undefined }),
    ...
    cubicSky: boundary<ObjectDiagnosticsOptions['cubicSky']>({ setStarExposure: unused, starGroup: null }),
    heliocentric: null,
    ...
    context: { own: disposer => disposers.push(disposer) }, initialSelection: { lensId: 'test' },
