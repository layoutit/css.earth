...
const errors = [];
try {
  const page = await browser.newPage({ viewport: { width: 1995, height: 1236 }, deviceScaleFactor: 1 });
  ...
  const id = await page.evaluate(() => window.__cssEarthRecorder.id);
  ...
  await page.locator('[data-context-label][data-object-navigate="makemake"]').evaluate(node => node.click());
  ...
  assert.ok(recording.metadata.userAgent.includes('Chrome/155.'));
  ...
  assert.ok(recording.samples.some(sample => sample.state.geometry.retainedLeaves > 1000));
  const phases = recording.events.filter(event => event.type === 'mark' && event.name.startsWith('cssEarth:navigation:'));
  assert.ok(phases.some(event => event.name.endsWith(':finished')));
  assert.ok(phases.every(event => event.detail.recordingId === id));
  const traceEvents = JSON.parse(trace).traceEvents;
  assert.ok(traceEvents.some(event => event.name === 'cssEarth:navigation:requested' && JSON.stringify(event.args).includes(id)));
  assert.equal(await page.evaluate(() => window.__cssEarthRecorder.id), null);
  await page.getByRole('button', { name: 'Record diagnostic data' }).click();
  assert.notEqual(await page.evaluate(() => window.__cssEarthRecorder.id), id);
  await page.evaluate(() => window.__cssEarthRecorder.stop('dispose'));
