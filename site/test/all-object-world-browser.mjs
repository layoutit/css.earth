...
const report = { schema: 'cssearth-all-object-world-browser@1', startedAt: new Date().toISOString(),
...
try {
  ...
  await page.evaluate(expected => {
    ...
    if (roots.some(root => !root)) throw new Error('Shared application roots must exist.');
    const universe = [roots[3], ...roots[3].querySelectorAll('*')];
    const probe = window.__allObjectWorld = { expected, document, timeOrigin: performance.timeOrigin,
      selectors, roots, universe, events: [], samples: [], running: true };
    probe.residency = (diagnostic, id, resources = diagnostic.runtime.resources()) => {
    ...
    probe.capture = () => {
      ...
      return window[`__${id}`]?.camera.captureWorldCamera(probe.expected[id].frame);
    };
    ...
    const sample = () => {
      ...
      probe.samples.push({ time: performance.now(), id, roots: document.querySelectorAll('.polycss-camera').length,
        world: diagnostic?.camera.captureWorldCamera(probe.expected[id].frame),
        pending: diagnostic?.renderStats.textureStats.pendingInteractiveImageCount,
        residency: diagnostic ? probe.residency(diagnostic, id) : null,
        materialReady: diagnostic?.runtime.selection().ready,
        retained: probe.universe.every(node => node.isConnected) });
      requestAnimationFrame(sample);
    };
    sample();
    const id = window.__cssEarth.activeObjectId, diagnostic = window[`__${id}`], resources = diagnostic.runtime.resources();
  ...
  assert.ok(report.flights.some(flight => flight.orbitProjection?.pieces > 0), 'Actual midflight orbit segments were measured.');
  ...
  if (page) {
    ...
    report.failure.snapshot = await snapshot().catch(error => ({ error: error.message }));
...
async function ready(page, id) {
  await page.waitForFunction(id => {
    ...
    const diagnostic = window[`__${id}`];
    const selection = diagnostic?.runtime.selection();
    return window.__cssEarth?.ready && window.__cssEarth.activeObjectId === id && diagnostic?.ready && selection.ready && !selection.pending &&
...
async function distance() { return page.evaluate(() => window[`__${window.__cssEarth.activeObjectId}`].camera.state().distanceKilometers); }
async function snapshot() {
  return page.evaluate(() => {
    const probe = window.__allObjectWorld, id = window.__cssEarth.activeObjectId, diagnostic = window[`__${id}`];
    return { id, path: location.pathname, world: probe.capture(),
      ...
      universeNodes: probe.universe.length, liveUniverseNodes: document.querySelector('.prepared-universe').querySelectorAll('*').length + 1,
      markerIds: [...document.querySelectorAll('[data-context-body]')].map(node => node.dataset.contextBody),
      ...
      lensButtons: [...document.querySelectorAll('button[name="lens"]')].map(button => ({ id: button.value, disabled: button.disabled,
        visible: button.getClientRects().length > 0 && getComputedStyle(button).visibility === 'visible',
        pressed: button.getAttribute('aria-pressed') })) };
  });
}
function assertRetained(state, label) {
...
async function proveArrival(id) {
...
async function markerAlignment(id) {
  const measured = await page.evaluate(id => {
    ...
    const diagnostic = window[`__${id}`], camera = diagnostic.camera.state(), world = probe.capture();
    ...
    const marker = document.querySelector(`[data-context-body="${id}"]`).getBoundingClientRect();
    const depth = -eye[2];
...
async function flight(to) {
  ...
  const orbitMeasurement = (async () => {
    while (!finished) {
      ...
      if (measured?.pieces > 0) return measured;
  ...
  report.flights.push({ from, to, durationMs: trace.durationMs, frames: trace.samples.length,
    ...
    speculativeImageFrames: incoming.filter(sample => sample.pending > 0).length,
  ...
  const worlds = trace.samples.filter(sample => sample.world).map(sample => sample.world);
...
async function orbitProjection() {
  return page.evaluate(() => {
    ...
    const contextElement = document.querySelector('.prepared-world-context');
    if (!contextElement || contextElement.hidden || !(Number(getComputedStyle(contextElement).opacity) > 0)) return null;
    const camera = diagnostic.camera.state(), world = probe.capture(), root = cameraElement.getBoundingClientRect();
    ...
    for (const { point } of Object.values(probe.expected)) {
      ...
      for (const node of document.querySelectorAll(`[data-context-orbit="${point.id}"]`)) {
        ...
        for (let index = 0; index < vertices.length; index++) {
          ...
          const error = point => Math.abs(dy * (point[0] - a[0]) - dx * (point[1] - a[1])) / length;
...
async function savedWorldRoundTrip(id, sampledWorlds) {
  const original = await capture(), frame = expected[id].frame;
  let reader, measured;
  try {
    for (const world of sampledWorlds) {
      await page.evaluate(({ id, frame, world }) => window[`__${id}`].camera.applyWorldCamera(world, frame), { id, frame, world });
      measured = await page.evaluate(({ id, frame }) => {
        const camera = window[`__${id}`].camera.state(), center = camera.bodyCenterKilometers;
        return { world: window[`__${id}`].camera.captureWorldCamera(frame), center,
    ...
    reader = await browser.newPage({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1 });
    ...
    const restored = await reader.evaluate(({ id, frame }) => ({
      world: window[`__${id}`].camera.captureWorldCamera(frame), center: window[`__${id}`].camera.state().bodyCenterKilometers,
      roots: document.querySelectorAll('.polycss-camera').length, ready: window[`__${id}`].runtime.selection().ready,
    }), { id, frame });
    sameOrientation(restored.world, measured.world, `${id} saved URL`);
    const positionErrorM = Math.hypot(...restored.world.pose.positionM.map((value, axis) => value - measured.world.pose.positionM[axis]));
    const centreErrorKm = restored.center ? Math.hypot(...restored.center.map((value, axis) => value - measured.center[axis])) : Infinity;
    ...
    await page.evaluate(({ id, frame, world }) => window[`__${id}`].camera.applyWorldCamera(world, frame), { id, frame, world: original });
    assertRetained(await snapshot(), `${id} URL proof restoration`);
  }
}
async function visibleMarker(id) {
  for (const distanceKilometers of [1e8, 3e8, 8e8]) for (let yaw = 0; yaw < 360; yaw += 30) {
    const found = await page.evaluate(({ id, distanceKilometers, yaw }) => {
      ...
      const targets = document.querySelectorAll(`[data-object-navigate="${id}"],.planet-heliocentric-body-target[data-body="${id}"]`);
...
async function galaxyRoundTrip(id, initialBackground) {
...
function sameOrientation(actual, expected, id) {
