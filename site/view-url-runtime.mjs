...
export function bindViewUrl({ windowTarget, view, getMotion, setMotion, onError = () => {}, listenHistory = true, listenToPopState = true }) {
  ...
  async function restore(beforeRestore = async () => {}) {
    clear();
    const current = ++revision;
    restoring = true;
    try {
      const query = new URLSearchParams(windowTarget.location.search);
      let saved = null, invalid = null;
      try {
        if (query.getAll("v").length > 1) throw new TypeError("This URL contains more than one saved view.");
        saved = query.has("v") ? parseSharedView(`v=${query.get("v")}`) : null;
      } catch (error) { invalid = error; }
      // Entity/lens restoration may fetch prepared data. Hold camera URL
      // publication throughout it, then restore the original saved pose.
      await beforeRestore({ hasSavedView: Boolean(saved) });
  ...
  if (listenHistory && listenToPopState) windowTarget.addEventListener("popstate", onPopState);
  return Object.freeze({
    restore, schedule, flush,
    destroy() {
      if (destroyed) return;
      destroyed = true; revision++; clear(); unsubscribe();
      if (listenHistory && listenToPopState) windowTarget.removeEventListener("popstate", onPopState);
    },
  });
}
