...
export function selectGalaxyNeighbor(card: HTMLElement, selectedId: string): void {
  ...
  if (!parent) return;
  const selectedName = rows.find(row => row.dataset.neighborId === selectedId)?.querySelector('.planet-object-name')?.textContent ?? '';
  for (const { row, value } of distances) {
    const active = row.dataset.neighborId === selectedId;
    const link = row.querySelector('a');
    if (active) link?.setAttribute('aria-current', 'true'); else link?.removeAttribute('aria-current');
    const label = row.querySelector<HTMLElement>('.planet-object-distance');
    if (label) {
      label.hidden = active;
      label.title = 'Catalogue center distance from ' + selectedName;
      const ly = value * 3.261563777, divisor = ly >= 1e6 ? 1e6 : ly >= 1e3 ? 1e3 : 1;
      const number = label.querySelector('.planet-object-distance-value'), unit = label.querySelector('.planet-object-distance-unit');
