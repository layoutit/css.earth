...
- Before committing, inspect the staged diff, including added files and their sizes. Each added artifact must support a named claim, explanation or test. Follow the [PR rules](docs/provenance/CONTRACT.md#pull-requests) for the title and content, and use the [PR template](.github/pull_request_template.md) to report the change and its checks. Inspect every image on the published GitHub PR before handing it off.
