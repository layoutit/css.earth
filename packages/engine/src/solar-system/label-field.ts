...
export interface LabelMetrics {capPixels:number;gapPixels:number;spacingPixels:number;boxHeightCaps:number;capHeightEm:number;}
export interface LabelPolicy extends LabelMetrics {model:string;poolSize:number;candidateCapacity:number;maxAlpha:number;maxAlphaStep:number;}
export interface DeclutterInput {owner:number;id:string;priority:number;anchor:Vector2;widthPx:number;bottomOffsetPx:number;topOffsetPx:number;}
export interface DeclutterCandidate {owner:number;id:string;priority:number;left:number;right:number;top:number;bottom:number;accepted:boolean;}
export interface LabelEntry {key:string;text:string;anchor:Vector2;bottomOffsetPx:number;alpha:number;priority:number;}
export interface LabelSlot {occupant:string|null;text:string;anchor:Vector2;bottomOffsetPx:number;alpha:number;target:number;changed:boolean;}
export type LabelDeclutter = ReturnType<typeof createLabelDeclutter>;
export type LabelSlots = ReturnType<typeof createLabelSlots>;
...
export function validateLabelPolicy(policy: LabelPolicy) {
...
export function createLabelDeclutter({ capacity, spacingPixels }: {capacity:number;spacingPixels:number}) {
  ...
  return Object.freeze({
    ...
    add({ owner, id, priority, anchor, widthPx, bottomOffsetPx, topOffsetPx }: DeclutterInput) {
    ...
    accepted(owner:number, id:string) {
...
export function createLabelSlots({ poolSize, maxAlpha: initialMaxAlpha, maxAlphaStep }: {poolSize:number;maxAlpha:number;maxAlphaStep:number}) {
  let maxAlpha = initialMaxAlpha;
  const slots: LabelSlot[] = Array.from({ length: poolSize }, () => ({
...
export function labelFontPixels(policy: LabelMetrics) {
  return policy.capPixels / policy.capHeightEm;
}

export function labelBox(policy: LabelMetrics, { widthPerCapHeight, markerRadiusPx }: {widthPerCapHeight:number;markerRadiusPx:number}) {
