...
export interface SphereDragInput extends PointerDelta { centerX: number; centerY: number; radius: number; focalLength: number; opticalCenterX?: number; opticalCenterY?: number; }
...
export function composeDragRotation(next: Quaternion, previous: Quaternion): Quaternion {
  const [x, y, z, w] = next;
...
export function rotationFromAngularVelocity(velocity: Vector3, elapsedMilliseconds: number): Quaternion {
