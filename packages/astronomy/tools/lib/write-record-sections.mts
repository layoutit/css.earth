...
export function readRecordSections(destination: URL, symbol: string) {
...
export function writeRecordSections(destination: URL, source: string, kind: 'satellites' | 'horizons') {
  ...
  for (const [group, records] of sections) {
    let chunk: string[] = [], lines = 0, part = 1;
    for (const record of records) {
