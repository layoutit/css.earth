...
export interface ElpTerm { iv: number; pow: number; amp: number; c: number[] }
...
const dms = (d: number, m: number, s: number) => (d + m / 60 + s / 3600) * DEG
...
const num = (line: string, a: number, b: number) => Number(line.slice(a, b).trim())
const int = (line: string, a: number, b: number) => {
...
export function elpPositionKm(terms: readonly ElpTerm[], jdTt: number) {
