import { cached } from './sources.mts'

export interface VsopTerm { a: number; b: number; c: number; cText: string }
 ...
 */
export function parseVsop87(text: string) {
  const series: VsopSeries = [[], [], []]
  let current: { terms: VsopTerm[]; declared: number } | null = null
...
export async function loadVsop87a(bodyKey: string) {
...
export function evalVsop87(series: VsopSeries, T: number) {
  const out = [0, 0, 0]
...
export function evalVsop87Rate(series: VsopSeries, T: number) {
  const out = [0, 0, 0]
  for (let ic = 0; ic < 3; ic++) {
    let sum = 0
