...
export interface LinearTrend { intercept: number; slope: number }
export function fitHarmonics(days: readonly number[], values: readonly number[], line: LinearTrend, count = 3, { trend = true, separation = 0 } = {}) {
  if (days.length < 2 || values.length !== days.length || ![...days, ...values, line.intercept, line.slope].every(Number.isFinite) || !(days[days.length - 1] > days[0]) || !Number.isInteger(count) || count < 0) throw new TypeError("Harmonic fit requires finite paired samples and a positive time span")
  const middle = (days[0] + days[days.length - 1]) / 2, span = days[days.length - 1] - days[0];
  const times = days.map(d => d - middle);
  const residual = values.map((v, i) => v - line.intercept - line.slope * days[i]);
  const frequencies: number[] = [];
  function solve(rates: readonly number[]) {
  ...
  function refine(index: number, guess: number) {
    const phi = (Math.sqrt(5) - 1) / 2;
    let lo = Math.max(low, guess - step), hi = Math.min(high, guess + step);
    const error = (w: number) => solve(frequencies.map((v, i) => i === index ? w : v)).error;
  ...
  if (!c) throw new TypeError("Harmonic fit has no nonsingular solution");
  return {
