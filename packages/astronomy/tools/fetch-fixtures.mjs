...
import { readRecordSections, writeRecordSections, literalRecords } from './lib/write-record-sections.mjs'
...
const SATELLITES = bodyRecords.filter(record => record.acquisition?.fixture)
  .map(({ id, acquisition: { fixture: f } }) => [id, f.target, f.center, f.range])
