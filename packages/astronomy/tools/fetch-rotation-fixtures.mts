...
${rows
  ...
  .join('\n')}
    ...
    failures.push(`${id}: ${(error instanceof Error ? error.message : String(error)).split('\n')[0]}`)
  }
}

const out = `${HEADER(
  "JPL Horizons, Sun's vector from each body's centre and from two body-fixed sites on it",
  'fetch-rotation-fixtures.mts',
)}
