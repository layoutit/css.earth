...
import type { VsopSeries, VsopTerm } from './lib/vsop87.mts'
import type { ElpTerm } from './lib/elp2000.mts'
import { loadVsop87a, evalVsop87 } from './lib/vsop87.mts'
import { loadElpTerms, elpPositionKm, W1_POLY, P_POLY, Q_POLY, A0, ATH } from './lib/elp2000.mts'
...
const closestApproachAu = (key: keyof typeof ORBIT) => {
...
const BODIES: [keyof typeof ORBIT, string][] = [
  ['mercury', 'mer'],
  ['venus', 'ven'],
  ['emb', 'emb'],
  ['mars', 'mar'],
...
const weight = (term: VsopTerm | ElpTerm, alpha: number, tMax: number) => Math.abs('a' in term ? term.a : term.amp) * tMax ** alpha

function sigmaBoundVsop(dropped: VsopSeries) {
...
function splitVsop(series: VsopSeries, threshold: number) {
  const kept: VsopSeries = [[], [], []]
  const dropped: VsopSeries = [[], [], []]
...
function chooseVsopThreshold(series: VsopSeries, targetAu: number) {
  let lo = 1e-14
  let hi = 1e-3
  for (let i = 0; i < 200; i++) {
...
const countTerms = (kept: VsopSeries) => kept.reduce((n, coord) => n + coord.reduce((m, terms) => m + terms.length, 0), 0)
...
function elpBound(dropped: ElpTerm[]) {
...
function splitElp(terms: ElpTerm[], angleThresholdArcsec: number) {
  const distanceThresholdKm = angleThresholdArcsec * KM_PER_ARCSEC_OF_ANGLE
  const kept: ElpTerm[] = []
  const dropped: ElpTerm[] = []
