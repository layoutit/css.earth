...
const readChecked = (path: string, symbol: string): Record<string, unknown> => {
  const text = readFileSync(new URL(path, import.meta.url), 'utf8');
  return objectValue(JSON.parse(text.split(`export const ${symbol} = `)[1].replace(/ (?:satisfies|as const)[\s\S]*$/, '')));
};
...
const records: Record<string, ElementRecord> = selected.size || formatOnly ? recordMap(readElements(), readElementRecord) : {};
const fixtures: Record<string, VectorFixture> = selected.size || formatOnly ? recordMap(readFixtures(), readVectorFixture) : {};
...
const header = '// Generated by tools/generate-asteroids.mts. Do not edit.\n' +
...
const serializeElements = (value: Record<string, ElementRecord>) => JSON.stringify(value, null, 2)
...
const serializeFixtures = (value: Record<string, VectorFixture>) => JSON.stringify(value, null, 2)
