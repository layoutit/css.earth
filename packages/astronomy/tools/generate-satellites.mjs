...
import { readBodyRecords, prepareBodyRecords } from './body-records.mjs'
...
const satelliteRecords = bodyRecords.filter(record => record.acquisition?.satellite)
const SATELLITES = satelliteRecords.map(({ id, acquisition: { satellite: s } }) =>
  [id, s.target, s.center, s.parent, s.fromJd, s.toJd, s.stepDays, s.barycentreCompanion])
const RADIAL_FIT_IDS = new Set(satelliteRecords.filter(record => record.acquisition.satellite.radial).map(record => record.id))
const LIBRATION_FIT_IDS = new Set(satelliteRecords.filter(record => record.acquisition.satellite.libration).map(record => record.id))
const POSITION_CORRECTION_FIT_IDS = new Set(satelliteRecords.filter(record => record.acquisition.satellite.positionCorrection).map(record => record.id))
const POSITION_CORRECTION_OPTIONS = Object.fromEntries(satelliteRecords.map(record => [record.id, record.acquisition.satellite.positionCorrectionOptions]))
...
const entry = (r) => `  ${JSON.stringify(r.id)}: {
    parent: '${r.parent}',
