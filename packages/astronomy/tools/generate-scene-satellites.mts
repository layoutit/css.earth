...
const sources: [string, string, number | null, number | null][] = [
...
type BodyState = Awaited<ReturnType<typeof loadBodyEpochEphemeris>>;
const records: Record<string, Omit<BodyState, 'provenance'> & { provenance: Omit<BodyState['provenance'], 'validation'> }> = {};
for (const [bodyId, centerBodyId, target, center] of sources) {
  ...
  const provenance: Omit<BodyState['provenance'], 'validation'> & { validation?: unknown } = { ...state.provenance };
  if ('validation' in provenance) delete provenance.validation;
  records[bodyId] = { ...state, provenance };
}
const output = '// Generated by tools/generate-scene-satellites.mts from validated object-owned source states.\n' +
