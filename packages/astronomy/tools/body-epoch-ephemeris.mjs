...
export async function loadBodyEpochEphemeris({ bodyRoot, bodyId, centerBodyId, target, center, epochJdTt }) {
  ...
  const provenance = source => Object.freeze({ model: 'Horizons geometric state at prepared epoch',
  ...
  async function readSource(source, expectedTarget, expectedCenter) {
    ...
    if (url.origin !== 'https://ssd.jpl.nasa.gov' || url.pathname !== '/api/horizons.api' ||
        ...
        Math.abs(Number(query.get('TLIST')) - record.requestEpochJdUtc) > 1e-9) {
    ...
    if (Math.abs(Number(fieldsCsv[0]) - record.requestEpochJdUtc) > 1e-9 ||
    ...
    return { positionKm: values.slice(0, 3), velocityKmPerDay: values.slice(3), gm: sourceGm };
  }
}

async function loadPublishedRecord({ record, bodyRoot, bodyId, centerBodyId, epochJdTt }) {
  ...
  for (const key of ['positionKm', 'velocityKmPerDay']) {
  ...
  if (parameters.independentPlaneReference) {
    ...
    if (!Number.isFinite(angle) || angle > 7 || !Number.isFinite(record.validation.orbitalPlaneVsIndependentRadarDegrees) ||
...
async function readPinnedText(bodyRoot, source) {
...
function numberedAsteroidRows(source, text, target, center, correction) {
  ...
  return lines.map((line, index) => {
    ...
    return { epochJdUtc, positionKm: values.slice(0, 3), velocityKmPerDay: values.slice(3, 6), lightTimeDays: values[6] };
  });
}

function verifyPublishedProjections(parameters, record, sources) {
  ...
  const close = (a, b, tolerance = .00001) => Number.isFinite(a) && Number.isFinite(b) && Math.abs(a - b) < tolerance;
  const project = (comparison, sourceKey, jdUtc) => {
    const rows = numberedAsteroidRows(record.sourcePins[sourceKey], sources.get(sourceKey), target, 399, 'LT');
  ...
  for (const comparison of validation.comparisons) {
    ...
    const source = record.sourcePins[miriadeKey]; const xml = sources.get(miriadeKey);
    const url = new URL(source.url);
    ...
    const difference = Math.hypot(...projected.map((value, i) => value - row[i + 1]));
...
export function evaluatePublishedOrbit(parameters, epochJdTt) {
  const keys = ['epochJd', 'semiMajorAxisKm', 'eccentricity', 'inclinationDegrees', 'ascendingNodeDegrees',
    'argumentPeriapsisDegrees', 'meanAnomalyDegrees', 'meanMotionDegreesPerDay', 'quadraticMeanAnomalyDegreesPerYear2'];
  ...
  const rotateX = ([x, y, z], angle) => [x, y * Math.cos(angle) - z * Math.sin(angle), y * Math.sin(angle) + z * Math.cos(angle)];
  const rotateZ = ([x, y, z], angle) => [x * Math.cos(angle) - y * Math.sin(angle), x * Math.sin(angle) + y * Math.cos(angle), z];
  const rotate = vector => {
  ...
  return { positionKm: rotate(position), velocityKmPerDay: rotate(velocity) };
}
