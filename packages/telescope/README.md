...
ASCL is available on demand for software discovery: `telescope ascl Astroquery` searches the [Astrophysics Source Code Library](https://ascl.net/), while `telescope ascl --product NEW_DIRECTORY/output.product.json` checks the software names in a verified product receipt against exact ASCL title aliases. Results include ASCL IDs, entry links, and preferred citations when supplied. A title match is a citation lead; only the receipt says which software and version this run recorded. No catalog lookup changes the saved product or claims that a source contains the target. The live catalog is fetched once per explicit command with an 8 MiB limit; `outputs` remains offline.
