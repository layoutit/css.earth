...
Exploration samples numbered archive sources for Keck, Gemini, Chandra and Spitzer; OPUS also offers its sampled spacecraft images as separate source choices. Run `telescope fetch RUN/explore.json --archive ARCHIVE --pick N --out NEW_DIRECTORY` with `keck`, `gemini`, `chandra`, `spitzer` or `opus` to recheck one saved choice and preserve its original files within the saved transfer limit. Keck is the default archive for older commands. If a Chandra ObsID or Spitzer AOR has several science FITS files, fetch lists the exact choices and requires `--file NAME`; it never silently chooses a detector or channel. A source receipt pins discovery, current archive metadata, file hashes and unresolved limitations. For a FITS source, `telescope outputs NEW_DIRECTORY/output.product.json` reports the operations its bytes actually support. A raw frame, event file, Spitzer product or native OPUS image is not thereby qualified for target detection, calibration or scientific fitness. Unavailable archive access is reported as unavailable, never as an empty search.

For a fetched Keck, Gemini, Chandra, Spitzer or OPUS source, `outputs` also gives a **source relevance** assessment. It keeps the archive's target name, the saved ICRS position's placement in a native FITS science image, and target detection as separate facts. A position is tested with Astropy's celestial WCS only when the exploration saved a SIMBAD position or an explicit ICRS circle; the test concerns the nominal position or circle centre, not the full source extent. A moving body's nominal catalog position is not an observation-time ephemeris. A missing or unusable WCS and event or PDS products without a supported sky-image grid remain `unknown`. Even `in-field` says nothing about a detection or an unmasked pixel at that location.

`outputs RUN/pick-N/result.json` now makes the same distinction for a verified `get` delivery. The delivery saves the original kind, wavelength, family and available position question, so the assessment travels with its pinned files. A scientific query's existing fulfilled/unresolved/refused verdict remains the acceptance result; source relevance describes what the native structures establish. Older deliveries without the saved question can still be inspected, but may lack the exploration filters.

ASCL is available on demand for software discovery: `telescope ascl Astroquery` searches the [Astrophysics Source Code Library](https://ascl.net/), while `telescope ascl --product NEW_DIRECTORY/output.product.json` or `telescope ascl --product RUN/pick-N/result.json` checks software names against exact ASCL title aliases. The latter verifies the portable delivery and its original producing record; the copied original record need not resolve paths from its new location. Results include ASCL IDs, entry links, and preferred citations when supplied. The current run's software and source-declared earlier processing are separate lists. For example, Betelgeuse's authored source manifest names SQUEEZE 3.0 and links its reconstruction evidence, while a new qualification run records cssEarth and Node. A title match is only a citation lead; it does not prove a version or scientific result. The live catalog is fetched once per explicit command with an 8 MiB limit; `outputs` remains offline.

The assessment lists each recognized native structure's dimensions, usable-sample count, units, mask, uncertainty and recorded calibration fields. It compares a requested kind with the structures or available operations and compares a requested wavelength interval only with usable, native bin edges in **one** structure. A filter name or a band centre alone does not establish continuous spectral coverage. An unsupported or outside-field result suggests another source; an unknown result names the missing evidence. The original source receipt and `outputs` operation list remain the authority for export. Use `--json` to retain the same assessment as structured data.

Use `--instrument NAME` and `--from ISO --to ISO` on `explore` to narrow live Keck, Gemini and Chandra source queries before their bounded samples. Spitzer filters its returned AOR list before taking 100; its archive endpoint has no corresponding server-side filters here. The Keck and Chandra queries use UTC calendar-day bounds, so their narrowed source lists can include records outside the exact times on the boundary days. `--kind` and `--wavelength` still apply to indexed and VO observation candidates, not these raw source leads; after fetching an exact lead, `outputs` tests those saved requests against its bytes. An `overflow` result remains incomplete: narrow the dates or instrument and explore again in a new directory.

Gemini, OPUS, Chandra and Spitzer fetches retain completed files at `NEW_DIRECTORY.partial` if a later transfer or output step fails. Repeat the **same** command with `--resume` and the same `--out`: it rechecks the saved selection against current archive metadata, verifies each completed file's size and SHA-256, and downloads only unfinished files. Changed archive metadata or altered local files are refused; use a new output directory for a new selection. A successful fetch moves the verified result to `NEW_DIRECTORY` and removes the retry record. Delete an abandoned `.partial` directory when its bytes are no longer needed. Keck fetch handles one file through its existing bounded retry path and does not accept `--resume`.

When `outputs` reports family operations for a fetched source, `telescope family-assess REQUEST.json NEW_DIRECTORY/output.product.json --out ASSESSMENT_DIRECTORY` verifies the source receipt and its pinned descriptor before assessing compatibility. The saved assessment includes the descriptor path, byte count and SHA-256. It reports `matched`, `unresolved` or `refused`; even `matched` does not establish target detection, calibration or scientific fitness. Sources without a supported descriptor remain explicit in `outputs` and cannot be assessed as a family descriptor.
