...
Keck exploration now samples up to three exact public raw FITS files per matching instrument. These are separately numbered **Keck source** leads, not qualified observation choices. Run `telescope fetch RUN/explore.json --pick N --out NEW_DIRECTORY` to recheck one KOA row, download the original FITS within the saved byte limit, and pin the source bytes and metadata. `telescope outputs NEW_DIRECTORY/output.product.json` then reports what the verified FITS actually supports. Target detection, calibration and scientific fitness remain unresolved for a raw frame. Gemini archive leads remain metadata-only if public file access is denied.
