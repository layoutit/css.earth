...
import type { VolumeSliceQuad, SliceAxis } from './volume-slices.js';
import type { Vector3 } from './volume-source.js';
...
const output = resolve(process.argv[2] ?? '.local/milky-way-proof/volume');
...
await writeFile(resolve(output, 'image-world-mapping.json'), JSON.stringify(receipt, null, 2) + '\n');
