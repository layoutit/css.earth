...
import { createStarRemovalJobs, removalJobActive, type RemovalJob } from './star-removal-jobs.ts';
import type { RemovalProgress, StarRemovalResult } from './star-removal-types.ts';
...
export const StarRemovalControls = forwardRef<Controls, Options & { host: HTMLElement }>(function StarRemovalControls({ host, onApply }, ref) {
  ...
  return <section className="automatic-star-removal" hidden={!state.context}>
...
export function createStarRemovalControls(host: HTMLElement, options: Options & { controls: ControlPortals }) {
  const root = options.controls.mount(host);
  ...
  return { setContext(context: Context | null) {
    ...
    if (destroyed) return;
    destroyed = true; controls?.setContext(null); root.unmount();
  } };
}
