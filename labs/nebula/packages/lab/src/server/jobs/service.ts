...
export interface ProcessingJob {
  ...
  progress?: JobProgress; result?: unknown; error?: string;
}
...
export function createProcessingJobs<Request>(root: string, options: {
  namespace?: string; label?: string; schema?: string;
  ...
  /** Stable operation scope; persisted as imageId for existing receipt compatibility. */
  requestKey: (request: Request) => string;
  sample: (request: Request, signal: AbortSignal, progress: (value: JobProgress) => void) => Promise<unknown>;
  validateResult: (result: unknown) => Promise<void>;
}) {
  const namespace = options.namespace ?? 'processing', label = options.label ?? 'Processing';
  ...
  const schema = options.schema ?? `cssearth-${namespace}-job@1`;
  ...
  function readSavedJob(value: unknown, id: string): SavedJob<Request> {
    if (!object(value) || value.schema !== schema || value.id !== id || typeof value.imageId !== 'string' ||
        ...
        (value.error !== undefined && typeof value.error !== 'string')) throw new Error('Saved processing job is invalid.');
    const request = options.parseRequest(value.request);
    if (value.imageId !== options.requestKey(request)) throw new Error('Saved processing job is invalid.');
  ...
  async function visible(job: SavedJob<Request>, archived = false): Promise<ProcessingJob> {
  ...
  async function lookup(id: unknown): Promise<{ job: SavedJob<Request>; archived: boolean } | undefined> {
    if (!uuid(id)) throw new TypeError('Invalid processing job identity.');
  ...
  async function find(id: unknown) {
    const found = await lookup(id); if (!found) throw new HttpError(404, 'Saved processing job is unavailable.'); return found;
  }
  ...
  function launch(job: SavedJob<Request>) {
    ...
    const task = (async () => {
      ...
      job.updatedAt = new Date().toISOString(); await save(job);
    })().catch(error => { job.status = 'failed'; job.error = `Could not persist processing completion: ${message(error)}`; })
  ...
  async function start(input: unknown) {
    ...
    const requestKey = options.requestKey(request);
    if (!requestKey || typeof requestKey !== 'string') throw new TypeError('Invalid processing scope.');
    return exclusive(async () => {
      ...
      if (closed) throw new HttpError(503, 'The local processing worker is shutting down.');
      if (workers.size >= 4) throw new HttpError(429, `${label} is busy; wait for an active job to finish.`);
      await archiveOldReceipts();
      if (records.size >= (history?.maxRecords ?? 512)) throw new HttpError(507, 'The local processing job history is full.');
      const now = new Date().toISOString(), job: SavedJob<Request> = { schema, id: input.requestId as string,
        imageId: requestKey, request: structuredClone(request), status: 'queued', createdAt: now, updatedAt: now };
