...
import {createEnvelopeSampler,envelopeChromaticity,validateEnvelopeSettings} from '@cssearth/nebula-reconstruction/methods/inference/simulation-envelope';
...
export async function bakeFiniteLens(root:string,input:FiniteLensInput,signal?:AbortSignal,progress?:(p:FiniteLensProgress)=>void):Promise<PreparedReconstruction>{
 ...
 const modelBytes=await readFile(resolve(modelDir,'source/provenance.json')),sourceBytes=await readFile(resolve(sourceDir,'source/provenance.json')),fieldBytes=await readFile(resolve(modelDir,'source/emission-field.json'));
 const model=parseLabModelJson(modelBytes.toString()),source=parseLabModelJson(sourceBytes.toString()),oldResult=parseLabModelJson(await readFile(resolve(modelDir,'result.json'),'utf8')),sourceResult=parseLabModelJson(await readFile(resolve(sourceDir,'result.json'),'utf8'));
 ...
 if(envelopeRecord){
  const settings=validateEnvelopeSettings(envelopeRecord.settings),gw=envelopeRecord.width,gh=envelopeRecord.height;
  ...
  envelopeColor=envelopeChromaticity(lensRgb,lensCoverage,gw,gh,envelopeRecord.bounds,settings.scalePixels);
 }
 ...
 const slabMaterial=typeof fullChromaAlphaByte==='number'?alphaLimitedSlabMaterial(compilerSlabMaterial(sampleEmission,sampleMaterial),sampleEmission,modelSettings.exposureGain,fullChromaAlphaByte):compilerSlabMaterial(sampleEmission,sampleMaterial);
 ...
 await json(resolve(staging,'inspection-object.json'),{...descriptor,properties:{...descriptor.properties,preparation:{source:'source/cloud-parts.json',sha256:partsSha}},prepared:{format:prepared.format,url:'prepared/inspection.json',sha256:inspectionSha}});
 ...
 const result={...oldResult,resultId,finiteMaterial:{modelResultId:input.modelResultId,sourceResultId:input.sourceResultId},imageId:sourceResult.imageId,removalResultId:sourceResult.removalResultId,placement:sourceResult.placement,appearance,subject};await json(resolve(staging,'result.json'),result);
