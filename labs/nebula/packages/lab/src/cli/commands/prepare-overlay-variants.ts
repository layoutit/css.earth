...
export async function prepareOverlayVariants(planPath: string, selected: string[] = []) {
  ...
  for (const selection of plan.selections) {
    ...
    await pinned('labs/nebula/packages/reconstruction/src/star-removal/star-separation.py', receipt.scriptSha256);
    ...
    files.push({ path: receiptPath, bytes: JSON.stringify({ ...receipt, delivery: { sourceReceiptSha256: hash(receiptBytes),
      scriptSha256: hash(await readFile('labs/nebula/packages/lab/src/cli/commands/prepare-overlay-variants.ts')), layers: row.layers,
