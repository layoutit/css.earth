  ...
  return build({ ...options, plugins: [{
    ...
    setup(builder) {
      builder.onResolve({ filter: /^@cssearth\// }, args => {
        // `@cssearth/core`, `@cssearth/fits`, `@cssearth/spice` and `@cssearth/telescope` ship built JavaScript, not TypeScript owners. Node loads their ESM builds at
        // run time: inlining the CommonJS build `require.resolve` finds would leave `require('node:crypto')` in an ESM
        // bundle, which fails.
        if (/^@cssearth\/(?:core|fits|spice|telescope)(?:\/|$)/.test(args.path)) return { path: args.path, external: true };
