...
export function buildLabModule(options: BuildOptions) {
  const entries = new Set((options.entryPoints as string[] | undefined ?? []).map(entry => resolve(entry)));
  return build({ ...options, plugins: [{
    ...
    setup(builder) {
      builder.onLoad({ filter: /\.[cm]?[jt]sx?$/ }, async args => {
        ...
        if (!text.includes('import.meta.url')) return undefined;
        const extension = args.path.slice(args.path.lastIndexOf('.'));
        return { contents: text.replaceAll('import.meta.url', JSON.stringify(pathToFileURL(args.path).href)),
          loader: LOADERS[extension] ?? 'ts', resolveDir: args.path.slice(0, args.path.lastIndexOf('/')) };
      });
    },
  }, ...(options.plugins ?? [])] });
}
