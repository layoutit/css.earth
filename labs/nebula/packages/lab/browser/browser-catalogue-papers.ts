...
import { paperRoot,readPaperIndex } from '../src/features/catalogue/papers/types';
...
const browser=await chromium.launch({headless:true}),page=await browser.newPage({viewport:{width:1500,height:1050},serviceWorkers:'block'});
const guard=await blockProcessingWrites(page.context());
...
try{
  ...
  assert.deepEqual(errors,[]);assert.deepEqual(posts,[]);assert.deepEqual(guard.blocked,[]);assert.deepEqual(downloads,[]);
