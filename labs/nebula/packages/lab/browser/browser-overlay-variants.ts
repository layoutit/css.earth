...
import { overlayVariantsPath, parseOverlayVariants } from '../src/features/legacy-viewer/overlay-variants.js';
...
try {
  await page.goto(`${process.argv[2] ?? 'http://127.0.0.1:4331'}/alignment?subject=lmc-clouds`);
  await page.waitForFunction(count => document.querySelector<HTMLElement>('#viewer')?.dataset.ready === 'true' &&
    document.querySelector<HTMLElement>('#viewer')?.dataset.subject === 'lmc-clouds' && document.querySelectorAll('#overlay-choice option').length === count,
  rows.length, { timeout: 30000 });
