...
import { blockProcessingWrites } from './browser-regression';
import { readMessierCatalogue, readMessierInventory, type MessierInventory } from '../src/features/catalogue/types';
...
const browser = await chromium.launch({ headless: true }), context = await browser.newContext({ viewport: { width: 1500, height: 1050 }, serviceWorkers: 'block' });
const guard = await blockProcessingWrites(context);
...
try {
  ...
  assert.deepEqual(errors, []); assert.deepEqual(processing, []); assert.deepEqual(guard.blocked, []); assert.deepEqual(science, []);
