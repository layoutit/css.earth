...
Exact directional compositor checks remain in the source-owned renderer fixture `src/renderers/css/volume/prepared-volume-runtime.test.ts`, which supplies explicit camera directions and tests axis transitions. Browser gesture screenshots complement those numerical checks; they do not replace them.
