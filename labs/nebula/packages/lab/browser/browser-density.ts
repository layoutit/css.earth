import { cameraOrientation, settleCamera, earthCamera, gestureCamera } from './browser-camera.ts';
...
const subjects = JSON.parse(await readFile('labs/nebula/packages/lab/src/state/subjects.json', 'utf8')) as
...
async function capture(id: string, pose: 'reference' | 'horizontal-positive-wide') {
  await gestureCamera(page, pose); await settleCamera(page);
...
try {
  ...
  await earthCamera(page);
  assert.equal(await cameraOrientation(page), direct.pose, 'direct Density differs from the public Earth view');
  ...
  report.subjects.push(await capture('lmc-particles', 'reference'));
  ...
  report.subjects.push(await capture('lmc-particles', 'horizontal-positive-wide')); const retained = await dragAndZoom();
  ...
  report.subjects.push(await capture('smc-particles', 'reference'), await capture('smc-particles', 'horizontal-positive-wide'));
