...
try {
  ...
  for (const pose of ['reference', 'horizontal-positive-wide', 'horizontal-positive-long', 'vertical-positive-long', 'reference']) {
  ...
  if (peer) {
    ...
    assert.equal(await cameraOrientation(page), retainedOrientation, 'Comparison switch must retain the view.');
  }
  assert.deepEqual(errors, []); assert.deepEqual(writes, []);
  await writeFile(`${directory}/result.json`, JSON.stringify({ passed: true, errors, writes,
    checks: ['prepared PolyCSS leaves', 'Earth and horizontal/vertical drag screenshots', 'pointer rotation', 'refresh', 'no processing on mount'] }, null, 2));
