...
import { decomposeStructures } from './structure-wavelets.js';
import type { StructureRegion, WaveletSettings } from './structure-wavelets.js';
import { digest, readBenchmarkImage, writeMethodProducts } from './benchmark-products.js';
import type { BenchmarkImage, BenchmarkMethod, ComponentMaps } from './benchmark-products.js';
...
async function importGetsf(path: string, recipe: Recipe, image: BenchmarkImage) {
  const data: GetsfImport = JSON.parse(await readFile(path, 'utf8'));
  ...
  await writeFile(resolve(recipe.outputDirectory, 'getsf/import-receipt.json'), JSON.stringify({
    ...
    provenanceSha256: digest(provenanceBytes), provenance: JSON.parse(provenanceBytes.toString()),
  }, null, 2) + '\n');
...
if (getsfArgument) methods.push(await importGetsf(getsfArgument.slice('--getsf-import='.length), recipe, image));
  ...
  const attempt = bytes ? JSON.parse(bytes.toString()) : null;
...
const codePaths = ['prepare-structures.ts', 'benchmark-products.ts', 'structure-wavelets.ts'];
