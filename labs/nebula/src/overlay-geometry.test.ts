...
import { overlayCorners, rayToOverlayPlane, type OverlayFrame } from './overlay-wcs.js';
...
async function prepared() {
  ...
  return Promise.all(recipe.targets.map(async (target: { directory: string; referenceObject: string }) => ({ target,
    provenance: JSON.parse(await readFile(`${target.directory}/source/provenance.json`, 'utf8')),
    frame: JSON.parse(await readFile(target.referenceObject, 'utf8')).properties.volume as OverlayFrame,
  })));
}
