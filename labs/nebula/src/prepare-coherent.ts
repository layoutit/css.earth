...
import { digest, readBenchmarkImage } from './benchmark-products.js';
import { decomposeStructures, type WaveletSettings } from './structure-wavelets.js';
import { createCoherentVolumeSampler, type CoarseStellarDensityPrior } from './coherent-volume.js';
...
const benchmark = JSON.parse((await pinned(recipe.benchmark.path, recipe.benchmark.sha256)).toString()) as {
...
const codeNames = ['prepare-coherent.ts', 'coherent-volume.ts', 'coherent-validation.ts', 'structure-wavelets.ts', 'benchmark-products.ts', 'master-slices.ts'];
...
for (const variant of selected) {
  ...
  if (existing) {
    ...
    slices = (await deriveMasterVolumeSlices({ masters: JSON.parse(existing), masterDirectory, deliveryBanks }))[0]!.slices;
  } else {
