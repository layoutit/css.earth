...
import { decomposeFilledComponents } from '../reconstruction/filled-components.js';
import { createFilledVolumeSampler } from '../reconstruction/filled-volume.js';
import { createIntegratedSignalSampler } from '../density/cloud-density.js';
...
export async function loadStarCloudModel(frame: DensityVolumeFrame) {
  ...
  const implementation=Object.fromEntries(await Promise.all(['stars/lmc-star-cloud-model.ts','cli/prepare-lmc-stars.ts','reconstruction/filled-products.ts',
    'reconstruction/filled-components.ts','reconstruction/filled-volume.ts','density/observation-prior.ts','density/cloud-density.ts'].map(async name=>
