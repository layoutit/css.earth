...
import { detectShapes } from './detect-shapes.js';
import { readDetectionRequest, readDetectionResult, geometryRecord, type DetectionResult } from './jobs-model.js';
import { geometrySha, readGeometryPin, readRegisteredGeometrySource } from './registered-source.js';
...
export async function prepareDetection(root: string, value: unknown, progress: (value: DetectionProgress) => void): Promise<DetectionResult> {
  ...
  const directory = 'labs/nebula/src/reconstruction/geometry';
  const dependencies = (await readdir(resolve(root, directory))).filter(name => name.endsWith('.ts') && !name.endsWith('.test.ts')).map(name => `${directory}/${name}`)
    .concat(['labs/nebula/src/alignment/observations-ui/geometry-model.ts', 'labs/nebula/src/alignment/observations-ui/structures-model.ts', 'pnpm-lock.yaml']).sort();
  const implementation = await Promise.all(dependencies.map(async path => ({ path, sha256: geometrySha(await readFile(resolve(root, path))) })));
