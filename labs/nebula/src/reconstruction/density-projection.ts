/** Offline source-independent density projection for cutoff and catalogue support. */
...
export function prepareDensityProjection(source: VolumeSource, distance: number, width = 256) {
  const bounds = source.recipe.grid.bounds;
  ...
  const densityAt = (x: number, y: number, z: number) => {
    sampleEncoded(source, x, y, z, encoded); return channelDensity(encoded[3], source.recipe.grid.encoding);
  };
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
    ...
    const value = -Math.expm1(-column * Math.hypot(1, tx / distance, ty / distance) * source.recipe.material.exposureGain);
