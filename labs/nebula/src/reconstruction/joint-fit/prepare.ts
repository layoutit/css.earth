...
import { prepareJointInput } from './input';
import { fitJointModels } from './fitter';
import { jointBounds, sampleJointEmission } from './geometry';
import { bakeJointVolume } from './volume';
import { readJointResult, type JointPin, type JointResult, type JointCandidate } from './result';
import type { JointRequest } from './model';
...
export async function prepareJointFit(root: string, request: JointRequest, signal: AbortSignal, progress: (message: string) => void): Promise<JointResult> {
  ...
  const implementation = await Promise.all(owners.map(name => readFile(resolve(root, 'labs/nebula/src/reconstruction/joint-fit', name)).then(geometrySha)));
