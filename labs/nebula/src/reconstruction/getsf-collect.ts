...
import { digest } from './benchmark-products.js';
import { decodeFits, float32LittleEndian } from './getsf-fits.js';
...
export async function collectGetSfBenchmark(options: {
  ...
  for (const [name, filename] of Object.entries(filenames)) {
    ...
    rawOfficialMaps[name] = { originalFitsPath: relative(process.cwd(), path), fitsSha256: digest(original),
      path: relative(process.cwd(), rawPath), sha256: digest(raw), bytes: raw.length, statistics: stats };
    ...
    maps[name] = relative(process.cwd(), importPath);
  }
  ...
  for (const path of catalogs) {
    ...
    catalogReceipts.push({ name, originalPath: relative(process.cwd(), path), sha256: digest(bytes), bytes: bytes.length,
  ...
  const manifest = { sourceSha256: input.sourceSha256, width: input.width, height: input.height, maps, note,
    provenance: relative(process.cwd(), receiptPath), metrics: { software: 'getsf260706',
