...
import { loadObservationDiffuse, readObservationStructureRecipe } from './observation-structure-source.js';
import type { ObservationRecipe } from '../../alignment/observations/recipe.js';
import type { Observation } from '../../alignment/observations-ui/model.js';
...
test('structure input cannot silently rerun star removal when a native receipt is absent', async () => {
  ...
  try {
    ...
    const recipe: ObservationRecipe = { schema: 'cssearth-nebula-observation-recipe@1', id: basename(directory), referenceId: 'test',
      ...
      nativeRemoval: { model: { path: modelPath, sha256: sha(model) }, scriptSha256: sha(await readFile('labs/nebula/src/star-removal/star-removal.py')) } };
