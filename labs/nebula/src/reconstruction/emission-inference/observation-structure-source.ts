...
import { readObservationRecipe, scienceObservationSources, type ObservationRecipe } from '../../alignment/observations/recipe.js';
import { nativeStarless } from './native-source.js';
import { nativePreserved } from './native-preserved.js';
import { verifyTransferPins } from '../../alignment/observations/registration-transfer.js';
import { composeAffine } from '../../alignment/observations/registration.js';
...
export async function readStructureObservations(observationRecipePath: string, cataloguePath: string) {
  ...
  for (const image of observations.images) {
    ...
    if (source.registrationTransfer) {
      ...
      if (receipt.sourceSha256 !== bridge.sha256 || receipt.recipeSha256 !== sha(recipeBytes) || evidence.status !== 'verified' || !Array.isArray(receipt.matrix) || receipt.matrix.length !== 6 || receipt.matrix.some(value => typeof value !== 'number' || !Number.isFinite(value))) throw new Error('Registration bridge receipt differs.');
