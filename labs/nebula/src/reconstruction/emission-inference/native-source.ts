...
export async function nativeStarless(source: Buffer, dimensions: [number, number], settings: NativeRemoval,
  ...
  const working = await sharp(source).removeAlpha().toColourspace('srgb').png().toBuffer();
  ...
  try { receipt = JSON.parse(await readFile(receiptPath, 'utf8')); }
    ...
    await new Promise<void>((accept, reject) => {
      ...
      child.on('exit', (code, signal) => { clearTimeout(timer); code === 0 ? accept() : reject(new Error(`NOX stopped: ${code ?? signal}`)); });
      child.stdin.on('error', error => { child.kill('SIGTERM'); reject(error); });
  ...
  const diffuse = await sharp(diffuseBytes).removeAlpha().raw().toBuffer({ resolveWithObject: true });
