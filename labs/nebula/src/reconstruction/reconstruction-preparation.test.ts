...
import { parseReconstructionRequest, reconstructionCatalogue } from './reconstruction-preparation.js';
import { defaultOverlayPlacement } from '../alignment/overlay-placement.js';
import { createStarRemovalJobs } from '../utils/processing-jobs.js';
...
test('the catalogue restores the newest completed placement, independent of hash ordering', async () => {
  ...
  try {
    ...
    await save('labs/nebula/src/subjects.json', [{ id: request.subjectId, density: { overlays: `${overlay}/overlays.json` } }]);
