...
import { imageCorners, type Matrix } from '../../alignment/observations-ui/model.js';
import { readRegisteredGeometrySource, readGeometryLocal, geometrySha } from '../geometry/registered-source.js';
import { extractEvidenceFields, registerEvidenceRaster, validateMatrix } from './fields.js';
import { type EvidenceInputs, type EvidenceGrid, type EvidenceSource } from './model.js';
...
export function evidenceGrid(frame: { width: number; height: number; fieldArcminutes: [number, number] },
  ...
  const corners = images.flatMap(image => imageCorners({ imageToFrame: image.imageToFrame, source: { width: image.nativeWidth, height: image.nativeHeight } }, image.imageToFrame));
...
function samplingArcseconds(image: { nativeWidth: number; nativeHeight: number; width: number; height: number; imageToFrame: Matrix }, grid: EvidenceGrid): number {
...
export async function prepareEvidenceInputs(root: string, cataloguePath: string, options: PrepareEvidenceOptions = {}): Promise<EvidenceInputs> {
  ...
  const implementation = await Promise.all(ownerFiles.map(async name => geometrySha(await readFile(resolve(root, 'labs/nebula/src/reconstruction/evidence-fusion', name)))));
