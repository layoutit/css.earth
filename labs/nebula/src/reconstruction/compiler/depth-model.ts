/** Offline, evidence-addressed surface hypotheses. No object names or image-dependent branches. */
import { readFile, realpath } from 'node:fs/promises';
import { resolve, relative, isAbsolute } from 'node:path';
import { geometrySha } from '../geometry/registered-source';
import { jointPath, jointRecord } from '../joint-fit/model';
import type { EmissionComponent } from './field-types';
...
export function readDepthRecipe(v: unknown): DepthRecipe {
  if (!jointRecord(v) || v.schema !== 'cssearth-nebula-depth-model@1' || !id(v.id) || !jointRecord(v.evidence) ||
      !jointPath(v.evidence.path) || !v.evidence.path.startsWith('labs/nebula/models/') || typeof v.evidence.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(v.evidence.sha256) ||
