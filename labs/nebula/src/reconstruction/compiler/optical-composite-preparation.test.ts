...
import { bakeMasterVolumeSlices } from '../master-slices';
import { jointRecord } from '../joint-fit/model';
import { compilerAlphaDigest, compilerFrame } from './bake';
import { assertCompilerLensGeometry } from './bank-validation';
import { readCompilerRecipe, defaultCompilerControls } from './model';
...
import { opticalCompositeSourcePins, restoreOpticalCompositeSources } from './optical-composite-inputs';
import { prepareOpticalComposite, prepareOpticalCompositeForResult, readOpticalCompositeRecipe } from './optical-composite-preparation';
...
async function fixture(t: TestContext) {
  ...
  // Fixture code pins use the actual owners; no historical processing outputs are copied.
  for (const owner of ['labs/nebula/src/reconstruction/compiler', 'labs/nebula/src/alignment/observations'])
    for (const name of (await readdir(owner)).filter(name => name.endsWith('.ts') && !name.endsWith('.test.ts')))
      await put(`${owner}/${name}`, await readFile(`${owner}/${name}`));
  for (const path of ['labs/nebula/src/cli/prepare-observations.ts', 'labs/nebula/src/alignment/overlay-wcs.ts',
    'labs/nebula/src/reconstruction/emission-inference/native-source.ts', 'labs/nebula/src/star-removal/star-removal.py']) await put(path, await readFile(path));
...
test('composite source readiness distinguishes missing native input from changed evidence and restores through the source owner', async t => {
  ...
  await writeFile(join(f.root, 'labs/nebula/src/run.ts'), `import { copyFile } from 'node:fs/promises';
  ...
  await writeFile(join(f.root, 'labs/nebula/src/run.ts'), 'process.exitCode = 0;');
