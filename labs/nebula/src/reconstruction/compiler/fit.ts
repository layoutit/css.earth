...
import { jointRayDepths } from '../joint-fit/geometry.js';
...
interface FitGrid { width: number; height: number; target: Float64Array; coverage: Float64Array; dx: number; dy: number }
...
function fitGrid(input: EmissionFitInput, detail: number): FitGrid {
  ...
  return { width, height, target, coverage, dx: (input.bounds.max[0] - input.bounds.min[0]) / width, dy: (input.bounds.max[1] - input.bounds.min[1]) / height };
}
function newBasis(x: number, y: number, sigma: number, grid: FitGrid, input: EmissionFitInput, depths: number[], halo: boolean): Basis {
  ...
  for (let py = minY; py <= maxY; py++) {
    ...
    for (let px = minX; px <= maxX; px++) {
      ...
      const value = gy * emissionKernel((input.bounds.min[0] + (px + .5) * grid.dx - x) / sigma); if (value < 1e-8) continue;
