...
type ImageSample = (x: number, y: number, out: Vector3) => boolean;
/** Color follows the same emitting sub-samples as neutral alpha, not an empty slab midpoint. */
export function compilerSlabMaterial(sampleEmission: Sample, sampleImage: ImageSample) {
  ...
  return (x: number, y: number, z: number, out: Vector3, slab: { axis: 'x' | 'y' | 'z'; pitch: number; samples: number }): boolean => {
    ...
    for (let i = 0; i < slab.samples; i++) {
      ...
      if (!(light > 0)) continue;
      weight += light;
      const observed = sampleImage(sx, sy, color), peak = observed ? Math.max(...color) : 0;
      if (observed && peak > 0) covered += light;
      // Missing/black source material retains the same neutral contribution as other lab lenses.
      for (let c = 0; c < 3; c++) out[c] += light * (peak > 0 ? color[c]! / peak : 1);
    }
