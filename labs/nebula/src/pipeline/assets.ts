...
import { prepareVolumeSlices, type VolumeSlices } from '../../../../src/preparation/volume/slices.js';
import { acquire, hash, json, pinned, writeAtomic } from './io.js';
...
export async function bakeDensity(root: string, directory: string) {
  ...
  const recipe = parseVolumeRecipe(JSON.parse((await pinned(root, { path: recipePath, sha256: object.properties.preparation.sha256 })).toString()));
  ...
  for (const quad of expected.quads) {
    try { await pinned(prepared, { path: quad.texturePath, sha256: quad.sha256 }); }
  ...
  try {
    ...
    for (const quad of baked.quads) await writeAtomic(join(prepared, quad.texturePath), await pinned(temporary, { path: quad.texturePath, sha256: quad.sha256 }));
...
export async function bakeSeparationPreviews(root: string, planPath: string, imageId: string) {
  ...
  const recipe = JSON.parse((await pinned(root, { path: selection.recipe, sha256: selection.recipeSha256 })).toString());
  ...
  for (const layer of variant.layers) {
    ...
    const original = await pinned(root, { path: `${recipe.outputDirectory}/${file}`, sha256: receipt.outputs[file].sha256 });
