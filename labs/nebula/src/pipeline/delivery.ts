...
import { hash, localPath, pinned, writeAtomic, type Pin } from './io.js';
...
export async function restoreDelivery(root: string, delivery: BakeDelivery, results: {imageId: string; resultId?: string; directory?: string}[]) {
  ...
  for (const result of results) {
    const sourceDirectory = result.directory ?? await reconstructionDirectory(root, result);
...
async function restoreAtlasDelivery(root: string, delivery: BakeDelivery, results: {imageId: string; resultId?: string; directory?: string}[],
...
async function reconstructionDirectory(root: string, result: {imageId: string; resultId?: string}) {
  ...
  return `${prepared.subject.directory}/prepared`;
}
