...
async function fixture(root: string) {
  ...
  const starsOwner = 'labs/nebula/src/reconstruction/compiler/stars.ts';
...
test('delivery restores missing impostors, rejects drift and rebuilds when their generator changes',async()=>{
  ...
  try {
    ...
    await assert.rejects(prepareNebulaObject(root,directory,true),/Nebula source hash mismatch/);
