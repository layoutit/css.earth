...
import { bakeMasterVolumeSlices } from "../reconstruction/master-slices";
...
import type { CompilerPin } from "../reconstruction/compiler/bake-types";
...
export async function replayCompactSymmetry(
  root: string,
  pin: CompilerPin,
  outputDirectory: string,
) {
  ...
  const volume = validatePreparedCssVolume(
    compileCssVolume({
      id: r.id,
      frame,
