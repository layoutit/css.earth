...
import { readSourceDossier } from '../alignment/observations-ui/source-dossier';
...
const errors: string[] = [], writes: string[] = [], results: unknown[] = [];
...
try {
  for (const id of ids) {
    ...
    assert.equal(await page.locator('#observation-image option').count(), recipe.images.length);
    ...
    await page.locator('#observation-image').selectOption(recipe.images[0]!.id);
    ...
    results.push({ id, images: recipe.images.length, papers: dossier.papers.length,
      registration: observations.images.map(i => ({ id: i.id, status: i.registration.status })) });
  }
