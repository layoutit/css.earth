...
import { loadVolumeSource, sha256 } from '../../../src/preparation/volume/source.js';
...
import { createObservationMapping, reprojectObservationPrior } from './observation-prior.js';
import type { ImageWcs } from './overlay-wcs.js';
import { decomposeFilledComponents, type FilledComponentOptions } from './filled-components.js';
import { createFilledVolumeSampler } from './filled-volume.js';
import { rectifyObservation, writeObservationPanel, extendedMap, validateObservationProjection } from './filled-products.js';
...
const recipeBytes = await readFile(recipePath), recipe: Recipe = JSON.parse(recipeBytes.toString());
...
const frameSource = JSON.parse((await pinned(recipe.frame)).toString()) as { properties: { volume: DensityVolumeFrame } };
...
const sourceRecipe = JSON.parse((await pinned(recipe.stellarPrior)).toString()) as VolumeRecipe;
...
const pins = Object.fromEntries(await Promise.all(['prepare-filled.ts', 'filled-products.ts', 'filled-components.ts',
  'filled-volume.ts', 'observation-prior.ts', 'overlay-wcs.ts', 'overlay-geometry.ts', 'master-slices.ts'].map(async name =>
  [name, sha256(await readFile(`labs/nebula/src/${name}`))])));
const bounds = prior.boundsKpc;
function physicalEnvelope(input: { min: Vec3; max: Vec3 }) {
  const corners = [input.min[2], input.max[2]].flatMap(z =>
    [input.min[0], input.max[0]].flatMap(x => [input.min[1], input.max[1]].map(y => mapping.pointAtDepth(x, y, z))));
  return { min: [0, 1, 2].map(axis => Math.min(...corners.map(p => p[axis]!))) as Vec3,
...
const evidence = { schema: 'cssearth-filled-observation-evidence@1', recipe: { path: recipePath, sha256: sha256(recipeBytes) },
  photo: recipe.photo, wcs: recipe.wcs, pins, observation: { tangentBoundsKpc: bounds, physicalBoundsKpc: physicalEnvelope(bounds),
...
for (const variant of selected) {
  ...
  const physicalBounds = physicalEnvelope(sampler.supportBoundsKpc);
  ...
  if (cached) slices = (await deriveMasterVolumeSlices({ masters: JSON.parse(cached), masterDirectory, deliveryBanks }))[0]!.slices;
