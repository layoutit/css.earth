...
import { loadVolumeSource } from '../../../src/preparation/volume/source.js';
...
import { createIntegratedSignalSampler } from './cloud-density.js';
...
export async function loadStarCloudModel(frame: DensityVolumeFrame) {
  const manifest = JSON.parse(await readFile('labs/nebula/models/lmc-stars/source/catalogue.json', 'utf8'));
  ...
  const recipe = JSON.parse(buffers.cloudRecipe.toString()), evidence = JSON.parse(buffers.cloudProvenance.toString());
  const analysis = JSON.parse(buffers.analysis.toString()), object = JSON.parse(buffers.cloudObject.toString());
  ...
  const stellarObject = JSON.parse(buffers.object.toString());
  ...
  const parts = JSON.parse(buffers.parts.toString()).parts as {id:string}[];
  ...
  const source=await loadVolumeSource(dirname(refs.recipe.path),JSON.parse(buffers.recipe.toString()));
  const implementation=Object.fromEntries(await Promise.all(['lmc-star-cloud-model.ts','prepare-lmc-stars.ts','filled-products.ts',
    'filled-components.ts','filled-volume.ts','observation-prior.ts','cloud-density.ts'].map(async name=>
