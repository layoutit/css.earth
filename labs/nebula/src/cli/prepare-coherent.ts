...
import { digest, readBenchmarkImage } from '../reconstruction/benchmark-products.js';
import { decomposeStructures, type WaveletSettings } from '../reconstruction/structure-wavelets.js';
import { createCoherentVolumeSampler, type CoarseStellarDensityPrior } from '../reconstruction/coherent-volume.js';
import { validateCoherentColumns, validateCoherentAxisSampling } from '../reconstruction/coherent-validation.js';
...
const codeNames = ['cli/prepare-coherent.ts', 'reconstruction/coherent-volume.ts', 'reconstruction/coherent-validation.ts', 'reconstruction/structure-wavelets.ts', 'reconstruction/benchmark-products.ts', 'reconstruction/master-slices.ts'];
const codePins = Object.fromEntries(await Promise.all(codeNames.map(async name =>
  [name, digest(await readFile(`labs/nebula/src/${name}`))])));
