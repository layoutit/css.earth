...
import { convertParticlesToDensityVolume } from '../stars/particles.js';
import { extractExtendedSource, type ExtractionOptions, type NativeExtractionReceipt } from '../reconstruction/extraction.js';
import { createPhotoMasterEmissionSampler } from '../reconstruction/photo-master.js';
...
export async function preparePhotoMaster(recipePath: string) {
  ...
  const pipeline = await Promise.all(['reconstruction/extraction.ts', 'reconstruction/native-extraction.ts', 'stars/particles.ts', 'reconstruction/photo-master.ts', 'reconstruction/master-slices.ts', 'cli/prepare-master.ts']
