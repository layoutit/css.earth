...
import { createFilledPartsSampler, type FilledVolumePart } from '../reconstruction/filled-parts.js';
import { rectifyObservation } from '../reconstruction/filled-products.js';
...
async function bakePart(part: FilledVolumePart): Promise<{ part: FilledVolumePart; data: PreparedCssVolume }> {
  ...
  const key = sha256(Buffer.from(JSON.stringify({ recipe: sha256(recipeBytes), part: part.id, physicalBounds,
    counts, width, implementation: sha256(await readFile('labs/nebula/src/reconstruction/filled-volume.ts')) }))).slice(0, 16);
