...
import { analyzeStructureMap, colorStructureLayer, structureLayers } from '../reconstruction/emission-inference/structure-map.js';
import type { WaveletSettings } from '../reconstruction/structure-wavelets.js';
...
await json(resolve(staging, 'structure-map.json'), { schema: 'cssearth-nebula-structure-map@1', dimensions, panels,
  ...
  provenance: { recipePath, recipeSha256: sha(recipeBytes), sourceRecipePath, sourceRecipeSha256: sha(sourceRecipeBytes),
    ...
    implementation: await Promise.all(['cli/prepare-nebula-structures.ts', 'reconstruction/emission-inference/structure-map.ts',
      'reconstruction/structure-wavelets.ts', 'reconstruction/emission-inference/native-source.ts'].map(async file =>
      ({ file: `labs/nebula/src/${file}`, sha256: sha(await readFile(`labs/nebula/src/${file}`)) }))) },
