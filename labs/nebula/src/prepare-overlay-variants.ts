...
import { overlayVariantsPath, parseOverlayVariants, type OverlayVariants } from './overlay-variants.js';
import { overlayCorners } from './overlay-wcs.js';
import { registeredOverlayCorners } from './overlay-registration.js';
...
export async function prepareOverlayVariants(planPath: string, selected: string[] = []) {
  ...
  const alignment = JSON.parse((await pinned(plan.alignmentReport.path, plan.alignmentReport.sha256)).toString());
  ...
  for (const selection of plan.selections) {
    ...
    const recipe = JSON.parse((await pinned(selection.recipe, selection.recipeSha256)).toString());
    ...
    const receiptBytes = await readFile(`${recipe.outputDirectory}/receipt.json`), receipt = JSON.parse(receiptBytes.toString());
    ...
    await pinned('labs/nebula/src/star-separation.py', receipt.scriptSha256);
    ...
    files.push({ path: receiptPath, bytes: JSON.stringify({ ...receipt, delivery: { sourceReceiptSha256: hash(receiptBytes),
      scriptSha256: hash(await readFile('labs/nebula/src/prepare-overlay-variants.ts')), layers: row.layers,
  ...
  const previous = selected.length ? parseOverlayVariants(JSON.parse(await readFile(overlayVariantsPath, 'utf8'))) : [];
