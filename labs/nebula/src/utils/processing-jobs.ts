...
export function createStarRemovalJobs<Request extends { imageId: string; action: string } = RemovalRequest>(root: string, options: {
  ...
  function save(job: SavedJob<Request>) {
  ...
  const ready = (async () => {
    ...
    for (const name of await readdir(directory)) {
      ...
      const job = JSON.parse(await readFile(resolve(directory, name), 'utf8')) as SavedJob<Request>;
      if (job.schema !== schema || job.id !== name.slice(0, -5) || job.imageId !== job.request?.imageId ||
  ...
  async function visible(job: SavedJob<Request>): Promise<RemovalJob> {
