...
import { defaultOverlayTone, updateOverlayTone, type OverlayTone } from '../viewer/overlay-tone';
...
export function createToneControls({ host, target, onApply }: {
  host: HTMLElement; target: ToneTarget;
  onApply(context: ToneContext, resources: ToneResource[], isCurrent: () => boolean): Promise<void>;
}) {
  const root = createRoot(host), listeners = new Set<() => void>(), saved = savedTones, touched = new Set<string>();
  ...
  return Object.freeze({
    ...
    getValue: () => ({ ...tone }),
    destroy() { disposed = true; revision++; pending?.abort(); window.clearTimeout(timer); window.clearTimeout(feedback); listeners.clear(); queueMicrotask(() => root.unmount()); },
  });
}
