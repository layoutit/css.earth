...
export function createOverlayPlacementControls(initial: Props) {
  const host = document.createElement('div'), root = createRoot(host); let disposed = false;
  const update = (props: Props) => { if (!disposed) root.render(<OverlayPlacementControls {...props} />); };
  update(initial);
  return Object.assign(host, { update, destroy() { if (!disposed) { disposed = true; queueMicrotask(() => root.unmount()); } } });
}
