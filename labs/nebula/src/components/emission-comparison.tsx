...
export function EmissionComparison({ directory, structureDirectory, modeled = false, methodUrl, credit, statusNote }: {
  directory: string; structureDirectory?: string; modeled?: boolean; methodUrl?: string; credit?: string; statusNote?: string;
}) {
  const [mode, setMode] = useState<'structure' | 'volume'>('structure');
  ...
  return <aside className="floating-panel cloud-adjustment-panel" aria-label="Emission inference comparison">
    {structureDirectory && <div className="emission-view-buttons" role="group" aria-label="Inspection mode">
      ...
      <button type="button" aria-pressed={!structureMode} title="Previous volume baseline; this structure map has not supplied new depths yet." onClick={() => setMode('volume')}>Volume</button>
    </div>}
    {structureMode && structureDirectory ? <EmissionStructures key={structureDirectory} directory={structureDirectory} /> : <fieldset>
    ...
    {!structureMode && <a className="model-source" href={methodUrl ?? 'https://doi.org/10.1111/cgf.12216'} target="_blank" rel="noreferrer">Reconstruction paper ↗</a>}
    {credit && <p className="interaction-hint">{credit}</p>}
  </aside>;
}
