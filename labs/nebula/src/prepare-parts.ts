...
import { compileCssVolume, type PreparedCssVolume } from '../../../src/renderers/css/preparation/volume.js';
import { decomposeFilledComponents, type FilledComponentOptions } from './filled-components.js';
import { createFilledPartsSampler, type FilledVolumePart } from './filled-parts.js';
import { rectifyObservation } from './filled-products.js';
...
function envelope(mapping: ReturnType<typeof createObservationMapping>, bounds: Bounds3): Bounds3 {
...
const stellarRecipe = JSON.parse((await pinned(recipe.stellarPrior)).toString()) as VolumeRecipe;
...
const reference = JSON.parse(referenceBytes.toString()) as PreparedObject;
...
async function bakePart(part: FilledVolumePart): Promise<{ part: FilledVolumePart; data: PreparedCssVolume }> {
  ...
  const key = sha256(Buffer.from(JSON.stringify({ recipe: sha256(recipeBytes), part: part.id, physicalBounds,
    counts, width, implementation: sha256(await readFile('labs/nebula/src/filled-volume.ts')) }))).slice(0, 16);
