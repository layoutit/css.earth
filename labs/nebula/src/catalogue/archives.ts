...
import { fetchTap, tapUrl } from './tap';
import { readArchiveImage, safeArchiveUrl, type ArchiveImage, type ArchiveProvider, type ArchiveQuery, type MessierObject } from './types';
...
export function queryWhere(provider: ArchiveProvider, object: MessierObject) {
  const radius = searchRadiusDegrees(object), ra = object.raDegrees, dec = object.decDegrees;
...
export function pendingQuery(provider: ArchiveProvider, object: MessierObject): ArchiveQuery {
  return { provider, status: 'pending', queriedAt: null, endpoint: endpoints[provider], query: '', radiusDegrees: searchRadiusDegrees(object),
    matchedCount: null, matchedEstimatedBytes: null, matchedUnknownSizeCount: null, images: [] };
}
export async function inventoryQuery(provider: ArchiveProvider, object: MessierObject, maxRecords: number, signal?: AbortSignal): Promise<ArchiveQuery> {
  const base = pendingQuery(provider, object), where = queryWhere(provider, object);
  ...
  try {
    const result = await fetchTap(base.endpoint, query, maxRecords + 1, signal);
