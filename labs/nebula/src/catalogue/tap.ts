...
export async function fetchTap(endpoint: string, query: string, maxRecords: number, signal?: AbortSignal) {
  const url = tapUrl(endpoint, query, maxRecords);
  const response = await fetch(url, { signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(55000)]) : AbortSignal.timeout(55000),
    headers: { Accept: 'application/json', 'User-Agent': 'cssEarth-NebulaLab-MetadataInventory/1' } });
