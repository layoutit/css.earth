...
import { createIntegratedSignalSampler, filterCloudDensityRgba, validateCloudDensityFilter, type CloudDensityFilter } from './cloud-density.js';
...
export function createCloudDensityPreparer(repositoryRoot: string, options: {
  ...
  async function safe(path: string): Promise<string> {
    const candidate = resolve(root, path), offset = relative(root, candidate);
  ...
  const json = async (path: string) => JSON.parse(await readFile(await safe(path), 'utf8'));
  ...
  async function context(request: CloudDensityPreparationRequest): Promise<{
    ...
    const recipeBytes = await readFile(await safe(recipePin.path));
    ...
    const frame = JSON.parse(frameBytes.toString()).properties?.volume;
    ...
    const target = await sharp(targetBytes).flop().removeAlpha().raw().toBuffer({ resolveWithObject: true });
