...
async function fixture(changeResult?: (result: any) => void) {
  ...
  await write(source.path, sourceBytes); await write(modelPin.path, 'synthetic pinned model'); await write('labs/nebula/src/star-removal/star-removal.py', script);
...
test('relocating catalogue metadata preserves saved native output but changed native inputs still fail', async () => {
  const f = await fixture();
  try {
    const result = await f.remove({ ...request, action: 'apply' }) as any;
    ...
    assert.equal(f.calls.length, 1);
    await f.write(f.source.path, 'changed native source');
    await assert.rejects(f.remove.resolveApplied(result.applied.resultId, request.imageId, hash(f.overview)), /hash differs/);
    assert.equal(f.calls.length, 1);
  } finally { await rm(f.root, { recursive: true, force: true }); }
});
