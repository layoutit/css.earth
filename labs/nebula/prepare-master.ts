...
export async function preparePhotoMaster(recipePath: string) {
  ...
  const recipe: MasterExperiment = JSON.parse(recipeBytes.toString());
  ...
  const base = JSON.parse(await readFile(recipe.base.path, 'utf8'));
  ...
  const imported = JSON.parse(await readFile(resolve(target.directory, 'source/import.json'), 'utf8'));
  ...
  const descriptor = JSON.parse(await readFile(resolve(target.directory, 'object.json'), 'utf8'));
