...
- Shape-cloud inference groups near-duplicate detected boundaries before initializing ellipsoidal emission; do not sum competing contours as separate glowing shells. Its source is the pinned working raster already derived from native NOX, with unchanged registration. Depth, thickness, softness and component weights remain authored assumptions. Preserve the full image window and finite shell extent; never fit away geometry errors with photograph-ray normalization. Neutral/textured banks must share geometry and every alpha byte. Keep draft/job identities tied to the source, structure map, geometry, settings and implementation; do not transfer them silently to changed evidence.
