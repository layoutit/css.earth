...
- The visible Detector controls change sensitivity, minimum radius and the proposal limit. Slider changes never process; **Detect** starts a durable, cancellable CPU job and shows proposals on the registered image. Keep the current cloud mounted and its edits intact until **Apply proposals**. Application selects an immutable geometry pin per browser/source; never rewrite the shared catalogue. **Restore previous cloud** returns to the previous pin and its saved edits. Persist pending jobs/proposals and settings by source/map identity; refresh reconnects without detecting again. Higher sensitivity can expose stellar halos and noise, so new proposals are not automatically accepted nebular structures.
