...
Front, oblique, west-side and north-side browser inspection confirms that the coarse cloud still loses filaments, contains stellar halos, and becomes a thin bright ribbon edge-on. **Full visual acceptance remains withheld.** These are the historical local-comparison results, before the app delivery below. Source switches, Original overlay, pose retention and refresh work without processing requests or browser errors; 38 affected tests and strict lab TypeScript checking pass. The fractional-RGB boundary regression fails before the numerical clipping fix.
...
The standalone experiment explicitly depends on the pinned historical cloud snapshot and its retained assets. A normal lab Compile nebula or geometry-slider rebuild still recreates the original four-source result. The shared app delivery is a separate reproducible path: its declared `compositeRecipe` first compiles the pinned current request, then adds this material to that exact new cloud. It restores missing registered observations and native NOX products through their existing owners, preserves the supplied geometry, every alpha byte and the catalogue stars, and writes an immutable result without changing the running lab's publication. It does not need the historical comparison cache. See the [complete app preparation instructions](../../../../docs/nebulae/README.md#reproduce-from-a-clean-checkout).
