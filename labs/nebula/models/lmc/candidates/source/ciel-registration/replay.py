...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='ciel-registration-') as directory:
    ...
    for item in pin['preservedFiles']:
        ...
        destination = target.parent / item['file']
        if source.suffix == '.py':
            # Execute a path-adapted working copy; the verified historical helper stays unchanged.
            destination.write_text(relocated(source.read_text()))
        else:
    ...
    if not gate['pass'] or gate['uniqueMatchedStars'] < 4000 or gate['heldOutCount'] < 1300:
        raise ValueError('Ciel registration did not reproduce its positive evidence.')
    print('REGISTRATION_REPLAY_FINISHED; direction pass:', gate['pass'])
