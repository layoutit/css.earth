...
- In **Shapes**, the visible **Detector** has Sensitivity (25–400%, default 100%), Min. size (minimum ellipse radius, 2–40% of the shorter working-image side; default 7%), and Max. shapes (1–32, default 12). Press **Detect** to prepare new projected proposals. Inspect the solid supported arcs and dashed extrapolations, then **Apply proposals** to initialize their cloud, or **Discard** to keep the current fit. **Restore previous cloud** recovers its geometry and saved edits. Settings/proposals/jobs are source-scoped and survive refresh; merely moving a slider never processes. Detector jobs reuse the existing starless raster, not another NOX run. Sensitivity lowers contrast and edge thresholds; it cannot establish whether faint light belongs to the nebula.
