...
**Choose the method from the evidence:** `density` uses an independent spatial prior; `symmetry` declares an axis/inclination; `inference` preserves image evidence and compares alternative geometry. All methods share the lab shell and source-registration/separation utilities. Helix's current stage is human inspection and filtering of 2D structures from the three aligned ESO images, using their existing native star removal. Review candidates before fitting any new depth hypothesis. Observation colors and shapes can differ by wavelength even when field stars align correctly.
