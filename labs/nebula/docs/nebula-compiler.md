...
### Detector controls and proposal acceptance

The **Detector** is visible in both Shapes views. It has three one-row sliders and an explicit **Detect** button:
...
**Detect** verifies the existing source/map pins, runs the CPU fit in a separate process, and atomically saves an immutable geometry file and recipe receipt. Progress and Cancel remain responsive. It never runs star removal, modifies source pixels, replaces the catalogue, or bakes a cloud. Completion overlays the proposed lines on the registered image. **Apply proposals** selects that geometry for this browser/source and starts its normal cloud preview; **Discard** leaves the previous cloud intact. **Back to cloud** lets the operator inspect the old fit while retaining pending proposals. Changing controls after detection marks the proposals stale until Detect is pressed again.

The previously edited cloud remains mounted while proposals are inspected. Each applied geometry hash has its own existing shape draft and completed preview. **Restore previous cloud** swaps back to that saved geometry and its edits. Detector settings, pending/applied/previous pins, and active job IDs persist against the source and map identity. Refresh observes existing work and completed receipts without silently redetecting. Other images have independent sessions. The CLI reads the same validated settings; its explicit publication behavior remains separate from browser proposals.

Higher sensitivity still admits false positives. The browser check observed VISTA changing from the original 12 proposals at 100%/7%/12 to 32 at 400%/2%/32, including clear stellar halos. This proves the controls reach the detector, not improved astrophysical classification. Default proposal geometry matched the unchanged canonical geometry exactly. The same check verified no cloud bake before Apply, retained current scene, refresh reconnection after server acknowledgement, cancellation, source isolation, unchanged source/map/catalogue hashes, and restoration of an edited cloud. The detector-controls checkpoint passed 227 lab tests, strict lab/changed-test TypeScript, and the lab build; actual layouts were inspected at 1000px and 1600px. Evidence remains local in `geometry-controls-browser`.
