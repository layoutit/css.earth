...
Each image has **Adjust placement** controls: local X/Y/Z position in kpc, rotation, uniform size, and optional X/Y tilt. The numeric fields allow precise adjustments; sliders update the retained image immediately. Rotation and size use the prepared image centre. **Reset placement** restores its exact original sky placement. **Copy positioning** copies JSON with the subject, image, reference catalogue, position, rotation, size, and opacity for sharing or saving. The density cloud and other images are unaffected. Adjustments survive tab and subject switches during this lab session; refreshing the page clears them. These are manual alignment experiments, not changes to source astrometry or production assets.
