...
This is discovery: it neither adds reconstruction subjects nor downloads FITS files, removes stars or compiles clouds. The recognition thumbnails are small DSS2 colour survey cutouts, separate from the archive products below. Published product previews also display inline when available; missing previews are not fabricated from a different observation. Source, Preview and Data links open separately.
