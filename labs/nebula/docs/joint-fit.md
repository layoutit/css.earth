...
The complete [compiler setup](emission-compiler.md#reproduce-from-a-clean-checkout) restores the shared observations, starless working evidence and pinned CDS table. Open **Joint fit** to inspect its independent candidates. The diagnostic's `prepare-joint-fit` entry also accepts `--fit-only` to compare hypotheses without baking. Command defaults use the checked-in source registration and unit source weights; browser inspection adjustments are included only in browser requests.
...
The [emission compiler](emission-compiler.md) now uses a fitted molecular surface as an optional scaffold for multiscale emission. These diagnostic baselines and their velocity holdout remain independent. Keep ionized core, molecular wall and faint halo separate. A calibrated forward spectrum and a defensible likelihood require original spectra or justified centroid errors and a tracer-specific emissivity model; higher texture resolution cannot supply those constraints.
