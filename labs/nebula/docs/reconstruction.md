...
3. Adjust **Brightness**, **Gamma**, **Saturation**, **Detail strength** and **Detail scale**, then press **Preview**. This captures the selected native NOX result and the image's current saved Alignment placement. Merely choosing a source does not bake it.
...
- Paints source chromaticity with optional local contrast and RGB tone: every quad and decoded alpha byte stays identical to Alignment. Outside the photo or at zero RGB, retain explicitly counted neutral density color.
...
The five material sliders save drafts per image; they do not launch work. **Preview** prepares and saves a local result using the existing native starless image. Brightness scales RGB (0–200%); gamma changes RGB midtones (0.25–3, higher is lighter); saturation ranges from neutral gray to stronger source color (0–250%). Detail strength (0–200%) deepens local dark lanes relative to bright knots; scale (2–128 reference pixels) selects finer or broader features. Brightness and gamma also affect neutral material outside image coverage. Catalogue stars and all cloud alpha bytes stay unchanged. These are authored display controls, not new 3D structure or measured radiance.

Changing an Alignment placement requires Preview again. An older saved reconstruction records its original fit; it cannot silently track a later edit to the image. Source switching preserves the camera and loads completed variants without reprocessing.
