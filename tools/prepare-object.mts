 ...
 *
 *   node tools/prepare-object.mts <object-id> [--from <step>]
 *
...
export interface PreparationStep { readonly name: string; readonly purpose: string; readonly commands: (id: string) => Promise<readonly (readonly string[])[]> }
...
export const PREPARATION_STEPS: readonly PreparationStep[] = Object.freeze<PreparationStep[]>([
  ...
  { name: 'prepare', purpose: 'prepare lenses, scene and presentation; refresh derived legend labels and the world frame', commands: async id => [node('tools/objects/dist/prepare-authored.js', id, '--write')] },
...
export async function prepareObject(id: string, { from }: { from?: string } = {}) {
  ...
  for (const step of PREPARATION_STEPS.slice(start)) {
    const commands = await step.commands(id);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const ids = args.filter((argument, index) => argument !== '--from' && (fromIndex < 0 || index !== fromIndex + 1));
  if (ids.length !== 1) throw new TypeError(`Usage: prepare-object <object-id> [--from <step>]; steps: ${PREPARATION_STEPS.map(step => step.name).join(', ')}.`);
  if (!await prepareObject(ids[0]!, from === undefined ? {} : { from })) process.exitCode = 1;
}
