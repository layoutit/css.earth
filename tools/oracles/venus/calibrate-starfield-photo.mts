...
const frames: Awaited<ReturnType<typeof prepareFrame>>[] = [];
...
async function prepareFrame(bytes: Buffer, latitude: number, longitude: number) {
...
function scoreRotation(xDegrees: number, yDegrees: number, zDegrees: number) {
...
function samplePhoto(direction: readonly number[]) {
...
function cameraToEquatorial(direction: readonly number[]) {
  return normalize([
...
function transformEuler(direction: readonly number[], x: number, y: number, z: number) {
  return rotateZ(rotateY(rotateX(direction, x), y), z);
}

function transform(matrix: readonly (readonly number[])[], direction: readonly number[]) {
  return [
...
function centeredStats(values: readonly number[], valid: readonly boolean[]) {
  let sum = 0;
  let count = 0;
...
function rotateZ([x, y, z]: readonly number[], degrees: number) {
  const radians = degrees * Math.PI / 180;
  const cosine = Math.cos(radians);
  const sine = Math.sin(radians);
...
function endpointId(latitude: number, longitude: number) {
