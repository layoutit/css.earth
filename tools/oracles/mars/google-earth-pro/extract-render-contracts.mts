...
import { parseNativeSnapshot, type NativeSnapshot, type NativeTexture } from "./render-contract-values.mts";
import { parseWindowAudit, type WindowAudit } from "./native-evidence.mts";
import type { ViewInfo } from "./controller.mts";
import type { MarsOraclePose } from "./profile.mts";

type CapturedState = Awaited<ReturnType<typeof captureIsolatedState>> & { snapshots: readonly NativeSnapshot[] };
interface Capture { readonly [key: string]: unknown; pose: MarsOraclePose; requestedCamera: ViewInfo;
  observedCamera: ViewInfo; normalizationDelta: ReturnType<typeof numericDelta>; off: CapturedState; on: CapturedState;
  sunDifference: Awaited<ReturnType<typeof compareAndRenderDifference>>; }
interface BufferResource { role: string; kind: "buffer"; id: number; bytes: number; dumpPath?: string; }
type Resource = ReturnType<typeof textureResource> | BufferResource;
...
execFileSync(process.execPath, [
  resolve(import.meta.dirname, "build-render-contract-hook.mts"),
], { stdio: "inherit" });
const hookManifest = parseHookManifest(parseJson(await readFile(hookManifestPath, "utf8")));
...
async function captureIsolatedState({
  ...
  frameRoot,
  suffix,
}: { pose: MarsOraclePose; frameNumber: string; sun: boolean; frameRoot: string; suffix: string }) {
...
async function ensureInstrumentedOracle(manifest: ReturnType<typeof parseHookManifest>) {
...
async function waitForRevisionSnapshots(revision: number) {
  for (let attempt = 0; attempt < 100; attempt += 1) {
...
async function snapshotsForRevision(revision: number) {
  try {
    const source = await readFile(snapshotLogPath, "utf8");
    return source.trim().split("\n").filter(Boolean).map((line) =>
      parseNativeSnapshot(parseJson(line))).filter((entry) => entry.revision === revision);
  } catch (error) {
...
function bestSnapshots(snapshots: readonly NativeSnapshot[]) {
  const selected = new Map<string, { score: number; snapshot: NativeSnapshot }>();
...
function deriveContracts(captureEntries: readonly Capture[], interactionEntries: Awaited<ReturnType<typeof captureInteractionContract>>, resources: Awaited<ReturnType<typeof materializeResources>>) {
...
function pickDrawContract(snapshot: NativeSnapshot) {
  return Object.freeze({
...
async function materializeResources(captureEntries: readonly Capture[], pid: number) {
  ...
  const firstStars = captureEntries[0].off.snapshots.find(({ classification }) =>
    classification.catalogueStars);
  if (!firstSky || !firstStars) throw new Error("Missing sky or catalogue-star renderer contract.");
  ...
  const sunCandidates = new Map<string, ReturnType<typeof textureResource> & { observedAtPose: string }>();
...
function textureResource(role: string, texture: NativeTexture) {
  return Object.freeze({
    role,
...
function textureRecords(snapshots: readonly NativeSnapshot[]) {
  return snapshots.flatMap(({ samplers = {} }) => Object.values(samplers))
    .filter(({ texture, width, height }) => texture > 0 && width > 0 && height > 0);
}

function textureSet(snapshots: readonly NativeSnapshot[]) {
  return new Set(textureRecords(snapshots).map(({ texture, width, height }) =>
    `${texture}:${width}x${height}`));
}

async function compareAndRenderDifference(leftPath: string, rightPath: string, outputPath: string) {
...
async function createSunContactSheet(captureEntries: readonly Capture[], outputPath: string) {
  ...
  const panels: [string, string][] = selected.flatMap((entry): [string, string][] => [
...
async function readControlLog() {
  ...
  return source.trim().split("\n").filter(Boolean).map((line) => {
    ...
    return { ...fields, revision: integer(Number(fields.revision)), pid: integer(Number(fields.pid)),
      atmosphere: fields.atmosphere === "1", sun: fields.sun === "1" };
  });
}

function numericDelta(left: ViewInfo, right: ViewInfo) {
...
function largestComponent(mask: Uint8Array, width: number, height: number) {
  ...
  let best: { pixelCount: number; bounds: Bounds | null } = Object.freeze({ pixelCount: 0, bounds: null });
...
async function safeDirectoryNames(path: string) {
  try {
...
function parseHookManifest(value: unknown) {
  const row = object(value), artifacts = object(row.artifacts), hook = object(artifacts.hook);
  return { ...row, artifacts: { ...artifacts, hook: { ...hook, path: text(hook.path) } } };
}
