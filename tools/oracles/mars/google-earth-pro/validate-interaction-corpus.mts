...
import { loadInteractionCorpus, type InteractionEvent } from "./interaction-corpus.mts";
...
for (const scenario of corpus.scenarios) {
  ...
  for (const event of scenario.events) {
    ...
    for (const forbidden of ["x", "y", "clientX", "clientY",
      ...
      assert.equal(Object.getOwnPropertyDescriptor(event, forbidden)?.value, undefined,
...
function validateCamera(camera: { latitude: number; longitude: number; distance: number; tilt: number; azimuth: number }, id: string) {
