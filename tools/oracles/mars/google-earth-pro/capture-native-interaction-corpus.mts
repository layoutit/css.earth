import { object, text, integer, boolean, parseJson, isNodeError } from "./oracle-values.mts";
import { parseWindowAudit, type WindowAudit } from "./native-evidence.mts";
import { parseCaptureEvent, eventNumber, eventString, parseLayerManifest, parseCalibrationManifest, parseNativeRegistration, parseBrowserRegistration, parseMapping, type EventRecord } from "./native-capture-values.mts";
import type { NativeGesture } from "./rendered-motion-inputs.mts";
import type { ViewInfo } from "./controller.mts";
import type { InteractionScenario } from "./interaction-corpus.mts";
import type { NativeMotionFrame } from "./native-motion-trace-reader.mts";
type DriverEntry = Awaited<ReturnType<typeof nativeInteractionScenarios>>[number];
type CapturedRun = Awaited<ReturnType<typeof captureRepeat>>;
interface HookPaths { layerControlPath: string; layerAuditPath: string; controlLogPath: string; installLogPath: string; drawAuditPath: string; rawRoot: string; mappingPath: string; bindingLogPath: string }
...
import { loadInteractionCorpus } from "./interaction-corpus.mts";
import {
  decodeNativeMotionTrace,
  matrixMaximumError,
  nativeScenarioTrace,
} from "./native-motion-trace-reader.mts";
import { nativeInteractionScenarios } from "./native-interaction-driver.mts";
...
const launchScript = resolve(import.meta.dirname, "launch-headless.mts");
const calibrationMapScript = resolve(
  import.meta.dirname,
  "prepare-google-calibration-map.mts",
);
const calibrationVerificationScript = resolve(
  import.meta.dirname,
  "prepare-calibration-surface.mts",
);
...
const [
  ...
  readJson(layerManifestPath).then(parseLayerManifest),
  readJson(calibrationManifestPath).then(parseCalibrationManifest),
  readJson(nativeRegistrationPath).then(parseNativeRegistration),
  readJson(browserRegistrationPath).then(parseBrowserRegistration),
  readFile(corpusPath),
]);
assert.equal(
...
const calibrationVerification = object(parseJson((await execFileAsync(
  process.execPath,
  [calibrationVerificationScript, "--verify"],
  { cwd: workspaceRoot, maxBuffer: 16 * 1024 * 1024 },
 )).stdout), "calibration verification");
assert.equal(
...
const geometryFor = (scenario: InteractionScenario) => {
...
assert.ok(lastRun, "No native capture runs completed.");
...
const gates = Object.freeze({
  ...
  noInstrumentationLongTask: runs.every(({ scenarios }) =>
    scenarios.every(({ trace }) =>
      (trace.instrumentation.maximumMilliseconds ?? Infinity) <
  ...
  calibrationBoundAfterTiming: lastRun.calibration !== null &&
    lastRun.calibration.mappingCount > 0 &&
    lastRun.calibration.bindingCount > 0,
  visualReplayCapturedAfterTiming: lastRun.visualReplay !== null &&
    lastRun.visualReplay.scenarios.every(({ frames }) =>
      frames.length === 4),
...
async function captureRepeat({ repeat, captureVisualReplay }: { repeat: number; captureVisualReplay: boolean }) {
  ...
  let oraclePid: number | null = null;
  let termination = null;
  const headless: ReturnType<typeof headlessRecord>[] = [];
  const measured: Awaited<ReturnType<typeof runMeasuredScenario>>[] = [];
  ...
  try {
    ...
    const launch = object(parseJson(launchResult.stdout), "native launch");
    oraclePid = integer(launch.pid, "native pid");
    await waitForEvent(
      launchRoot,
    ...
    for (const measurement of measured) {
      ...
      const revisionEvents = allEvents.filter(({ revision }) =>
        typeof revision === "number" && measurement.revisions.includes(revision));
...
async function runMeasuredScenario({ entry, revision, inputPath, launchRoot, oraclePid }: { entry: DriverEntry; revision: number; inputPath: string; launchRoot: string; oraclePid: number }) {
  ...
  const traceTailMilliseconds = Math.max(
    1600,
    Math.ceil(
      settlementMilliseconds - lastGesture(entry.events).atMilliseconds + 250,
    ),
  );
...
async function captureVisualReplays({ runRoot, inputPath, launchRoot, oraclePid }: { runRoot: string; inputPath: string; launchRoot: string; oraclePid: number }) {
  ...
  for (let index = 0; index < driverScenarios.length; index += 1) {
    ...
    await waitForStableCamera();
    await delay(200);
    const finalInputOffset = lastGesture(entry.events).atMilliseconds;
  ...
  async function captureVisualFrame({ scenarioRoot, index, targetMilliseconds, actualMilliseconds }: { scenarioRoot: string; index: number; targetMilliseconds: number; actualMilliseconds: number }) {
...
function qualifyScenarios(runs: readonly CapturedRun[]) {
  return driverScenarios.map(({ scenario }) => {
    const entries = runs.map((run) => {
      const entry = run.scenarios.find(({ id }) => id === scenario.id);
      assert.ok(entry, `Missing scenario ${scenario.id}.`);
      return entry;
    });
...
function summarizeInput(requested: readonly NativeGesture[], revisions: readonly number[], recorded: readonly EventRecord[]) {
  const accepted = recorded.filter(({ event }) => event === "native-input-accepted").map((event) => ({
    ...event, id: eventString(event, "id"), acceptedMonotonicSeconds: eventNumber(event, "acceptedMonotonicSeconds"),
  }));
...
function summarizeRun(run: CapturedRun) {
  return Object.freeze({
    repeat: run.repeat,
    runRoot: run.runRoot,
    process: run.process,
...
async function prepareCalibrationBinding({ oraclePid, layerControlPath, layerAuditPath, controlLogPath, installLogPath, drawAuditPath, rawRoot, mappingRoot, mappingPath, bindingLogPath }: HookPaths & { oraclePid: number; mappingRoot: string }) {
  attachLayerHook({
    pid: oraclePid,
  ...
  const mapping = parseMapping(parseJson((await execFileAsync(process.execPath, [
...
function attachLayerHook({ pid, hookPath, layerControlPath, layerAuditPath, controlLogPath, installLogPath, drawAuditPath, rawRoot, mappingPath, bindingLogPath }: HookPaths & { pid: number; hookPath: string }) {
...
async function waitForHookInstallation({ pid, hookPath, installLogPath }: { pid: number; hookPath: string; installLogPath: string }) {
...
async function writeLayerControl(path: string, { mode, atmosphere, sun, revision }: { mode: string; atmosphere: boolean; sun: boolean; revision: number }) {
...
async function waitForLayerControl(path: string, revision: number) {
...
async function waitForRevisionDelivered(launchRoot: string, revision: number, ids: readonly string[]) {
...
async function requestGracefulExit({ inputPath, launchRoot, pid }: { inputPath: string; launchRoot: string; pid: number }) {
...
async function writeInputControl(path: string, revision: number, events: readonly (NativeGesture & { sourceOffsetMilliseconds?: number })[], { terminate = false }: { terminate?: boolean } = {}) {
...
async function waitForEvent(launchRoot: string, predicate: (event: EventRecord) => boolean, timeout = 20_000) {
  const started = Date.now();
...
async function readEvents(launchRoot: string) {
  try {
    const source = await readFile(resolve(launchRoot, "events.jsonl"), "utf8");
    return source.trim().split("\n").filter(Boolean).map((line) =>
      parseCaptureEvent(parseJson(line)));
  } catch (error) {
...
async function readJsonLines(path: string) {
  try {
    const source = await readFile(path, "utf8");
    return source.trim().split("\n").filter(Boolean).map((line) =>
      object(parseJson(line), "binding record"));
  } catch (error) {
    if (isNodeError(error, "ENOENT")) return [];
    throw error;
  }
...
async function cropNativeScreenshot(rawPath: string, path: string) {
  ...
  assert.ok(metadata.height !== undefined && metadata.height >= crop.top + crop.height);
  await sharp(rawPath).extract(crop).png().toFile(path);
}

function headlessRecord(pid: number, stage: string) {
...
function auditWindows(pid: number) {
  return parseWindowAudit(parseJson(execFileSync(windowAuditPath, [String(pid)], {
    encoding: "utf8",
  })));
}

function assertHeadless(audit: WindowAudit, pid: number, stage: string) {
...
function frameSequenceGapCount(frames: readonly NativeMotionFrame[]) {
  let count = 0;
...
function readProcessState(pid: number) {
  try {
...
function camera(latitude: number, longitude: number, distance: number) {
  return Object.freeze({ latitude, longitude, distance, tilt: 0, azimuth: 0 });
}

function sameCamera(first: ViewInfo, second: ViewInfo) {
...
function numericDelta(first: ViewInfo, second: ViewInfo) {
  return Object.freeze({
...
function longitudeDelta(first: number, second: number) {
...
function lldbSetEnvironment(name: string, value: string) {
  return `expr (int)setenv(${lldbString(name)}, ${lldbString(value)}, 1)`;
}

function lldbString(value: string) {
...
function parseArguments(args: readonly string[]) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!["--set", "--repeat", "--output"].includes(key) ||
        value === undefined) {
      throw new Error(
        "Usage: capture-native-interaction-corpus.mts --set training " +
        "--repeat 3 --output <native-training>",
      );
    }
    values[key.slice(2)] = value;
  }
  if (values.set !== "training" && values.set !== "heldout" && values.set !== "all") throw new Error("Invalid native corpus set.");
  if (!values.output || !values.repeat) throw new Error("Native corpus output and repeat are required.");
  return Object.freeze({
    set: values.set,
...
function assertSafeOutputRoot(path: string) {
...
function lastGesture(events: readonly NativeGesture[]): NativeGesture {
  const event = events.at(-1);
  if (!event) throw new Error("A native interaction scenario needs at least one event.");
  return event;
}
