...
import { object, array, text, finite, integer, boolean, parseJson, isNodeError } from "./oracle-values.mts";
...
const cacheExportScript = resolve(
  import.meta.dirname,
  "export-google-imagery-cache.mts",
);
...
const calibrationVerification = parseVerification(parseJson((await execFileAsync(
  process.execPath,
  [calibrationScript, "--verify"],
  { cwd: workspaceRoot, maxBuffer: 16 * 1024 * 1024 },
)).stdout));
const cacheExport = object(parseJson((await execFileAsync(
  process.execPath,
  [
    cacheExportScript,
    "--cache-root",
    resolve(homedir(), "Library/Caches/Google Earth"),
    "--output",
    cacheIndexPath,
  ],
  { cwd: workspaceRoot, maxBuffer: 16 * 1024 * 1024 },
...
const calibrationMasterPath = resolve(dirname(calibrationManifestPath), calibrationManifest.source.path);
...
try {
  ...
  const launch = object(parseJson(launchResult.stdout));
  const launchedPid = integer(launch.pid, "launched process id");
  if (launchedPid <= 0) throw new TypeError("Expected a positive native process id.");
  oraclePid = launchedPid;
  ...
  for (let index = 0; index < poses.length; index += 1) {
    ...
    const mapping = parseMappingResult(parseJson((await execFileAsync(
      process.execPath,
      [
    ...
    const mappingReport = parseMappingReport(await readJson(mapping.reportPath));
    ...
    const bindingValidation = (await readJsonLines(bindingLogPath))
      .filter((binding) => binding.revision === revision).map(parseBinding);
...
function pose(id: string, latitude: number, longitude: number, distance: number, coverage: readonly string[]) {
...
function attachHook(pid: number, hookPath: string) {
...
async function waitForHookInstallation(pid: number, hookPath: string) {
...
async function writeLayerControl({ mode, atmosphere, sun, revision }: { mode: string; atmosphere: boolean; sun: boolean; revision: number }) {
...
async function waitForControlRevision(revision: number) {
...
async function waitForEvent(predicate: (event: Record<string, unknown>) => boolean, timeoutMilliseconds = 25_000) {
...
async function waitForStableFile(path: string, minimumLines: number) {
  let previousSize = -1;
  let stable = 0;
  for (let attempt = 0; attempt < 100; attempt += 1) {
    try {
...
async function waitForRevisionEvidence(path: string, revision: number) {
  let previousCount = -1;
  let stable = 0;
  for (let attempt = 0; attempt < 160; attempt += 1) {
...
async function waitForStableCamera({ appName }: { appName: string }) {
  const started = Date.now();
  const samples = [];
  let stableReadings = 0;
  let previous: ViewInfo | null = null;
...
async function analyzeLandmarkSignature(path: string) {
...
async function cropSceneCapture(rawCapture: Awaited<ReturnType<typeof saveScreenShot>>, path: string) {
...
async function createContactSheet(captures: readonly { id: string; final: { path: string } }[], path: string) {
  const tileWidth = 480;
...
function coverageReport(captures: readonly { coverage: readonly string[]; camera: ViewInfo }[]) {
...
function cameraResidual(requested: ViewInfo, observed: ViewInfo) {
  return Object.freeze({
...
function assertHeadless(audit: WindowAudit, pid: number, phase: string) {
...
async function terminateExactProcess(pid: number) {
...
function parseArguments(args: readonly string[]) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!["--renderer", "--poses", "--output"].includes(key) ||
        value === undefined) {
      throw new Error(
        "Usage: capture-calibration-registration.mts --renderer native " +
...
function parseVerification(value: unknown) {
  const row = object(value); return { ...row, sourceDecodedRgbaSha256: text(row.sourceDecodedRgbaSha256) };
}
function parseLayerManifest(value: unknown) {
  const row = object(value), hook = object(row.hook); return { ...row, hook: { ...hook, path: text(hook.path) } };
}
function parseCalibrationManifest(value: unknown) {
  const row = object(value), source = object(row.source); return { ...row, source: { ...source, path: text(source.path),
    encodedSha256: text(source.encodedSha256), decodedRgbaSha256: text(source.decodedRgbaSha256) } };
}
function parseMappingResult(value: unknown) {
  const row = object(value); return { ...row, ok: boolean(row.ok), mappingCount: integer(row.mappingCount), reportPath: text(row.reportPath) };
}
function parseMappingReport(value: unknown) {
  const row = object(value); return { ...row, qualification: text(row.qualification), mappings: array(row.mappings).map(value => {
    const mapping = object(value), tile = object(mapping.calibrationTile); return { ...mapping,
      registrationQualification: text(mapping.registrationQualification), calibrationTile: { ...tile, uploadRgbaSha256: text(tile.uploadRgbaSha256) } };
  }) };
}
function parseBinding(value: unknown) {
  const row = object(value); return { ...row, revision: integer(row.revision), mapped: boolean(row.mapped), decodedRgbaSha256: text(row.decodedRgbaSha256) };
}
