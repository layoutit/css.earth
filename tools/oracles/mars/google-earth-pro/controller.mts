export interface ViewInfo { latitude: number; longitude: number; distance: number; tilt: number; azimuth: number }
export interface ViewRequest extends Pick<ViewInfo, "latitude" | "longitude" | "distance"> { tilt?: number; azimuth?: number; speed?: number; appName?: string }
interface AppOptions { appName?: string }
interface CoordinateOptions extends AppOptions { x: number; y: number }
...
export async function getViewInfo({ appName = DEFAULT_APP_NAME }: AppOptions = {}) {
...
export async function getCurrentVersion({ appName = DEFAULT_APP_NAME }: AppOptions = {}) {
  const output = await runGoogleEarthAppleScript(
    appName,
    "GetCurrentVersion",
  );
...
export async function waitForStreaming({
  ...
  stableReadings = 4,
}: AppOptions & { timeoutMilliseconds?: number; intervalMilliseconds?: number; stableReadings?: number } = {}) {
...
export function parseViewInfo(value: string): Readonly<ViewInfo> {
  ...
  return Object.freeze({ latitude: fields.latitude, longitude: fields.longitude, distance: fields.distance, tilt: fields.tilt, azimuth: fields.azimuth });
}

async function runGoogleEarthAppleScript(appName: string, command: string) {
...
async function runRawAppleScript(script: readonly string[]) {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  ...
  if (command === "get-view") {
    ...
    result = await setViewInfo(requireViewRequest(parseNamedNumbers(args)));
  } else if (command === "wait-stream") {
    result = await waitForStreaming();
  } else if (command === "terrain") {
    result = await getPointOnTerrain(requireCoordinates(parseNamedNumbers(args)));
  } else if (command === "move") {
    result = await moveCamera(requireCoordinates(parseNamedNumbers(args)));
...
function parseNamedNumbers(args: readonly string[]) {
...
function requireViewRequest(values: Record<string, number>): ViewRequest {
  for (const field of ["latitude", "longitude", "distance"]) if (!Number.isFinite(values[field])) throw new TypeError(`Missing --${field}.`);
  return { latitude: values.latitude, longitude: values.longitude, distance: values.distance, tilt: values.tilt, azimuth: values.azimuth, speed: values.speed };
}
function requireCoordinates(values: Record<string, number>): CoordinateOptions {
  if (!Number.isFinite(values.x) || !Number.isFinite(values.y)) throw new TypeError("Both --x and --y are required.");
