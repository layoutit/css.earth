import { parseNativeSnapshot, type NativeSnapshot } from "./render-contract-values.mts";
import { object, text, finite, integer, parseJson, isNodeError } from "./oracle-values.mts";
import { parseWindowAudit } from "./native-evidence.mts";
import type { ViewInfo } from "./controller.mts";
interface RequestedCamera { latitude: number; longitude: number; distance: number; tilt: number; heading: number }
interface PathPlan extends RequestedCamera { id: string; family: string }
interface GranularSample extends Record<string, unknown> {
  index: number; familyIndex: number; requested: PathPlan; observed: ViewInfo;
  skyMap: NativeSnapshot; catalogue: NativeSnapshot; sun: NativeSnapshot | null;
  sunPresentation: { visibility: string; halfExtentPerCameraDistance?: number };
}
...
import { getViewInfo, saveScreenShot, setViewInfo } from "./controller.mts";
...
function buildPathPlan() {
  ...
  const add = (family: string, values: readonly RequestedCamera[]) => {
...
function sequence<T,>(length: number, create: (index: number) => T) {
  return Array.from({ length }, (_, index) => Object.freeze(create(index)));
}

function attachHook(pid: number) {
...
async function waitForSnapshots(revision: number) {
  for (let attempt = 0; attempt < 160; attempt += 1) {
    try {
      const source = await readFile(snapshotPath, "utf8");
      const snapshots = source.trim().split("\n").filter(Boolean)
        .map((line) => parseNativeSnapshot(parseJson(line)))
...
function isSunDraw(entry: NativeSnapshot) {
...
async function deriveSunPresentation(draw: NativeSnapshot) {
  ...
  return Object.freeze({
    ...
    cameraDistance,
    localHalfExtent,
    halfExtentPerCameraDistance: (localHalfExtent ?? 0) / cameraDistance,
  });
}

async function deriveGranularContract(samples: readonly GranularSample[], shaderManifest: Awaited<ReturnType<typeof collectShaderManifest>>) {
  ...
  const billboardRatios = samples.flatMap(({ sunPresentation }) =>
    typeof sunPresentation.halfExtentPerCameraDistance === "number" && Number.isFinite(sunPresentation.halfExtentPerCameraDistance)
  ...
  if (firstSun == null) throw new Error("Sun was never drawn.");
...
async function readCompletedSamples(): Promise<GranularSample[]> {
  try {
    const source = await readFile(sampleLogPath, "utf8");
    return source.trim().split("\n").filter(Boolean).map((line) =>
      Object.freeze(parseGranularSample(parseJson(line))));
  } catch (error) {
...
function decodeFloatRecords(bytes: Buffer, components: number, strideBytes: number) {
  const records = [];
...
function project(matrix: readonly number[], position: readonly number[]) {
...
function numericDelta(requested: RequestedCamera, observed: ViewInfo) {
  return Object.freeze({
...
function angularDelta(actual: number, expected: number) {
  return ((actual - expected + 180) % 360 + 360) % 360 - 180;
}

function vectorLength(value: readonly number[]) {
...
function assertHeadless(pid: number, stage: string) {
  const audit = parseWindowAudit(parseJson(execFileSync(windowAuditPath, [String(pid)], {
    encoding: "utf8",
  })));
...
function parseGranularSample(value: unknown): GranularSample {
  const entry = object(value, "completed granular sample");
  const requested = object(entry.requested, "requested camera");
  const observed = object(entry.observed, "observed camera");
  const presentation = object(entry.sunPresentation, "Sun presentation");
  return { ...entry, index: integer(entry.index), familyIndex: integer(entry.familyIndex),
    requested: { ...requested, id: text(requested.id), family: text(requested.family), latitude: finite(requested.latitude), longitude: finite(requested.longitude), distance: finite(requested.distance), tilt: finite(requested.tilt), heading: finite(requested.heading) },
    observed: { ...observed, latitude: finite(observed.latitude), longitude: finite(observed.longitude), distance: finite(observed.distance), tilt: finite(observed.tilt), azimuth: finite(observed.azimuth) },
    skyMap: parseNativeSnapshot(entry.skyMap), catalogue: parseNativeSnapshot(entry.catalogue), sun: entry.sun === null ? null : parseNativeSnapshot(entry.sun),
    sunPresentation: { ...presentation, visibility: text(presentation.visibility),
      ...(presentation.halfExtentPerCameraDistance === undefined ? {} : { halfExtentPerCameraDistance: finite(presentation.halfExtentPerCameraDistance) }) },
  };
}
