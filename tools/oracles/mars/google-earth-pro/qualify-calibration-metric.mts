...
import { parseNativeRegistration, parseBrowserRegistration, type Disc, type DecodedImage, type PairOptions } from "./calibration-metric-values.mts";
...
const [nativeReport, browserReport] = await Promise.all([
  readJson(nativeReportPath).then(parseNativeRegistration),
  readJson(browserReportPath).then(parseBrowserRegistration),
]);
assert.equal(
...
for (let index = 0; index < nativeReport.captures.length; index += 1) {
  ...
  const browserCapture: ReturnType<typeof parseBrowserRegistration>["densities"][number]["captures"][number] = browserDensity.captures[index];
...
export function localLandmarkFlow(native: DecodedImage, browser: DecodedImage, mask: Uint8Array) {
...
function compareEdgeAntialiasing(native: DecodedImage, browser: DecodedImage, nativeDisc: Disc, browserDisc: Disc) {
...
function edgeAntialiasing(image: DecodedImage, disc: Disc) {
  const widths = [];
...
function sampleRgb(image: DecodedImage, x: number, y: number) {
...
function crossing(profile: readonly { offset: number; normalized: number }[], threshold: number) {
...
function featurePlane(image: DecodedImage) {
...
function compareFeatureCentroids(native: DecodedImage, browser: DecodedImage, mask: Uint8Array) {
  const definitions = Object.freeze({
    cyan: (red: number, green: number, blue: number) => green > 120 && blue > 120 &&
      green + blue > red * 3,
    magenta: (red: number, green: number, blue: number) => red > 120 && blue > 70 &&
      red + blue > green * 3,
    yellow: (red: number, green: number, blue: number) => red > 140 && green > 110 &&
      red + green > blue * 4,
    white: (red: number, green: number, blue: number) => Math.min(red, green, blue) > 165 &&
    ...
    ([name, predicate]) => {
      ...
      return [name, Object.freeze({
        ...
        residualPixels: reference.count >= 5 && candidate.count >= 5
          ? Math.hypot(
            (candidate.x ?? NaN) - (reference.x ?? NaN),
            (candidate.y ?? NaN) - (reference.y ?? NaN),
          )
          : null,
      })];
    },
...
function centroid(image: DecodedImage, mask: Uint8Array, predicate: (red: number, green: number, blue: number) => boolean) {
  let count = 0;
  let xTotal = 0;
  let yTotal = 0;
...
export function intersectionDiscMask(width: number, height: number, first: Disc, second: Disc, inset: number) {
...
async function writeMasked(path: string, image: DecodedImage, mask: Uint8Array) {
...
export function maskedColorDelta(native: DecodedImage, browser: DecodedImage, mask: Uint8Array) {
  let channelTotal = 0;
  let pixelCount = 0;
  for (let pixel = 0; pixel < mask.length; pixel += 1) {
    if (!mask[pixel]) continue;
...
async function shiftImage(sourcePath: string, outputPath: string, x: number, y: number) {
...
function score(residuals: readonly number[]) {
...
function summarize(poses: readonly Awaited<ReturnType<typeof evaluatePair>>[]) {
  const mean = (selector: (pose: Awaited<ReturnType<typeof evaluatePair>>) => number) => poses.reduce((sum, pose) =>
...
function summarizeCoverage(poses: readonly Awaited<ReturnType<typeof evaluatePair>>[]) {
