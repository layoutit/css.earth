...
import { parseJson, isNodeError } from "./oracle-values.mts";
...
import { GOOGLE_EARTH_PRO_MARS_POSES } from "./profile.mts";
...
execFileSync("/usr/bin/lldb", [
  ...
  "-o", lldbSetEnvironment("CSSMARS_ORACLE_CONTRACT_UNIFORM_LOG_V10", uniformPath),
  "-o", lldbSetEnvironment("CSSMARS_ORACLE_CONTRACT_TEXTURE_ROOT_V10", rawRoot),
  "-o", lldbSetEnvironment("CSSMARS_ORACLE_CONTRACT_INSTALL_LOG_V10", installPath),
...
const firstDraw = samples.find(({ rendererDraw }) => rendererDraw !== null)
  ?.rendererDraw;
if (firstDraw == null) throw new Error("Default Sun billboard was never bound.");
...
async function derivePresentation(draw: NativeSnapshot) {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    try {
      ...
      const snapshots = source.trim().split("\n").filter(Boolean)
        .map((line) => parseNativeSnapshot(parseJson(line))).filter((entry) =>
...
async function locateTextureDump(texture: NativeTexture) {
...
function assertHeadless(pid: number) {
