...
export function prepareAtmosphereFrame({
  ...
  const terms = layers.map(layer => {
    if (!(layer.scaleHeightKm > 0) || !Number.isFinite(layer.scaleHeightKm) ||
        ...
        layer.scatteringPerKm.some(value => !Number.isFinite(value) || value < 0) ||
    ...
    return { height: layer.scaleHeightKm / radiusKm,
      coefficients: layer.scatteringPerKm.map(value => value * radiusKm),
...
export function compositePreparedAtmosphere(base, atmosphere) {
