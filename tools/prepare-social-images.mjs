...
try {
  ...
  for (const object of OBJECTS.filter(({ id }) => ids.includes(id))) {
    ...
    if (values.card) {
      ...
      const background = cardBackground.match(/\d+/g).map(Number);
      ...
      for (let attempt = 0; attempt < 8; attempt++) {
        ...
        await page.evaluate(({ id, scale }) => {
          ...
          if (!camera) throw new Error('Portrait fitting requires a development or performance server.');
          camera.setState({ zoom: camera.state().zoom * scale });
        }, { id: object.id, scale });
