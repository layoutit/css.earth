...
test('restores a missing transport from its pinned runtime without rebaking or repinning', async () => {
  ...
  try {
    ...
    assert.deepEqual(await restoreObjectJson(['thetis'], root), { objects: 1, written: 1, skipped: 0, reused: 0 }, 'a changed runtime rewrites the transport');
    assert.deepEqual(await readFile(resolve(directory, 'object.json')), descriptorBytes, 'restoration never changes the descriptor');
