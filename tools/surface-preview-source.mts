...
export const parsePreviewControls=shape({controls:array(shape({id:text,surfaceUrl:optional(text),thumbnailUrl:optional(text),view:optional(text),overlayId:optional(text)}))});
export const parsePolarPreview=shape({dimensions:shape({width:number,height:number}),packing:shape({latitudeBoundsDegrees:array(number),gutter:number}),lenses:array(shape({id:text,files:shape({surface:text})}))});
