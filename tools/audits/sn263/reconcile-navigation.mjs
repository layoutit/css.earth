...
for (const [name, ref] of Object.entries(refs)) {
  ...
  banks[name] = (await import('data:text/javascript;base64,' + source.toString('base64'))).PREPARED_NAVIGATION_MARKERS;
}
...
const markers = {}, report = { refs, objects: descriptors.length, method: 'Copy decoded committed tiles by body identity and encode losslessly. Visible RGB and alpha must match each committed input tile exactly. Fully transparent RGB is not a visible value.', densities: [], contexts: [] };
for (const [index, descriptor] of descriptors.entries()) {
  const name = additions.has(descriptor.planetId) ? 'companions' : 'main';
  const previous = banks[name][descriptor.planetId];
  assert.ok(previous, descriptor.planetId);
  assert.deepEqual(descriptor, JSON.parse(original(refs[name], `src/planets/${descriptor.planetId}/source/preparation/navigation.json`)));
