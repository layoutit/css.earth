...
try{
 for(const dpr of (navigationOnly||defaultsOnly)?[1]:[1,2]){
  ...
  await page.route(new RegExp('/scenes/('+ids.join('|')+')/'),async route=>{
   ...
   const bytes=await readFile(resolve(reportDirectory,'fresh-runtime',id,filename));
  ...
  for(const id of navigationOnly?[]:ids){
   ...
   if(dpr===1){
    ...
    await page.locator('input[name="shadows"]').evaluate(node=>node.click());
    ...
    await page.locator('input[name="shadows"]').evaluate(node=>node.click());
    await page.locator('input[name="orbit"]').evaluate(node=>node.click());
    assert.equal(await page.locator('input[name="orbit"]').isChecked(),true);
    await page.locator('input[name="orbit"]').evaluate(node=>node.click());
    assert.equal(await leaves.count(),faceCount);
    results.at(-1).optInControls={shadows:true,bodyPixelsChanged:true,orbit:true};
   }
  }
  if(dpr===1&&!defaultsOnly){
   ...
   const listed=await entries.evaluateAll(nodes=>nodes.map(n=>n.dataset.objectId));for(const id of bodies.filter(body=>body.classification==='trans-neptunian').map(body=>body.id))assert.ok(listed.includes(id));
   ...
   for(const id of [mobileId,'gkunhomdima']){
    ...
    await page.goto(`${origin}/${id}/`,{waitUntil:'networkidle'});
    await page.waitForFunction(id=>document.documentElement.dataset.ready==='true'&&document.querySelector('.planet-stage')?.dataset.objectId===id,id);
    const layout=await page.evaluate(()=>{const viewport=document.querySelector('.planet-viewport'),r=viewport.getBoundingClientRect();return{position:getComputedStyle(viewport).position,viewport:r.toJSON(),cardTop:document.querySelector('.planet-sidebar').getBoundingClientRect().top,overflow:document.documentElement.scrollWidth>innerWidth,diagnostics:window.__cssEarth!==undefined};});
