...
test("publishes the complete prepared batch atomically", async (context) => {
  ...
  await publishObjectInformation(preparedBatch(), { outputDirectory });

  const files = (await readdir(outputDirectory)).sort();
  assert.deepEqual(files, OBJECT_INFORMATION_SOURCES
    .map(({ id }) => `${id}.json`).sort());
  for (const file of files) {
    validateObjectInformationSnapshot(JSON.parse(
...
test("restores the previous complete batch when publication fails", async (context) => {
  ...
  await assert.rejects(
    publishObjectInformation(preparedBatch(), { outputDirectory, fileOperations: operations }),
...
test("preserves both publication and restoration failures", async (context) => {
  ...
  await assert.rejects(
    publishObjectInformation(preparedBatch(), {
      outputDirectory,
...
function preparedBatch() {
  return OBJECT_INFORMATION_SOURCES.map((source) => ({
    schemaVersion: 1,
    id: source.id,
    planet: source.name,
