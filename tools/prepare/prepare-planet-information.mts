...
import {
  ...
  requirePlanetInformationSnapshot,
} from "../sources/planet-information-sources.mts";

import type { PlanetInformationSource, PlanetInformationSnapshot } from '../sources/planet-information-sources.mts';
...
async function preparePlanet(config: PlanetInformationSource): Promise<PlanetInformationSnapshot> {
...
function selectSections(blocks: readonly unknown[], planetName: string) {
  const sections: PlanetInformationSnapshot["sections"] = [];
  let activeSection: PlanetInformationSnapshot["sections"][number] | null = null;
...
function validateRecord(input: unknown, config: PlanetInformationSource): NasaRecord {
...
async function verifyLocalSnapshots(ids: readonly string[], outputDirectory: string) {
  ...
  for (const id of snapshotIds) {
    ...
    const snapshot = requirePlanetInformationSnapshot(JSON.parse(await readFile(
...
export async function publishPlanetInformation(
  ...
  const serialized = snapshots.map((input) => {
    const snapshot = requirePlanetInformationSnapshot(input);
    return Object.freeze({
      id: snapshot.id,
