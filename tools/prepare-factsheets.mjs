...
import { orderFacts } from '../site/fact-order.mjs';
...
export async function prepareFactsheet(objectDirectory, { check = false, editorial = false } = {}) {
  const read = async file => JSON.parse(await readFile(resolve(objectDirectory, file), 'utf8'));
  const descriptor = await read('object.json');
    ...
    if (fact.source) {
      ...
      if (fact.source.path) {
        const evidence = [...manifest.inputs, ...manifest.documents, ...manifest.generatedIntermediates]
          .find(entry => `source/${entry.path}` === fact.source.path);
  ...
  if (editorial) {
    ...
    const authored = isStatic ? source.controls.lenses : source.lenses;
    ...
    await publish('prepared/controls.json', { ...controls,
      lenses: { ...controls.lenses, controls: updateControls(controls.lenses.controls) } });
    const runtime = await read('prepared/runtime.json');
    await publish('prepared/runtime.json', { ...runtime, controls: { ...runtime.controls,
      lenses: { ...runtime.controls.lenses, controls: updateControls(runtime.controls.lenses.controls) } } });
    if (!isStatic) {
  ...
  for (const path of ['prepared/authored-preparation.json', 'prepared/world-navigation.json']) {
    try {
      ...
      await publish(path, { ...receipt,
        sources: receipt.sources.map(source => source.id === 'content' ? { ...reference } : source) });
    } catch (error) { if (error.code !== 'ENOENT') throw error; }
  }
