...
// Node built-ins, like tools/object-scope-gate.mts, so it can run as an early CI step right after checkout.
...
async function gitChangedPaths(mode: ChangeMode, ref: string, root: string): Promise<readonly string[] | undefined> {
  ...
  // pr: a three-dot diff against the PR's base branch, like tools/object-scope-gate.mts — what the PR actually
