...
test("root test aggregates preserve every lane and audit scripts use existing moved paths", async () => {
  ...
  assert.equal(scripts['test:browser'], 'pnpm test:browser:conformance && pnpm test:browser:planets');
...
test("default preparation covers every actual OBJECTS package exactly once", async () => {
  ...
  assert.ok(report.results.every(result => result.status === "succeeded" && result.exitCode === 0 && result.elapsedMilliseconds >= 0));
...
test("parallel preparation refills a bounded queue while preserving selected result order", async () => {
  const selected = ids.slice(0, 4), gates = new Map(selected.map(id => [id, deferred()]));
  const started = [], firstTwo = deferred(), third = deferred();
...
test("preparation validates selected registry IDs, capacities, and all scripts before starting", async () => {
  let calls = 0;
  const options = { projectRoot: root, onEvent: quiet, runCommand: async () => { calls++; return success; } };
