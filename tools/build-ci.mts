...
export function ciBuildPlan(root: string, mode: CiBuildMode, preparationDts = true): readonly CiBuildTask[] {
  ...
  const tasks: CiBuildTask[] = [
    ...
    node('titles', 'tools/prepare-shell-titles.mts'),
  ...
  if (mode === 'full') tasks.push(
    ...
    node('icons', 'tools/prepare-shell-icons.mts'),
    node('navigation', 'tools/prepare-navigation.mts', ['solar'], ['--catalog-only']),
    ...
    node('moon-labels', 'tools/prepare-moon-labels.mts', ['world']),
    node('wordmark', 'tools/prepare-wordmark-rail.mts', ['font']),
    // The title generator imports planet title preparation, whose shell imports the real world context.
    node('overview-titles', 'tools/prepare-overview-titles.mts', ['font', 'world']),
  );
  return tasks;
}
