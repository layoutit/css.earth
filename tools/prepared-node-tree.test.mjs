...
test('one prepared raster plane preserves the full projective mapping and scaled texture addresses', () => {
  ...
  const leaf = builder.leaf({ style: 'width:32px;height:16px;--surface-position:-8px -12px;background-position:var(--surface-position);background-size:512px 256px;background-image:url(/prepared.webp)',
  ...
  for (const propertyId of node.properties) { const property = tree.properties[propertyId]; if (property.custom) style.setProperty(property.name, property.value); else style[property.name] = property.value; }
  assert.equal(tree.nodes.length, 3); assert.equal(leaf.children.length, 0);
  assert.equal(style.width, '128px'); assert.equal(style.height, '64px');
  assert.equal(style.getPropertyValue('--surface-position'), '-32px -48px');
  assert.equal(style.backgroundSize, '2048px 1024px'); assert.equal(style.backgroundPosition, 'var(--surface-position)');
  assert.equal(style.backgroundImage, 'url(/prepared.webp)');
  const combined = style.transform.slice(9, -1).split(',').map(Number);
