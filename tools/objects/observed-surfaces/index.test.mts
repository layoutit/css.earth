...
test('pole products bypass coordinate-preserving lens resizes while retained maps keep their declared size',async()=>{
  ...
  try{
    ...
    const result=await prepareObservedSurfaces({sourceDirectory:directory,config:{schema:'cssearth-observed-surfaces@1',sources:[{path,expectedBytes:bytes.length,expectedSha256:createHash('sha256').update(bytes).digest('hex')}],lenses:[{id:'normal',source:path,decode:{kind:'raster',channels:3},transforms:[{kind:'resize',width:16,height:8}],products:[{kind:'poles',filename:'normal-poles.webp',projection,encoding:{lossless:true}}]}]}});
