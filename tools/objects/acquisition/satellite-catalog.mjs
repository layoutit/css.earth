...
export function prepareSatelliteCatalog({config,documents}) {
validateSatelliteCatalogRecipe(config);
...
for (let index=0;index<discoveryOnly.length;index++) {const orbit=config.discoveryOnly[index];const semiMajorAxisKm=orbit.document?parseApproximateOrbit(documents[orbit.document],orbit.radiusPattern):orbit.semiMajorAxisKm;moons.push(discoveryOnlyMoon(discoveryOnly[index],{...orbit,semiMajorAxisKm}));}
...
function parseApproximateOrbit(html,pattern) {
...
export function validateSatelliteCatalogRecipe(config) {
...
for(const field of ['start','end'])if(typeof config.discoverySection[field]!=='string'||!config.discoverySection[field])throw new TypeError('Invalid discovery section.');
for(const item of config.discoveryOnly){if(typeof item.identity!=='string'||!item.identity||item.document&&(!config.sources[item.document]||typeof item.radiusPattern!=='string')||!item.document&&(!Number.isFinite(item.semiMajorAxisKm)||item.semiMajorAxisKm<=0))throw new TypeError('Invalid approximate satellite orbit.');}
return config;
}
