...
export function prepareProjectedCatalog({bytes,template,selection,selectedCount,catalogRows}) {
  ...
  const radians=value=>value*Math.PI/180,centerRa=radians(selection.centerRaDegrees),centerDec=radians(selection.centerDecDegrees),tangentX=Math.tan(radians(selection.horizontalFovDegrees/2)),tangentY=tangentX/selection.aspectRatio,candidates=[];
  ...
  return new TextEncoder().encode(JSON.stringify({...template,presentation:{...template.presentation,visibleCatalogStars:candidates.length,selectedStars:stars.length,brightestMagnitude:stars[0].magnitude,faintestMagnitude:stars.at(-1).magnitude},stars},null,2)+'\n');
}
