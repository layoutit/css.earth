...
test('DSK input size and digest are checked before any converter is launched',async()=>{
 const sourceRoot=await mkdtemp(resolve(tmpdir(),'dsk-pin-test-'));
 try {await writeFile(resolve(sourceRoot,'shape.bds'),Buffer.alloc(16));await assert.rejects(prepareDskMesh({sourceRoot,recipe}),/Pinned DSK source bytes differ/);}
