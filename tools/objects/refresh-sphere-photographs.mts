...
type Recipe = {sourceWidth:number;sourceHeight:number;width:number;height:number;latitudeBands:number;polarTile:number;resample:string;unpackedResizeBeforePack?:boolean;polesCombined:boolean;polesOutput:string;publicBase:string;surfaces:Surface[]};

// Prepared maps carry two texels per layout pixel.
const density = 2;
const hash = (bytes:Uint8Array) => createHash('sha256').update(bytes).digest('hex');
const outputName = (template:string, id='') => template.replaceAll('{id}',id);
...
function recipe(value:RecordValue):Recipe {
  ...
  const surfaces=requireArray(value.surfaces).map(value=>{
  ...
  if(![sourceWidth,sourceHeight,width,height,latitudeBands,polarTile].every(value=>Number.isSafeInteger(value)&&value>0)) throw new TypeError('Unsupported raster dimensions.');
  ...
  return {sourceWidth,sourceHeight,width,height,latitudeBands,polarTile,resample,unpackedResizeBeforePack:value.unpackedResizeBeforePack===true,polesCombined:value.polesCombined,polesOutput,publicBase,surfaces};
}
...
function encodeBand(image:Sharp, encoding:RecordValue|undefined) {
  if(!encoding) return image.webp({quality:90,smartSubsample:true});
...
async function stagePoles({lens,config,sourceDirectory,manifest,stage,publicDirectory}:{lens:Surface;config:Recipe;sourceDirectory:string;manifest:RecordValue;stage:string;publicDirectory:string}) {
  ...
  const {sampler}=await nativeSampler(lens,config,sourceDirectory,manifest);
  const tile=config.polarTile*density, filename=outputName(config.polesOutput,lens.id), previous=contained(publicDirectory,filename);
  ...
  return [{filename,url:config.publicBase+filename,width:current.width!,height:current.height!,bytes:bytes.length,sha256:hash(bytes)}];
}
async function stageMercuryBand({lens,config,sourceDirectory,manifest,stage,publicDirectory}:{lens:Surface;config:Recipe;sourceDirectory:string;manifest:RecordValue;stage:string;publicDirectory:string}) {
  ...
  const packed=packLatitudeRaster(target,width,height,config.latitudeBands,Math.max(2,height/config.latitudeBands/4)), filename=outputName(lens.output,lens.id), previous=contained(publicDirectory,filename);
  const old=await existingMetadata(previous), bytes=await encodeBand(sharp(packed.data,{raw:{width:packed.packedWidth,height:packed.packedHeight,channels:4}}),lens.encoding).toBuffer();
  const stagePath=resolve(stage,filename); await writeFile(stagePath,bytes);
