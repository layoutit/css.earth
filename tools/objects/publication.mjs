...
/** Replace the complete prepared minimap set; retain superseded lenses for rollback. */
export async function publishPreparedMinimaps({ stage, destination, recovery }) {
  const { images } = JSON.parse(await readFile(resolve(stage, 'minimaps.json'), 'utf8'));
  const expected = new Set(images.map(image => {
    if (!/^minimaps\/[a-z0-9-]+\.webp$/u.test(image.path)) throw new Error('Invalid prepared minimap path.');
    return image.path.slice('minimaps/'.length);
  }));
  if (expected.size !== images.length) throw new Error('Duplicate prepared minimap path.');
  const incoming = resolve(stage, 'minimaps');
  const entries = await readdir(incoming, { withFileTypes: true }).catch(error => {
    if (error.code === 'ENOENT' && !expected.size) return []; throw error;
  });
  if (entries.length !== expected.size || entries.some(entry => !entry.isFile() || !expected.has(entry.name))) {
    throw new Error('Prepared minimap files differ from their declaration.');
  }
  let previous = false;
  try { await rename(destination, recovery); previous = true; }
  catch (error) { if (error.code !== 'ENOENT') throw error; }
  try { if (expected.size) await rename(incoming, destination); }
  catch (error) { if (previous) await rename(recovery, destination); throw error; }
}
