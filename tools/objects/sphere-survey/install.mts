...
export async function installSetup(objectId: string) {
  ...
  if (Array.isArray(lenses) && lenses.some(lens => requireRecord(lens).id === LENS_ID)) throw new Error(`${objectId} already has a ${LENS_ID} lens; the setup run compares with it and installs nothing.`);
  const setup = await buildSetup(objectId), work = resolve(ROOT, 'output/sphere-survey', objectId), scratch = resolve(work, 'source');
  ...
  const lensFrames = setup.apparition.frames, nights = setup.apparition.nights;
  ...
  const plan = await readJson(resolve(packageSource, 'preparation/acquisition.json')), operations = requireArray(plan.operations).map(value => requireRecord(value));
  ...
  controls.push({ id: LENS_ID, label: 'SPHERE photograph', thumbnail: `/scenes/${objectId}/${objectId}-${LENS_ID}-thumbnail.webp`,
    surface: `${objectId}-${LENS_ID}-surface@2x.webp`, poles: `${objectId}-${LENS_ID}-surface@2x.webp`, source: { id: `${objectId}-adam-shape`, path: '../manifest.json' },
    falseColor: false, noData: true,
    notes: `${lensFrames} deconvolved VLT/SPHERE/ZIMPOL frames, ${nightsText(nights)}, cast onto the ADAM reconstruction from the same survey. The survey’s rotation record describes that frame, so the photograph rides it rather than the model the Shape view uses. Pointing and orientation are computed from that record, JPL Horizons geometry and each frame’s header, at the midpoint of its exposure; the disc centre is fitted to the limb of the mesh. With these cameras the mesh reproduces Vernazza et al. (2021) Figure ${figure}. Grayscale is photographed illumination and matched relative frame brightness. The deconvolution carries no radiometric calibration, so this is not measured albedo or colour. The grid marks surface that was unphotographed, too grazing, or rejected.` });
  ...
  const close = (id: string, finding: (earlier: string) => string, link?: string) => {
    ...
    if (!entry) return;
    if (entry.survey !== undefined) throw new Error(`${objectId}'s ${id} entry quotes a shared record; give it its own finding before installing.`);
  ...
  close('surface-imagery', earlier => `Included ${today} as the SPHERE photograph lens: ${lensFrames} camera-1 deconvolved frames, ${nightsText(nights)}, cast onto the ADAM mesh with cameras computed from the release rotation record, JPL Horizons and each frame’s header. Its registration is the published comparison recorded in ${COMPARISON_ENTRY}.${unused ? ` The other ${unused} released camera-1 frames are not used: a lens keeps one apparition and at most 32 frames.` : ''} Earlier finding, kept: ${earlier}`, listing);
  ...
  close('sphere-cross-frame-registration', earlier => `Decided ${today} by the published comparison instead: the lens is prepared from ${lensFrames} camera-1 frames on the ADAM mesh with the rotation record read ${setup.columnOrder.order}, and ships on ${COMPARISON_ENTRY}; the registration stage’s numbers are in the README. Earlier result, kept: ${earlier}`);
  ...
  await writeFile(resolve(objectDirectory, 'README.md'), readmeWithLens(await readFile(resolve(objectDirectory, 'README.md'), 'utf8'),
    { number, name, figure, lensFrames, nights, order: setup.columnOrder.order, source: setup.evidence.source, spinRecordUrl: setup.spinRecordUrl, block: comparisonBlock(evidence),
      bodyName: name, latitudes: [Math.min(...latitudes), Math.max(...latitudes)] }));
...
/** Every pinned input preparation will read, copied from a sibling checkout when it is missing here and a byte-identical copy exists. */
export async function restorePinnedInputs(objectId: string) {
  ...
  for (const value of requireArray(manifest.inputs)) {
...
export function readmeWithLens(readme: string, lens: { number: number; name: string; figure: string; lensFrames: number; nights: readonly string[]; order: string; source: string; spinRecordUrl: string; block: string; bodyName: string; latitudes: readonly [number, number] }) {
  ...
  const problems = lines.indexOf('## Known problems'), footer = lines.findIndex((line, index) => index > problems && line.startsWith('[Investigation ledger]'));
  if (problems < 0 || footer < 0) throw new Error('The README has no Known problems section ending in the package links.');
  lines.splice(footer, 0, `The SPHERE photograph is photographed illumination from the survey's deconvolved frames, with matched relative frame levels. It is not albedo or colour. The frames see ${lens.bodyName} from ${latitudeSpan(lens.latitudes)}, so surface the survey did not see keeps the missing-imagery grid.`, '');
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const [objectId, ...rest] = process.argv.slice(2);
  if (!objectId || rest.length) { console.error('usage: node tools/objects/sphere-survey/install.mts <object-id>'); process.exit(2); }
  await installSetup(objectId);
}
