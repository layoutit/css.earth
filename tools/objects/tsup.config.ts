...
export default {
  entry: ['prepare-authored.ts', 'prepare-world-navigation.ts', 'prepare-spatial-context.ts', 'prepare-volume.ts', 'prepare-stars.ts', 'celestial/prepare.ts', 'content/prepare.ts', 'operations.ts'].map(path => resolve(root, 'tools/objects', path)),
