...
const manifestPath = resolve(sourceRoot, '../candidates/giotto-hmc.json');
...
export function decodeGiottoFrame(headerBytes, imageBytes, labelBytes) {
  ...
  const field = key => label.match(new RegExp(`^\\s*${key}\\s*=\\s*([^\\r\\n]+)`, 'm'))?.[1].trim();
...
export async function loadPinned(directory, entry, download = false) {
...
async function main() {
  ...
  if (args.some(a => a !== '--download' && !a.startsWith('--output='))) {
    throw new Error('Usage: node tools/objects/comet-1p/inspect-giotto.mjs [--download] [--output=directory]');
  }
  ...
  for (let i = 0; i < manifest.frames.length; i++) {
    ...
    const bytes = await Promise.all(['header', 'image', 'label'].map(k => loadPinned(input, source[k], args.includes('--download'))));
    const frame = decodeGiottoFrame(...bytes), h = frame.header;
    ...
    frames.push({ id: source.id, imageId: source.imageId, sensor: h.SENSOR, filter: h.FILTER,
      ...
      sourceFiles: ['header', 'image', 'label'].map(k => source[k]),
  ...
  const report = {
    schema: 'cssearth-halley-giotto-intake-report@1', manifest: { path: 'src/planets/comet-1p/candidates/giotto-hmc.json', sha256: sha256(manifestBytes) },
