...
import {verifyObservationSources} from './observations.mjs';
...
const ellipseBoundary=(rx,ry,dx,dy)=>1/Math.hypot(dx/rx,dy/ry);

/** Linear-light transfer for a retained source-over photometric overlay. */
export function encodeAttenuatedSrgb(channel,factor){
...
export function prepareNormalizedDiscProjection(config,pitchDegrees=config.scenePitchDegrees){
...
function projectedNormal(localX,localY,projection,config){
  ...
  const equatorial=1/config.shape.equatorialRadius**2,polar=1/config.shape.polarRadius**2,weight=axis=>axis===2?polar:equatorial;
...
export function phaseLightDirection(z,reference){const[x,y]=reference,scale=Math.sqrt(Math.max(0,1-z*z))/Math.hypot(x,y);return[x*scale,y*scale,z];}

export function rasterPhotometricDisc(config,lightViewZ,{shadowless=false}={}){
...
export function parsePhotometricDiscRecipe(config){
...
export async function resolvePhotometricDiscRecipe({sourceDirectory,config}){
  parsePhotometricDiscRecipe(config);await verifyObservationSources(sourceDirectory,config.sources);
  if(!config.sources.some(source=>source.path===config.minnaertSource))throw new TypeError('Minnaert source is not pinned.');
  const source=JSON.parse(await readFile(resolve(sourceDirectory,config.minnaertSource),'utf8'));
  ...
  if(!minnaertChannels.every(value=>Number.isFinite(value)&&value>0&&value<=2))throw new TypeError('Invalid observed Minnaert coefficients.');
  return{...config,minnaertChannels};
}

/** Generates transparent normalized-disc overlays, not replacement globe maps. */
export async function preparePhotometricDisc({sourceDirectory,publicDirectory,config,write=false}){
  config=await resolvePhotometricDiscRecipe({sourceDirectory,config});
  if(write)await mkdir(publicDirectory,{recursive:true});
  const{bank}=config,stride=config.frameSize+bank.gutter*2,rowCount=Math.ceil(bank.frames/bank.framesPerRow),assets=[],rows=[],presentations=[];
  const output=async(filename,data,width,height)=>{const asset={filename,width,height,bytes:data.length,sha256:createHash('sha256').update(data).digest('hex'),data};if(write)await writeFile(resolve(publicDirectory,filename),data);assets.push(asset);return asset;};
  for(let row=0;row<rowCount;row++){
    const first=row*bank.framesPerRow,count=Math.min(bank.framesPerRow,bank.frames-first),columns=Math.min(bank.columns,count),lines=Math.ceil(count/bank.columns),width=columns*stride,height=lines*stride,composites=[],filename=config.rowOutput.replace('{row}',String(row).padStart(2,'0'));
