...
import {prepareObservedSurfaces,verifyObservationSources} from './observations.mjs';
import {prepareEllipsoidMaterials} from './materials.mjs';
import {parseRadialLayerRecipe,rasterAnnularField} from './index.mjs';
const root=fileURLToPath(new URL('../../../',import.meta.url));
export async function assertMaterialPreparationParity(id){
  ...
  const config=await json('source/preparation/materials.json'),observations=await prepareObservedSurfaces({sourceDirectory,config:await json('source/preparation/observations.json')});
  let radialLayer;
  if(config.radialLayer){const recipe=parseRadialLayerRecipe(await json('source/preparation/rings.json'));await verifyObservationSources(sourceDirectory,recipe.sources);radialLayer={...config.radialLayer,data:rasterAnnularField(recipe.layers[config.radialLayer.layerIndex],config.radialLayer.size)};}
  const result=await prepareEllipsoidMaterials({config,maps:observations.maps,radialLayer,write:false});
  const accepted=await json('runtime-assets.json');
