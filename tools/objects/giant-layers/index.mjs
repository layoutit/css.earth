...
import { rasterAnnularField, rasterObservedRadialField, colorizeRadialField, rasterProjectedStripShadow } from './rings.mjs';
export { mapRadius, ringRayOccluded, rasterAnnularField, rasterObservedRadialField, sampleRadialProfile } from './rings.mjs';

const digest = bytes => createHash('sha256').update(bytes).digest('hex');
function fail(message) { throw new TypeError(`Radial preparation: ${message}`); }
function positive(value) { return Number.isFinite(value) && value > 0; }
function pair(value) { return Array.isArray(value) && value.length === 2 && value.every(Number.isFinite) && value[1] > value[0]; }
function color(value) { return Array.isArray(value) && value.length === 3 && value.every(channel => Number.isInteger(channel) && channel >= 0 && channel <= 255); }
function validateFiniteTree(value) {
  ...
  if (value && typeof value === 'object') for (const child of Object.values(value)) validateFiniteTree(child);
}

export function parseRadialLayerRecipe(value) {
  if (!value || value.schema !== 'cssearth-radial-layer-recipe@1' || value.units !== 'kilometers' ||
      !Array.isArray(value.sources) || !Array.isArray(value.layers) || value.layers.length === 0) fail('invalid recipe.');
  ...
  for (const layer of value.layers) {
    if (!['annular-field', 'observed-radial-profile'].includes(layer.kind) ||
        ...
        !Array.isArray(layer.densities) || layer.densities.length === 0 ||
    ...
    for(const overlay of layer.overlays??[]){
      if(overlay.kind!=='projected-strip-shadow'||layer.densityMode==='downsample-highest'||!positive(overlay.bodyRadius)||!positive(overlay.outerRadius)||overlay.outerRadius<=overlay.bodyRadius||!Array.isArray(overlay.direction)||overlay.direction.length!==2||!overlay.direction.every(Number.isFinite)||Math.hypot(...overlay.direction)===0||!color(overlay.color)||!positive(overlay.startFraction)||!positive(overlay.edgeFraction)||!Number.isInteger(overlay.maximumAlpha)||overlay.maximumAlpha<1||overlay.maximumAlpha>255||![0,1].includes(overlay.centerInset)||!Number.isFinite(overlay.marginPixels)||overlay.marginPixels<0||!overlay.encoding||!/^[a-z0-9][a-z0-9-]*\{suffix\}\.webp$/u.test(overlay.output)||outputs.has(overlay.output))fail('invalid radial overlay.');
    ...
    for (const variant of layer.variants ?? []) {
      if (!/^[a-z0-9][a-z0-9-]*\{suffix\}\.webp$/.test(variant.output) || outputs.has(variant.output) ||
          !Array.isArray(variant.palette) || variant.palette.length !== 3 || !variant.palette.every(color) ||
          !positive(variant.outerRadius) || !positive(variant.exponent) || !positive(variant.gain) || !positive(variant.outerGain) ||
          !Array.isArray(variant.luminance) || variant.luminance.length !== 3 || !variant.luminance.every(positive) ||
          !Array.isArray(variant.radialGains) || variant.radialGains.some((band, index) => !positive(band.upperBound) || !positive(band.gain) ||
    ...
    if (layer.kind === 'annular-field') {
      ...
      if (mapping.kind === 'linear' ? !positive(mapping.scale) : !Array.isArray(mapping.knots) || mapping.knots.length < 2 ||
        mapping.knots.some((knot, index) => !Array.isArray(knot) || knot.length !== 2 || knot.some(value => !positive(value)) ||
          (index > 0 && (knot[0] <= mapping.knots[index - 1][0] || knot[1] <= mapping.knots[index - 1][1])))) fail('invalid radius mapping.');
      if (!Array.isArray(layer.bands) || layer.bands.length === 0) fail('empty annular field.');
      for (const band of layer.bands) {
        ...
        if (band.envelope === 'smooth-annulus' && (!Array.isArray(band.fade) || band.fade.length !== 2 || !band.fade.every(positive) || band.bounds[0] <= band.fade[0])) fail('invalid annular fade.');
        if (band.arcs && (!Array.isArray(band.arcs.centers) || band.arcs.centers.length === 0 || !band.arcs.centers.every(Number.isFinite) || !positive(band.arcs.halfWidth))) fail('invalid arcs.');
      }
      if (layer.shadow && (!Array.isArray(layer.shadow.direction) || layer.shadow.direction.length !== 3 ||
      ...
      if (!pair(layer.bounds) || !pair(layer.sourceBounds) || layer.sourceBounds[0] <= layer.bounds[0] ||
          ...
          !Array.isArray(layer.channelFactors) || layer.channelFactors.length !== 3 || !layer.channelFactors.every(positive) ||
          !layer.interior || !color(layer.interior.color) || !Array.isArray(layer.interior.centers) || !layer.interior.centers.length ||
          !layer.interior.centers.every(positive) || !positive(layer.interior.sigma) ||
          !Array.isArray(layer.operations) || layer.operations.some(operation => !pair(operation.bounds) ||
            !['alpha-cap', 'clear', 'edge-core', 'alpha-gain'].includes(operation.kind)) ||
          !layer.readability || !Array.isArray(layer.readability.features) || !positive(layer.readability.alphaGain) ||
...
async function verifyInputs(root, sources) {
...
async function observedInputs(layer, inputs) {
...
export async function prepareGiantLayers({ sourceDirectory, publicDirectory, config, write = true }) {
  ...
  for (const layer of recipe.layers) {
    ...
    const raster = (size, density) => profile
      ? rasterObservedRadialField(layer, size, layer.readability.minimumPixels[density], profile)
