...
test('unobserved hemisphere uses a bounded scientific baseline without new local features',()=>{
  ...
  const result=completeUniformCoverage({data,width:4,height:10,channels:4},{color:[10,20,30],provenance:{authority:'hypothetical pinned observation',model:'central-disc mean'}},{alphaValidity:true,minimumRowCoverage:0.5,equatorialInsetRows:0,allowedBoundaryRange:[0.25,0.75],transitionRows:2});
...
test('polar projection uses explicit pole count, sampling and RGBA coverage',()=>{
  const map={data:Buffer.from(Array.from({length:32},(_,i)=>[i,100,200,255]).flat()),width:8,height:4,channels:4};
...
test('recipe and source failures are rejected before output writes',async()=>{
  ...
  for(const mutate of[c=>c.lenses[0].products[0].filename='../escape.webp',c=>c.lenses[1].decode.color.palette[0][0]=Infinity,c=>c.lenses[0].coverage.kind='invented-pixels',c=>c.lenses[0].products.push(c.lenses[0].products[0])]){const copy=structuredClone(config);mutate(copy);assert.throws(()=>parseObservedSurfaceRecipe(copy));}
