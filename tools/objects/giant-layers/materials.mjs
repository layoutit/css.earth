...
import {intersectViewRayWithEllipsoid,normalizeVector,dotVector,rotateSequence} from '../material-composition/ellipsoid.mjs';
import {writeMaterialAtlasTile,sampleRgbaBilinear} from '../material-composition/raster.mjs';
export {writeMaterialAtlasTile} from '../material-composition/raster.mjs';
import {optimizePreparedDisplayLosslessWebp,optimizePreparedQ75Webp,PREPARED_Q75_WEBP_ENCODING} from '../../prepared-webp.mjs';

const clamp=(value,low=0,high=1)=>Math.max(low,Math.min(high,value));
const scale=(vector,value)=>vector.map(component=>component*value);
const add=(...vectors)=>[0,1,2].map(axis=>vectors.reduce((sum,vector)=>sum+vector[axis],0));
const hash=bytes=>createHash('sha256').update(bytes).digest('hex');
const safeName=name=>typeof name==='string'&&/^[a-z0-9][a-z0-9-]*(?:@2x)?\.webp$/u.test(name);
function transform(vector,steps,state){for(const step of steps){if(step.kind==='normalize')vector=normalizeVector(vector);else vector=rotateSequence(vector,[{axis:step.axis,degrees:step.state?state[step.state]*step.factor:step.degrees}]);}return vector;}

export function parseEllipsoidMaterialRecipe(config){
  const fail=message=>{throw new TypeError(`Ellipsoid material: ${message}`);},positive=value=>Number.isFinite(value)&&value>0,unit=value=>Number.isFinite(value)&&value>=0&&value<=1,integer=(value,min,max)=>Number.isInteger(value)&&value>=min&&value<=max;
  const vector=value=>Array.isArray(value)&&value.length===3&&value.every(Number.isFinite)&&Math.hypot(...value)>0;
  const color=value=>Array.isArray(value)&&value.length===3&&value.every(channel=>integer(channel,0,255));
  if(config?.schema!=='cssearth-ellipsoid-materials@1'||!Array.isArray(config.lenses)||!config.lenses.length||!/^\/scenes\/[a-z][a-z0-9-]*\/$/u.test(config.urlPrefix))fail('invalid recipe.');
  ...
  const rotations=steps=>{if(!Array.isArray(steps))fail('missing orientation.');for(const step of steps){if(step.kind==='normalize')continue;if(step.kind!=='rotate'||!['x','y','z'].includes(step.axis)||(step.state?!['scenePitchDegrees','systemObliquityDegrees'].includes(step.state)||!Number.isFinite(step.factor):!Number.isFinite(step.degrees)))fail('invalid orientation operation.');}};
  ...
  for(const lens of config.lenses){
    if(!/^[a-z][a-z0-9-]*$/u.test(lens.id)||ids.has(lens.id)||(!lens.atmosphereFromMap&&!color(lens.atmosphere))||(silhouette&&!color(lens.fixedBase))||!Array.isArray(lens.fixed)||!lens.fixed.length)fail('invalid lens material.');ids.add(lens.id);
...
function sampleRadialPlane(x,y,layer) {
...
function radialIntersection(origin,direction,layer,minimumDistance) {
...
export function rasterEllipsoidMaterial(config,{size,state,palette,radialLayer,shadowless=false,textureUrl='',geometryOnly=false}) {
  ...
  const projected=direction=>Math.sqrt(shape.equatorialRadius**2*(direction[0]**2+direction[1]**2)+shape.polarRadius**2*direction[2]**2);
  const radiusX=projected(right),radiusY=projected(down),frontDepth=projected(view),output=Buffer.alloc(size*size*4),center=(size-1)/2,pixelToObject=grid.span/size;
  const intersect=(origin)=>intersectViewRayWithEllipsoid(origin,view,shape);
  ...
  for(let y=0;y<(geometryOnly?0:size);y++)for(let x=0;x<size;x++){
    ...
    if(silhouette){
      ...
      if(!surfaceHit)continue;const materialScale=grid.coverageScale/grid.contentScale;
    ...
    if(ringShadow>1/255&&direct>0){direct*=1-ringShadow*lighting.radialShadowGain;ringShadowTexelCount++;}
    ...
    if(silhouette){
      const coverage=normalAlignment=>clamp((silhouette.fadeStart-normalAlignment)/(silhouette.fadeStart-silhouette.opaqueAt));
    ...
    if(radialLayer){
      const surfaceDistance=dotVector(hit.position.map((value,i)=>value-origin[i]),view),foreground=radialIntersection(origin,view,radialLayer,surfaceDistance);
  ...
  if(grid.kind==='projected-ellipsoid'){
    ...
    const matrix=[...worldPositionToCss(scale(right,scaledX*2/size)),0,...worldPositionToCss(scale(down,scaledY*2/size)),0,...worldPositionToCss(view),0,...worldPositionToCss(add(scale(right,-scaledX),scale(down,-scaledY),scale(view,frontDepth+grid.depthBias))),1].map(value=>Number(value.toFixed(12)));
...
async function encodeMaterial(data,width,height,encoding,optimization) {
...
export async function prepareEllipsoidMaterials({config,maps,radialLayer,publicDirectory,write=false}) {
  parseEllipsoidMaterialRecipe(config);
  ...
  const assets=[],lenses={};
  const publish=async(filename,frame,width,height,encoding,optimization)=>{if(!safeName(filename))throw new TypeError('Invalid material output.');const data=await encodeMaterial(frame,width,height,encoding,optimization);const record={filename,width,height,bytes:data.length,sha256:hash(data),data};assets.push(record);return record;};
  for(const lens of config.lenses){
    const map=maps.get(lens.id);if(!map)throw new Error(`Material lens ${lens.id} has no observed map.`);
    const atmosphere=lens.atmosphereFromMap?map.atmosphereColor:lens.atmosphere;
    const result={fixed:{},shadowless:{}};
    ...
    result.bank={rows,presentations,frameRawSha256,frameForegroundRingTexelCounts,frameRingShadowTexelCounts,...(bank.optimization==='q75'?{encoding:PREPARED_Q75_WEBP_ENCODING}:{})};lenses[lens.id]=result;
  }
  if(write){await mkdir(publicDirectory,{recursive:true});for(const asset of assets)await writeFile(resolve(publicDirectory,asset.filename),asset.data);}
