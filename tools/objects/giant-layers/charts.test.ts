...
for(const id of['jupiter','uranus','neptune'])test(`${id} authored chart parameters reproduce all accepted source-driven SVG bytes`,async()=>{
 const sourceDirectory=resolve('src/planets',id,'source'),config=JSON.parse(await readFile(resolve(sourceDirectory,'content/charts.json'),'utf8')),publicDirectory=await mkdtemp(join(tmpdir(),'giant-chart-parity-'));
