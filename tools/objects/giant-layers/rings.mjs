...
function interval(value, bounds, inclusive = [true, true]) {
...
export function mapRadius(value, mapping, inverse = false) {
...
export function ringRayOccluded(x, y, shadow) {
...
function prepareBand(band, mapping, kilometersPerPixel) {
...
function bandCoverage(radius, angle, band) {
  if (band.arcs && !band.arcs.centers.some(center => angularDistance(angle, center) < band.arcs.halfWidth)) return 0;
...
export function rasterAnnularField(recipe, size) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      let alpha = 0, color = recipe.defaultColor ?? [0, 0, 0];
...
export function applyLinearTint(channel, factor) {
...
export function sampleRadialProfile(radius, recipe, inputs) {
...
export function prepareRadialReadability(recipe, size, minimumPixels, inputs) {
  const samples = new Map(), outerPixels = (size - 1) / 2;
...
export function rasterObservedRadialField(recipe, size, minimumPixels, inputs) {
...
export function colorizeRadialField(source, width, height, recipe) {
...
export function rasterProjectedStripShadow(data,size,config) {
