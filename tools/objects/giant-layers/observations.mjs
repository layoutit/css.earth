...
export function parseObservedSurfaceRecipe(config) {
  if(config?.schema!=='cssearth-observed-surfaces@1'||!Array.isArray(config.sources)||!Array.isArray(config.lenses)||!config.lenses.length)throw new TypeError('Invalid observed-surface recipe.');
  ...
  for(const lens of config.lenses){
    ...
    if(lens.decode.kind==='fits'){
      ...
      if(lens.decode.bitpix!==-32||!dimensions(lens.decode.width,lens.decode.height)||!color||![3,4].includes(color.channels)||!Array.isArray(color.palette)||color.palette.length!==3||color.palette.some(rgb=>!Array.isArray(rgb)||rgb.length!==3||rgb.some(c=>!Number.isSafeInteger(c)||c<0||c>255))||!Array.isArray(color.percentiles)||color.percentiles.length!==2||!color.percentiles.every(fraction)||color.percentiles[0]>=color.percentiles[1]||color.percentiles[1]===1||!['sqrt','power'].includes(color.transfer)||!positive(color.exponent)||!fraction(color.minimumCoverage))throw new TypeError('Invalid scientific colour mapping.');
    }
    ...
    for(const continuation of[lens.decode.continuation,lens.coverage?.kind==='boundary-mean'?lens.coverage:null])if(continuation&&(!positive(continuation.exponent)||!positive(continuation.boundaryFraction)||continuation.boundaryFraction>=1||!Number.isFinite(continuation.minimumBoundarySum)))throw new TypeError('Invalid boundary continuation.');
    if(lens.calibration&&(!sourcePaths.has(lens.calibration.source)||lens.decode.channels!==3))throw new TypeError('Invalid true-colour calibration input.');
    operations(lens.transforms);
    if(!Array.isArray(lens.products)||!lens.products.length)throw new TypeError('Observation has no products.');
    for(const product of lens.products){
      ...
      if(product.kind==='poles'){const p=product.projection;if(!p||!dimensions(p.tileSize,p.tileSize)||!Array.isArray(p.poles)||!p.poles.length||p.poles.some(value=>!['north','south'].includes(value))||!positive(p.latitudeSegments)||!['nearest-closed','bilinear-wrapped'].includes(p.sampling)||!['direct-segment','boundary-difference'].includes(p.angularMode))throw new TypeError('Invalid polar projection.');}
    }
  }
  return config;
}
...
export async function verifyObservationSources(directory, sources) {
...
export function rgbMoments(source,width,height,sample) {
...
export function affineColorCalibration(source,target,config) {
...
export function continueBoundaryMean(data,{width,height,channels,boundaryFraction,exponent,minimumBoundarySum=0}) {
  const output=Buffer.isBuffer(data)?Buffer.from(data):new data.constructor(data),boundary=Math.round(height*boundaryFraction),means=[];
...
const presentPixel=(data,offset,config)=>config.alphaValidity?data[offset+3]>0:data[offset]+data[offset+1]+data[offset+2]>config.minimumBrightness;
export function observedMean(map,config) {
...
export function centralDiscBaseline(map,config) {
...
export function completeUniformCoverage(map,baseline,config) {
  if(map.channels!==4||!Array.isArray(baseline.color)||baseline.color.length!==3)throw new TypeError('Uniform coverage requires RGBA and an RGB baseline.');
  ...
  const detectedBoundaryRow=validRows.at(-1),boundaryRow=Math.min(detectedBoundaryRow,Math.floor((height-1)/2)-config.equatorialInsetRows);
...
export function percentileFalseColor(values,width,height,config) {
...
export function brightTailColor(data,{share,maximum,luminance}) {
  const histogram=new Uint32Array(256),pixelCount=data.length/3;
  const luma=offset=>Math.round(data[offset]*luminance[0]+data[offset+1]*luminance[1]+data[offset+2]*luminance[2]);
...
function sampleMap(map,latitude,longitude,mode) {
...
export function polarDiscAtlas(map,config) {
...
async function decodeRaster(bytes,config) {
...
async function transformMap(map,operations) {
...
export async function prepareObservedSurfaces({sourceDirectory,publicDirectory,config,write=false}) {
  parseObservedSurfaceRecipe(config);
  const inputs=await verifyObservationSources(sourceDirectory,config.sources),baselines=new Map(),maps=new Map(),assets=[];
  ...
  for(const lens of config.lenses){
    ...
    if(lens.coverage?.kind==='uniform-baseline'){
      ...
      if(!baseline)throw new TypeError('Observation baseline is undefined.');map=completeUniformCoverage(map,baseline,lens.coverage);
    }
