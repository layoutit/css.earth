...
async function prepareAuthoredStages({ objectDirectory, publicDirectory, outputDirectory, write = false }: AuthoredPreparationContext): Promise<AuthoredPreparationResult> {
  if (write) {
    ...
    const { publishPreparedAssets, publishPreparedMinimaps, readPreparedJsonOutputs } = await import(pathToFileURL(resolve(process.cwd(), 'tools/objects/publication.mjs')).href) as typeof import('./publication.mjs');
    ...
    await publishPreparedMinimaps({ stage: stagedData, destination: resolve(outputDirectory, 'minimaps'), recovery: resolve(stage, 'previous-minimaps') });
  ...
  if (source(sources, 'paged-ellipsoid')) {
    ...
    await prepareRuntimeManifest({ id: descriptor.id, publicRoot: publicDirectory,
      ...
      values: [prepared.definition, prepared.content, prepared.geographicAssets] });
