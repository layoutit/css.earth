...
export async function verifySkyBandRecipe(recipePin: { readonly path: string }, input: SkyBandIo['input']) {
  ...
  const files = [{ path: recipePin.path, sha256: sha256(recipeBytes), bytes: recipeBytes.length }];
  for (const band of recipe.bands) if ('tiles' in band) {
    ...
    files.push({ path: band.tiles.path, sha256: sha256(list), bytes: list.length });
  }
  return { recipe, files };
}
