...
export function cityPageRasterDensity(level) {
...
export function pageBounds({ level, x, y }) {
...
export function pageKey({ level, x, y }) {
  return `${level}-${x}-${y}`;
}
...
export function createCityGeographicSampler(page) {
...
const multiply3=(a,b)=>Array.from({length:9},(_,i)=>[0,1,2].reduce((sum,k)=>sum+a[Math.floor(i/3)*3+k]*b[k*3+i%3],0));
function inverse3(m) {
...
function preparePolarCityPageGeometry(address, scene, bounds, options) {
  ...
  const geographicProjection = { type: "polar", sign: north ? 1 : -1,
...
function polarSourceBounds(projection) {
  const vertices = [[0, 0], [1, 0], [1, 1], [0, 1]];
  const points = [...vertices];
  for (let edge = 0; edge < 4; edge++) {
    const a = vertices[edge], b = vertices[(edge + 1) % 4];
    const wa = polarWorldPoint(projection, ...a), wb = polarWorldPoint(projection, ...b);
    for (let i = 0; i < 32; i++) {
  ...
  const geographic = points.map(point => sampler(...point));
...
export function createCityCoverageSampler(page) {
...
export function childAddresses({ level, x, y }) {
