...
import { preparedCityAssetUrl } from '../../../../src/platform/prepared-map/city-asset-url.mjs';
...
export async function prepareCoarseRelease({ plan, directory, scene, fine, project, assetPath, onProgress = async () => {} }) {
  assert.match(assetPath, /^\/scenes\/[a-z][a-z0-9-]*\/$/, 'Explicit object asset path is required');
  const keyPrefix=assetPath.slice(1,-1);
  ...
  const files = ['tools/objects/geographic-pages/operations/coarse-release.mjs', 'tools/objects/geographic-pages/operations/prepare-index.mjs',
  ...
  const identity = { schema: 'cssearth-global-coarse-release-inputs@1', assetPath, planSha256: hash(JSON.stringify(plan)),
  ...
  for (const entry of plan.pages) {
    ...
    if (record) {
      ...
      const url = preparedCityAssetUrl(fine.assetOrigin, keyPrefix, filename);
  ...
  const index = prepareCityIndex(pages, fine.dataset, scene, { assetOrigin: fine.assetOrigin, keyPrefix });
  let indexBytes = 0;
