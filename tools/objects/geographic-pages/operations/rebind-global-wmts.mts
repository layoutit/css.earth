...
assert.equal(sha256(previousBytes),inputs.hashes[sceneInput]);
...
for(const [path,expected] of Object.entries(inputs.hashes)){
  const actual=sha256(await readFile(context.projectUrl(path)));hashes[path]=actual;
...
const proof={version:"",schema:"cssearth-wmts-geographic-rebind@1",previousVersion:inputs.version,
  previousSceneSha256:sha256(previousBytes),sceneSha256:hashes[sceneInput],
  geographicFrames:frames(scene).reduce((sum,b)=>sum+b.frames.length,0),
  geographicFramesSha256:sha256(JSON.stringify(frames(scene))),changedInputs:changed,
