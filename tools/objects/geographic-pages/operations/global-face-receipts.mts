...
export function validateGlobalCityFaceReceipt(value: unknown, expected: ReturnType<typeof expectedGlobalCityFace>, source: Pick<ReturnType<typeof parseCitySource>, 'dataset' | 'delivery'>, catalogSha256: string) {
  assertGlobalFaceReceipt(value); const receipt = value;
  ...
  if (!directory || !Number.isSafeInteger(directory.bytes) || directory.bytes < 1 ||
      !isPreparedCityAssetUrl({ assetPath: `/${source.delivery.keyPrefix}/`, dataset: source.dataset, assetOrigin: source.delivery.assetOrigin },
