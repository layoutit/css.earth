...
export async function readPublishedCoverage(path) {
  const bytes = await readFile(path).catch(error => error.code === "ENOENT" ? null : Promise.reject(error));
...
export function assembleCityCoveragePlan(base, snapshot, {assetPath=base.assetPath}={}) {
...
function validateSnapshot(snapshot) {
  if (snapshot.schema !== "cssearth-published-city-coverage@1" || !Array.isArray(snapshot.faces) ||
