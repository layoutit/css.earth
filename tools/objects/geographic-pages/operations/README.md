...
When the source recipe declares `coarseReleasePath`, ordinary preparation also
verifies that source pin and binds its intermediate imagery to the exact fine
root closure. Rebuilding the fine binding therefore preserves the declared
coarse imagery. A changed fine release requires a compatible coarse certificate.

`integrate:earth-coarse <release-directory>` verifies the complete publication
receipt and authors this compact source pin. It records the release and source
identities, bounded roots and fine compatibility without copying the full asset
inventory or private preparation paths. Normal object preparation owns the
resulting runtime JSON and local publication. Integration itself uploads nothing.

Observation recipes declare preparation operators and card ownership in
`source/observations.json`. `pnpm datasets prepare --object=earth` delegates to
the shared object preparer, which emits `prepared/observation-<id>.json`, entity
bindings and the complete asset manifest. A dataset filter scopes source
admission; preparation rebuilds the complete object package and can be expensive.
`pnpm datasets validate --object=earth` checks the existing prepared package
without rebuilding or uploading it.
