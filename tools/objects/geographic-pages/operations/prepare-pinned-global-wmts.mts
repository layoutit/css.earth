...
export async function preparePinnedGlobalWmts({objectId,projectRoot=projectDirectory,scene,pages,
  writeOutput=true,verifyOnly=false,onProgress=()=>{}}: { objectId: string; projectRoot?: string; scene?: Parameters<typeof prepareHierarchy>[0]['scene']; pages?: Parameters<typeof prepareHierarchy>[0]['pages']; writeOutput?: boolean; verifyOnly?: boolean; onProgress?: Parameters<typeof prepareHierarchy>[0]['onProgress'] }) {
  const context=createOperationContext({objectId,projectRoot});
  pages??=(await context.readSource('preparation/paged-ellipsoid.json',parsePreparationRecipe)).geographic.pages;
  const result=await prepareHierarchy({sourceRoot:context.sourceRoot,packDirectory:context.projectPath('.local/wmts-global'),pages,
    scene:scene??await context.readPrepared('scene',parseGeographicScene),namespace:objectId,displayName:objectId,assetPath:context.assetPath,verifyOnly,onProgress});
