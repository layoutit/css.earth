...
export function resamplePageRgba(rgba: Uint8Array, sourceWidth: number, sourceHeight: number, bounds: { x0: number; y0: number; x1: number; y1: number }, width: number, height: number) {
  ...
  const weights = (start: number, end: number, count: number) => Array.from({ length: count }, (_, i) => {
...
export function resampleMappedPageRgba(rgba: Uint8Array,sourceWidth: number,sourceHeight: number,map: (u: number,v: number) => readonly number[],width: number,height: number,covered: ((u: number,v: number) => boolean) | null=null) {
  const output=Buffer.alloc(width*height*4);
  const row=(y: number)=>Array.from({length:width+1},(_,x)=>map(x/width,y/height));
  let previous=row(0);
