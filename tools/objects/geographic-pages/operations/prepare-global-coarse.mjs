...
const scene=await context.readPrepared('scene'),fine=await context.readPrepared('pages');
import { readWorldCoverCatalog } from '../worldcover-catalog.mjs';
import { planGlobalCoarse, coarseSourceAddress, COARSE_PREPARATION_LIMITS } from './coarse-plan.mjs';
import { acquireCoarseInputs } from './coarse-inputs.mjs';
import { prepareCoarseRelease } from './coarse-release.mjs';
...
const sources = [
  context.relativeProject(context.preparedPath('scene.json')),
  'tools/objects/geographic-pages/operations/coarse-plan.mjs',
  'tools/objects/geographic-pages/operations/coarse-page.mjs',
  'tools/objects/geographic-pages/operations/coarse-replacements.mjs',
  'tools/objects/geographic-pages/page-geometry.mjs',
  'tools/objects/geographic-pages/worldcover-catalog.mjs',
  'tools/objects/geographic-pages/wmts-page-geometry.mjs',
  'tools/objects/geographic-pages/wmts-polar-geometry.mjs',
  'tools/objects/geographic-pages/wms-page-geometry.mjs',
...
const inputs = { schema: 'cssearth-global-coarse-inputs@1', sourceHashes, assetPath: context.assetPath,
...
if (values.phase !== 'plan') {
  ...
  if (values.phase === 'acquire') {
    ...
    await prepareCoarseRelease({ plan, directory, scene, fine, project: root, assetPath: context.assetPath,
