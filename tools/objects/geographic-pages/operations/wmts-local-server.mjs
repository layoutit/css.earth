...
import {createOperationContext} from './context.mjs';
...
export function wmtsLocalMirror({objectId,projectRoot,directory,assetOrigin,fetcher=fetch}={}) {
  const context=createOperationContext({objectId,projectRoot});
  directory??=context.projectUrl('.local/wmts-global/');
  assetOrigin??=JSON.parse(readFileSync(context.sourceUrl('city/manifest.json'),'utf8')).delivery.assetOrigin;
