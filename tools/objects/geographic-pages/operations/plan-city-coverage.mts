#!/usr/bin/env node
import { parseGeographicScene, shape, array, number } from '../source-records.mts';
...
import { CITY_PAGE_PIXELS, pageKey } from '../page-geometry.mts';

const {pin,entries}=await readWorldCoverCatalog({directory:context.sourceUrl("city/")});
const root=context.projectRoot;
const ancestors=new Set<string>(),sourceTiles=new Set<string>();
interface CoverageSummary { schema:string;dataset:string;catalogSha256:string;qualification:string;jobs:number;jobsByLevel:Record<string,number>;jobsWithUnavailableSourceTiles:number;blockedJobs:number;blockedExamples:{id:string;window:import('../contracts.mts').CitySourceWindow}[];maximumWindowPixels:number;pagesByLevelUpperBound:Record<string,number>;sourceWindowPixels:number;faceBuild:{acceptedFaces:number;nonemptyFaces:number;maximumJobs:number;maximumJobsFace:string|null;maximumPages:number;maximumPagesFace:string|null;maximumLosslessCoreRgbaBytes:number;maximumLosslessCoreRgbaBytesFace:string|null};pagesUpperBound?:number;sourceObjectsReferenced?:number;sourceObjectBytes?:number;localAvailableBytes?:number;sampleExtrapolation?:{qualification:string;samplePages:number;medianFinePageBytes:number;finePageUpperBound:number;compressedFineBytes?:number}}
const summary: CoverageSummary={schema:'cssearth-city-coverage-preflight@1',dataset:pin.dataset,
...
for(const face of cityCoverageRoots()) {
  ...
  for(const job of planCityCoverage(PREPARED_SCENE,entries,[face])) {
    faceJobs++;
    summary.jobs++;
...
const fixture=shape({pages:array(shape({level:number,bytes:number}))})(JSON.parse(await readFile(resolve(root,`output/${context.objectId}-city`,pin.dataset,'manifest.json'),'utf8')));
...
const output=resolve(root,`output/${context.objectId}-city/global-coverage-preflight.json`);
await mkdir(resolve(root,`output/${context.objectId}-city`),{recursive:true});
