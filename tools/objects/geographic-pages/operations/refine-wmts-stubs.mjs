...
import { hashBytes } from "../prepare-wmts-tree.mjs";
import {commandContext} from './context.mjs';

function coverageParts({zoom,x,y},scene){
...
async function refineRegion(directory,address,version,toolHash,scene){
...
export async function refineWmtsStubs(inputDirectory,{context,workers=4}={}){
  ...
  inputs.hashes["tools/objects/geographic-pages/operations/refine-wmts-stubs.mjs"]=toolHash;
...
if(!isMainThread){parentPort.on("message",async address=>{try{await refineRegion(workerData.directory,address,workerData.version,workerData.toolHash,workerData.scene);parentPort.postMessage({ok:true});}catch(error){parentPort.postMessage({error:error.stack});}});}
