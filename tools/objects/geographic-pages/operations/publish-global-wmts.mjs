...
import { releaseFiles, verifyLocalPack, publishedPackMatches, WMTS_CACHE_CONTROL } from "../wmts-release.mjs";
import { CITY_R2_WRANGLER_VERSION } from "./r2-publish.mjs";
import { wmtsS3Environment,publishWmtsS3,verifyWmtsS3 } from "./wmts-s3-publish.mjs";
...
const { delivery } = JSON.parse(await readFile(context.sourceUrl("city/manifest.json"), "utf8"));
...
try {
  ...
  if (values.upload && !values.sample) {
    await new Promise((accept, reject) => {
      const child = spawn(process.execPath, [resolve(import.meta.dirname, "verify-global-wmts-delivery.mjs"),`--object=${context.objectId}`], { cwd: root, stdio: "inherit" });
