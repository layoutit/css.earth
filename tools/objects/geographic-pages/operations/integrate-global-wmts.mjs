...
const existing=await context.readPrepared('pages');
const scene=await context.readPrepared('scene');
import { readFile,writeFile } from "node:fs/promises";


import {prepareWmtsPagePresentation} from '../pinned-hierarchy.mjs';
import {preserveCoarseBacking} from './coarse-integration.mjs';
...
const pages=(await context.readSource('preparation/paged-ellipsoid.json')).geographic.pages;
const release=JSON.parse(await readFile(context.sourceUrl("city/wmts-release.json"),"utf8"));
const latest=context.args[0]==="--latest"?JSON.parse(await readFile(context.projectUrl(".local/wmts-global/latest.json"),"utf8")):release;
...
const plan=preserveCoarseBacking(existing,{schema:pages.schema,dataset:manifest.dataset,qualification:manifest.qualification,credit:existing.credit,sourcePage:existing.sourcePage,
...
plan.opaqueDiscs=(await prepareOpaquePolarDiscs(scene,
  await readFile(context.projectUrl(`public${canonicalPreparedAsset(scene.body.assets.poles)}`)))).discs;
