...
import { prepareGeographicTextureQuad } from "./wms-page-geometry.mts";
...
import { parseOverlayConfig, parseOverlayPin, parseOverlayData, parseBodyAttitude } from './source-records.mts';
export async function prepareVectorOverlay({sourceDirectory,publicDirectory,config: value,scene}: { sourceDirectory: string; publicDirectory: string; config: unknown; scene: GeographicScene }) {
...
const data=parseOverlayData(JSON.parse(bytes.toString('utf8'))),polygons=(feature: (typeof data.features)[number])=>feature.geometry.type==="Polygon"?[feature.geometry.coordinates]:feature.geometry.coordinates;
...
const project=([x,y]: readonly number[])=>[(x-bounds.west)/(bounds.east-bounds.west)*size,(bounds.north-y)/(bounds.north-bounds.south)*size];
...
const plan={schema:"cssearth-earth-city-pages@1",dataset:recipe.dataset,assetOrigin:recipe.assetOrigin,qualification:pin.qualification,
  ...
  bounds,legend,assets,camera:prepareLocationCamera(scene,prepareLocationPoint(scene,recipe.camera.longitude,recipe.camera.latitude),recipe.camera.zoom,{body:parseBodyAttitude(scene[config.sceneBodyKey]),camera:config.camera})};
return plan;
}
