import { PREPARED_BLOCK_SCHEMA, PREPARED_BLOCK_LIMITS } from "../../../src/platform/prepared-map/prepared-block.mjs";
...
export function encodePreparedBlock(records, { envelope = null } = {}) {
  if (!Array.isArray(records) || records.length < 1 || records.length > PREPARED_BLOCK_LIMITS.rows) throw new Error("Invalid prepared record count.");
  const columns = [], payloads = [];
  const bitsView = new DataView(new ArrayBuffer(8));
  const template = (values, depth = 0) => {
    ...
    if (typeof first === "number" || typeof first === "string") {
      ...
      if (typeof first === "string") columns.push({ type: "string", values });
      else {
        const bits = values.map(value => { bitsView.setFloat64(0, value, true); return bitsView.getBigUint64(0, true); });
    ...
    if (Array.isArray(first)) {
      if (!values.every(value => Array.isArray(value) && value.length === first.length)) throw new Error("Prepared array shapes differ.");
      return ["array", ...first.map((_, i) => template(values.map(value => value[i]), depth + 1))];
    }
    if (first && typeof first === "object") {
      const keys = Object.keys(first);
      if (!values.every(value => value && !Array.isArray(value) && typeof value === "object" && Object.keys(value).length === keys.length && keys.every(key => Object.hasOwn(value, key)))) throw new Error("Prepared object shapes differ.");
      return ["object", ...keys.map(key => [key, template(values.map(value => value[key]), depth + 1)])];
    }
...
export function packWmtsRecords(pages) {
