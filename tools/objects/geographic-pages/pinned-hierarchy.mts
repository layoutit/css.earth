...
import { createPreparedBlockDecoder, PREPARED_BLOCK_LIMITS } from "../../../src/platform/prepared-map/prepared-block.mts";
import { isPreparedBlockReference, PREPARED_BLOCK_ENCODING } from "../../../src/platform/prepared-map/prepared-block-transport.mts";
...
import type { GeographicScene, PageGeometry, WmtsCoverage } from './contracts.mts';
import { parseWmtsRelease, parseCitySource, parsePageRecipe } from './source-records.mts';
type PageRecipe = ReturnType<typeof parsePageRecipe>;
const sourcePaths = ["city/wmts-release.json", "city/catalog-pin.json", "city/worldcover-rgbnir-2021.json.gz", "city/manifest.json"];
const addresses = (level: WmtsCoverage) => level.bands.flatMap(band => Array.from({ length: band.y1 - band.y0 }, (_, dy) =>
...
export async function preparePinnedGlobalWmts({ sourceRoot, packDirectory, scene, namespace, displayName, assetPath, pages, verifyOnly = false, onProgress = () => {} }: { sourceRoot: string; packDirectory: string; scene: GeographicScene; namespace: string; displayName: string; assetPath: string; pages: PageRecipe; verifyOnly?: boolean; onProgress?: (progress: { verifiedPacks: number; totalPacks: number }) => void }) {
  ...
  const release = parseWmtsRelease(JSON.parse(pinned.get("city/wmts-release.json")!.toString("utf8"))), content = parseCitySource(JSON.parse(pinned.get("city/manifest.json")!.toString("utf8")));
  ...
  for (const file of files) {
    assert.ok(typeof file.tiles === "number" && Number.isSafeInteger(file.tiles) && file.tiles > 0, "Invalid prepared pack tile total");
    assert.ok(typeof file.leaves === "number" && Number.isSafeInteger(file.leaves) && file.leaves >= 0, "Invalid prepared pack leaf total");
  }
  assert.equal(release.tiles, files.reduce((sum, file) => sum + (file.tiles ?? 0), 0), "Worldwide pack tile totals disagree");
  assert.equal(release.leaves, files.reduce((sum, file) => sum + (file.leaves ?? 0), 0), "Worldwide pack leaf totals disagree");
  ...
  for (const address of coarse) {
    ...
    assert.ok(file, `Missing pinned coarse pack ${filename}`);
    ...
    for (const stub of envelope.external) {
      ...
      assert.ok(stub.stub && stub.level === 8 && ref && isPreparedBlockReference(ref, assetPath), "Invalid pinned region reference");
      ...
      assert.ok(target && name === stub.key.replace("wmts-tile-", "") + ".pack" && (ref.offset ?? 0) + ref.bytes <= target.bytes,
  ...
  const plan = { schema: pages.schema, dataset: release.dataset, qualification: release.qualification,
    ...
    ...presentation };
  return { report: { ...report, roots: roots.length, runtimeBinding: "regenerated-from-pinned-coarse-packs" }, plan };
}

/** Presentation and initial location are authored data; placement uses shared geometry. */
export function prepareWmtsPagePresentation(scene: GeographicScene,value: unknown) {
  const pages = parsePageRecipe(value);
  ...
  const counts=['rasterScale','poolSize','decodedPageBytes','maximumDecodedBytes','maximumConcurrentLoads','minimumZoom','maximumZoom','targetCssPixels'] as const;
  ...
  const bounds=['maximumDirectories','maximumBytes','maximumDirectoryBytes','maximumConcurrentLoads'] as const;
