...
const decode = (value: string) => value.replace(/&(?:quot|apos|amp|lt|gt);/g,
  (entity: string) => ({"&quot;":'"',"&apos;":"'","&amp;":"&","&lt;":"<","&gt;":">"} as Record<string, string>)[entity]);
...
function fields(xml: string, allowed: readonly string[]) {
  const result: Record<string, string> = {};
  const remainder = xml.replace(/<([A-Za-z]+)>([^<>]*)<\/\1>/g, (_: string, name: string, value: string) => {
...
export function worldCoverTileBounds(tile: string) {
...
export function worldCoverTileId(west: number,south: number) {
...
export function worldCoverSourceEntry(value: unknown) {
  const entry = parseWorldCoverEntry(value);
...
export function parseWorldCoverInventory(xml: string) {
  ...
  const entries: WorldCoverEntry[]=[];
  const rest=root[1].replace(/<Contents>([\s\S]*?)<\/Contents>/g,(_: string,body: string)=>{
...
export function sourceTilesForBounds(bounds: GeographicBounds, catalog: ReadonlyMap<string, WorldCoverEntry>) {
...
export async function readWorldCoverCatalog({directory}: { directory: URL }) {
  const pin=parseCatalogPin(JSON.parse(await readFile(new URL("catalog-pin.json",directory),"utf8")));
  ...
  const snapshot=parseWorldCoverSnapshot(JSON.parse(decoded.toString("utf8")));
  ...
  const entries=new Map<string, WorldCoverEntry>();
  let sourceBytes=0;
