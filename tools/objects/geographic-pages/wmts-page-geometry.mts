import { preparePolarWmtsTile } from "./wmts-polar-geometry.mts";
import { prepareCityPageGeometry } from "./page-geometry.mts";
import { preparePageTextureQuad, geographicTextureMapping } from "./wms-page-geometry.mts";
...
const facesByScene=new WeakMap<GeographicScene, PageGeometry[]>();
function preparedFaces(scene: GeographicScene){
...
export const wmtsLatitude = (row: number, zoom: number) => Math.atan(Math.sinh(Math.PI*(1-2*row/2**zoom)))*180/Math.PI;
export const wmtsRow = (latitude: number, zoom: number) => (1-Math.asinh(Math.tan(latitude*Math.PI/180))/Math.PI)/2*2**zoom;

export function wmtsAddress(longitude: number, latitude: number, zoom: number) {
...
export function mercatorStripErrorBound(zoom: number, row0: number, row1: number) {
...
export function prepareWmtsTile({zoom,x,y}: TileAddress,scene: GeographicScene): WmtsPage[] {
  const n=2**zoom;
  ...
  const pieces: WmtsPage[]=[];
  const split=(row0: number,row1: number): void=>{
