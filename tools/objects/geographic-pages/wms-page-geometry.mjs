...
import { prepareCityPageGeometry, CITY_PAGE_RASTER_SCALE, cityPageRasterDensity } from "./page-geometry.mjs";
...
const multiply = (a, b, size) => Array.from({ length: size * size }, (_, i) =>
  Array.from({ length: size }, (_, k) => a[Math.floor(i / size) * size + k] * b[k * size + i % size])
    .reduce((sum, value) => sum + value, 0));
const transpose4 = m => Array.from({ length: 16 }, (_, i) => m[(i % 4) * 4 + Math.floor(i / 4)]);
function inverse3(m) {
...
export function prepareWmsPage(address, scene) {
...
export function prepareGeographicTextureQuad(page, {west,east,south,north}, rasterScale) {
  return preparePageTextureQuad(page, geographicTextureMapping(page,{west,east,south,north}),rasterScale);
}

const geographicInverses = new WeakMap(), pageFrames = new WeakMap();
export function geographicTextureMapping(page,{west,east,south,north}) {
...
export function preparePageTextureQuad(page,h,rasterScale) {
