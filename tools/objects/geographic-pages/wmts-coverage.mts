import { wmtsRow } from "./wmts-page-geometry.mts";
import { worldCoverTileBounds } from "./worldcover-catalog.mts";

import type { WmtsCoverage, CoverageBand, WorldCoverEntry } from './contracts.mts';
const union = (ranges: [number, number][]) => {
...
export function prepareWmtsCoverage(entries: readonly Pick<WorldCoverEntry, 'tile'>[], zoom: number, { blockSide = 8, includePolar = false } = {}): WmtsCoverage {
  ...
  const events = new Map<number, { id: string; range?: [number, number] }[]>();
  const add = (row: number, value: { id: string; range?: [number, number] }) => { if (!events.has(row)) events.set(row, []); events.get(row)!.push(value); };
  ...
  const rows = [...events.keys()].sort((a,b)=>a-b), active = new Map<string, [number, number]>(), bands: CoverageBand[] = [];
  for (let i = 0; i + 1 < rows.length; i++) {
    ...
    const ranges = union([...active.values()].map(range => [...range] as [number, number]));
  ...
  const blockRows = new Map<number, [number, number][]>();
  for (const band of bands) for (let y = Math.floor(band.y0 / blockSide); y < Math.ceil(band.y1 / blockSide); y++) {
    ...
    blockRows.get(y)!.push(...band.ranges.map(([a,b]): [number, number]=>[Math.floor(a/blockSide),Math.ceil(b/blockSide)]));
  }
