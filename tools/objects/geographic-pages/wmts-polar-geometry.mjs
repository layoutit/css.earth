...
import { preparePageTextureQuad } from "./wms-page-geometry.mjs";

const caps = new WeakMap();
const latitude = (row, zoom) => Math.atan(Math.sinh(Math.PI*(1-2*row/2**zoom)))*180/Math.PI;
...
const apply=(h,u,v)=>{const w=h[6]*u+h[7]*v+h[8];return [(h[0]*u+h[1]*v+h[2])/w,(h[3]*u+h[4]*v+h[5])/w];};
...
export function polarGeographicUv(page, longitude, lat) {
  const p=page.geographicProjection,m=p.matrix;
...
export function preparePolarWmtsTile({zoom,x,y},scene,url,startIndex=0){
  if(!caps.has(scene))caps.set(scene,[0,15].map(b=>prepareCityPageGeometry({level:0,x:0,y:b},scene)));
  const pieces=[],n=2**zoom,west=-180+x/n*360,longitudeSpan=360/n;
  ...
  for(const coarse of caps.get(scene)){
    ...
    const point=(u,v)=>polarGeographicUv(coarse,west+longitudeSpan*u,latitude(y+v,zoom));
    const split=(u0,u1,v0,v1,depth)=>{
      const points=[[u0,v0],[u1,v0],[u1,v1],[u0,v1]].map(([u,v])=>point(u,v));
      const h=quad(points),inv=inverse(h);
      let error=0;
      for(let j=0;j<=4;j++)for(let i=0;i<=4;i++){
        const u=i/4,v=j/4,actual=point(u0+(u1-u0)*u,v0+(v1-v0)*v),mapped=apply(inv,...actual);
