...
export function releaseFiles(release, sample = false) {
  if (release.schema !== "cssearth-global-wmts-release@1" ||
      !/^[a-f0-9]{16}$/u.test(release.version ?? "") || !Array.isArray(release.files)) {
...
export async function verifyLocalPack(directory, file) {
...
export async function publishedPackMatches(url, file, { fetcher = fetch, origin = "https://css.earth" } = {}) {
