...
export async function prepareSolarSystemMarkerStrip({ tiles, schema, provenance, urls, tilePixels = 16 }) {
  if (!Array.isArray(tiles) || tiles.length === 0 || new Set(tiles.map(tile => tile.id)).size !== tiles.length ||
      tiles.some(tile => !/^[a-z][a-z0-9-]*$/u.test(tile.id ?? "") || !(tile.size > 0) ||
        !Array.isArray(tile.color) || tile.color.length !== 3 || tile.color.some(value => !Number.isInteger(value) || value < 0 || value > 255)) ||
