 ...
 *
 *   node tools/objects/interferometry/oifits-select.mts <in.fits> <out.fits> [--window <min>:<max> ...] [--wavelength-scale <factor>]
 *
 ...
 * reconstruction's pixel scale is the sky's. Already-flagged cells stay flagged. */
import { readFile, writeFile } from 'node:fs/promises';
import { resolve } from 'node:path';
import { pathToFileURL } from 'node:url';
import { binaryTable, numbers, readFitsHdus, tableColumn, writeCell } from './fits-table.mts';

export interface SelectOptions { readonly windowsMetres?: readonly (readonly [number, number])[]; readonly wavelengthScale?: number }

export function selectOifits(input: Buffer, { windowsMetres = [], wavelengthScale = 1 }: SelectOptions) {
  ...
  if (windowsMetres.length) {
    for (const hdu of hdus.filter(hdu => ['OI_VIS', 'OI_VIS2', 'OI_T3'].includes(hdu.extname))) {
      ...
      const inside = list.map(wavelength => windowsMetres.some(([min, max]) => wavelength >= min && wavelength <= max));
      for (let row = 0; row < table.rows; row++) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (!input || !output) throw new TypeError('Usage: oifits-select <in.fits> <out.fits> [--window <min>:<max> ...] [--wavelength-scale <factor>]');
  ...
  const result = selectOifits(await readFile(input), { windowsMetres, wavelengthScale });
