...
export function selectOifits(input: Buffer, { windowsMetres = [], mjdRange, half, wavelengthScale = 1 }: SelectOptions) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  let wavelengthScale = 1, mjdRange: [number, number] | undefined, half: 'even' | 'odd' | undefined;
  ...
  const result = selectOifits(await readFile(input), { windowsMetres, wavelengthScale, ...(mjdRange ? { mjdRange } : {}), ...(half ? { half } : {}) });
