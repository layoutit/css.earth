...
async function getExplorationSession(root: string, directory: string, pick: number, progress: (text: string) => void = () => {}, api: SessionServices = services, options: { readonly offline?: boolean } = {}) {
  return locked(directory, async () => {
    ...
    if (choice.state === 'qualify') {
      ...
      answer = await exploreForRequest(api, root, { ...request, target: answer.target }, saved.observation);
...
export interface FamilyRequestSession {readonly schema:typeof FAMILY_REQUEST_SESSION_SCHEMA;readonly createdAt:string;readonly original:import('./family-request.mts').NormalizedFamilyRequest;readonly assessment:import('./family-request.mts').FamilyRequestAssessment;readonly status:'fulfilled'|'unresolved'|'refused'}
export async function saveFamilyRequestSession(directory:string,input:import('./family-request.mts').FamilyScientificRequest,descriptor:unknown):Promise<FamilyRequestSession>{
  const {assessFamilyRequest}=await import('./family-request.mts');const assessment=assessFamilyRequest(input,descriptor),answers=Object.values(assessment.verdicts).filter((value):value is NonNullable<typeof value>=>value!==undefined),status=answers.some(v=>v.answer==='no')?'refused':answers.length&&answers.every(v=>v.answer==='yes')?'fulfilled':'unresolved';
