...
export async function selectedProductInput(root:string, selection:ObservationSelection) {
 ...
 const current=(await loadQualifiedObservations(root,selection.request.target)).find(p=>p.product===stated.product && p.productRecord===stated.productRecord && p.receipt===stated.receipt && p.program===selection.programme && p.telescope===selection.telescope && p.mode===selection.mode && p.observation===stated.observation);
