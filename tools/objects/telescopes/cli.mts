...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='new-object'){
    const rest=args.slice(1),fromArchive=rest.indexOf('--from-archive'),outAt=rest.indexOf('--out'),out=outAt>=0?rest[outAt+1]:undefined;
    const flags=rest.filter(arg=>arg.startsWith('-')&&arg!=='--from-archive'&&arg!=='--out'),unknown=flags.filter(flag=>!['--json','--verbose','--check','--skip-existing'].includes(flag));
    if(unknown.length)throw new TypeError(`Unknown new-object option ${unknown[0]}.`);
    const common={check:flags.includes('--check'),skipExisting:flags.includes('--skip-existing'),json:flags.includes('--json'),verbose:flags.includes('--verbose')};
    if(fromArchive>=0){
      const hosts=rest.slice(fromArchive+1).filter((arg,i,list)=>!arg.startsWith('-')&&list[i-1]!=='--out');
      if(!hosts.length||!out)throw new TypeError('Use telescope new-object --from-archive HOST... --out SPEC.json [--json].');
      return {command,hosts,out:resolve(out),...common};
    }
    const positional=rest.filter((arg,i,list)=>!arg.startsWith('-')&&list[i-1]!=='--out');
    if(positional.length!==1)throw new TypeError('Use telescope new-object SPEC.json [--check] [--skip-existing] [--json], or --from-archive HOST... --out SPEC.json.');
  ...
  try {
    ...
    try {
      if(options.command==='wwt-fits'){
        ...
        const {runNewObject,formatNewObject,specFromArchive,checkGenerated}=await import('../new-object/generate.mts');
        if(options.hosts){
          ...
          if(options.check&&results.length)await checkGenerated(results.map(result=>result.id),root,line=>io.error(`${line}\n`));
          text=options.json?`${JSON.stringify(results)}\n`:formatNewObject(results)+(options.check&&results.length?`${results.length} objects passed the bake's first steps.\n`:'');code=0;
        }
