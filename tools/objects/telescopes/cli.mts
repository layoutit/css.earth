...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='new-object'){
    ...
    const flags=rest.filter(arg=>arg.startsWith('-')&&arg!=='--from-archive'&&arg!=='--out'),unknown=flags.filter(flag=>!['--json','--verbose','--check','--skip-existing','--bake','--refresh'].includes(flag));
