...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='new-object'){
    ...
    const flags=rest.filter(arg=>arg.startsWith('-')&&arg!=='--from-archive'&&arg!=='--out'),unknown=flags.filter(flag=>!['--json','--verbose','--check','--skip-existing','--bake'].includes(flag));
    if(unknown.length)throw new TypeError(`Unknown new-object option ${unknown[0]}.`);
    const common={check:flags.includes('--check'),bake:flags.includes('--bake'),skipExisting:flags.includes('--skip-existing'),json:flags.includes('--json'),verbose:flags.includes('--verbose')};
    if(fromArchive>=0){
    ...
    if(positional.length!==1)throw new TypeError('Use telescope new-object SPEC.json [--check | --bake] [--skip-existing] [--json], --bake ID..., or --from-archive HOST... --out SPEC.json.');
    ...
    try {
      if(options.command==='wwt-fits'){
        ...
        if(options.ids){
          ...
          const baked=(options.check||options.bake)&&results.length?await prepareObjects(results.map(result=>result.id),{progress,...(options.bake?{}:{to:'page'})}):true;
