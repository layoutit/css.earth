...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='papers'){
    ...
    for(let i=1;i<args.length;i++){
      ...
      if(!['--instrument','--out'].includes(arg)||values.has(arg))throw new TypeError(`Unknown or repeated papers option ${arg}.`);
      const value=args[++i];if(!value||value.startsWith('--'))throw new TypeError(`Missing value for ${arg}.`);values.set(arg,value);
    }
    if(positional.length!==1)throw new TypeError('Use telescope papers OBJECT [--instrument NAME] [--json] [--out DIRECTORY].');
    const instrument=values.get('--instrument'),directory=values.get('--out');
    return {command,target:positional[0]!,...(instrument?{instrument}:{}),...(directory?{directory:resolve(directory)}:{}),json:flags.has('--json'),verbose:flags.has('--verbose')};
  }
  if(command==='candidates'||command==='associate'){
    ...
    for(let i=1;i<args.length;i++){
      ...
      if(!arg.startsWith('-')){positional.push(arg);continue;}
      if(arg==='--json'||arg==='--verbose'){if(flags.has(arg))throw new TypeError(`Repeated option ${arg}.`);flags.add(arg);continue;}
      const allowed=command==='candidates'?['--epoch','--out','--figure-background']:['--system','--out','--figure-background'];
    ...
    const common={directory:resolve(directory??''),...(background?{figureBackground:background as 'transparent'|'opaque'}:{}),json:flags.has('--json'),verbose:flags.has('--verbose')};
  ...
  try {
    ...
    try {
      if(options.command==='family-run'){let raw:Record<string,unknown>;if(options.parameters){const value:unknown=JSON.parse(await readFile(options.parameters,'utf8'));if(!value||typeof value!=='object'||Array.isArray(value))throw new TypeError('Family operation parameters must be a JSON object.');raw=value as Record<string,unknown>;if(raw.operationId!==undefined&&raw.operationId!==options.operationId)throw new TypeError(`Parameter operationId ${String(raw.operationId)} disagrees with ${options.operationId}.`);}else{if(familyOperationNeedsParameters(options.operationId))throw new TypeError(`Family operation ${options.operationId} requires --params PARAMS.json.`);raw={};}const result=await api.executeFamilyOperation(options.descriptor,{...raw,operationId:options.operationId}as unknown as FamilyOperationParameters,options.directory);text=options.json?`${JSON.stringify(result)}\n`:`Product: ${result.product}\nEvidence: ${result.record}\nOperation: ${result.operation.id}\n`;code=0;
        ...
        const result=await searchPapers(root,{target:options.target,...(options.instrument?{instrument:options.instrument}:{}),...(options.directory?{directory:options.directory}:{}),progress:line=>io.error(`${line}\n`)});
        ...
        const {runCandidates,epochMjd}=await import('./sky-association.mts'),result=await runCandidates(options.system,epochMjd(options.epoch),options.directory,options.figureBackground?{figureBackground:options.figureBackground}:{});
        ...
        const {runAssociation}=await import('./sky-association.mts'),result=await runAssociation(options.measurements,options.system,options.directory,options.figureBackground?{figureBackground:options.figureBackground}:{});
