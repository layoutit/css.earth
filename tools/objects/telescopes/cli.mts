...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='new-object'){
    const positional=args.slice(1).filter(arg=>!arg.startsWith('-')),flags=args.slice(1).filter(arg=>arg.startsWith('-'));
    const unknown=flags.filter(flag=>flag!=='--json'&&flag!=='--verbose');
    if(unknown.length||positional.length!==1)throw new TypeError('Use telescope new-object SPEC.json [--json].');
    return {command,spec:resolve(positional[0]!),json:flags.includes('--json'),verbose:flags.includes('--verbose')};
  }
  if(command==='papers'){
    const positional:string[]=[],values=new Map<string,string>(),flags=new Set<string>();
    for(let i=1;i<args.length;i++){
  ...
  try {
    ...
    try {
      if(options.command==='wwt-fits'){
        ...
        const {runNewObject,formatNewObject}=await import('../new-object/generate.mts'),results=await runNewObject(options.spec,{root,progress:line=>io.error(`${line}\n`)});
        text=options.json?`${JSON.stringify(results)}\n`:formatNewObject(results);code=0;
