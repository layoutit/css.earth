...
export function parseCli(args: readonly string[]): CliOptions {
  ...
  if(command==='fetch'){
    ...
    for(let i=1;i<args.length;i++){
      ...
      if(!['--pick','--out','--archive'].includes(arg)||values.has(arg))throw new TypeError(`Unknown or repeated fetch option ${arg}.`);
    ...
    return {command,archive:archive as 'keck'|'gemini'|'opus'|'chandra'|'spitzer',exploration:resolve(positional[0]!),pick,directory:resolve(values.get('--out')!),json:flags.has('--json'),verbose:flags.has('--verbose')};
  }
  if(command==='wwt-fits'){
    const positional:string[]=[],values=new Map<string,string>(),flags=new Set<string>();
    for(let i=1;i<args.length;i++){
    ...
    try {
      if(options.command==='wwt-fits'){
        ...
        const result=options.archive==='keck'?await fetchKeckSource(options.exploration,options.pick,options.directory)
          ...
          :options.archive==='chandra'?await fetchChandraSource(options.exploration,options.pick,options.directory)
          :await fetchSpitzerSource(options.exploration,options.pick,options.directory);
