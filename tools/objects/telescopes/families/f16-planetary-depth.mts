...
export interface PinnedDepthFile {readonly path:string;readonly bytes:number;readonly sha256:string}
...
export async function exportPlanetaryDepthNative(pin:PinnedDepthFile,output:string){const actual=await pinFile(pin.path);if(actual.bytes!==pin.bytes||actual.sha256!==pin.sha256)throw new Error('Planetary grid pin changed.');await copyFile(pin.path,output);return{path:output,...actual};}
