...
h=fits.Header();h['CTYPE1']='CRLN';h['CRPIX1']=1.;h['CRVAL1']=0.;h['CDELT1']=1.;h['CUNIT1']='deg';h['CTYPE2']='CRLT';h['CRPIX2']=91.;h['CRVAL2']=0.;h['CDELT2']=1.;h['CUNIT2']='deg';h['CTYPE3']='HECR';h['CRPIX3']=1.;h['CRVAL3']=1.5;h['CDELT3']=.05;h['CUNIT3']='solRad';h['BUNIT']='cm^-3';h['INSTRUME']='SECCHI';h['DATE-AVG']='2007-02-11T14:13:00.012';fits.PrimaryHDU(a.astype('>f4'),header=h).writeto(sys.argv[1])`, file], { env: { ...process.env, ...toolchain.env }, encoding: 'utf8' });
    ...
    const prepared = await executeFamilyOperation(descriptorPath, { operationId: 'spherical-grid-prepare-volume', size: 32, transfer: { kind: 'sqrt', range: [0, 500000], color: [1, .7, .35], strength: 1, clipping: 'clamp', invalid: 'transparent' }, displayWeight: { kind: 'radial-smoothstep', unit: 'solRad', inner: [1.5, 1.7], outer: [3.7, 4] } }, resolve(work, 'prepared')), record = await readProductRecord(prepared.record), wrapper = JSON.parse(await readFile(prepared.product, 'utf8'));
    assert.equal(wrapper.schema, 'cssearth-physical-grid-volume@1'); assert.equal(wrapper.resampling.method, 'trilinear'); assert.equal(wrapper.displayWeight.path, 'source/display-weight.fits'); assert.ok(wrapper.displayWeight.partialSamples > 0); assert.ok(wrapper.displayWeight.zeroSamples > 0); assert.ok(record?.outputs.some(output => output.path.endsWith('/source/display-weight.fits'))); assert.ok(record?.software.some(item => item.name.includes('f16-spherical-grid'))); assert.ok(await readFile(resolve(prepared.directory, wrapper.prepared)));
    ...
    const innerTransition = voxel(22, 16, 16); // r ~= 1.63 solRad: valid science, partial display weight.
    assert.ok(rgba[innerTransition + 3]! > 0 && rgba[innerTransition + 3]! < 255);
    assert.ok(rgba[innerTransition]! > 0 && rgba[innerTransition]! < rgba[innerTransition + 3]!, 'the display weight must attenuate the encoded emission channel consumed by shared-opacity');
