...
const owner=(name:string)=>({module:'tools/objects/telescopes/families/f16/f16-planetary-coverage.mts',export:name});
...
export const F16_PLANETARY_COVERAGE_HANDLER:FamilyHandler={
  ...
  profiles:[{id:F16_PLANETARY_COVERAGE_PROFILE,format:'Exact sparse planetary support: archive tracks, body-fixed rays, located stations and profiles',version:'1',families:['F16'],
    evidence:[{path:'tools/objects/telescopes/families/f16/f16-planetary-coverage.test.mts',establishes:'Real archive support geometry is retained sample for sample, and every grid, volume, material-property, interior-model and interpolation promotion is refused.',status:'complete'}]}],
  recognizes:()=>[],
  operations:product=>{
