...
raw=np.zeros(cart.shape+(4,),dtype=np.uint8);raw[...,0]=np.floor(scaled*255+.5).astype(np.uint8);raw[...,3]=np.where(valid,255,0).astype(np.uint8);raw.tofile(r['raw'])
...
json.dump({'inspection':inspection,'resampling':{'method':'trilinear','size':n,'validSamples':int(valid.sum()),'invalidSamples':int(valid.size-valid.sum()),'boundsSolarRadii':[[-rmax,-rmax,-rmax],[rmax,rmax,rmax]]},'dimensions':[n,n,n]},sys.stdout)`;
...
async function run(pin: PinnedGridFile, context: SphericalGridContext, operation: 'inspect' | 'resample', options?: { readonly size: number; readonly transfer: PhysicalGridTransfer; readonly raw: string; readonly native: string }): Promise<any> {
...
export async function preparePhysicalSphericalVolume(input: { readonly id: string; readonly pin: PinnedGridFile; readonly context: SphericalGridContext; readonly size: number; readonly transfer: PhysicalGridTransfer; readonly destination: string }): Promise<SphericalVolumeResult> {
  const destination = resolve(input.destination), source = resolve(destination, 'source'); await mkdir(source, { recursive: true });
  const rawPath = resolve(source, 'density.rgba'), native = resolve(source, 'cartesian-grid.fits'), result = await run(input.pin, input.context, 'resample', { size: input.size, transfer: input.transfer, raw: rawPath, native });
  ...
  return { object: objectPath, prepared: resolve(destination, 'prepared/volume.json'), native, files: await files(destination), inspection: result.inspection, resampling: result.resampling, transfer: input.transfer, frame };
}
