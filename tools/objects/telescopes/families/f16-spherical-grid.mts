...
valid=np.isfinite(cart); scaled=np.zeros_like(cart,dtype=np.float64);scaled[valid]=np.clip((cart[valid]-lo)/(hi-lo),0,1)
if t['kind']=='sqrt':scaled[valid]=np.sqrt(scaled[valid])
weight=np.where(valid,1.,0.).astype(np.float32); display_weight=None
if r.get('displayWeight') is not None:
 d=r['displayWeight']
 def smoothstep(edge0,edge1,x):
  q=np.clip((x-edge0)/(edge1-edge0),0.,1.);return q*q*(3.-2.*q)
 rr3=np.sqrt(xyz[None,None,:]**2+xyz[None,:,None]**2+xyz[:,None,None]**2)
...
json.dump({'inspection':inspection,'resampling':{'method':'trilinear','size':n,'validSamples':int(valid.sum()),'invalidSamples':int(valid.size-valid.sum()),'boundsSolarRadii':[[-rmax,-rmax,-rmax],[rmax,rmax,rmax]]},'dimensions':[n,n,n],**({'displayWeight':display_weight} if display_weight is not None else {})},sys.stdout)`;
...
function validateDisplayWeight(value: SphericalDisplayWeight | undefined): SphericalDisplayWeight | undefined {
  ...
  if (value.kind !== 'radial-smoothstep' || value.unit !== 'solRad' || !finitePair(value.inner) || !finitePair(value.outer) || value.inner[0] < 1.5 || value.inner[1] > value.outer[0] || value.outer[1] > 4) throw new TypeError('Spherical display weight must be ordered radial smoothsteps inside the measured 1.5–4.0 solRad shell.');
  return value;
}
async function run(pin: PinnedGridFile, context: SphericalGridContext, operation: 'inspect' | 'resample', options?: { readonly size: number; readonly transfer: PhysicalGridTransfer; readonly raw: string; readonly native: string; readonly displayWeight?: SphericalDisplayWeight; readonly displayWeightPath?: string }): Promise<any> {
  context = validateContext(context); const actual = await pinFile(pin.path);
  if (actual.bytes !== pin.bytes || actual.sha256 !== pin.sha256) throw new Error('Spherical-grid FITS pin changed.');
...
export async function preparePhysicalSphericalVolume(input: { readonly id: string; readonly pin: PinnedGridFile; readonly context: SphericalGridContext; readonly size: number; readonly transfer: PhysicalGridTransfer; readonly displayWeight?: SphericalDisplayWeight; readonly destination: string }): Promise<SphericalVolumeResult> {
  ...
  const frame = { referenceFrame: context.frame, epochJdTt: result.inspection.epochJdTt, originM: [0, 0, 0], localToReferenceXyzw: [0, 0, 0, 1], metersPerUnit: result.inspection.metresPerSolarRadius, boundsUnits: bounds }, objectPath = resolve(destination, 'object.json'); await writeFile(objectPath, JSON.stringify({ schema: 'cssearth-object@1', id: stable(input.id, 'spherical volume id'), type: 'density-volume', properties: { volume: frame, preparation: { source: 'source/volume.json', sha256: sha256(recipeBytes) } } }, null, 2) + '\n');
  const { prepareDensityVolumeObject } = await import('../../../../tools/objects/dist/prepare-volume.js'), { loadPreparedCssVolume } = await import('../../../../src/renderers/css/dist/universe.js'); await prepareDensityVolumeObject({ objectDirectory: destination }); const object = JSON.parse(await readFile(objectPath, 'utf8')); await loadPreparedCssVolume(object, { read: async path => { const bytes = await readFile(resolve(destination, path)); return bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) as ArrayBuffer; } });
