...
async function mapFixture(root:string,target='mercury'){
  ...
  await writeFile(files.navigation,JSON.stringify({radiiKm:[2439.7,2439.7,2439.7],shape:'reference ellipsoid',registration:{method:'disc'},uncertainty:'source uncertainty',normalization:{minimum:1,maximum:4,colormap:'viridis',missing:'#333941'},sourceRequest:{target},sourceSatisfaction:{status:'unresolved'}}));
...
test('sphere inspection validates the map contract, navigation and existing sphere owner',async()=>{
  ...
  try{
    ...
    assert.equal(ready.target,'mercury');assert.deepEqual(ready.sourceSatisfaction,{status:'unresolved'});assert.ok(ready.outputs.some(output=>output.kind==='sphere'&&output.available));
    ...
    assert.deepEqual(receipt.parameters.sourceRequest,{target:'mercury'});assert.deepEqual(receipt.parameters.sourceSatisfaction,{status:'unresolved'});
...
test('terminal products verify current output bytes without reopening historical sources',async()=>{
  ...
  try{
    ...
    await writeProductRecord(record,{telescope:'Fixture',stage:'telescope-sphere',inputs:[{role:'historical source',identity:resolve(root,'absent.fits'),bytes:1,sha256:'a'.repeat(64)}],parameters:{target:'mercury',sourceSatisfaction:{status:'unresolved'}},software:[]},[{path:'sphere.html',file:html}]);
