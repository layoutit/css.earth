...
test('qualification readback binds facts to the exact output, receipt and producing record', async () => {
  ...
  try {
    ...
    assert.equal(assessRequest({ ...request, angularResolutionArcsec: 1 }, loaded[0]!.facts).status, 'fulfilled');
    assert.equal(assessRequest({ ...request, angularResolutionArcsec: .2 }, loaded[0]!.facts).constraints.angularResolution!.answer, 'unknown');
