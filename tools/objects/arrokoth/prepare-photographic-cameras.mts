...
for (const frame of profile.frames) {
  ...
  const provenance = await Promise.all([profile.mesh,path,...profile.references].map(async path => ({path,sha256:await digest(path)})));
...
if (profile.mvic) {
  ...
  const provenance=await Promise.all([profile.mesh,path,mvic.referenceCamera,...mvic.references].map(async path=>({path,sha256:await digest(path)})));
