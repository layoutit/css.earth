...
interface MarkerAsset {url:string;width:number;height:number;tilePixels:number;bytes:number;sha256:string;}
// The original prepared flat-colour strip rasterizer, with body facts and
// output addresses supplied by the caller. It performs no filesystem writes.
import { createHash } from "node:crypto";
import sharp from "sharp";

export async function prepareSolarSystemMarkerStrip({ tiles, schema, provenance, urls, tilePixels = 16 }:{tiles:readonly MarkerTile[];schema:string;provenance:string;urls:Readonly<Record<number,string>>;tilePixels?:number}) {
  if (!isArray(tiles) || tiles.length === 0 || new Set(tiles.map(tile => tile.id)).size !== tiles.length ||
      ...
      [1, 2].some(density => typeof urls?.[density] !== "string" || !urls[density].startsWith("/scenes/"))) {
    throw new TypeError("System marker strip needs explicit tiles, provenance, and density addresses.");
  }
  const files:Record<number,MarkerAsset> = {}, assets = [];
  for (const density of [1, 2]) {
    ...
    files[density] = Object.freeze({ url: urls[density], width, height: tile, tilePixels: tile,
      bytes: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex") });
    assets.push({ url: urls[density], bytes });
  }
  return { plan: Object.freeze({ schema, model: "synthetic-flat-colour-discs", provenance, count: tiles.length,
    tiles: Object.freeze(Object.fromEntries(tiles.map(({ id, size }, index) =>
      [id, Object.freeze({ index, count: tiles.length, size })]))), density1: files[1], density2: files[2] }), assets };
}
