export interface ObservedRgb {data: Uint8Array; info: {width: number; height: number; channels: number};}
export interface PolarDetail {structure: ObservedRgb; palette: ObservedRgb; contrast: number; tint: number; structureRadius?: number;}
export interface PolarDetails {north?: PolarDetail; south?: PolarDetail;}
export interface PolarContinuationOptions {
  source: ObservedRgb; tileSize: number; firstMeasuredRow: number; lastMeasuredRow: number; measuredHeight: number;
  overlap?: number; detailLookbackDegrees?: number; polarDetails?: PolarDetails; detailBlendStartRadius?: number; detailBlendEndRadius?: number;
  edgeStructureWeight?: number; alphaOpaqueRadius?: number; alphaTransparentRadius?: number; measuredProjectionEdgeLatitudeDegrees?: number | null;
}
export interface PolarTransitionOptions {source: ObservedRgb; polarDetails: PolarDetails; transitionStartLatitudeDegrees?: number; transitionEndLatitudeDegrees?: number;
  edgeStructureWeight?: number; innerDetailRadius?: number; outerDetailRadius?: number;}
...
export function preparePolarContinuationAtlas({
  ...
  if (!source?.data || !source?.info || source.info.channels < 3 ||
    ...
    throw new TypeError("Observed polar continuation input is invalid.");
  }
  ...
  const scaleRow = (row: number) => row / (measuredHeight - 1) * (sourceHeight - 1);
      ...
      for (let x = 0; x < tileSize; x += 1) {
        ...
        let measured = measuredProjectionEdgeLatitudeDegrees === null
          ...
          : projectedPolarSample({
            ...
            longitude,
            radius,
            unmeasuredCoreRadius: unmeasuredCoreRadius ?? (() => { throw new Error("Polar projection requires a core radius."); })(),
...
function projectedPolarSample({
  ...
  measuredProjectionEdgeLatitudeDegrees,
  harmonic,
}: {source: ObservedRgb; pole: string; longitude: number; radius: number; unmeasuredCoreRadius: number; measuredProjectionEdgeLatitudeDegrees: number; harmonic: ReturnType<typeof measuredLongitudeHarmonics>}) {
...
function angularlySupersampledPolarSource(source: ObservedRgb, longitude: number, latitude: number, radius: number) {
  const sampleCount = 9;
...
export function preparePolarSurfaceTransition({
  ...
  if (!source?.data || !source?.info || source.info.channels < 3 ||
    ...
    throw new TypeError("Observed polar surface transition input is invalid.");
  }
...
function validPolarDetails(details: PolarDetails) {
  ...
  return [details.north, details.south].every((detail) =>
    !detail || (
      ...
      (!Number.isFinite(detail.structureRadius) ||
        (typeof detail.structureRadius === "number" && detail.structureRadius > 0 && detail.structureRadius <= 1))
    ));
}

function prepareDetailPlan(detail: PolarDetail) {
...
function injectPolarStructure({
...
}: {base: readonly number[]; sample: readonly number[]; detailPlan: ReturnType<typeof prepareDetailPlan>; detailWeight?: number; edgeStructureWeight?: number; structureWeight?: number; tintWeight?: number}) {
  if (explicitStructureWeight === undefined && (edgeStructureWeight === undefined || detailWeight === undefined) || explicitTintWeight === undefined && detailWeight === undefined) {
    throw new TypeError('Polar structure injection requires explicit weights or a complete blend.');
  }
  ...
  const structureWeight = explicitStructureWeight ?? (
    requireBlendWeight(edgeStructureWeight) + (1 - requireBlendWeight(edgeStructureWeight)) * requireBlendWeight(detailWeight)
  );
  ...
  const tintWeight = explicitTintWeight ?? detailPlan.tint * requireBlendWeight(detailWeight);
  return structured.map((value, channel) => Math.max(0, Math.min(255,
    value * (1 - tintWeight) + tinted[channel] * tintWeight)));
}

function imageStatistics(source: ObservedRgb, { maximumRadius, minimumLuminance }: {maximumRadius: number; minimumLuminance: number}) {
  ...
  if (count === 0) throw new Error("Observed polar detail has no usable pixels.");
...
function luminance(sample: readonly number[]) {
  return sample[0] * 0.2126 + sample[1] * 0.7152 + sample[2] * 0.0722;
}

function pixelSample(source: ObservedRgb, x: number, y: number) {
...
function harmonicSample(plan: ReturnType<typeof measuredLongitudeHarmonics>, angle: number, radius: number, maximumOrder = plan.order) {
...
function bilinearSample(source: ObservedRgb, sourceX: number, sourceY: number) {
  const { width, height, channels } = source.info;
...
function requireBlendWeight(value: number | undefined): number { if (value === undefined) throw new TypeError("Missing polar blend weight."); return value; }
