...
const optionalMeasurement = (value: string | undefined) => value === 'N/A' ? undefined : value;
...
/** Read dated mission images from package-owned detached labels. Other PDS products can still
 * enter through native intake; a map or incomplete label must not hide unrelated observations. */
export async function sourcePds3Observations(root: string, targetId: string,
  reportIssue: (path: string, reason: string) => void = () => {}): Promise<SourcePds3Observation[]> {
  ...
  for (const { input: labelInput, path: labelPath } of pinned.filter(entry => entry.path.toLowerCase().endsWith('.lbl'))) {
    ...
    try {
      ...
      const horizontal = optionalMeasurement(pds3Keyword(label, 'HORIZONTAL_PIXEL_SCALE', [])),
        vertical = optionalMeasurement(pds3Keyword(label, 'VERTICAL_PIXEL_SCALE', []));
      ...
      const center = optionalMeasurement(pds3Keyword(label, 'CENTER_FILTER_WAVELENGTH', []));
      ...
      reportIssue(`src/objects/${targetId}/source/${labelPath}`, error instanceof Error ? error.message : String(error));
    }
  }
