...
export const upstream = JSON.parse(readFileSync(new URL('upstream.json', sourceRoot)));
export function readCatalog() {
  ...
  const text = source.toString(), records=[];
  const header = /^"([^"\r\n]+)"\s+"Sol"\s*\{/gm;
  for (const match of text.matchAll(header)) {
    ...
    records.push({id,designation,name:periodic?periodic[2]:long[3],radiusKm,mesh:body.match(/\bMesh\s+"([^"]+)"/)?.[1],excerpt});
  }
