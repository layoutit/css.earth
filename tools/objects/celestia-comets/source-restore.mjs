...
for(const id of ['comet-17p']){
 const original=resolve('src/planets',id,'source'),sourceRoot=resolve(stage,id),manifest=JSON.parse(await readFile(resolve(original,'manifest.json'))),plan=JSON.parse(await readFile(resolve(original,'preparation/acquisition.json')));
 ...
 const requests=[];await restoreMissingSources({sourceRoot,manifest,plan,missing:[...wanted],transport:{fetch:async(url,init)=>{requests.push(url);return fetch(url,init);}}});
