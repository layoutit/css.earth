...
test('pinned upstream files and inventory retain their identities and license',()=>{
 ...
 assert.equal(records.find(r=>r.id==='comet-c2014-un271').designation,'C/2014 UN271');
...
test('packages use the native Celestia mesh, with only physical scaling',()=>{
 for(const c of candidates){
  const root=`src/planets/${c.id}/source/`,model=JSON.parse(readFileSync(root+'shape/model.json'));
  ...
  const content=JSON.parse(readFileSync(root+'content/object.json'));assert.equal(content.lenses.controls.length,1);
  assert.equal(content.lenses.controls[0].detail,'Celestia');assert.equal(content.panel.facts.length,2);
  for(const control of content.settings.controls.filter(c=>['shadows','orbit'].includes(c.name)))assert.equal(control.checked,false);
  assert.equal(JSON.parse(readFileSync(root+'preparation/rotation.json')).phase,'arbitrary-display-phase');
 }
});

test('native source closure and shared model exports retain exact hashes',()=>{
 const code=JSON.parse(readFileSync(new URL('../native/upstream.json',sourceRoot)));
 ...
 for(const mesh of ['asteroid','roughsphere']){
  const report=JSON.parse(readFileSync(new URL(`meshes/${mesh}.json`,sourceRoot)));
