...
for(const c of await readIntake()){
 ...
 const exclude=new Set(['manifest.json',...records(manifest.inputs).map(x=>requireString(x.path)),...records(manifest.generatedIntermediates).map(x=>requireString(x.path))]),documents:{path:string;expectedBytes:number;expectedSha256:string}[]=[];
 async function walk(dir:string,pre=''):Promise<void>{for(const e of await readdir(dir,{withFileTypes:true})){const rel=pre+e.name;if(e.isDirectory())await walk(resolve(dir,e.name),rel+'/');else if(!exclude.has(rel))documents.push(refreshSourceRecord(records(manifest.documents),{path:rel,...pin(await readFile(resolve(dir,e.name)))}));}}
