...
export async function meshSource(candidate: Pick<CatalogEntry, 'mesh' | 'radiusKm'>) {
  const stem=candidate.mesh.replace('.cms','');
  const root=new URL('./source/meshes/',import.meta.url);
  const rawReport=requireRecord(JSON.parse(await readFile(new URL(`${stem}.json`,root),'utf8')));
  const report={...rawReport,axisTransform:requireString(rawReport.axisTransform),volumeEquivalentRadius:requireFiniteNumber(rawReport.volumeEquivalentRadius),vertices:requireFiniteNumber(rawReport.vertices),faces:requireFiniteNumber(rawReport.faces)};
