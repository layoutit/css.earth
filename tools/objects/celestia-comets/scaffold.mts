...
for(const c of intake){
 ...
 const descriptor=await clone('object.json');const properties=requireRecord(descriptor.properties),page=requireRecord(properties.page);delete properties.worldFrame;delete descriptor.prepared;delete page.metadata;requireRecord(properties.preparation).label=name;requireRecord(requireRecord(properties.recipe).shape).radiusKm=model.volumeEquivalentRadiusKm;await write(`${p}/object.json`,descriptor);
