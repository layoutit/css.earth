...
if(upstream.commit!=='1993a082ee6307c0df7fdc0828eb117a0e8e9958')throw Error('Curate the SOURCES catalogue binding for the new Celestia revision before scaffolding.');
...
for(const c of intake){
 ...
 const manifest=await clone('source/manifest.json');manifest.schema=`css${c.id}-authoritative-sources@2`;const inputs=records(manifest.inputs).slice(0,2);manifest.inputs=inputs;
 inputs.push({id:'celestia-mesh',sourceBinding:{kind:'catalogued',references:[{catalogueId:'celestia-comet-catalog-1993a082',role:'method',evidence:`${p}/source/reference/source-record.json#/catalog`}]},path:'shape/model.obj',...await pin(`${s}/shape/model.obj`),origin:url,credit,license,licenseEvidence:['reference/celestia.ssc','reference/GPL-2.0-or-later.txt'],acquisition:'Exported from the pinned original Celestia generator; normalized and uniformly scaled.',redistribution:'Preserve GPL terms, upstream credit and the illustrative status of the model.',consumers:['shape']});manifest.generatedIntermediates=[];manifest.documents=[];await write(`${s}/manifest.json`,manifest);
