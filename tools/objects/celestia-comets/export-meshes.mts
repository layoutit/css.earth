...
for(const name of ['asteroid','roughsphere']){
 ...
 const vector=(k:string)=>cms.match(new RegExp(`\\b${k}\\s*\\[([^\\]]+)\\]`))![1].trim().split(/\s+/).map(Number);
 const scalar=(k:string)=>Number(cms.match(new RegExp(`\\b${k}\\s+([\\d.]+)`))![1]);
 ...
 const parsed=requireRecord(JSON.parse(run.stdout));
 const numbers=(value:unknown)=>requireArray(value).map(n=>requireFiniteNumber(n));
 const raw={vertices:requireArray(parsed.vertices).map(numbers),strips:requireArray(parsed.strips).map(numbers)},rings=scalar('Rings'),slices=scalar('Slices');
 ...
 const vertices:number[][]=[],mapping=new Map<string,number>(),ids:number[]=[];let maximumWeldError=0;
 for(let i=0;i<normalized.length;i++){
  ...
  const mapped=mapping.get(key);if(mapped===undefined)throw Error('Native vertex mapping missing');ids[i]=mapped;maximumWeldError=Math.max(maximumWeldError,Math.hypot(...normalized[i].map((v,k)=>v-vertices[ids[i]][k])));
 }
