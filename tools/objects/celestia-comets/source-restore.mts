...
for(const id of ['comet-17p']){
 const original=resolve('src/planets',id,'source'),sourceRoot=resolve(stage,id),manifest=parseSourceManifest(JSON.parse(await readFile(resolve(original,'manifest.json'),'utf8'))),plan=parseAcquisitionPlan(JSON.parse(await readFile(resolve(original,'preparation/acquisition.json'),'utf8')));
