...
**Refinement, by agreement, kept by measurement.** A lens may name `refinement: { by?: 'relief' | 'frames' | 'map', tilt?: 'silhouette', agreementDegrees?: number }` (a format's own refinement, which states a `method` such as the kernel route's `mesh-limb`, is that format's and is left alone). `by` turns every camera about the pole by the named reference's decisive median; `tilt` rolls every camera about its line of sight by the silhouette's median residual when at least three frames scored and the median stands above the frames' floor. Both are decided on the untouched measurement and applied once, and the corrected lens is measured again: a turn is kept only if the reference it came from moves toward zero, a tilt only if the limb residual falls, and a correction that fails that test is reverted, with the reason in the report. Psyche keeps a 5° tilt that took its residual from 4.5° to 3.2°; Kleopatra's 3° tilt and Kalliope's 1° turn were reverted by their own re-measurement. After the stage measures the lens, the named reference's median turn, over at least the rule's three decisive frames, is applied to every camera of the lens through `turnedCamera`, once, and the turned lens is measured again, so the report shows what the turn bought. It applies only when every other decisive reference agrees within the stated degrees (three by default); a conflict is reported and the provider's camera stands. The provider's camera and the turn stay side by side in each frame's report. Nothing is fitted quietly.

**A body from the survey in one command.** `node tools/objects/scaffold-frame-lens.mts <object> --survey <N><Name> --stem <N>_<Name> --order latitude-first|longitude-first [--from <dir>]` pins the frames, the reconstruction and the rotation record, asks Horizons at the frames' epochs (or takes them from a directory already fetched), writes the acquisition steps, the manifest inputs, the derivation record, the lens on the mesh the record describes with `refinement: { by: 'relief', tilt: 'silhouette' }`, the reader control and text, the README section with its report markers and a ledger entry, derives every camera, binds the source records, and measures the lens the way preparation will. If the simplifier cannot reach the face target within the base mesh's budget, the alternative's budget is raised in halves up to twice the base and the recipe states it. A lens ships only when every measurement that reached a verdict places it within three degrees; one agreeing reference does not outvote a disagreeing one. A lens that fails that is unwired again, and only the ledger keeps its numbers. Elektra, Daphne and Eugenia all entered this way and none ships: Elektra's relief places it at −2° while its silhouette stays at 7°, and the ledgers say so.
