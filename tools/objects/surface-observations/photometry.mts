...
export async function publishedPhotometry(sourceDirectory: string, manifest: SourceManifest | undefined, block: PublishedPhotometryBlock): Promise<ObservationPhotometry> {
  ...
  return { gain: resolved.normalize, report: resolved.report, units: resolved.units };
}

/** The historical disk functions: Lommel-Seeliger, Minnaert with its phase slope, or the retained acquisition illumination within angle limits. */
export function diskPhotometry(block: DiskPhotometry): ObservationPhotometry {
  const retained = block.model === 'retained-observation';
  return { gain: (incidence, emission, phase) => observationGain(incidence, emission, block, phase), report: { ...block,
    formula: retained ? 'Original acquisition illumination retained; no photometric disk correction.' : block.model === 'minnaert'
...
export function observedPhotometry(report: Record<string, unknown>): ObservationPhotometry {
  return { gain: () => 1, report: { ...report, limitations: 'Original acquisition shading retained; bounded relative display levels only, not albedo.' } };
}
