 ...
 * Dark calibrated pixels stay eligible; brightness never defines coverage. */
export function sampleFootprint({ image, camera, geometry, photometry }: FootprintSource, point: readonly number[],
  ...
  for (const i of ids) { const reason = geometry.reject(i); if (reason !== null) return { reason }; }
  for (const i of ids) { const reason = image.reject(i); if (reason !== null) return { reason }; }
  const emissionLimit = maximumEmissionDegrees * Math.PI / 180;
  if (ids.some(i => { const emission = geometry.emission(i); return !Number.isFinite(emission) || emission < 0 || emission > emissionLimit; })) return { reason: 'grazing' };
...
export function cameraFrame({ id, image, camera, geometry, photometry, limits, mesh, displayPercentiles, report = {} }: CameraFrameOptions): ObservationFrame {
  ...
  // A contributor on a continuous surface lies within one pixel diagonal of the sampled point; the diagonal stretches with emission.
  const diagonal = (i: number) => geometry.rangeMeters(i) * angle * Math.sqrt(1 + 1 / Math.cos(geometry.emission(i)) ** 2);
  const { maximumSeparationMeters, maximumSeparationFootprints } = limits;
  const separation = (allowance: number) => maximumSeparationFootprints === undefined ? allowance + (maximumSeparationMeters ?? NaN)
    : (ids: readonly number[]) => allowance + maximumSeparationFootprints * Math.max(...ids.map(diagonal));
  ...
  return { id, startTime: image.startTime, filter: image.filter, positionKm: camera.positionKm, cameraKind: camera.kind, geometrySource: geometry.source,
    ...
    sample: (point, allowance = 0) => sampleFootprint({ image, camera, geometry, photometry }, point, { maximumSeparationMeters: separation(allowance), maximumEmissionDegrees: limits.maximumEmissionDegrees }),
    visible: point => {
