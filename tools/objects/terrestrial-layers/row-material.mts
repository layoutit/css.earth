...
import type {preparePhotographicAtmosphere} from './photographic-atmosphere.mts';
import {array,number,object,parse,string} from '../material-composition/data-schema.mts';
type AtmospherePreparation = Awaited<ReturnType<typeof preparePhotographicAtmosphere>>;
export interface RowMaterialConfiguration {namespace: string; publicBase: string; shapePath: string; atmospherePath: string;
  lighting: {sourcePaths: readonly string[]; deliveryPath?: string; frameCount: number; frameGutter: number; minimumLightViewZ: number; maximumLightViewZ: number;
    pitchDegrees: number; atmosphereLabel: string; shadowLabel: string};}
export interface MaterialRow {rowIndex: number; url: string; encoding: string; bytes: number; sha256: string; width: number; height: number; decodedRgbaBytes: number;}
export interface MaterialPresentation {frameIndex: number; phaseFrame: number; shadows: boolean; lightViewZ: number; rowIndex: number; url: string; backgroundPosition: string; backgroundSize: string; cameraLightDirection: readonly number[];}
const deliverySchema=object({schema:string,bankId:string,rows:array(object({density:number,row:number,sha256:string,bytes:number}))});
...
export async function prepareRowMaterial({config,atmosphere:a,sun,sourceDirectory,publicDirectory,outputDirectory}: {config: RowMaterialConfiguration; atmosphere: AtmospherePreparation; sun: PreparedDirectionalSunPlan; sourceDirectory: string; publicDirectory: string; outputDirectory: string}) {
 const p=config.lighting;
 const fingerprint=createHash('sha256');
 const operationSources=['row-material.mts','photographic-atmosphere.mts','ellipsoid-geometry.mts','../../prepared-atmosphere.mts','../../../src/platform/directional-sun-coordinate.mts'];
 ...
 const delivery=p.deliveryPath?parse(JSON.parse(await readFile(resolve(sourceDirectory,p.deliveryPath),'utf8')),deliverySchema,'material delivery'):null;
 ...
 const banks: Record<number,Awaited<ReturnType<typeof prepareDensity>>>={},
  projectionSamples: (Pick<ReturnType<AtmospherePreparation['prepareProjectedSilhouette']>['metadata'],'pitchDegrees'|'radiusX'|'radiusY'|'right'|'down'|'view'> & {frameIndex:number;lightViewZ:number})[]=[],
  preparedFrames1x: Buffer[]=[];
 const phaseDirection=(viewZ: number)=>{const radians=azimuth*Math.PI/180,projectedLength=Math.sqrt(Math.max(0,1-viewZ**2));return [Math.cos(radians)*projectedLength,Math.sin(radians)*projectedLength,viewZ]};
 ...
 async function prepareDensity(density: number) {
  ...
  const rows: MaterialRow[]=[],presentations: MaterialPresentation[]=[],pending: Promise<void>[]=[];
