 ...
 */
export function parsePdsRgbLabel(label, entry, policy) {
...
export async function preparePdsRgbObservation(path, entry, policy, width, height) {
  const child = spawn('unzip', ['-p', path, policy.member], { stdio: ['ignore', 'pipe', 'pipe'] });
  let stderr = '';
  ...
  completion.catch(() => {});
  let header = Buffer.alloc(0), consumed = 0, metadata, rgb;
  try {
    ...
    await completion;
    if (!metadata || consumed !== metadata.totalBytes) throw new Error('PDS RGB image is incomplete.');
