...
export function validatePds4ObservationPolicy(p) {
...
export function readPds4ColorLabel(xml) {
  if (typeof xml !== 'string' || /<!DOCTYPE|<!ENTITY/.test(xml)) throw new Error('Unsupported PDS4 XML.');
  const all = (text, tag) => [...text.matchAll(new RegExp(`<${tag}(?:\\s[^>]*)?>([\\s\\S]*?)</${tag}>`, 'g'))];
...
export function decodePds4Color(bytes, xml, entry, policy) {
  validatePds4ObservationPolicy(policy);
...
export function mapPds4Color(source, policy, width, height) {
...
export async function preparePds4Observation(sourceDirectory, entry, policy, width, height) {
