...
import { decodeEncounterFits } from './encounter-fits.mjs';
...
const unit = a => { const l=Math.hypot(...a); return a.map(n=>n/l); };
const cross = (a,b) => [a[1]*b[2]-a[2]*b[1],a[2]*b[0]-a[0]*b[2],a[0]*b[1]-a[1]*b[0]];
const safePath = p => typeof p==='string' && p.length>0 && !p.startsWith('/') && !p.split(/[\\/]/).includes('..');
const positive = n => Number.isFinite(n) && n>0;
export function validateEncounterRecipe(recipe, geometry) {
...
export function sampleEncounterFootprint(frame, camera, plane, point, policy) {
...
function qualifiedFace(mesh,id) {
...
export function buildEncounterBackplane(frame,camera,mesh,recipe) {
  const count=frame.width*frame.height;
  const plane={accepted:new Uint8Array(count),xyz:new Float64Array(count*3),gains:new Float32Array(count),emissions:new Float32Array(count),reasons:[]};
  const tally={};const reject=(i,reason)=>{plane.reasons[i]=reason;tally[reason]=(tally[reason]??0)+1;};
  const projected=mesh.positions.map(p=>camera.project(p)).filter(Boolean);
  ...
  plane.report={projectedBounds:bounds,acceptedPixels:accepted,rejectedPixels:tally};
  return plane;
}

export async function loadEncounterSurface({sourceDirectory,source,recipe,radial,config}) {
  ...
  const observations=[];const metersPerUnit=config.geometry.radiusKm*1000/config.geometry.radius;
  for(const f of recipe.frames) {
    const control=JSON.parse(await readFile(resolve(sourceDirectory,f.controlPath),'utf8'));
    ...
    const sampleSource=point=>sampleEncounterFootprint(frame,camera,plane,point,recipe.transfer);
    observations.push({frame,camera,plane,sampleSource,control,registration});
  }
  const missing=(point,reason)=>({reason,color:missingCoverageColor(Math.atan2(point[1],point[0])*180/Math.PI,Math.atan2(point[2],Math.hypot(point[0],point[1]))*180/Math.PI,180/config.raster.height)});
  const sampleAll=displayPoint=>{
    ...
    return observations.map(o=>{
      ...
      const delta=sub(hit.point,o.camera.positionMeters),distance=Math.hypot(...delta),ray=radial.grid.intersect(o.camera.positionMeters,delta.map(n=>n/distance),distance+recipe.transfer.visibilityToleranceMeters);
  ...
  const choose=samples=>recipe.selection==='finest-resolution' ? samples.reduce((best,sample,i)=>!sample.reason && (best<0 || observations[i].registration.nominalPixelScaleMeters<observations[best].registration.nominalPixelScaleMeters) ? i : best,-1) : selectObservation(samples);
  ...
  const levels=observations.length===1?{gains:[1],pairs:[]}:fitObservationLevels(observations.map((_,i)=>samples.map(s=>s[i])),recipe.levelMatching);
  const values=[];
  for(const samplesAtPoint of samples){const i=choose(samplesAtPoint);if(i>=0)values.push(samplesAtPoint[i].radiance*levels.gains[i]);}
  values.sort((a,b)=>a-b);
  ...
  const samplePoint=point=>{
    ...
    const value=values[i],radiance=value.radiance*levels.gains[i],gray=Math.round(Math.max(0,Math.min(1,(radiance-low)/(high-low)))*255);
    return {...value,radiance,color:[gray,gray,gray],frameIndex:i,frameId:recipe.frames[i].id};
  };
  const areaCoverage={method:'Deterministic equal-area barycentric samples on every retained triangle; excludes atlas bleed',samplesPerTriangle:recipe.levelMatching?.samplesPerTriangle??8,totalSquareMeters:0,acceptedSquareMeters:0,sourceSquareMeters:{}};
