...
const safePath = p => typeof p === 'string' && p.length && !p.startsWith('/') && !p.includes('\\') && !p.split('/').includes('..');

export function validateFacetScalarProfile(lens, terrain) {
...
function centroidError(mesh, faceId, point, limit) {
...
function centroidBijection(mesh, tolerance) {
  const cells = new Map(), seen = new Uint8Array(mesh.faces);
  const centers = mesh.indices.map(indices => [0,1,2].map(axis => indices.reduce((sum, i) => sum + mesh.positions[i][axis], 0)/3));
  const cellFor = point => point.map(n => Math.floor(n/tolerance));
...
function record(result, faceId, value, sigma, valid, error) {
...
export function parseFacetCsv(text, profile, mesh) {
  const lines = text.trimEnd().split(/\r?\n/);
  const names = lines.shift().split(','), units = lines.shift().split(',');
...
function fitsHeader(bytes, start) {
  const keys = {}; let offset = start, ended = false;
...
export function parseFacetFits(bytes, xml, profile, mesh) {
  ...
  const tag = (text, name) => new RegExp('<' + name + '(?:\\s[^>]*)?>([^<]+)</' + name + '>').exec(text)?.[1]?.trim();
...
export function createFacetScalarSampler(mesh, table, lens) {
  const transform = raw => raw * (lens.valueTransform?.scale ?? 1) + (lens.valueTransform?.offset ?? 0);
  const valueAt = id => Number.isFinite(table.values[id]) ? transform(table.values[id]) : null;
  return {
...
export async function loadFacetScalarSurface(root, lens, mesh) {
  ...
  if (lens.table.format === 'sbmt-csv-zip') {
    const {stdout} = await exec('unzip', ['-p', resolve(root, lens.path), lens.table.member], {maxBuffer: 192 * 1024 * 1024});
    table = parseFacetCsv(stdout, lens.table, mesh);
  } else {
    const [bytes, xml] = await Promise.all([readFile(resolve(root, lens.path)), readFile(resolve(root, lens.table.labelPath), 'utf8')]);
