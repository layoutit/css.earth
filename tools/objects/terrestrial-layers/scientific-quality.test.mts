...
function cube(values){
 ...
 const bytes=Buffer.alloc(4096+values.length*4);bytes.write(header);values.forEach((v,i)=>bytes.writeFloatLE(v,4096+i*4));return bytes;
}
test('scientific quality masks intersect measured support without erasing valid zero or blending across boundaries',async()=>{
 ...
 try{
  ...
  assert.ok(Math.abs(surface.sample(45,-45)-.8)<1e-6,'Exact1500m threshold included');
