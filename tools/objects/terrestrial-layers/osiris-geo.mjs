...
export function decodeOsirisGeo(bytes) {
  ...
  for (const name of PLANE_NAMES) {
    ...
    if (!Number.isSafeInteger(w) || !Number.isSafeInteger(h) || w < 2 || h < 2 || w > 4096 || h > 4096 ||
        (width !== undefined && (w !== width || h !== height)) || Number(field(block, 'SAMPLE_BITS')) !== 32 ||
  ...
  const xyz = i => ['X', 'Y', 'Z'].map(axis => planes[`COORDINATE_${axis}_IMAGE`][i]);
  ...
  const valid = i => Number.isInteger(i) && i >= 0 && i < width * height &&
...
export function acceptOsirisQuality(flag, allowLossy) {
  return Number.isInteger(flag) && flag >= 0 && flag <= 255 && (flag & 1) === 1 &&
    (flag & ~(allowLossy ? 9 : 1)) === 0;
}

export function decodeOsirisQuality(bytes, geo) {
  ...
  const flags = Uint8Array.from(data.QUALITY_MAP_IMAGE), histogram = {};
...
export function lommelSeeligerGain(incidence, emission, policy) {
...
export function osirisRadianceFactorScale(history) {
  const quantity = (name, unit) => {
...
export function phaseGain(phase, policy) {
  if (!policy) return 1;
  if (!Number.isFinite(phase) || phase < policy.minimumDegrees * Math.PI / 180 || phase > policy.maximumDegrees * Math.PI / 180) return null;
  const scattering = angle => {
...
export function observationGain(incidence, emission, policy, phase) {
...
export function fitCamera(points, pixels, width, height) {
  ...
  const normal = Array.from({ length: 11 }, () => Array(11).fill(0)), rhs = Array(11).fill(0);
