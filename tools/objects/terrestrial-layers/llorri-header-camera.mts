...
export function llorriHeaderCamera(bytes: Buffer, kernels: KernelSet, bodyId: number) {
  const h: Record<string, unknown> = {};
  // Other mission cards may contain FITS undefined values. Decode only the camera's fields.
  scanFitsCards(bytes, 0, (key, card) => {
    if (!/^(NAXIS[12]|CTYPE[12]|CRVAL[12]|CRPIX[12]|CD[12]_[12]|[AB]_\d_\d|SPCTS[CO][XYZ]|MIDUTC|STARTUTC|TRGFOV1)$/.test(key)) return;
  ...
  return { schema: 'cssearth-archived-camera@1', target: text('TRGFOV1'), startTime: text('STARTUTC'), filter: 'PANCHROMATIC', width: image.width, height: image.height,
    ...
    sip: { referencePixel, a: terms('A'), b: terms('B'), offsetPixels: [0,0] }, imageSha256: createHash('sha256').update(bytes).digest('hex'),
