...
export function validateImageDemScience(lens) {
...
export async function loadImageDemScience(root, lens, mesh) {
  validateImageDemScience(lens);
  if (!mesh?.heightAt || !['height', 'difference'].includes(lens.quantity) || lens.relief ||
  ...
  const rawValue = point => {
    ...
    const height = point[2] - mesh.imageGrid.zOffsetMeters;
