...
const xyz = ([lat, lon, radius]: readonly number[]) => {
...
const attrs = (text: string) => Object.fromEntries([...text.matchAll(/([\w]+)="([^"]*)"/g)].map(m => [m[1], m[2]]));

export function decodeSbmtPaths(text: string, expectedShapeModel: string) {
...
export function decodeSbmtLocations(text: string) {
...
export async function loadSbmtSymbols(root: string, value: unknown, mesh: SourceMesh) {
  const profile=parseSymbols(value);
  ...
  const segments: SymbolSegment[] = [], report = {paths: paths.length, locations: locations.length, acceptedLocations: 0, rejectedLocations: 0,
    acceptedSegments: 0, rejectedSegments: 0, maximumRegistrationDistanceMeters: 0,
    lineWidthMeters: profile.lineWidthMeters, locationDiameterMeters: profile.locationDiameterMeters, features: [] as {id:number;name:string;category:number;kind:string}[]};
  ...
  function category(color: string) {
  ...
  const cellSize = Math.max(profile.lineWidthMeters, profile.locationDiameterMeters, profile.maximumSegmentMeters * 2), cells = new Map<string,PreparedSymbolSegment[]>();
  for (const raw of segments) {
    const delta=raw.b.map((n,i)=>n-raw.a[i]);
    const segment: PreparedSymbolSegment={...raw,delta,squared:delta.reduce((s,n)=>s+n*n,0)};
  ...
  return {report, sample(point: readonly number[]) {
    const candidates = cells.get(point.map(n => Math.floor(n / cellSize)).join(',')) ?? [];
    let selected: PreparedSymbolSegment | undefined, best = Infinity;
    for (const s of candidates) {
      ...
      if (score <= 1 && (score < best || score === best && (!selected || s.id < selected.id))) {selected = s; best = score;}
    }
    return selected;
  }};
}
