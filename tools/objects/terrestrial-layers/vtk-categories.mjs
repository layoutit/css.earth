...
import {loadSbmtSymbols} from './sbmt-symbols.mjs';

const local = p => typeof p === 'string' && p.length && !p.startsWith('/') && !p.split('/').includes('..');
...
export function validateVtkCategories(lens, terrain) {
...
export function decodeVtkCategories(text, grid) {
  ...
  const take = expected => {if (tokens[at++] !== expected) throw new Error('Unexpected categorical VTK field: ' + expected);};
...
export async function loadVtkCategories(root, lens, renderedMesh) {
