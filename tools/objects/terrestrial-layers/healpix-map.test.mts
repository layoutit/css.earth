...
test('each point takes the value of the HEALPix pixel that contains it, in RING and NESTED order', async () => {
  ...
  try {
    ...
    for (const ordering of ['ring', 'nested']) {
      const map = await loadHealpixNpyMap(work, { path: 'index.npy', ordering, transform: 'value' });
    ...
    assert.equal((await loadHealpixNpyMap(work, { path: 'index2.npy', ordering: 'ring', transform: 'value' })).sample(45, 66.4421870779), 0);
    assert.equal((await loadHealpixNpyMap(work, { path: 'index2.npy', ordering: 'nested', transform: 'value' })).sample(45, 66.4421870779), 3);
    ...
    assert.equal((await loadHealpixNpyMap(work, { path: 'variance.npy', ordering: 'ring', transform: 'sqrt' })).sample(0, 0), 2);
    await writeFile(resolve(work, 'negative.npy'), npy([-1, ...Array.from({ length: 11 }, () => 1)]));
    await assert.rejects(loadHealpixNpyMap(work, { path: 'negative.npy', ordering: 'ring', transform: 'sqrt' }), /no square root/u);
    await writeFile(resolve(work, 'short.npy'), npy([1, 2, 3]));
    await assert.rejects(loadHealpixNpyMap(work, { path: 'short.npy', ordering: 'ring', transform: 'value' }), /12 NSIDE/u);
...
luhman('Luhman 16 B: the deposited map has its dark patch just east of noon on the equator and its brightest pixels near the north pole', async () => {
  ...
  const mean = await loadHealpixNpyMap(root, { path: 'science/ureshino-2026/posterior_mean_chip1.npy', ordering: 'ring', transform: 'value' });
  ...
  const sigma = await loadHealpixNpyMap(root, { path: 'science/ureshino-2026/posterior_var_chip1.npy', ordering: 'ring', transform: 'sqrt' });
