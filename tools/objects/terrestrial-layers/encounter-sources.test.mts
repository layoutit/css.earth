...
for(const body of ['comet-81p','comet-9p','comet-103p'])test(`${body}: real source pixels match independent FITS decoding and all cameras pass held-out controls`,async()=>{
 const source=resolve(root,`src/planets/${body}/source`),recipe=JSON.parse(await readFile(resolve(source,'preparation/terrestrial.json'), 'utf8')),manifest=JSON.parse(await readFile(resolve(source,'manifest.json'), 'utf8'));
 ...
 for(const lens of recipe.raster.surfaceObservations)for(const frame of lens.frames){
  ...
  const camera=encounterCamera(decoded.header,control.camera),report=validateEncounterRegistration(camera,control.registration,String(shape.expectedSha256));
