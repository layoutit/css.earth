...
import { field, imageBlock, acceptOsirisQuality } from './osiris-geo.mjs';
...
export function decodeOsirisReflectance(bytes, camera, allowLossy) {
  ...
  const data = {}, ranges = [], count = camera.width * camera.height;
  ...
  return { width: camera.width, height: camera.height, planes: { IMAGE: data.IMAGE },
    acceptPixel: i => acceptOsirisQuality(data.QUALITY_MAP_IMAGE[i], allowLossy) &&
      Number.isFinite(data.IMAGE[i]) && Number.isFinite(data.SIGMA_MAP_IMAGE[i]) && data.SIGMA_MAP_IMAGE[i] >= 0,
    startTime: camera.startTime, filter: camera.filter, camera, isLossyPixel: i => Boolean(data.QUALITY_MAP_IMAGE[i] & 8),
...
export function attachSourceGeometry(frame, mesh) {
  const {camera,width,height,planes} = frame, count = width*height;
  ...
  frame.xyz = i => [planes.COORDINATE_X_IMAGE[i],planes.COORDINATE_Y_IMAGE[i],planes.COORDINATE_Z_IMAGE[i]];
  frame.valid = i => Number.isInteger(i) && i >= 0 && i < count && planes.DISTANCE_IMAGE[i] > 0;
