...
interface ShapeAtlas {
  ...
  plans: readonly { face: PreparedTriangle; rect: { x: number; y: number }; matrix: readonly number[];
...
export function neutralShapeAtlas(atlas: ShapeAtlas, sun: readonly number[]) {
  const { width, height, tileSize } = atlas;
  if (![width, height, tileSize].every(n => Number.isSafeInteger(n) && n > 0) || sun.length !== 3 || !sun.every(Number.isFinite))
  ...
  for (const { face, rect, matrix: m, geometry } of atlas.plans) {
    if (face.estimated || m.length !== 16 || m[3] !== 0 || m[7] !== 0 || m[15] !== 1 ||
        ![rect.x, rect.y].every(Number.isSafeInteger) || rect.x < 0 || rect.y < 0 || rect.x + tileSize > width || rect.y + tileSize > height)
