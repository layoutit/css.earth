...
test('orthographic center and east/north orientation follow the source projection', () => {
  ...
  assert.ok(Math.hypot(...center) < 1e-8);
  assert.ok(orthographicPoint(25, 18, p)[0] > 0);
  assert.ok(orthographicPoint(5, 18, p)[0] < 0);
  assert.ok(orthographicPoint(15, 28, p)[1] > 0);
  assert.ok(orthographicPoint(15, 8, p)[1] < 0);
  assert.equal(orthographicPoint(195, -18, p), null);
  assert.ok(Math.hypot(...orthographicPoint(375, 18, p)) < 1e-8);
});
