...
export function inspectOpenSurface(indices, positions) {
  const edges = new Map(), vertices = new Set(), neighbors = Array.from({ length: indices.length / 3 }, () => []);
...
export function orientObservedSurface(mesh) {
  ...
  const reorientedFaces = [], triangles = mesh.indices.map((face,i) => {
...
export function validateObservedReduction(sourceIndices, reducedIndices, positions) {
