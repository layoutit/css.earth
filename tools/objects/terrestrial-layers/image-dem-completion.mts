...
type Face = {vertices: readonly Point[]};
export interface ImageDemCompletionRecipe {method: string; depthMeters: number; faceBudget: number;}
export interface ImageDemReductionRecipe {targetFaces: number; maximumErrorMeters: number;}
...
function boundaryLoops(indices: readonly (readonly number[])[]) {
  ...
  for (const seed of next.keys()) if (!seen.has(seed)) {
    ...
    while (!seen.has(i)) {
      ...
      seen.add(i); loop.push(i); i=requireVertex(next.get(i));
    }
...
function triangulate(loop: readonly number[], positions: readonly Point[]) {
  ...
  if (!(cross2(positions[remaining[0]],positions[remaining[1]],positions[remaining[2]])>0)) throw new Error('Degenerate completion gap.');
  return [...triangles,remaining];
}

export function completeImageDem(observed: readonly Face[], recipe: ImageDemCompletionRecipe, unitsPerMeter: number) {
  ...
  const positions: Point[]=[], lookup=new Map<string, number>();
  const vertex=(p: Point) => { const k=p.join(','); if (!lookup.has(k)) {lookup.set(k,positions.length);positions.push(p);} return requireVertex(lookup.get(k)); };
  const front=observed.map(f=>f.vertices.map(vertex));
  if (front.some(f=>!(cross2(positions[f[0]],positions[f[1]],positions[f[2]])>0))) throw new Error('Completion needs a nonoverlapping positive-Z height field.');
  const loops=boundaryLoops(front), area=(loop: readonly number[])=>loop.reduce((s,a,i)=>{
  ...
  for (const ids of front) {
    ...
    if (count===3) {
      const [a,b,c]=ids,[ab,bc,ca]=mids.map(requireVertex);rear.push([a,ab,ca],[ab,b,bc],[ca,bc,c],[ab,bc,ca]);
    } else if (count===2) {
      const i=mids.findIndex(n=>n===undefined),a=ids[i],b=ids[(i+1)%3],c=ids[(i+2)%3],bc=requireVertex(mids[(i+1)%3]),ca=requireVertex(mids[(i+2)%3]);
      rear.push([a,b,bc],[a,bc,ca],[ca,bc,c]);
    } else if (count===1) {
      const i=mids.findIndex(n=>n!==undefined),a=ids[i],b=ids[(i+1)%3],c=ids[(i+2)%3],m=requireVertex(mids[i]);rear.push([a,m,c],[m,b,c]);
  ...
  const distance=(p: Point)=>Math.sqrt(Math.min(...segments.map(([a,b])=>{
...
export async function reduceCompletedImageDem(observed: readonly Face[], completion: ReturnType<typeof completeImageDem>, recipe: ImageDemReductionRecipe, scale: number) {
  ...
  const positions: Point[]=[], lookup=new Map<string, number>(), measured=new Set(observed.flatMap(f=>f.vertices.map(p=>p.join(','))));
  const sourceFlags: boolean[]=[];
  const indices=Uint32Array.from([...observed,...completion.faces].flatMap(f=>f.vertices.map(p=>{
    const k=p.join(',');if(!lookup.has(k)){lookup.set(k,positions.length);positions.push(p);sourceFlags.push(measured.has(k));}return requireVertex(lookup.get(k));
  })));
...
function requireVertex(index: number | undefined): number { if (index === undefined) throw new Error("Missing completion boundary vertex."); return index; }
