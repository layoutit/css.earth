...
import { closestTrianglePoint } from './obj-shape.mjs';

const safePath = path => typeof path === 'string' && path.length > 0 && !path.startsWith('/') && !path.includes('\\') && !path.split('/').includes('..');

export function validateObjUvFits(lens, terrain) {
...
export function parseObjTextureCoordinates(text, mesh) {
  const uv = [], faces = []; let vertexCount = 0;
...
export function createObjUvFitsSampler(mesh, mapping, fits, lens) {
  ...
  function valueAt(hit) {
    ...
    if (lens.sampling === 'bilinear') {
      ...
      if (four.some(value => value === null)) return null;
...
export async function loadObjUvFits(root, lens, mesh) {
  if (!mesh?.closestPoint) throw new Error('Source UV science requires the full source mesh.');
  const [text, bytes, label] = await Promise.all([readFile(resolve(root, lens.meshPath), 'utf8'), readFile(resolve(root, lens.path)), readFile(resolve(root, lens.labelPath), 'utf8')]);
  const fits = readFitsPrimary(bytes), mapping = parseObjTextureCoordinates(text, mesh);
