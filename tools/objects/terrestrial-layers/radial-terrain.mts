...
export async function prepareRadialMaterials({ radial, surfaces, config, source, sourceDirectory, publicDirectory, outputDirectory, sunDirection,
  ...
  for (const entry of snapshotEntries.filter(entry =>
    ...
    const png = await renderRadialSnapshot({ ...parseRadialSnapshot(requireRecord(entry).recipe), faces: radial.faces,
      ...
      map: resolve(publicDirectory, requireString(surface.map.url.split('/').at(-1))) });
    // A new body declares the snapshot with a placeholder pin (an all-zero digest). The first preparation writes the render beside the sources and stops
    // with the pin to record, so the checked-in image is the pipeline's own and the next run verifies it like any other input.
    if (/^0{64}$/u.test(entry.expectedSha256)) {
      if (sourceDirectory === undefined) throw new Error(`${config.namespace} context snapshot ${entry.path} has a placeholder pin but no source directory to write to.`);
      const path = resolve(sourceDirectory, entry.path);
      await mkdir(dirname(path), { recursive: true });
      await writeFile(path, png);
      throw new Error(`${config.namespace} context snapshot ${entry.path} was rendered and written (${png.length} bytes, sha256 ${createHash('sha256').update(png).digest('hex')}); pin it in the manifest and the navigation source, then prepare again.`);
    }
