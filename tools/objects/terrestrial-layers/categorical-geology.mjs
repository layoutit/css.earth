...
const localPath = path => typeof path === 'string' && path.length > 0 && !path.startsWith('/') && !path.split('/').includes('..');

/** The recipe binds archival symbols, not an ordinal or interpolated scale. */
export function validateGeologyProfile(lens) {
...
export function decodeGeologyAttributes(bytes) {
...
export function decodeGeologyPolygons(bytes, grid) {
  ...
  const polygons = [], degenerate = []; let offset = 100;
...
function rowSpans(polygons, latitude) {
  const events = [];
  ...
  const active = new Map(), spans = []; let i = 0;
...
export function createGeologySampler(polygons) {
  const bins = Array.from({length: 180}, () => []);
  for (const polygon of polygons.filter(Boolean)) {
    for (let y = Math.max(0, Math.floor(polygon.south + 90)); y <= Math.min(179, Math.floor(polygon.north + 90)); y++) bins[y].push(polygon);
  }
  let rowLatitude, spans;
...
export async function loadGeologySurface(root, lens) {
  validateGeologyProfile(lens);
  const {grid} = lens;
  ...
  const values = new Map(lens.categories.map((category, index) => [category.value, index])), counts = {};
  rows.forEach((row, i) => {
...
export function categoryColorForValue(value, lens) {
