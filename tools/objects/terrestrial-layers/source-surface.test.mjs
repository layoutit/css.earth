...
for (const fixture of fixtures.cases) test(`${fixture.id}: full-source regression coordinates keep height on the corresponding surface`, async () => {
  ...
  const [config, manifest] = await Promise.all(['preparation/terrestrial.json', 'manifest.json'].map(async path => JSON.parse(await readFile(new URL(path, root)))));
  const entry = manifest.inputs.find(input => input.path === fixture.sourcePath);
  ...
  near(first.radius*lens.valueTransform.scale+lens.valueTransform.offset, fixture.oldFirstRayHeight);
  ...
  if (fixture.oldRadialGrid) {
    ...
    near(radius*lens.valueTransform.scale+lens.valueTransform.offset, old.scalarValue);
    const outer = fixture.checks.find(check => check.kind === 'exact-outer-source-ray-surface');
    const corrected = sample.samplePoint(outer.query).value;
    const errorMeters = (corrected - old.scalarValue) / lens.valueTransform.scale;
    assert.ok(errorMeters > { itokawa: 86, ryugu: 46, eros: 799 }[fixture.id]);
...
test('the delivered scientific atlas samples source geometry, not the lossy flat preview', async () => {
  ...
  try {
    ...
    const lens = {id:'elevation',minimum:0,maximum:5,colors:['#000000','#ffffff'],valueTransform:{scale:1,offset:-1},surfaceSampling:{method:'closest-source-point',maximumDistanceMeters:.5}};
