...
test('Agenor focus transforms actual solid leaf coordinates to the camera centre',async()=>{
  ...
  const [x,y,z]=prepareEclipticPresentationFrame('europa').toPresentation([
    Math.cos(focus.latitudeDegrees*r)*Math.sin(focus.longitudeDegrees*r),
    Math.cos(focus.latitudeDegrees*r)*Math.cos(focus.longitudeDegrees*r),Math.sin(focus.latitudeDegrees*r)]);
