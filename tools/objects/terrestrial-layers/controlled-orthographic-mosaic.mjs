...
import {controlledBodyFrame, colorPhotometricGain, readObservationVectors} from './photometric-observations.mjs';
...
export async function prepareControlledOrthographicMosaic(sourceDirectory, entries, recipe, width, height) {
  ...
  const extract = async name => (await run('unzip', ['-p', archive, name], {encoding:'buffer',maxBuffer:128*1024*1024})).stdout;
  ...
  for (const id of recipe.imageIds) {
    ...
    const metadata = JSON.parse(xml.match(/<Metadata[^>]*>([\s\S]*?)<\/Metadata>/)[1]);
    ...
    const geometry = Object.fromEntries(['sun','observer'].map(name => {
      ...
      return [name, axes.map(axis => axis.reduce((sum,v,i)=>sum+v*sample.position[i],0))];
    }));
  ...
  try {
    for (const frame of frames) {
      ...
      try {
        ...
        if (image.getSampleFormat(0)!==3 || image.getSamplesPerPixel()!==1 || keys.ProjCoordTransGeoKey!==21 ||
        ...
        const band={data:(await image.readRasters())[0],width:image.getWidth(),height:image.getHeight(),
        ...
        for(let y=firstY;y<lastY;y++){
          ...
          for(let column=firstX;column<lastX;column++){
            ...
            if(!point)continue;
            const source=sampleColorBand(band,...point);if(source===null)continue;
