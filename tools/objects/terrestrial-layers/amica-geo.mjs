...
const string = value => value?.replace(/^'(.*)'$/, '$1').trim();
...
export function decodeAmicaGeo(compressed, label, originalBytes, flatBytes) {
  ...
  const original = readFitsPrimary(originalBytes), flat = readFitsPrimary(flatBytes), h = original.header;
  ...
  if (original.bitpix !== 8 || flat.bitpix !== -32 || [original, flat].some(f => f.width !== 1024 || f.height !== 1024 || f.scale !== 1 || f.zero !== 0) ||
      ...
      string(h.FILTER_0) !== 'v' || string(h.FILTER_1) !== 'v' || string(flat.header.FILTER) !== 'v' ||
  ...
  const planes = {}, nullValue = Buffer.from('f49dc5ae', 'hex').readFloatBE(), accepted = new Uint8Array(count);
  for (let band = 0; band < names.length; band++) if (names[band]) {
    const values = new Float32Array(count), angular = names[band].includes('ANGLE');
    for (let i = 0; i < count; i++) {
      const value = bytes.readFloatBE((band * count + i) * 4);
      values[i] = value === nullValue ? NaN : angular ? value * Math.PI / 180 : value;
    }
    planes[names[band]] = values;
  }
  ...
  const xyz = i => [planes.COORDINATE_X_IMAGE[i], planes.COORDINATE_Y_IMAGE[i], planes.COORDINATE_Z_IMAGE[i]];
  const valid = i => Number.isInteger(i) && i >= 0 && i < count && planes.DISTANCE_IMAGE[i] > 0 && xyz(i).every(Number.isFinite);
  return { width: 1024, height: 1024, planes, xyz, valid, acceptPixel: i => accepted[i] === 1,
