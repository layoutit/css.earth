...
const recipe = config.raster.surfaceObservations[0], reference = recipe.frames?.[0] ?? recipe;
const camera = JSON.parse(await readFile(resolve(source, reference.cameraPath), 'utf8'));
