...
import { loadStlShape, loadObjShape, loadPdsVertexFacetShape, loadPdsPlateShape, loadPdsRadiusTable } from './obj-shape.mjs';
...
export async function loadRadialTerrain({ config, sourceDirectory, source }) {
  ...
  const loader = profile.format === 'stl' ? loadStlShape : profile.format === 'pds-radius-table' ? loadPdsRadiusTable : ['wavefront-obj', 'wavefront-obj-zip'].includes(profile.format) ? loadObjShape : profile.format === 'pds-plate-model' ? loadPdsPlateShape : profile.format === 'pds-vertex-facet' ? loadPdsVertexFacetShape : loadPdsScalarGrid;
