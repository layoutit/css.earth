...
import { project } from './osiris-geo.mjs';

const unquote = s => s?.replace(/^'(.*)'$/, '$1').trim();
const polynomial = (terms, u, v) => terms.reduce((s, [i, j, a]) => s + a * u ** i * v ** j, 0);
...
export function sipPixel(camera, x, y, inverse) {
  ...
  for (let n = 0; n < 12; n++) {
    ...
    const derivative = (terms, axis) => terms.reduce((s, [i,j,c]) => s + (axis === 0
...
export function bindSipCamera(camera) {
  const s = camera.sip;
  ...
  return {
    projectPoint(point) { const p = project(camera.matrix, point); return [...sipPixel(camera,p[0],p[1],true),p[2]]; },
    rayPixel(x,y) { return sipPixel(camera,x,y,false); },
  };
}
...
export function decodeLlorri(bytes, camera) {
  ...
  return { width:1024,height:1024,planes:{IMAGE:values},camera,...bindSipCamera(camera),
    ...
    acceptPixel:i => flags[i] === 0 && Number.isFinite(values[i]) && Number.isFinite(sigma.values[i]) && sigma.values[i] >= 0,
