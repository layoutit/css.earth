...
export function validateOrthographicObservation(recipe, geometry) {
...
export async function loadOrthographicObservation({ sourceDirectory, source, recipe, radial, config }) {
  ...
  for (let i = 0; i < photo.data.length; i++) {
    ...
    const point = mesh.positions[sourceId];
  ...
  function samplePoint(point) {
    const missing = reason => ({ reason, color: missingCoverageColor(
