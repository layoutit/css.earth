...
export function decodeIsis3Raster(input, grid) {
  ...
  const value = (text, key) => text.match(new RegExp(`^\\s*${key}\\s*=\\s*([^\\s]+)`, 'm'))?.[1];
  const number = (text, key) => Number(value(text, key));
...
export async function loadIsis3Raster(path, grid) {
