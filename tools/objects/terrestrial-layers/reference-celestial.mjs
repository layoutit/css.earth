...
export async function prepareReferenceCelestial({config,sourceDirectory,publicDirectory,outputDirectory,sourceManifest}) {
  ...
  const contract=JSON.parse(bytes),projection=contract.camera.projection,catalogue=contract.sky.catalogue;
  const direction=value=>Array.isArray(value)&&value.length===3&&value.every(Number.isFinite)&&Math.hypot(...value)>0;
  ...
  return JSON.parse(JSON.stringify({sky,sun,contract}));
}
