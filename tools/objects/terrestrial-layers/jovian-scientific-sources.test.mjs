...
for (const id of ['io', 'ganymede']) test(`${id} independently archived label points anchor both hemispheres of the geology view`, async () => {
  ...
  const recipe = await json(`${source}/preparation/terrestrial.json`), lens = recipe.raster.scientific.find(entry => entry.id === 'geology');
  const directory = lens.path.slice(0, lens.path.lastIndexOf('/'));
  const audit = await json(`${source}/${directory}/registration-anchors.json`), surface = await loadGeologySurface(source, lens);
  ...
  for (const point of audit.anchors) {
    ...
    assert.equal(lens.categories[category].value, point.value, `Separate point record ${point.record}`);
...
test('Agenor preserves independently decoded source heights and withholds incomplete interpolation footprints', async () => {
  ...
  const lens = recipe.raster.scientific.find(entry => entry.id === 'elevation');
  ...
  for (const point of anchors.anchors.slice(1, -1)) assert.ok(Math.abs(display.sample(point.longitudeEastDegrees, point.latitudeDegrees) - point.heightMeters) < 1e-8);
  ...
  assert.ok(Number.isFinite(display.sample(lens.focus.longitudeDegrees, lens.focus.latitudeDegrees)));
