...
import { loadVrmlShape } from './obj-shape.mjs';

const safePath = p => typeof p === 'string' && p.length > 0 && !p.startsWith('/') && !p.includes('\\') && !p.split('/').includes('..');
export function validateScalarMapProfile(lens, terrain) {
...
export function parseScalarMap(bytes, label, lens) {
  const key = name => new RegExp(`^\\s*${name.replace('^', '\\^')}\\s*=\\s*(?:"([^"]*)"|([^\\s]+))`, 'm').exec(label)?.slice(1).find(v => v !== undefined);
...
export function scalarMapIndex(longitude, latitude, grid = { latitudeFirst: 90, latitudeStep: -.5 }) {
...
export function createScalarMapSampler(data, lens, mesh, reference, cellValidity = new Uint8Array(259200)) {
  const unique = (lon, lat) => Boolean(mesh.hit(lon, lat, true) && reference.hit(lon, lat, true));
  ...
  return {
    sample(lon, lat) { const index = scalarMapIndex(lon, lat, lens.grid); return qualified(index) && unique(lon, lat) ? data[index] : null; },
    samplePoint(point) {
...
export async function loadScalarMap(root, lens, mesh) {
