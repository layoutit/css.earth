...
export interface PolarStabilization {boundaryLatitudeDegrees: number; overlap: number; inpaintRadius: number; boundaryAngularSmoothingDegrees: number;}
// Prepared source-boundary stabilization for explicitly authored polar policies.

export function preparePolarAtlas(source: RasterResult, tileSize: number, {
  boundaryLatitudeDegrees,
  overlap,
  inpaintRadius,
  boundaryAngularSmoothingDegrees,
}: PolarStabilization) {
  if (!Number.isFinite(boundaryLatitudeDegrees) ||
      boundaryLatitudeDegrees <= 0 || boundaryLatitudeDegrees >= 90) {
    throw new TypeError("Ellipsoid polar boundary latitude is invalid.");
  }
  if (!Number.isInteger(tileSize) || tileSize < 2) {
    throw new TypeError("Ellipsoid polar tile size is invalid.");
  }
  if (!source?.data || !source?.info || source.info.channels < 3) {
    throw new TypeError("Ellipsoid polar source must contain raw RGB channels.");
  }

  if (![overlap, inpaintRadius, boundaryAngularSmoothingDegrees].every(value => Number.isFinite(value) && value > 0) || inpaintRadius >= 1) throw new TypeError('Invalid source-authored polar stabilization.');
  ...
  return Object.freeze({
    ...
    stabilization: Object.freeze({
      ...
      boundaryAngularSmoothingDegrees: boundaryAngularSmoothingDegrees,
...
function prepareSmoothedBoundaryProfile({
  ...
  inpaintRadius,
  boundaryAngularSmoothingDegrees,
}: {source: RasterResult; north: boolean; tileSize: number; overlap: number; boundaryLatitude: number; inpaintRadius: number; boundaryAngularSmoothingDegrees: number}) {
  ...
  const smoothingRadius = Math.max(1, Math.round(
    sampleCount * boundaryAngularSmoothingDegrees / 360,
  ));
...
function sampleProjectedSource({
  ...
  overlap,
  boundaryLatitude,
}: {source: RasterResult; north: boolean; radius: number; longitude: number; overlap: number; boundaryLatitude: number}) {
...
function interpolateBoundaryProfile(profile: readonly (readonly number[])[], longitude: number) {
...
function interpolatePolarInterior({
  ...
  radius,
  inpaintRadius,
}: {centerColor: readonly number[]; boundaryColor: readonly number[]; radius: number; inpaintRadius: number}) {
...
function boundarySourceWeight(radius: number, inpaintRadius: number) {
...
function bilinearSample(source: RasterResult, sourceX: number, sourceY: number) {
