...
test('a shape display requires a source mesh and explicit uniform display color', async () => {
 const profile=await read('ida');
 profile.raster.observations=[];profile.raster.scientific=[];
 profile.raster.shapeViews=[{id:'shape',label:'Shape',consumer:'geometry',color:'#aaaaaa'}];
 ...
 profile.raster.shapeViews[0].color='albedo';
 assert.throws(()=>parseTerrestrialProfile(profile),/display color/);
});
