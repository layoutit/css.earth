...
export function validateFitsObservationPolicy(policy) {
...
export async function prepareFitsObservation(path, entry, policy, width, height) {
  return mapFitsObservation(readFitsPrimary(await readFile(path)), entry, policy, width, height);
}

export function mapFitsObservation(fits, entry, policy, width, height) {
  validateFitsObservationPolicy(policy);
  ...
  const modulo = (n, d) => ((n % d) + d) % d, [low, high] = policy.displayRange;
  ...
  return { rgb, missing, sourceMissingPixels: fits.values.reduce((count, v) => count + (v === policy.noData), 0),
