 ...
 */
export async function loadPdsScalarGrid(path, profile, { width = 2049, height = 1025 } = {}) {
  ...
  let bytes = Buffer.alloc(0), consumed = 0, metadata, nextRow = 0;
  const xIndices = [], rowIndices = [];
  try {
  ...
  function sample(longitudeDegrees, latitudeDegrees) {
...
export function parsePdsScalarLabel(label, profile) {
  const field = key => label.match(new RegExp(`^\\s*${key.replaceAll('^', '\\^')}\\s*=\\s*([^\\r\\n]+)`, 'm'))?.[1].trim().replaceAll('"', '');
  const num = key => Number.parseFloat(field(key));
  ...
  if (field('PDS_VERSION_ID') !== 'PDS3' || field('SAMPLE_TYPE') !== 'IEEE_REAL' || num('SAMPLE_BITS') !== 32 || num('BANDS') !== 1 ||
      ...
      !Array.isArray(profile.validRange) || profile.validRange.length !== 2 || !(profile.validRange[0] < profile.validRange[1])) {
