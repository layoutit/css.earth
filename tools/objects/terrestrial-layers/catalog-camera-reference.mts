...
if (!sourceArg || !outputArg || process.argv.length !== 4) throw new TypeError('Usage: catalog-camera-reference.mts <source-directory> <output>');
...
const frame=await resolveCatalogCamera(source,requireArray(requireRecord(mosaics.find(lens=>lens.id==='calibrated')).frames)[0]);
