...
import { prepareAtmosphereFrame as integratePreparedAtmosphereFrame, compositePreparedAtmosphere } from "../../prepared-atmosphere.mjs";




/** Source-calibrated atmosphere and analytic oblate projection, prepared once. */
export async function preparePhotographicAtmosphere({ sourceDirectory, profile, geometry }) {
...
const ATMOSPHERE_PROFILE = Object.freeze({
  radiusKm: atmospherePlanetRadiusKm,
  heightKm: atmosphereHeightKm,
  layers: [Object.freeze({
    scaleHeightKm: rayleighScaleHeightKm,
    scatteringPerKm: Array(3).fill(-Math.log(1 - LIMB_MAXIMUM_ALPHA) /
...
function prepareAtmosphereFrame(
  ...
  { lightDirection, preparedProjection = null,
    integrationSamples = ATMOSPHERE_RESPONSE.integrationSamples } = {},
) {
  if (!MATERIAL_DENSITIES.includes(pixelDensity)) {
...
function prepareMaterialProjection(pitchDegrees, pixelDensity) {
  if (!MATERIAL_DENSITIES.includes(pixelDensity)) {
    throw new RangeError(`Unsupported prepared  density: ${pixelDensity}.`);
  }
  const size = MATERIAL_PRESENTATION_SIZE * pixelDensity;
...
function prepareProjectedSilhouette(pitchDegrees, pixelDensity) {
  if (!MATERIAL_DENSITIES.includes(pixelDensity)) {
    throw new RangeError(`Unsupported prepared  density: ${pixelDensity}.`);
  }
  const size = MATERIAL_PRESENTATION_SIZE * pixelDensity;
...
function applyLinearLight(channel, factor) {
...
function intersectViewRay(projection, screenX, screenY) {
...
function screenToObjectDirection(vector, pitchDegrees) {
