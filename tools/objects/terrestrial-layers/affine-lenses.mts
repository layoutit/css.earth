...
import type {AffineProfile,AffineLens} from './affine-source.mts';
import type {parseEllipsoidProfile} from './ellipsoid-geometry.mts';
interface RgbBuffer {data:Buffer;info:{width:number;height:number;channels:Channels};}
...
import {polarZeroCoverage,resizeObservedRgb,prepareMeasuredPolarAtlas} from '../observed-coverage.mts';



/** Source-authored observation processing over shared affine texture bands. */
export async function prepareAffineLenses({config,shape,sourceDirectory,publicDirectory,outputDirectory,sourceManifest}: {config:AffineProfile;shape:ReturnType<typeof parseEllipsoidProfile>;sourceDirectory:string;publicDirectory:string;outputDirectory:string;sourceManifest:Awaited<ReturnType<typeof createSourceManifest>>}) {
...
async function prepareLens(plan:AffineLens) {
  ...
  const complete = observed ? resizeObservedRgb(observed, BODY_2X_WIDTH, BODY_2X_HEIGHT) : source;
  const source1x = observed ? resizeObservedRgb(observed, BODY_WIDTH, BODY_HEIGHT) : await sharp(complete.data, { raw: complete.info })
  ...
  const polar = observed ? prepareMeasuredPolarAtlas(observed, POLAR_TILE_SIZE * 2, {projection:'orthographic', ...shape.polar}) : preparePolarAtlas(complete, POLAR_TILE_SIZE * 2, {
    ...shape.polar,
...
async function writeProjectiveSurface(source:RgbBuffer, outputPath:string) {
...
async function prepareThumbnail(inputPath:string, outputPath:string) {
  ...
  if (!metadata.width || !metadata.height) throw new Error('Affine thumbnail source has no dimensions.');
