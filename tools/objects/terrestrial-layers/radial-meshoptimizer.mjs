...
import { sampleRadialTriangles, shadeRadialFaces } from './radial-terrain.mjs';

/** Simplify before UV/lighting baking so atlas seams cannot constrain collapses. */
export async function simplifyRadialTerrain(sample, profile, scale) {
  ...
  const flags = ['ErrorAbsolute', ...(options.regularize ? ['RegularizeLight'] : [])];
