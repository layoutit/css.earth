...
import {validateMaterialRecipe,validateRelativePath} from './recipe.mjs';
/** Compose source-selected scalar/thermal surfaces and the corresponding material variants. */
export async function prepareSpectralMaterialVariants({sourceDirectory,publicDirectory,stagingDirectory,config}) {
...
const descriptor = {...config.descriptor,controls:config.controlOrder.map(id=>prepared.find(plan=>plan.id===id)??config.descriptor.controls.find(control=>control.id===id))};
...
async function prepareScalarSurface(plan) {
