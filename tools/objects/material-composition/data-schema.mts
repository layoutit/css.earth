...
/** Structural recipe validation precedes the operators' semantic and budget checks. */
...
export const record = (value: unknown): value is Record<string, unknown> => value !== null && typeof value === 'object' && !isArray(value);
...
export function dictionary<T>(guard: Guard<T>): Guard<Record<string, T>> { return (value, path = 'recipe'): value is Record<string, T> => record(value) && Object.entries(value).every(([key, child]) => guard(child, `${path}.${key}`)); }
...
export const json: Guard<JsonValue> = (value): value is JsonValue => value === null || string(value) || number(value) || boolean(value) || (isArray(value) ? value.every(item => json(item)) : record(value) && Object.values(value).every(item => json(item)));
