...
import {parseRadialLayerRecipe} from '../giant-layers/index.mjs';
import {sampleRadialProfile,rasterObservedRadialField} from '../giant-layers/rings.mjs';
import {verifyObservationSources} from '../giant-layers/observations.mjs';
import {cropTransparentRgba,responsiveTransparentCrop} from './rgba.mjs';
import {normalizeVector,rotateX as rotateVectorX,rotateZ as rotateVectorZ} from './ellipsoid.mjs';
/** Observed radial-profile sampling, seeded density tracers and ellipsoid penumbra. */
export async function prepareRadialMotionAndShadow({sourceDirectory,publicDirectory,config,radialRecipe}) {
 if(config?.schema!=='cssearth-radial-motion-shadow@1')throw new TypeError('Invalid radial motion recipe.');
 parseRadialLayerRecipe(radialRecipe);
 ...
 const rows=await Promise.all([layer.colorSource,layer.transparencySource].map(path=>sharp(inputs.get(path)).removeAlpha().raw().toBuffer({resolveWithObject:true})));
 if(rows.some(row=>row.info.height!==1||row.info.channels!==3)||rows[0].info.width!==rows[1].info.width)throw new TypeError('Radial profile inputs must be matching RGB rows.');
 const profile={color:rows[0].data,transparency:rows[1].data,width:rows[0].info.width};
 const preparedRingSample=radius=>{const[red,green,blue,alpha]=sampleRadialProfile(radius,layer,profile);return{red,green,blue,alpha};};
...
const bodyOnRings={...config.shadowModel[config.fields.bodyOnRings],directTransmission:BODY_SHADOW_RING_DIRECT_TRANSMISSION,shadowedPixelCount:shadow.shadowedPixelCount,meanCoverage:shadow.meanCoverage,denseAlphaRamp:[BODY_SHADOW_DENSE_ALPHA_START,BODY_SHADOW_DENSE_ALPHA_END],edgeFeather:{...config.shadowModel[config.fields.bodyOnRings].edgeFeather,sigmaTexturePixels:BODY_SHADOW_EDGE_FEATHER_SIGMA,ringSupportAlpha:BODY_SHADOW_RING_SUPPORT_ALPHA}};
...
function createBodyShadowOverlay(textureSize) {
...
function rayIntersectsBody([x, y, z], [dx, dy, dz]) {
