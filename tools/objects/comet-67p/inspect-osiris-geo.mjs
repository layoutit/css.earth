...
import { decodeOsirisGeo, fitCamera, project, sampleGeo } from '../terrestrial-layers/osiris-geo.mjs';
import { loadPinned } from '../comet-1p/inspect-giotto.mjs';
import { loadRadialTerrain } from '../terrestrial-layers/radial-terrain.mjs';
import { closestTrianglePoint } from '../terrestrial-layers/obj-shape.mjs';
...
const percentiles = values => {
  values.sort((a, b) => a - b);
  return Object.fromEntries([50, 95, 99, 100].map(p => [p, values[Math.min(values.length - 1, Math.floor(values.length * p / 100))] ?? null]));
};

async function main() {
  const args = process.argv.slice(2);
  if (args.some(a => a !== '--download' && !a.startsWith('--output=') && !a.startsWith('--tile-size='))) {
    throw new Error('Usage: node tools/objects/comet-67p/inspect-osiris-geo.mjs [--download] [--output=directory] [--tile-size=32|64]');
  }
  ...
  if (holdoutResidualPixels[100] > .01) throw new Error('Recovered camera does not explain held-out geometry pixels.');
  radiances.sort((a, b) => a - b);
  const low = radiances[Math.floor(radiances.length * .01)], high = radiances[Math.floor(radiances.length * .99)];
  const level = value => Math.round(Math.max(0, Math.min(1, (value - low) / (high - low))) * 255);
  ...
  config.geometry.radialTerrain.tileSize = tileSize;
  const source = { manifest: { inputs: [] }, validatePath: async path => {
    if (path !== manifest.shape.path || sha256(await readFile(resolve(sourceDirectory, path))) !== manifest.shape.sha256) throw new Error('Unexpected source path.');
  } };
  ...
  const counts = {}, distances = [], separations = [];
  ...
  for (let faceIndex = 0; faceIndex < radial.plans.length; faceIndex++) {
    ...
    for (let y = 0; y < tileSize; y++) for (let x = 0; x < tileSize; x++) {
      ...
      let hit;
      if (!sampled.reason) {
        hit = radial.grid.closestPoint(point, policy.maximumSourceDistanceMeters);
        ...
        if (!sampled.reason) {
          ...
          const ray = radial.grid.intersect(eye, direction.map(n => n / distance), distance + policy.visibilityToleranceMeters);
          if (!ray || Math.abs(ray.radius - distance) > policy.visibilityToleranceMeters) sampled = { reason: 'occluded' };
        }
      }
      const accepted = !sampled.reason;
      if (interior) {
        interiorTexels++;
        counts[sampled.reason ?? 'accepted'] = (counts[sampled.reason ?? 'accepted'] ?? 0) + 1;
        if (accepted) { distances.push(hit.distanceMeters); separations.push(sampled.separationMeters); }
      }
      ...
      const color = accepted ? Array(3).fill(level(sampled.radiance)) : missingCoverageColor(longitude, latitude, .35).map(n => Math.round(n * light));
...
<script>const stage=document.querySelector('#stage'),mesh=document.querySelector('.mesh'),state=document.querySelector('#state');let yaw=0,pitch=0,last=null;new ResizeObserver(()=>stage.style.setProperty("--fit",stage.clientWidth/640)).observe(stage);function paint(){mesh.style.transform='rotateY('+yaw+'deg) rotateX('+pitch+'deg)';state.textContent='Rotation '+yaw.toFixed(1)+'° / '+pitch.toFixed(1)+'° · 1,000 retained leaves'}stage.onpointerdown=e=>{last=[e.clientX,e.clientY];stage.setPointerCapture(e.pointerId)};stage.onpointermove=e=>{if(!last)return;yaw+=(e.clientX-last[0])*.45;pitch-=(e.clientY-last[1])*.45;last=[e.clientX,e.clientY];paint()};stage.onpointerup=stage.onpointercancel=()=>last=null;document.querySelector('#reset').onclick=()=>{yaw=pitch=0;paint()};document.querySelector('#turn').onclick=()=>{yaw+=180;paint()};document.querySelector('#coverage').onclick=e=>{const on=stage.classList.toggle('coverage');e.target.textContent=on?'Show photograph':'Show accepted coverage'};</script></html>`;
  ...
  const report = { schema: 'cssearth-osiris-geo-trial-report@1', status: 'experimental; not a production object qualification',
    inputs: { manifestSha256: sha256(manifestBytes), frame: manifest.frame, shape: manifest.shape,
      decoderSha256: sha256(await readFile(resolve(import.meta.dirname, '../terrestrial-layers/osiris-geo.mjs'))),
      preparerSha256: sha256(await readFile(import.meta.filename)) },
