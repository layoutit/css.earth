...
const demand = (ok, why) => { if (!ok) throw new Error(`MUR imagery: ${why}`); };

export function parseMurCapabilities(xml, today = new Date().toISOString().slice(0, 10)) {
...
export async function prepareMurMosaic(directory, receipt, outputPath, width = 16384) {
...
export async function acquireMurImagery(directory, { capabilitiesPath } = {}) {
  ...
  const records = new Array(3200); let cursor = 0, totalBytes = 0, completed = 0;
  const getTile = async index => {
  ...
  const receipt = { schema: 'cssearth-mur-gibs@1', checked: new Date().toISOString(), date, product: 'NASA MUR v4.1 via GIBS', baseline: '2003–2014', layer: murLayer,
...
export async function restoreMurMosaic(sourceDirectory) {
  const receipt = JSON.parse(await readFile(join(sourceDirectory, 'mur-gibs-receipt.json')));
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const result = mode === 'restore' ? await restoreMurMosaic(resolve(directory)) :
    await acquireMurImagery(resolve(directory), { capabilitiesPath });
  console.log(json({ date: result.date, complete: result.complete, sourceBytes: result.sourceBytes, archiveBytes: result.archiveBytes, mosaic: result.mosaic ?? result }));
}

export function murEnsoContent(recipe) {
...
export async function verifyPreparedMurImage(sourceDirectory, recipe) {
  const receipt = JSON.parse(await readFile(join(sourceDirectory, 'science/mur-gibs-receipt.json')));
...
export async function writeMurLegend(sourceDirectory, outputPath) {
