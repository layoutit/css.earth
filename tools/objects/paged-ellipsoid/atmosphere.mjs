import { prepareAtmosphereFrame } from "../../prepared-atmosphere.mjs";
import { viewSunDirectionToPreparedLightDirection } from "../../../src/platform/directional-sun-coordinate.mjs";
import { readFile } from "node:fs/promises";
import { resolve } from "node:path";


export function createAtmospherePreparation({ config, sourceDirectory, sourceManifest, sun }) {
...
function atmosphereProfile(model) {
  ...
  return {
    ...
    transfer: { mode: "scattering-rgb",
      ...
      maximumAlpha: response.cleanRoomTransfer[config.atmosphere.maximumOpacityKey], limbConcentration: true },
  };
}

function prepareAtmosphereMaterialFrame({ size, frame = ATMOSPHERE_DEFAULT_FRAME, model }) {
