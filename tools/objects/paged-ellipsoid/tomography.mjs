...
const normalizedLongitude = degrees => ((degrees + 180) % 360 + 360) % 360 - 180;
...
export async function readMantleTomography(sourceDirectory, interior, config) {
  if (!interior.tomographyPath) return null;
  const recipe = JSON.parse(await readFile(resolve(sourceDirectory, interior.tomographyPath), 'utf8'));
  ...
  const shellDepth = recipe.modelRadiusKm * (1 - interior.layers.find(layer => layer.id === 'mantle').outerRadiusKm / interior[config.interiorRadiusKey]);
  ...
  const bilinear = (data, width, row, column) => {
  ...
  const sample = (face, depthKm, latitudeDegrees) => {
  ...
  const color = value => tomographyColor(recipe, value);
  return {
    recipe, sample, color,
    sectionColor(face, radius, vertical, horizontal) {
    ...
    shellColor(longitudeDegrees, latitudeDegrees) {
...
export function tomographyColor(recipe, percent) {
  if (percent === null || !Number.isFinite(percent)) return rgb(recipe.missingColor);
  const stops = recipe.palette;
  const value = Math.max(stops[0].percent, Math.min(stops.at(-1).percent, percent));
...
export function tomographyLegend(recipe) {
  const { width, height } = recipe.legend;
  const data = Buffer.alloc(width * height * 3);
  for (let x = 0; x < width; x++) {
    const color = tomographyColor(recipe, interpolate(recipe.palette[0].percent, recipe.palette.at(-1).percent, x / (width - 1)));
