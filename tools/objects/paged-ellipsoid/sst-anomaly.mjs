...
const attribute = (object, key) => {
  ...
  return ArrayBuffer.isView(value) ? value[0] : value;
};
const demand = (condition, message) => { if (!condition) throw new Error(`CoralTemp: ${message}`); };
...
export async function readCoraltempAnomaly(path, recipe) {
  await h5.ready;
  const file = new h5.File(path, 'r');
  try {
    const field = file.get('sea_surface_temperature_anomaly'), maskField = file.get('mask');
    const lat = file.get('lat').value, lon = file.get('lon').value;
    ...
    const stamp = attribute(file, 'time_coverage_start');
    ...
    demand(/1991-2020/.test(attribute(file, 'title')) && /1991-2020/.test(attribute(file, 'history')) &&
    ...
    demand(attribute(time, 'units') === 'seconds since 1981-01-01 00:00:00' &&
      new Date(Date.UTC(1981,0,1) + time.value[0] * 1000).toISOString().slice(0,10) === date, 'time coordinate differs');
    ...
    demand(attribute(maskField, 'flag_meanings') === 'valid-water land missing ice' &&
      JSON.stringify([...maskField.attrs.flag_values.value]) === '[0,1,2,4]' && attribute(maskField, '_FillValue') === 251, 'mask definition differs');
    ...
    const values = field.value, mask = maskField.value, data = Buffer.alloc(7200 * 3600 * 3);
    ...
    const maskNames = { 0: 'water', 1: 'land', 2: 'missing', 4: 'ice', 251: 'fill' };
    let valid = 0, missing = 0, saturated = 0, minimum = Infinity, maximum = -Infinity;
    for (let i = 0; i < values.length; i++) {
      const packed = values[i], flag = mask[i];
      demand(Object.hasOwn(maskNames, flag), 'unknown mask flag');
      maskCounts[maskNames[flag]]++;
    ...
    return { data, info: { width: 7200, height: 3600, channels: 3 },
      receipt: { product: 'NOAA CoralTemp v3.1 SST anomaly', version: '3.1-clim19912020-v1', filename: recipe.filename,
        date, issued: attribute(file, 'date_issued'), baseline: recipe.baseline, units: '°C',
...
export function anomalyColor(value, recipe) {
...
export function ensoContent(recipe) {
