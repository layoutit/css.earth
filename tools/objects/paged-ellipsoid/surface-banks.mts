...
export function surfaceBankInventory(plan: SurfaceBankPlan, lenses: SurfaceBankLenses, assetPath: string) {
  ...
  const outerPages = requireSurfacePages(outer.urls, "Interior surface", assetPath);
  if (outer.url !== outerPages[0] || outerPages.length !== bodyPages.length || body.url !== bodyPages[0]) {
  ...
  const banks = lenses.controls.filter(lens => !lens.surfaceBankId).map((lens) => {
    const urls = lens.view === "interior" ? outerPages
