...
import { preparePolarAtlas } from "../geographic-pages/operations/polar-raster.mjs";
import { writeSurfaceDelivery } from "./surface-page-delivery.mjs";
...
const produced = new Set(), deliveries = {};
...
return { assets: [...produced].sort(), deliveries };
...
async function writeSphereAssets({ data, width, height, channels, density,
  ...
  if (projectiveSurface) {
    ...
    const receipt = await writeSurfaceDelivery({ plan: surfaceRasterPlan, density: surfaceWidth / 1024,
      ...
      writePage: (page, bytes) => writeFile(output(urls[page].split("/").at(-1)), bytes),
    });
    deliveries[`${name}${suffix}`] = receipt;
  } else {
