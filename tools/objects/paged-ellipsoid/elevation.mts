import type {ElevationGrid, ElevationRecipe, MapSource} from './contracts.mts';
...
export function decodeElevationDods(bytes: Buffer, grid: ElevationGrid, { rowOffset = 0, rows = grid.height } = {}) {
  ...
  const count = (expectedCount: number) => {
  ...
  const axis = (length: number, origin: number, startIndex = firstIndex) => {
    count(length);
...
function elevationSampler(values: Int16Array, latitudes: Float64Array, longitudes: Float64Array, step: number) {
...
export async function readElevationGrid(sourceDirectory: string, map: MapSource<ElevationRecipe>) {
  ...
  let nextRow = 0, longitudes: Float64Array | undefined;
  ...
  demand(nextRow === grid.height && longitudes, 'incomplete global coverage');
  return elevationSampler(values, latitudes, longitudes, grid.stride * grid.nativeCellDegrees);
}

export function elevationColor(value: number, recipe: Pick<ElevationRecipe, "palette">) {
  ...
  const index = value > stops[stops.length - 1].meters ? stops.length - 1 : i;
...
export async function prepareElevationMap({ sourceDirectory, map, width, height }: {sourceDirectory: string; map: MapSource<ElevationRecipe>; width: number; height: number}) {
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
    const value = source.sample(-180 + (x + .5) * step, 90 - (y + .5) * step);
    demand(value !== null, 'output cell lies outside the source grid');
...
export async function writeElevationLegend(recipe: ElevationRecipe, path: string) {
