...
const demand = (ok, why) => { if (!ok) throw new Error(`Night lights: ${why}`); };
const prepared = new Map();

export function validateNightLightGrid(image, recipe) {
...
export function accumulateRadianceRow(values, sourceRow, grid, target, sums, weights) {
...
export function finishRadianceGrid(sums, weights, grid, target) {
...
export function nightLightColor(value, display) {
...
async function readNightLightGrid(archivePath, recipe, width, height) {
  ...
  const work = (async () => {
    ...
    try {
      ...
      const exited = new Promise((done, fail) => { child.once('error', fail); child.once('close', code =>
...
export async function prepareNightLightsMap({ sourceDirectory, map, width, height }) {
...
export async function writeNightLightsLegend(recipe, path) {
