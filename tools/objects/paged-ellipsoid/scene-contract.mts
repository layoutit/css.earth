...
export type PagedGeometryParameters = Record<'BODY_LATITUDE_SEGMENTS' | 'BODY_LONGITUDE_SEGMENTS' | 'EQUATORIAL_RADIUS' | 'TILE_SIZE' | 'SEAM_BLEED' | 'PLANET_SEAM_BLEED' | 'INTERIOR_PROJECTIVE_TEXTURE_RASTER_SCALE' | 'SURFACE_OVERLAP' | 'POLAR_CAP_BAND_SPAN' | 'POLAR_SURFACE_OVERLAP' | 'POLAR_INNER_OVERLAP' | 'POLAR_INNER_INSET' | 'OBLIQUITY_DEGREES' | 'PRESENTATION_NODE_DEGREES' | 'MESH_ROTATION_Z' | 'CAMERA_ZOOM' | 'CAMERA_SCENE_PITCH_DEGREES' | 'CAMERA_MINIMUM_CONTROL_PITCH_DEGREES' | 'CAMERA_MAXIMUM_CONTROL_PITCH_DEGREES' | 'CAMERA_DEFAULT_CONTROL_PITCH_DEGREES' | 'CAMERA_MILLISECONDS_PER_CONTROL_DEGREE' | 'INTERIOR_LATITUDE_SEGMENTS' | 'INTERIOR_LONGITUDE_SEGMENTS' | 'rotationSeconds', number> & {interiorCutaway: Cutaway};
