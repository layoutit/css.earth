...
async function verifySources(sourceDirectory: string, paths: readonly string[]): Promise<void> {
  ...
  for (const path of paths) { const expected = known.get(path); if (typeof expected !== 'string' || !sourceDigest.test(expected)) throw new TypeError(`Celestial source ${path} is not pinned in the source manifest.`); const bytes = await readFile(resolve(sourceDirectory, path)); if (sha256(bytes) !== expected) throw new TypeError(`Celestial source ${path} changed from its pinned digest.`); }
}
