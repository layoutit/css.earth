import type { RasterInfo, SynopticEmissionRecipe, EmissionVariant, ContinuumVariant } from './contracts.mts';
...
interface PolarProxyModel {
  map: Uint8Array; width: number; height: number; centerX: number; centerY: number; radiusX: number; radiusY: number; gains?: readonly number[];
}
import { readFile, mkdir } from 'node:fs/promises';
import { resolve } from 'node:path';
import sharp from 'sharp';
import { orientLatitudeBands } from './projection.mts';
import { prepareFitsMap } from './fits-map.mts';

/** Source-pinned observation mosaics, polar continuation, and emissive layers. */
export async function prepareSynopticEmission({ sourceDirectory, publicDirectory, config }: {sourceDirectory: string; publicDirectory: string; config: SynopticEmissionRecipe}) {
  ...
  let continuumFramesPromise: Promise<readonly ContinuumFrame[]> | undefined;
  await mkdir(publicDirectory, { recursive: true });
  for (const variant of config.variants) await prepareVariant(variant);
async function prepareVariant(variant: EmissionVariant) {
...
async function loadGlobalMap(variant: EmissionVariant, width: number, height: number) {
...
function createPolarSprite(map: Uint8Array, detailMap: Uint8Array, width: number, height: number, tileSize: number) {
...
function createPolarLeafAtlas(polar: Uint8Array, tileSize: number, cellSize: number) {
...
function samplePolarMap(map: Uint8Array, width: number, height: number, sourceX: number, sourceY: number, radial: number, longitude: number, boundaryModel: PolarBoundaryModel, proxyModel: PolarProxyModel, unitX: number, unitY: number) {
...
function preparePolarProxyModel(map: Uint8Array, width: number, height: number, north: boolean, boundaryModel: PolarBoundaryModel) {
  const model = {
    map,
    width,
    height,
...
function samplePolarProxy(model: PolarProxyModel, unitX: number, unitY: number, applyGains = true) {
  ...
  if (!applyGains || !gains) return sample;
  return sample.map((value, channel) => channel === 3
    ? 255
    : clamp(value * gains[channel], 0, 255));
}
...
function sampleClampedBilinearRgba(map: Uint8Array, width: number, height: number, sourceX: number, sourceY: number) {
...
async function prepareOffLimbContext(variant: EmissionVariant, density: number) {
...
async function prepareLimbMaterial(variant: EmissionVariant, map: Uint8Array, mapWidth: number, mapHeight: number, density: number) {
...
async function prepareContinuumLimbMaterial(variant: ContinuumVariant, map: Uint8Array, mapWidth: number, mapHeight: number, density: number) {
  const size = LIMB_SIZE * density;
  const output = Buffer.alloc(size * size * 4);
  const representative = representativeMapColor(map, mapWidth, mapHeight);
