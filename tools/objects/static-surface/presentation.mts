...
import type { prepareBandSurfaceScene } from './band-scene.mts';
import type { prepareSegmentedSurfaceScene } from './segmented-scene.mts';
import type { BandLenses, EmissiveLenses } from './source-contract.mts';
...
export async function prepareBandSurfacePresentation({ namespace, plan, lenses, sky, sun }: {namespace: string; plan: ReturnType<typeof import('../../prepared-replay-source.mts').parseBandReplayScene>; lenses: BandLenses; sky: PreparedCubicSkyPlan; sun: PreparedDirectionalSunPlan | null}) {
  ...
  const required = (id: string) => [`surface:${id}`, `poles:${id}`];
  ...
  const surface: PreparedNode[] = [], polar: PreparedNode[] = [];
  ...
  material.style.backgroundImage = `url("${canonicalPreparedAsset(lenses.material)}")`;
...
export async function prepareEmissiveSurfacePresentation({ namespace, plan, lenses }: {namespace: string; plan: ReturnType<typeof prepareSegmentedSurfaceScene>; lenses: EmissiveLenses}) {
