...
import { prepareStaticSurfaceObject } from './index.mts';

/** Compile in an isolated destination and compare the complete published bank. */
export async function verifyStaticSurfaceReproduction({ id, projectRoot }: {id: string; projectRoot: string}) {
  ...
  const manifest = requireRuntimeAssetManifest(id, JSON.parse(await readFile(resolve(objectDirectory, 'runtime-assets.json'), 'utf8')));
