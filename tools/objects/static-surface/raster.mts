import type { ObservationLens, ObservationRasterRecipe, RasterImage, RasterAtlas, TonalPresentation } from './contracts.mts';
import type { SourceScalar } from '../terrestrial-layers/contracts.mts';
import type { prepareSurfaceRasterCell } from './inverse-homography.mts';
...
import {loadScienceSurface, paintScienceSurface, colorForValue} from '../terrestrial-layers/scientific-raster.mts';
import sharp from 'sharp';
import type { Sharp } from 'sharp';
import { blackFillCoverage, sampleCoverage, paintMissingCoverage } from '../../../src/platform/prepare-missing-coverage.mts';
import { orientLatitudeBands, preparePolarAtlas } from './projection.mts';
import { writeCurvatureMaterial } from './curvature.mts';
import { decodeElevationGrid, elevationRaster, elevationColor } from './elevation.mts';
import { bakeSurfaceRaster } from './inverse-homography.mts';

/** Observation interpretation is selected by the source recipe, never body ID. */
export async function prepareObservationLenses({ sourceDirectory, publicDirectory, config, geometry, surfaceRasterCells }: {sourceDirectory: string; publicDirectory: string; config: ObservationRasterRecipe; geometry?: {rasterAtlas?: RasterAtlas}; surfaceRasterCells?: readonly ReturnType<typeof prepareSurfaceRasterCell>[]}) {
  ...
  for (const plan of config.lenses) {
    ...
    if (plan.elevation || plan.scientific) {
      ...
      for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
        ...
        if (plan.scientific) {
          const recipe = plan.scientific;
          const value = recipe.categories
            ? Math.min(recipe.categories.length - 1, Math.floor(x * recipe.categories.length / width))
            : recipe.minimum + x / (width - 1) * (recipe.maximum - recipe.minimum);
          color = colorForValue(value, recipe);
        } else {
          if (!plan.elevation) throw new Error('Elevation legend recipe is missing.');
          color = elevationColor((2 * x / (width - 1) - 1) * plan.elevation.rangeMetres, plan.elevation);
        }
    ...
    if (config.thumbnail === 'prepared-centered' || plan.scientific) {
      if (!thumbnailRaster) throw new Error('Observation thumbnail requires a prepared 2x raster.');
      ...
      if (metadata.width === undefined || metadata.height === undefined) throw new Error('Observation image dimensions are missing.');
...
function applyTonalPresentation(pipeline: Sharp, presentation: TonalPresentation) {
