...
export function decodeElevationGrid(bytes, recipe) {
  ...
  if (!strips || !counts || strips.type !== 4 || counts.type !== 4 || strips.count !== height || counts.count !== height || !missing || bytes.toString("ascii", missing.start, missing.start + missing.count).replace(/\0/gu, "") !== String(recipe.noData)) throw new Error("Signed elevation elevation strip/no-data metadata drifted.");
  ...
  return { width, height, recipe, sample: (x, y) => bytes.readInt16LE(offsets[y] + x * 2) };
}

export function elevationColor(metres, recipe) {
