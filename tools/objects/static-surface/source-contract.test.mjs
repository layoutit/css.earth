...
const read = async (id, path) => JSON.parse(await readFile(new URL(`../../../src/planets/${id}/source/${path}`, import.meta.url), 'utf8'));
...
for (const id of ['moon', 'pluto', 'sun']) {
  test(`${id} source boundaries preserve pinned records and their provenance`, async () => {
    for (const [path, parse] of [['preparation/geometry.json', parseSurfaceGeometry], ['preparation/raster.json', parseSurfaceRaster],
      ...
      ['content/static.json', parseSurfaceContent], ['presentation/title-mark.json', parseTitleRecipe]]) {
    ...
    const {lenses} = await read(id, 'content/static.json');
...
test('camera and material addresses must be numeric and complete before compiling scene leaves', async () => {
  const geometry = await read('pluto', 'preparation/geometry.json');
  geometry.metadata.camera.defaultZoom = '1.1';
  assert.throws(() => parseSurfaceGeometry(geometry), /static surface geometry/);
  const {lenses} = await read('sun', 'content/static.json');
  delete lenses.controls[0].limbUrl;
...
test('observation variants retain the correlation between measurements and processing', async () => {
  const raster = await read('sun', 'preparation/raster.json');
  ...
  const continuum = await read('sun', 'preparation/raster.json');
  continuum.variants[0].kind = 'fits-map';
