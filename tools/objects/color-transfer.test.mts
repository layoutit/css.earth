...
import { parseBandColorDisplay, linearToSrgb, srgbToLinear, encodeBandColor } from './color-transfer.mts';

const policy = { kind:'band-composite',inputQuantity:'radiance-factor',bands:['IR3','IR1','UV3'],displayRange:[0,1],outputEncoding:'srgb' };
...
test('ICC sRGB display values and channel ratios survive the final byte boundary',()=>{
  ...
  const display=parseBandColorDisplay(policy,policy.bands);
...
test('clipping and quantization happen after a complete floating composite',()=>{
  const display=parseBandColorDisplay(policy);
  const mean=(1.5+.1)/2;
...
test('band interpretation cannot be promoted to natural color or silently changed',()=>{
  assert.throws(()=>parseBandColorDisplay(undefined),/object|record/i);
  assert.throws(()=>parseBandColorDisplay({...policy,kind:'natural-color'}),/no inferred natural color/);
  assert.throws(()=>parseBandColorDisplay({...policy,gains:[1,1,2]}),/no inferred natural color/);
  assert.throws(()=>parseBandColorDisplay({...policy,whiteBalance:'auto'}),/no inferred natural color/);
  assert.throws(()=>parseBandColorDisplay(policy,['RED','GRN','BL1']),/actual source bands/);
  assert.throws(()=>parseBandColorDisplay({...policy,outputEncoding:'linear'}),/explicit scientific/);
});
