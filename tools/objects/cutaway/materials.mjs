...
import {dotVector as dot3} from '../material-composition/ellipsoid.mjs';
import {validateMaterialRecipe,validateRelativePath} from '../material-composition/recipe.mjs';
/** Declared radial composition, two-face cutaway shading, and polar wedge rasters. */
export async function prepareCutawayMaterials({sourceDirectory,publicDirectory,config,objectLightDirection}) {
...
const manifest = JSON.parse(await readFile(resolve(sourceDirectory,config.source),'utf8'));
...
function publicPath(url) { const filename=url.startsWith(config.publicPrefix)?url.slice(config.publicPrefix.length):'';if(!/^[a-z0-9@.-]+$/u.test(filename))throw new TypeError('Invalid cutaway texture URL.');return resolve(publicDirectory,filename); }
