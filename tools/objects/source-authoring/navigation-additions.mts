#!/usr/bin/env node
// Carry forward exact checked-in marker pixels for unchanged bodies. Selected markers
...
const baseRef = argument('base'), inputsPath = argument('inputs'), evidencePath = argument('evidence');
if (!baseRef || !inputsPath || !evidencePath) throw new Error('--base, --inputs and --evidence are required.');
...
const old = Object.fromEntries(Object.entries(requireRecord(JSON.parse(oldText.split('Object.freeze(')[1].slice(0, -3)))).map(([id, value]) => {
  const marker = requireRecord(value);
  const context = marker.context === undefined ? undefined : requireRecord(marker.context);
  return [id, {index: requireFiniteNumber(marker.index), ...(context ? {context: {url: requireString(context.url), pixels: requireFiniteNumber(context.pixels)}} : {})}];
}));
const bodies = requireArray(requireRecord(JSON.parse(await readFile(resolve(root, inputsPath), 'utf8'))).bodies).map(value => ({id: requireString(requireRecord(value).id)}));
const selected = new Set(bodies.map(b => b.id));
const planets = OBJECTS.toSorted((a, b) => a.distanceAu - b.distanceAu);
const newIds = bodies.filter(b => !old[b.id]).map(b => b.id);
const updatedIds = bodies.filter(b => old[b.id]).map(b => b.id);
if (selected.size !== bodies.length || bodies.some(b => !planets.some(p => p.id === b.id)) ||
    planets.length !== Object.keys(old).length + newIds.length || planets.some(p => !old[p.id] && !selected.has(p.id))) throw new Error('Unreviewed registry change.');
const descriptors = await loadMarkerDescriptors({ planets, projectRoot: root });
for (const d of descriptors.filter(d => !selected.has(d.planetId))) {
...
for (const density of [1, 2]) {
  ...
  for (let index = 0; index < descriptors.length; index++) {
    ...
    const input = selected.has(d.planetId)
  ...
  evidence.push({ filename, baseSha256: createHash('sha256').update(bytes).digest('hex'), outputSha256: createHash('sha256').update(output).digest('hex'), preservedMarkers: Object.keys(old).length - updatedIds.length });
}
const newContexts = await prepareContextMarkers({ projectRoot: root, outputRoot: resolve(root, 'public/navigation'), descriptors: descriptors.filter(d => selected.has(d.planetId)), planets });
...
await writeFile(resolve(root, evidencePath), JSON.stringify({ base, added: newIds, updated: updatedIds, atlases: evidence }, null, 2) + '\n');
console.log(JSON.stringify({ added: newIds, updated: updatedIds, preservedMarkers: Object.keys(old).length - updatedIds.length }));
