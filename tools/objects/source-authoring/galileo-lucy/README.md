...
6. Run `node tools/objects/source-authoring/galileo-lucy/navigation.mts` to retain accepted marker pixels and prepare the three new markers. The script verifies unchanged existing recipes and exact visible pixel equality at both densities against its pinned baseline. Regenerate shared world context, then run `node tools/prepare-object-json.mts dactyl dinkinesh selam ida sun`.
