...
for (const [id, parent, a, periodHours, eccentricity] of [['dactyl', 'ida', 82.3, .96534 * 24, .15], ['selam', 'dinkinesh', 3.11, 52.67, 0]]) {
  test(`${id}: declared approximate state preserves period, separation, orbital pole and phase qualification`, async () => {
    ...
    const parameters = JSON.parse(await readFile(resolve(bodyRoot, 'source/orbit/published-parameters.json')));
    const state = await loadBodyEpochEphemeris({ bodyRoot, bodyId: id, centerBodyId: parent, epochJdTt });
    assert.equal(state.provenance.placement, 'approximate');
    assert.match(state.provenance.timeQualification, /not a propagation/);
    ...
    const rotation = JSON.parse(await readFile(resolve(bodyRoot, 'source/preparation/rotation.json')));
    ...
    await assert.rejects(loadBodyEpochEphemeris({ bodyRoot, bodyId: id, centerBodyId: parent, epochJdTt: epochJdTt + 1 }), /epoch/);
  });
}
