...
for(const c of bodies){
 ...
 const manifest=parseAuthoringManifest(await read(resolve(s,'manifest.json')));manifest.generatedIntermediates=[refreshSourceRecord(manifest.generatedIntermediates,{...nav.source,id:'prepared-radial-context'})];
 ...
 async function walk(dir: string,pre=''){for(const e of await readdir(dir,{withFileTypes:true})){const rel=pre+e.name;if(e.isDirectory())await walk(resolve(dir,e.name),rel+'/');else if(!exclude.has(rel))documents.push(refreshSourceRecord(manifest.documents,{path:rel,...pin(await readFile(resolve(dir,e.name)))}));}}
