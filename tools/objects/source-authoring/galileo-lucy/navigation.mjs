...
import { loadMarkerDescriptors, prepareContextMarkers } from '../../../prepare-navigation.mjs';
import { renderMarker } from '../../../../src/navigation/marker-recipe.mjs';
import { bodies } from './catalog.mjs';
const baseline = 'bbfbf86b5c9b71fc56ef5e6b4759fd00af11d8ec';
const original = path => execFileSync('git', ['show', `${baseline}:${path}`], { maxBuffer: 8 * 1024 * 1024 });
...
const report = { baseline, objects: planets.length, method: 'Preserve every existing decoded visible pixel; render additions from their source-pinned navigation recipes; encode losslessly.', densities: [] };
