...
export async function author(defaultLens = 'zimpol-v') {
  ...
  // The catalogue records are generated by tools/author-source-records.mts from these bindings; their revision is a
  // placeholder until that tool is rerun with --evidence once the manifest is committed.
  const binding = (id: string) => ({ dependencies: [], sourceBinding: { kind: 'catalogued', references: [
    { catalogueId: `source-betelgeuse-shell-${id}`, role: 'material',
      evidence: `${packageBase}/manifest.json@${'0'.repeat(40)}#/inputs/${inputs.length}` }] } });
