...
export async function installPdsToolchain() {
  ...
  run('micromamba', ['create', '-y', '-q', '-p', prefix, '-c', requireString(mamba.channel),
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (mode === 'install') console.log(`PDS packages are installed at ${await installPdsToolchain()}`);
    ...
    const found = run(toolchain.python, ['-c', 'from importlib.metadata import version; import pdr, pds.peppi; print(version("pds.peppi"), version("pdr"))'], toolchain.env).trim();
