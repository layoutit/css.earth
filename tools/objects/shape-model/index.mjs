...
import { prepareModelMarker, prepareModelRasters, prepareRingRaster } from './raster.mjs';
...
export async function prepareShapeModel({ descriptor, sources, objectDirectory, publicDirectory, outputDirectory, prepareContent }) {
  ...
  if (config.schema !== 'cssearth-shape-model@1' || shape.kind !== 'ellipsoid') throw new TypeError('Expected an ellipsoidal shape model.');
  const axes = [shape.radiusKm, shape.secondaryRadiusKm ?? shape.radiusKm, shape.polarRadiusKm];
  const { latitudeSegments, longitudeSegments, width, height, poleSize } = config.mesh;
  const quadCount = (latitudeSegments - 2) * longitudeSegments + 4 + config.ring.segments;
  if (![latitudeSegments, longitudeSegments, width, height, poleSize, config.ring.segments].every(n => Number.isInteger(n) && n > 0) ||
  ...
  const textures = await prepareModelRasters({ config, axes, publicDirectory, publicBase, sourceDirectory });
  await writeFile(resolve(publicDirectory, 'marker.webp'), await prepareModelMarker(axes));
  const ringTexture = await prepareRingRaster({ config, axes, publicDirectory, publicBase });
  ...
  const sunPresentation = { ...prepareSolarSystemSunPresentation(cameraOptions), source: 'JPL Kepler orbit and authored observed pole; arbitrary display phase',
  ...
  const ringLeaves = prepareRingLeaves(config, ringTexture, axes[0]);
  ...
  const entries = [{ key: 'marker-phase', url: publicBase + 'marker-phase.webp', pool: 'mounted' }, ...preparedSunResources(sun, 'mounted'), ...Object.entries(textures).map(([key, url]) => ({ key, url, pool: 'mounted' })),
    { key: 'ring', url: ringTexture.url, pool: 'mounted' }, { key: 'marker', url: publicBase + 'marker.webp', pool: 'mounted' }];
  ...
  const system = b.mesh('shape-model-system', `transform:${scene.systemTransform}`), body = b.mesh('shape-model-body'), ring = b.mesh('shape-model-ring');
  b.append(null, camera); b.append(camera, root); b.append(root, system); b.append(system, body, ring);
  ...
  const definition = JSON.parse(JSON.stringify({ schema: 'cssearth-object-runtime@4', id, camera: { ...scene.camera, responsiveFit: { ...scene.camera.responsiveFit, maximumHeightShare: config.camera.maximumHeightShare } }, sky: scene.starfield, sun,
    ...
    variants: [{ when: {}, required: ['surface', 'poles', 'ring'], writes: [
      ...
      { kind: 'texture', target: index(ring), name: '--shape-ring', resource: 'ring', quoted: true },
    ], materials: [] }], materials: [], animations: [], viewBindings: [],
  ...
  const geometry = { ...scene, bodyLeaves, ringLeaves, counts: { bodyQuads: bodyLeaves.length, ringQuads: ringLeaves.length, totalQuads: quadCount, budget: config.quadBudget },
    model: { semiAxesKm: axes, ring: config.ring, surface: config.surfaceModel ? 'NASA VTAD illustrative model texture; no observed terrain' : 'unresolved; uniform neutral display material', phase: 'arbitrary-display-phase', lighting: 'even flood illumination; original illustrative texture, no added directional shadows' } };
