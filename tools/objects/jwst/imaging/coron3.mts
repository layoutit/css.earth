...
import { imagingMembers } from './image3.mts';
...
export async function runCoron3(id: string, band: string, work: string, options: { sources?: readonly string[]; maxRssBytes?: number } = {}) {
  ...
  return { mosaic: resolve(output, `${product}_i2d.fits`), peakRssBytes: result.peakRssBytes, seconds, members: files.length, references: references.length };
}
