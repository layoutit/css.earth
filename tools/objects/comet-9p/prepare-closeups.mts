...
import {parseEncounterPolicy,parseEncounterSourceControl,parseMeshProfile} from '../terrestrial-layers/source-records.mts';
...
export async function prepareCloseups(sourceDirectory:string,write=false) {
  ...
  await pinned('preparation/closeups.json'); await pinned(geometry.path);
  ...
  for(const entry of recipe.frames){
    ...
    validateEncounterControls(refCamera,refControl.registration);
