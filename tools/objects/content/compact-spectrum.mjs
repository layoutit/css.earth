...
export function renderCompactSpectrum({ title, description, metadata, points, maximum }) {
  if (!Array.isArray(points) || points.length < 2 || !Number.isFinite(maximum) || maximum <= 0 ||
  ...
  const x = wavelength => (wavelength - .35) / .65 * 300;
  // Reserve room above the plot for the full height of its 13px tick labels.
  const y = value => 64 - value / maximum * 46;
...
${[[.35,'350','start'],[.55,'550','middle'],[.75,'750','middle'],[1,'1000 nm','end']].map(([wavelength,label,anchor]) => `<text x="${x(wavelength)}" y="86" fill="#666" text-anchor="${anchor}">${label}</text>`).join('\n')}
</svg>`;
}
