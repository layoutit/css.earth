...
interface Spectrum extends Identity {kind:'spectrum';source:string;format:'json-columns'|'numeric-lines';pointCount:number;maximum:number;requiredHeader?:string;xField?:string;yField?:string;countField?:string;countValue?:number;xScale?:number;minimumX?:number;maximumX?:number;metadataFields?:Record<string,string>;}
interface Pressure extends Identity {kind:'pressure';source:string;layerCount:number;temperatureMinimum:number;temperatureMaximum:number;pressureTicks:{pressure:number;label:string}[];}
...
export function parseChartAssetRecipe(value:unknown):ChartAssetRecipe {
 ...
 for(const value of recipe.charts){const chart=record(value,'chart');for(const key of ['id','title','description','output'])string(chart[key],key);path('.',String(chart.output));record(chart.metadata,'metadata');if(!['spectrum','pressure','phase'].includes(String(chart.kind)))throw new TypeError('Unknown chart operator.');
  if(chart.kind==='spectrum'){string(chart.source,'source');if(!['json-columns','numeric-lines'].includes(String(chart.format))||typeof chart.pointCount!=='number'||!Number.isSafeInteger(chart.pointCount)||chart.pointCount<2||typeof chart.maximum!=='number'||chart.maximum<=0)throw new TypeError('Invalid spectrum sampling profile.');}
  if(chart.kind==='pressure'){string(chart.source,'source');if(typeof chart.layerCount!=='number'||chart.layerCount<2||typeof chart.temperatureMinimum!=='number'||typeof chart.temperatureMaximum!=='number'||!Array.isArray(chart.pressureTicks))throw new TypeError('Invalid pressure profile.');}
...
export async function prepareChartAssets({sourceDirectory,publicDirectory,config}:{sourceDirectory:string;publicDirectory:string;config:unknown}) {
 ...
 for(const chart of recipe.charts){const identity:ChartIdentity={id:chart.id,title:chart.title,description:chart.description,metadata:{...chart.metadata}};let svg:string;
  if(chart.kind==='spectrum'){
   ...
   if(points.length!==chart.pointCount)throw new TypeError('Spectrum sample count drifted.');svg=renderReflectanceChart({...identity,points,maximum:chart.maximum});
   ...
   const pressureMinimum=layers[layers.length-1].pressure,pressureMaximum=layers[0].pressure;identity.metadata.pressureRangeBar=[pressureMinimum,pressureMaximum];
   svg=renderTemperaturePressureChart({...identity,layers,pressureMinimum,pressureMaximum,temperatureMinimum:chart.temperatureMinimum,temperatureMaximum:chart.temperatureMaximum,pressureTicks:chart.pressureTicks});
  }else{
