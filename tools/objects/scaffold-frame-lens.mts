...
// The ship rule: every measurement that reached a verdict must place the lens within three degrees, and at least one must have.
// One agreeing reference does not outvote a disagreeing one; a conflict is a conflict, and the ledger keeps its numbers.
const verdicts = [scored >= 3 && systematic !== null ? systematic <= 3 : null, reliefMedian !== null ? Math.abs(reliefMedian) <= 3 : null, referenceMedian !== null ? Math.abs(referenceMedian) <= 3 : null].filter((verdict): verdict is boolean => verdict !== null);
const registered = verdicts.length > 0 && verdicts.every(Boolean);
