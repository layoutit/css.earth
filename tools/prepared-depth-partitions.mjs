...
export function restoreDepthSource(definition) {
  ...
  const chain = [];
  for (let id = source.surfaceHit.target; id !== camera; id = nodes[id].parent) {
    if (id < 0) throw new TypeError('Depth source branch is detached.');
    chain.unshift(id);
  }
  const removed = new Set(), bodyParents = new Set();
  ...
  const target = binding => ({ ...binding, target: remap(binding.target) });
  const restored = { ...source,
    tree: { ...source.tree, camera: remap(camera), scene: remap(scene), properties: properties.slice(0, -1),
      nodes: nodes.flatMap((node, id) => removed.has(id) ? [] : [{ ...node,
        parent: remap(bodyParents.has(node.parent) ? source.surfaceHit.target : node.parent),
    ...
    surfaceHit: { ...source.surfaceHit, target: remap(source.surfaceHit.target) },
  };
...
export function partitionSurface(triangles, maximumLeaves = MAXIMUM_DEPTH_LEAVES, frontSigns) {
  ...
  for (const points of triangles) for (let edge = 0; edge < points.length; edge++) {
    ...
    if (normal.find(value => Math.abs(value) > 1e-8) < 0) normal = normal.map(value => -value);
...
export function prepareDepthPartitions(definition, surface) {
  ...
  const remap = id => id < insertion ? id : id + extra;
  const nodes = original.map(node => ({ ...node, parent: remap(node.parent) }));
  const added = [], clones = new Map(chain.map(id => [id, []])), partitions = [];
  const append = record => { const id = insertion + added.length; added.push(record); return id; };
  for (const members of groups) {
    ...
    let parent = root, projectedScene;
