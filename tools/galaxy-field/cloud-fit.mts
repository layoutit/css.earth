...
export function fitClouds(points: readonly Point[]) {
  const k = Math.min(160, points.length);
  ...
  return groups.flatMap((g,i)=>{
    if(g.count<20)return [];
    const covariance=g.covariance.map((v,j)=>v/g.count+(j%4===0?.25:0));
    ...
    return [{position:centers[i],covariance,radius:3*Math.sqrt(Math.max(covariance[0]!,covariance[4]!,covariance[8]!)),brightness:Math.min(.14,.045*Math.log1p(density)),count:g.count}];
  });
}
