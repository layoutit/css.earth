...
export function instrumentCss(source, id) {
  ...
  if (id.endsWith('/paging/city-pages.ts')) {
    replace('  function clearDesired() {', `  function oracleView(){
      if(!view)return null;
      ...
      const matrix=(physical?new DOMMatrix(Array.from(physical.eyeFromScene)):new DOMMatrix(getComputedStyle(scene).transform))
        .multiply(new DOMMatrix(getComputedStyle(system).transform)).multiply(new DOMMatrix(getComputedStyle(carrier).transform));
      return {projection:Array.from(matrix.toFloat64Array()),scale:physical?1:Number.parseFloat(getComputedStyle(camera).scale),
        viewport:{width:stage.clientWidth,height:stage.clientHeight,originX:c.left+c.width/2-s.left,originY:c.top+c.height/2-s.top,...(physical?{projection:physical}:{})}};
    }
    if(plan.topology==='wmts-quadtree@1')globalThis.__cssOracleView=oracleView;
  function clearDesired() {`);
    replace('  function release(slot: PageSlot) {', `  function release(slot: PageSlot) {
  ...
  } else if (id.endsWith('/paging/api-image-transport.ts')) {
  ...
  } else if (id.endsWith('/paging/city-index.ts')) {
