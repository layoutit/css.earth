...
//
//   node tools/sync-upstream.mts
//   UPSTREAM_ROOT=/path/to/checkout node tools/sync-upstream.mts
//
...
const IMPORT_NOTE =
  ...
  "(tools/prepare-solar-geometry.mts first). A catalog consumer would be " +
  "wired the same way.";
...
export function locallyMaintainedFile(target:Pick<SyncTarget,'id'>, rel:string) {
  ...
  return ["README.md", "AGENTS.md", "CLAUDE.md", "package.json", "tools/tsconfig.json",
...
async function syncPackage(pkg:SyncPackage, { root, upstream, license, allowDirty, log }:SyncContext & {license:string;allowDirty:boolean}) {
  ...
  const provenance = {
    ...
    syncScript: "tools/sync-upstream.mts",
    transform:
