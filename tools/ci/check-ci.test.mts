...
test('the deploy consumes installed assets, rebuilds only catalogues and rejects uninventoried output',async()=>{
 ...
 // `prepare:deploy` and `prepare:deploy-catalogues` were single-caller stages; both are inlined
 // into `build:deploy`, which is the script the deploy workflow actually runs.
 assert.match(packageFile.scripts['build:deploy']??'',/node tools\/nebula\/prepare\.mts --if-missing/);
...
test('nebula application discovery runs new test filenames in place and propagates failures',async t=>{
 ...
 for(const path of ['tools/nebula/future-boundaries.test.mts','tools/sources/nebula-provenance-source-closure.test.mts']){
...
test('trusted body publication prepares only the requested object',async()=>{
 ...
 assert.match(command,/node tools\/prepare\/prepare-planets\.mts "--object=\$OBJECT"/);
...
test('--typecheck appends the typecheck only when shared code changed',()=>{
 ...
 for(const path of ['tools/ci/check-ci.mts','site/objects.mts','src/platform/a.mts','src/renderers/css/b.ts','packages/astronomy/c.ts'])
...
test('appended --typecheck restores declarations and prepared inputs after a no-DTS lane, despite local reuse',async()=>{
 ...
 const fixtures=`
node() {
 test "$*" = 'tools/ci/build-ci.mts full'
...
`;
 try{
  const typed={name:'earlier typed build',run:'node tools/ci/build-ci.mts full',env:{CI_PREPARATION_DTS:'true'}};
  const untyped={name:'later no-DTS lane',run:'node tools/ci/build-ci.mts full',env:{CI_PREPARATION_DTS:'false'}};
  ...
  assert.deepEqual(SHARED_TYPECHECK_STEP.run.split('\n'),['node tools/ci/build-ci.mts full','pnpm prepare:typecheck','pnpm typecheck']);
  ...
  const mutated={...SHARED_TYPECHECK_STEP,run:SHARED_TYPECHECK_STEP.run.replace('node tools/ci/build-ci.mts full\n','')};
...
test('the actual workflow dispatches every native lane command locally and retains main authoring',async()=>{
 ...
 try{
  ...
  assert.equal(await readFile(join(root,'dispatch'),'utf8'),[
   ...lanes.flatMap(lane=>['node tools/prepare/prepare-facilities.mts --catalog-only',`pnpm test:universe:${lane}`]),'',
  ].join('\n'));
 }finally{await rm(root,{recursive:true,force:true});}
});
...
test('the advisory audit dispatches the relocated source and reproduction commands in full',async()=>{
 ...
 assert.equal(await run({GITHUB_EVENT_NAME:'pull_request'}),[
  'node tools/prepare/prepare-ci-inputs.mts universe','node tools/assets/restore-object-json.mts',
  'node tools/prepare/prepare-provenance.mts --objects-only','node tools/prepare/prepare-feature-index.mts',
  'node site/minimap/prepare.mts --data-only','pnpm test:sources:pr',
  'node tools/prepare/prepare-ci-inputs.mts universe-preparation','node tools/assets/restore-source-inputs.mts --repository-volumes',
  'pnpm test:galaxy-field','pnpm test:ci-preparation:world','',
 ].join('\n'));
 assert.equal(await run({GITHUB_EVENT_NAME:'push'}),[
  'pnpm setup:asset-data','node tools/assets/restore-object-json.mts',
  'node tools/prepare/prepare-provenance.mts --objects-only','node tools/prepare/prepare-feature-index.mts',
  'node site/minimap/prepare.mts --data-only','node tools/assets/restore-source-inputs.mts --repository-volumes','pnpm test:sources',
  'node tools/prepare/prepare-ci-inputs.mts universe-preparation','node tools/assets/restore-source-inputs.mts --repository-volumes',
  'pnpm test:galaxy-field','pnpm test:ci-preparation:world','',
...
test('shared preparation expands to each expensive task once, in dependency order',async()=>{
 ...
 for(const name of ['build:tools','build:deploy']){
  const commands=expand(name);
  for(const task of ['node tools/prepare/prepare-catalog.mts','node tools/prepare/prepare-shell-titles.mts','node tools/prepare/prepare-overview-titles.mts'])
  ...
  const overview=commands.indexOf('node tools/prepare/prepare-overview-titles.mts');
