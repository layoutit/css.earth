import {shape,array,text,number} from './objects/terrestrial-layers/source-records.mts';
import {requireRecord,hasErrorCode} from './source-values.mts';
const mission=(value:unknown)=>Object.assign({},requireRecord(value),shape({id:text,description:text})(value));
const imageEntry=shape({id:text,url:text,width:number,height:number,bytes:number,sha256:text,source:shape({kind:text,sourcePage:text,credit:text})});
const emblemEntry=shape({id:text,src:text,width:number,height:number,bytes:number,sha256:text,source:requireRecord});
type PreparedSpacecraft=ReturnType<typeof mission> & {image:{src:string;width:number;height:number;bytes:number;sha256:string;kind:string;sourceUrl:string;credit:string};emblem:Record<string,unknown> & {kind:string;src:string;width:number;height:number;bytes:number;sha256:string}};
...
export async function prepareSpacecraft() {
  const catalog = shape({spacecraft:array(mission)})(JSON.parse(await readFile(resolve(root, 'site/source/spacecraft/catalog.json'), 'utf8')));
  const library = shape({entries:array(imageEntry)})(JSON.parse(await readFile(resolve(root, 'site/source/spacecraft/render-library.json'), 'utf8')));
  const emblemLibrary = shape({entries:array(emblemEntry)})(JSON.parse(await readFile(resolve(root, 'site/source/spacecraft/emblem-library.json'), 'utf8')));
  ...
  const prepared:Record<string,PreparedSpacecraft> = {};
