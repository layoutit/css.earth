...
export async function loadPreparedSceneMarkup(id: string) {
  ...
  // so publishing an object's assets to R2 never requires a rebake.
  const decodeDescriptor = await withPreparedAssetOrigin(descriptor);
  const definition = await loadPreparedCssObject(decodeDescriptor, {
    async read() { return Uint8Array.from(bytes).buffer; },
  });
  return { ...serializePreparedScene(definition), descriptor: decodeDescriptor };
}
