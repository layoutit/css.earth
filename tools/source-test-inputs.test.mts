...
async function fixture(t: TestContext) {
  ...
  await write('tools/prepare-facilities.mts', "import assert from 'node:assert/strict'; import { appendFileSync } from 'node:fs'; assert.deepEqual(process.argv.slice(2), ['--catalog-only']); appendFileSync('events.log', 'published\\n');");
...
test('native discovery covers every configured source family without admitting volume or context authoring', async t => {
  ...
  await write('tools/object-provenance.test.mts', "import test from 'node:test'; test('native-legacy-provenance', () => {});");
  for (const path of ['tools/context-provenance.test.mts', 'tools/prepare-volume-provenance.test.mts'])
