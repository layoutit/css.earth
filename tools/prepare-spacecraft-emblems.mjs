...
const records=JSON.parse(await fs.readFile(path.join(root,'source-records.json'),'utf8'));
...
for(const e of records){
 ...
 const white=n=>Math.min(data[n*4],data[n*4+1],data[n*4+2])>=225&&Math.max(data[n*4],data[n*4+1],data[n*4+2])-Math.min(data[n*4],data[n*4+1],data[n*4+2])<=30;
 ...
 if(opaqueWhite){
  ...
  const push=n=>{if(!seen[n]&&(data[n*4+3]<=8||white(n))){seen[n]=1;queue[tail++]=n;}};
