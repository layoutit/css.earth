...
import type { BodyId, SceneSatelliteRecord, RotationElements, Vsop87BodyKey } from "@cssearth/astronomy";
...
function planetKey(id: string): Vsop87BodyKey {
  if (id === "earth") return "emb";
  if (isIncluded(["mercury", "venus", "mars", "jupiter", "saturn", "uranus", "neptune"] as const, id)) return id;
  throw new TypeError(`No planetary state for ${id}.`);
}
...
const primaryStates = new Map<string, NonNullable<SceneSatelliteRecord["parentHeliocentricState"]>>();
...
const planetPosition = (id: string) => systemBarycentreHeliocentricAu(planetKey(id), EPOCH_JD_TT)
  .map((value, axis) => value + (id === "earth" ? epochStates.get("earth")!.positionKm[axis] / ASTRONOMICAL_UNIT_KILOMETERS : 0));
const planetVelocity = (id: string) => systemBarycentreVelocityAuPerDay(planetKey(id), EPOCH_JD_TT)
...
export const BODY_POSITION_PROVENANCE = Object.freeze(${JSON.stringify(Object.fromEntries([...epochStates, ...primaryStates].map(([id, state]) => [id, { ...requireRecord(state.provenance, "State provenance"), ...(id === "earth" ? {model:"VSOP87A EMB plus Horizons Earth-center offset"} : {}) }])), null, 2)});
