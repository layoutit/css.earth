...
export function visibilityComponents(triangles, frontSigns) {
  ...
  const outgoing = components.map(() => new Set()), incoming = new Int32Array(components.length);
  ...
  const ready = components.flatMap((_, i) => incoming[i] === 0 ? [i] : []), ordered = [];
  while (ready.length) {
