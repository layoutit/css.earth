...
import { createProjectiveSurfaceRasterLayout } from '../src/platform/projective-surface-raster.mjs';
import { orientLatitudeBands } from './objects/static-surface/projection.mjs';
import { latitudeRasterBands } from './objects/giant-layers/geometry.mjs';
import { createEllipsoidGeometry } from './objects/terrestrial-layers/ellipsoid-geometry.mjs';
...
export function unpackSurfacePreview({ data, info }, options) {
  ...
  return { data: output, info: { width: layout.width, height: layout.height, channels: 4 } };
}

export function assertSurfacePreviewCoverage(controls, images, bindings = []) {
...
export async function* recipeSurfacePreviews({ objectDirectory, publicDirectory, outputDirectory }) {
  const config = name => optionalJson(resolve(objectDirectory, 'source/preparation', `${name}.json`));
  const [raster, observations, terrestrial, paged, spectral, geometry, lenses] = await Promise.all([
  ...
  const read = file => sharp(resolve(publicDirectory, basename(file))).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  const packed = async (file, layout) => unpackSurfacePreview(await read(file), layout);
  if (raster?.kind === 'synoptic-emission') {
  ...
  if (observations?.schema === 'cssearth-observed-polar-surfaces@1') {
  ...
  if (observations?.schema === 'cssearth-observed-surfaces@1') {
  ...
  if (spectral?.schema === 'cssearth-spectral-material-variants@1') {
